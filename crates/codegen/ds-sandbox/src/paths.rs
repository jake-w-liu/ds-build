...
/// DS state directory — always writable (`$DS_HOME` or `~/.ds`).
pub(crate) fn ds_home() -> PathBuf {
    ds_config::ds_home()
}
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), ds_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.ds + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
#[cfg(all(feature = "enforce", unix))]
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![ds_home()];
    paths.extend(temp_writable_paths());
    paths
}

...
//! OS-level sandboxing for DS Build via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use ds_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__DS_INSIDE_BWRAP";
...
/// Zero-permission placeholder (file or dir) under `ds_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent ds
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::ds_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.ds/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("ds-{tag}-{}-{nanos}", std::process::id()));
        let ds = ws.join(".ds");
        std::fs::create_dir_all(&ds).unwrap();
        std::fs::write(ds.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("ds-deny-dir-{}", std::process::id()));

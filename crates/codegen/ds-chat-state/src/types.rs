...
/// emitter in `ds-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (ds login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, DEEPSEEK_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `ds_sampling_types::SamplingConfig` (which is secret-free).

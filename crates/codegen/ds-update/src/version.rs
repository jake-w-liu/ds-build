...
use ds_shell::env::DsBuildEnvironment;
use ds_shell::util::ds_home::ds_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@ds-official/ds";
pub const GH_RELEASE_REPO: &str = "deepseek-ai-shared/ds-build";

/// Primary CLI base URL: Cloudflare-fronted deepseek.com endpoint with edge caching
/// for binaries and origin-respecting no-cache for channel pointers.
pub(crate) const CLI_BASE_URL_PRIMARY: &str = "https://www.deepseek.com/cli";
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/ds-build-public-artifacts/cli";
...
///
/// Constructed once from `DsBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `DsBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://api.deepseek.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.ds/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (DS_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &DsBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: ds_shell::auth::DsComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = ds_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = DsVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<DsVersion>(&version_str)
...
pub use ds_version::installed as get_installed_ds_version;

/// Version of the managed ds binary currently on disk, read from the
/// `~/.ds/bin/ds` symlink target (`../downloads/ds-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `ds update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.ds/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = ds_shell::util::ds_home::ds_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "ds")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`ds-0.1.150-macos-aarch64`, including
/// pre-releases: `ds-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`ds-0.1.150`,
/// `ds-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`ds-latest`, `ds-pager-*` when `bin_prefix` is
/// `ds`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.ds/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: DsVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(ds_version::VERSION, &stable)
    })
}
...
/// `~/.ds/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(ds_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = DsVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`ds-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("ds-0.2.46-darwin-arm64", Some("0.2.46")),
            ("ds-0.1.220-linux-x86_64", Some("0.1.220")),
            ("ds-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("ds-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("ds-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("ds-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("ds-garbage-darwin-arm64", None),               // unparseable version
            ("ds-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("ds-latest", None),                             // symlink alias, not a version
            ("ds", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "ds").as_deref(),
                *expected,
        ...
        // prefix but not under "ds".
        assert_eq!(
            version_from_versioned_binary_name("ds-pager-0.1.5-darwin-arm64", "ds-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // DsVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: DsVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = DsVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: DsVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<DsVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<DsVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = DsVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use ds_shell::env::DsBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&DsBuildEnvironment::Production);

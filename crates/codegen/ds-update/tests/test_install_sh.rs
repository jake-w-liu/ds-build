...
//!
//! > After any install attempt, `$BIN_DIR/ds` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `ds-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../ds-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> ds installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".ds").join("downloads");
    let bin = home.join(".ds").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("ds-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/ds-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/ds` from disk and re-run it: the active ds must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_ds_runs(home: &Path) {
    let link = home.join(".ds").join("bin").join("ds");
    assert!(link.is_symlink(), "ds must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("ds symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active ds must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active ds must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("DS_BIN_DIR", home.join(".ds").join("bin"))
        .env("DS_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one ds installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_ds_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_ds_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // ds always runs (new good binary on success, previous-good on
        // rejection).
        assert_active_ds_runs(home.path());
    }
}

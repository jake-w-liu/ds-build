...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `ds update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `ds update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `ds-0.1.tmp`
...
use ds_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use ds_update::version::installed_on_disk_version;

/// Assert the active `~/.ds/bin/ds` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("ds");
    assert!(link.is_symlink(), "ds must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active ds symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("ds-{version}-{platform}"),
        "active ds must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active ds must pass the smoke-test");
}

/// Lay down a managed-install layout in the test DS_HOME:
/// `bin/ds -> ../downloads/ds-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("ds-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `ds update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.ds/bin/ds` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.ds/downloads cleanup), the probe must report None — otherwise
...
// shared one `ds-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/ds-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("ds-{version}-{platform}"));
        assert_eq!(

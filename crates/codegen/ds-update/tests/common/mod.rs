...
//! its own `OnceLock<DS_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the DS_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes DS_HOME once per binary
...
/// Returns a process-wide test `DS_HOME`, initialized exactly once per test
/// binary. Once initialized, `ds_config::ds_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("DS_INSTALLER");
            std::env::remove_var("DS_MANAGED_BY_NPM");
            std::env::remove_var("DS_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `DS_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_ds_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("DS_TEST_VERSION", v) };
}
...
/// Minimal [`ds_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> ds_update::UpdateConfig {
    ds_update::UpdateConfig {
...
/// Backdate every file in `DS_HOME/downloads` by ~2 hours.
///

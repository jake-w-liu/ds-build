...
//!
//! > `~/.ds/bin/ds` resolves to a binary that passes the smoke-test, OR it
...
use ds_update::auto_update::install_internal_from_base;
...
/// (`ds` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `ds` should resolve to after an install attempt.
...
/// to both `ds` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p ds-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("DS_BLITZ_ITERS")
        .ok()

...
//!
//! These tests touch global process state — `DS_HOME` (a `OnceLock` in
//! `ds-config`), `DS_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `DS_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the DS_HOME isolation used in other
//! integration tests.
...
use ds_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_ds_home() {
    let _ = test_home();
    reset();
...
/// Write a `DsVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `DsVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under DS_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every ds launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_ds_version env override
//
// The function honors `DS_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_ds_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("DS_TEST_VERSION", "9.9.9");
    }
    let v = ds_update::version::get_installed_ds_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("DS_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        ds_update::version::get_installed_ds_version()
    };

    unsafe {
        std::env::set_var("DS_TEST_VERSION", "0.0.0-test");
    }
    let overridden = ds_update::version::get_installed_ds_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("DS_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = ds_update::version::get_installed_ds_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("DS_TEST_VERSION", "not-a-version");
    }
    let v = ds_update::version::get_installed_ds_version();

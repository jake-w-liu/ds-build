...
use super::*;
use ds_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
/// legacy `ds.cli/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!(
        ...
        "Compare GPU vendors", "source" : "conversation", "_meta" : { "ds.cli/session" : {
        "kind" : "chat" } } }] }
    );
...
fn picker_keeps_old_conversation_past_cutoff() {
    let payload = serde_json::json!(
        ...
        "_meta" : { "ds.cli/session" : { "kind" : "chat" } } }] }
    );
    let entries = parse_session_picker_entries(&payload);
...
fn picker_drops_local_with_missing_updated_at() {
    let payload = serde_json::json!(
        { "sessions" : [{ "sessionId" : "local_no_ts", "cwd" : "/Users/me/ds", "summary"
        : "no timestamp", "source" : "local" }] }
    );
    let entries = parse_session_picker_entries(&payload);
    assert!(entries.is_empty(), "local rows still require a parseable updatedAt");
}
/// Untitled ds.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!(
            { "sessions" : [], "_meta" : { "ds.cli/partial" : { "conversations" : true,
            "reason" : reason } } }
        )
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!(
        { "sessions" : [], "_meta" : { "ds.cli/partial" : { "conversations" : false } } }
    );
...
fn parse_kill_outcome_reads_result_envelope() {
    use ds_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use ds_workspace::session::git::RestoreDegree;
...
/// Spawn a fake ACP agent that counts `ds.cli/yolo_mode_changed`
/// notifications. Exits when the channel closes.
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<ds_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let ds_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
                if args.request.method.as_ref() == "ds.cli/yolo_mode_changed" {
...
/// Redirect `DS_HOME` to a tempdir for test isolation.
fn setup_ds_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use ds_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        ds_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        ds_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_ds_home_in_tempdir();
    ...
    assert_eq!(
        ...
        "ACP `ds.cli/yolo_mode_changed` notification must fire exactly once \
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use ds_acp_lib::AcpAgentMessage;
    use ds_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "ds.cli/marketplace/action" => {
                        ...
                        let req: ds_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: ds_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            ds_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "ds.cli/session/list");
...
/// Default flags produce no agent profile (uses ds-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (ds-build already has TaskTool).
#[test]
...
/// Neutralize `DS_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(DS_AGENT)]` (process-global env).
fn without_ds_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("DS_AGENT", "")
}
...
#[serial_test::serial(DS_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_ds_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "ds-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ds-build-plan-no-subagents");
...
/// --ask-user alone selects the ds-build-ask-user profile.
#[serial_test::serial(DS_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ds-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `ds-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce ds-build-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["ds.cli/session"] ["kind"], "chat");
    assert!(
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("ds.cli/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
            "ds.cli/cloud_existing_workspace".into(),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> ds_shell::session::SessionInfoResponse {
    use ds_shell::session::acp_types::{ContextInfo, SessionInfoData};
    ds_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("deepseek-v4-pro.5", Some("deepseek-v4-pro.3"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: deepseek-v4-pro.5 (deepseek-v4-pro.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("deepseek-v4-pro.5", Some("deepseek-v4-pro.3"), 1000, 10000);
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model: deepseek-v4-pro.5"));
    assert!(! text.contains("deepseek-v4-pro.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("deepseek-v4-pro.5", Some("deepseek-v4-pro.5"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: deepseek-v4-pro.5"));
    assert!(! text.contains("(deepseek-v4-pro.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("ds-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    assert_eq!(roster.model_id.as_deref(), Some("deepseek-v4-pro"));

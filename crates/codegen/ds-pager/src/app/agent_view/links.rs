...
impl AgentView {
    ...
    pub(super) fn push_promo_cta_link_span(
        &self,
        link_spans_out: &mut Vec<ds_ratatui_inline::LinkSpan>,
        banner_announcements: &[ds_announcements::RemoteAnnouncement],
    ...
    fn push_cta_link_span(
        ...
        {
            link_spans_out.push(ds_ratatui_inline::LinkSpan {
                row: rect.y,
                col_start: rect.x,
...
mod link_click_tests {
    ...
    fn open_prompt_dropdown_suppresses_announcement_hide_click_target() {
        ...
        let critical = [ds_announcements::RemoteAnnouncement {
    ...
    fn header_upgrade_cta_rect_and_ctrl_o_override() {
        ...
        let reg = ActionRegistry::defaults();
        let cta = || {
            Some(ds_announcements::AnnouncementCta {
                label: Some("Upgrade Account".into()),
                url: Some("https://www.deepseek.com/promo".into()),
                caption: None,
            })
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let pinned = [ds_announcements::RemoteAnnouncement {
            id: Some("promo-pin".into()),
            severity: Some("promo".into()),
            message: Some("ZZPROMO".into()),
            dismissible: Some(false),
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let dismissible = [ds_announcements::RemoteAnnouncement {
    ...
    fn frame_occluder_over_banner_swallows_clicks_and_drops_cta_link_span() {
        ...
        assert_eq!(&*spans[0].url, "https://www.deepseek.com/promo");
        let outcome = agent.handle_input(&Event::Mouse(mouse_down(cta.x + 1, cta.y)), &reg);
        assert!(
            matches!(
                outcome,

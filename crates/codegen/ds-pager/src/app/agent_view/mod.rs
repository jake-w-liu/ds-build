...
impl McpInitProgress {
    ...
    ///   `ds.cli/mcp_initialized` clears the progress.
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + DeepSeek) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_ds_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<ds_hooks_plugins_types::MarketplacePluginEntry>,
...
/// (`ds.cli/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the DeepSeek stream keeps its own highwater
    /// ([`Self::last_applied_ds_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while DeepSeek lines are emitted direct-to-gateway, so a fresh DeepSeek
    ...
    /// DeepSeek-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_ds_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and DeepSeek paths; dropped updates (dedup,
    ...
    pub context_state: Option<ds_shell::session::ContextInfo>,
    ...
    /// (optimistic echo). The shell broadcasts `ds.cli/session/interjection` to
    ...
    pub(crate) deferred_session_mode: Option<ds_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `ds.cli/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`ds.cli/session/info` `agentName`).
    ...
    /// Armed when an `ds.cli/session/prompt_complete` broadcast arrives for the
    ...
    /// `ds.cli/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `ds.cli/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `ds.cli/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`ds.cli/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `ds.cli/follow_ups` that arrived for a turn that is NOT yet the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(ds_telemetry::events::CreditLimitChoice::PayAsYouGo);
            ds_telemetry::session_ctx::log_event(
                ds_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            ds_telemetry::session_ctx::log_event(
                ds_telemetry::events::DsProUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use ds_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        ds_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = ds_tools::util::ds_home::ds_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &ds_tools::implementations::ds_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `ds.cli/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `ds.cli/follow_ups`
    ...
    /// upstream by the `_meta["ds.cli/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `ds.cli/follow_ups` for a turn that

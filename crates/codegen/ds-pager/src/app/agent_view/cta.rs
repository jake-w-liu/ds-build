...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::PromptSuggestion {
            action: ds_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `ds.cli/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `ds.cli/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `ds.cli/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `ds.cli/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `ds.cli/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `ds.cli/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        ds_telemetry::session_ctx::log_event(
            ds_telemetry::events::PluginCtaConnectClicked {
        ...
        self.pending_effects
            .push(super::actions::Effect::InstallPluginFromCta {
                agent_id: self.session.id,
                session_id,
                source_url_or_path: ds_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
                    .to_string(),
                plugin_relative_path,
            });
    }
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> ds_hooks_plugins_types::MarketplacePluginEntry {

...
use ds_shell::sampling::types::ReasoningEffort;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: ds_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open ds.com in the browser for DS Pro subscription upsell.
    OpenDsUrl,
    /// Re-check subscription status via the shell's `ds.cli/auth/check_subscription`.
    CheckSubscription,
    /// Open an arbitrary URL in the system browser (with scheme validation).
    OpenUrl(String),
    /// Open ds.com managed connectors, appending session teamId when set.
    ...
    /// `prompt_id`. Routed to the agent as `ds.cli/queue/remove`;
    /// the resulting `ds.cli/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `ds.cli/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `ds.cli/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `ds.cli/queue/edit`; the rebroadcast of
    /// `ds.cli/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `ds.cli/queue/interject`;
    /// the `ds.cli/session/interjection` + `ds.cli/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`ds --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: ds_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(ds_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(ds_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(ds_hooks_plugins_types::MarketplaceAction),
    /// Add or update an MCP server via ds.cli/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<ds_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via ds.cli/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via ds.cli/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `ds.cli/bundle/status`.
    RequestBundleStatus,
    ...
    AnnouncementsOpenCta(ds_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"dsnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `ds dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.ds/config.toml` and route ACP
/// `ds.cli/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: ds_workspace::foreign_sessions::EnabledForeignSessionSources,
        ds_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `ds.cli/session/list` as `query` (chat
    ...
    /// `ds.cli/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `ds.cli/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `ds.cli/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.ds/config.toml`.
    ...
    /// Persist a typed setting to `~/.ds/config.toml`. On failure,
    ...
    /// `ds.cli/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `ds.cli/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `ds.cli/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `ds.cli/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// fire-and-forget `ds.cli/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`ds.cli/session/info`).
    ...
    /// Fetch MCP server list from the shell (ds.cli/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (ds.cli/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    },
    /// Fetch hooks list from the shell (ds.cli/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (ds.cli/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (ds.cli/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via ds.cli/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Install a plugin from the inline CTA via `ds.cli/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `ds.cli/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `ds.cli/mcp/list`,
    ...
    /// the same `ds.cli/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via ds.cli/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via ds.cli/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via ds.cli/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via ds.cli/session/info.
    ShowSessionInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        show_resolved_model: bool,
    },
    /// Fetch and display detailed context usage via ds.cli/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch current bundle cache status via `ds.cli/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `ds.cli/bundle/entry/get`.
    ...
    /// Send raw note to ds.cli/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via ds.cli/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    },
    /// Request a session recap via the ds.cli/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via ds.cli/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `ds.cli/session/interjection`
    ...
    /// registry (`~/.ds/active_sessions.json`).
    ...
    /// Call `ds.cli/session/fork` to create a peer session that resumes
    ...
    /// Fetch billing/credit usage from the agent's `ds.cli/billing` extension.
    ...
    /// Send an ACP `ds.cli/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `ds.cli/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`ds-build-0.1` when the
...
/// Outcome of an `ds.cli/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<ds_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["ds.cli/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["ds.cli/partial"]`), surfaced
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<ds_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `ds.cli/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `ds.cli/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<ds_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<ds_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `ds.cli/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<ds_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<ds_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<ds_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<ds_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<ds_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<ds_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    /// `ds.cli/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<ds_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `ds.cli/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `ds.cli/session/fork` failed. The placeholder agent stays in
    ...
    GateRefreshed {
        settings: Option<ds_shell::util::config::RemoteSettings>,
    },
    ...
    /// Shell suggestions loaded from ACP `ds.cli/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `ds.cli/suggestPrompt`.

...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `ds.cli/runningPromptId` from the reload response: the turn another
...
/// [`decide`](ds_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("DS_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = ds_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(ds_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = ds_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match ds_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = ds_config::env_bool("DS_PLUGIN_CTA")
    ...
    // switch; `DS_VOICE_MODE` overrides for local dev (env > remote > default on).
    // Free/X Basic still hit DS Pro upsell via tier gates (separate).
    let voice_mode_enabled = crate::app::resolve_voice_mode_enabled(
        ds_config::env_bool("DS_VOICE_MODE"),
        remote_settings.as_ref().and_then(|s| s.voice_mode_enabled),
    );
    app.apply_voice_mode_enabled(voice_mode_enabled);
    app.session_picker_grouped = std::env::var("DS_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            ds_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the ds.com method from the advertised list
            let ds_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "ds.com");
            if let Some(method) = ds_com {
                ...
                // No ds.com method available, use the first method as fallback
        ...
        // fail-closed: do not invent ds.com / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    ds_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<ds_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == ds_shell::agent::auth_method::DEEPSEEK_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = ds_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = ds_shell::config::load_merged_requirements();
    let user_config = ds_shell::config::load_from_disk().ok();
    let managed_config = ds_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match ds_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = ds_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = ds_voice::VoiceConfig::from_config_table(table);
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to ds-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = ds_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        ds_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        ds_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        ds_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use ds_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = ds_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let ds_home = ds_tools::util::ds_home::ds_home();
            app.tip = ds_shell::util::tips::pick_and_advance(&app.tips, &ds_home);
        }
    }

    let hints = ds_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.ds/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        ds_shell::util::config::clamped_display_permission_mode(
            ds_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        ds_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = ds_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = ds_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // DS_SCROLL_SPEED/_MODE/_LINES + DS_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<ds_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`ds "fix the bug"`). When
    ...
    // `ds dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `DS_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("DS_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(ds_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart ds");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.ds/config.toml [ui]`
                // rather than `~/.ds/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> ds_shell::agent::config::UiConfig {
    use ds_shell::agent::config::UiConfig;
    let Ok(root) = ds_shell::config::load_effective_config() else {

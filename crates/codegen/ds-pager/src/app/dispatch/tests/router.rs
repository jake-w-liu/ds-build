...
fn critical_announcement(id: &str) -> ds_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> ds_announcements::RemoteAnnouncement {
    ds_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(ds_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://www.deepseek.com/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `DS_TEST_OPEN_URL_FILE` seam, from every
...
#[serial_test::serial(DS_TEST_OPEN_URL_FILE)]
#[test]
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    use ds_telemetry::events::AnnouncementCtaSurface;
    let url_file = std::env::temp_dir().join(format!("ds-cta-open-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    unsafe { std::env::set_var("DS_TEST_OPEN_URL_FILE", &url_file) };
    ...
    for surface in [
        ...
        assert!(
            opened().lines().any(|l| l == "https://www.deepseek.com/promo-open"),
    ...
    unsafe { std::env::remove_var("DS_TEST_OPEN_URL_FILE") };
    let _ = std::fs::remove_file(&url_file);
}
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.ds/skills/pick-best/SKILL.md", }
    );

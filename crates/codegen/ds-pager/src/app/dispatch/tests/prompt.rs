...
/// Turn end with prompt suggestions enabled fires the `ds.cli/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `ds-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `ds-build-0.1`
...
/// `ds.cli/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `DS_SUGGESTIONS` semantics

...
fn cta_entry(name: &str, status: &str) -> ds_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: ds_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `ds_home()` (OnceLock-cached;
/// do not rely on setting `DS_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = ds_shell::util::ds_home::ds_home();
    let encoded = ds_shell::util::ds_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `ds.cli/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: ds_tools::implementations::ds_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    ds_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    ds_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = ds_acp_lib::AcpArgs {

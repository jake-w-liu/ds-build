...
fn plugins_list_response() -> ds_hooks_plugins_types::PluginsListResponse {
    use crate::views::extensions_modal::test_plugin_info;
    ds_hooks_plugins_types::PluginsListResponse {
        plugins: vec![
            test_plugin_info(
                "user-tool",
                Some(ds_hooks_plugins_types::PluginOrigin::UserDs),
            ),
            test_plugin_info(
                "claude-tool",
                Some(ds_hooks_plugins_types::PluginOrigin::UserClaude),
            ),
        ],
    }
}

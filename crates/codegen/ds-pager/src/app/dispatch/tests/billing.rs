...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("ds_heavy")));
    assert!(is_max_tier(Some("DS Heavy")));
    assert!(is_max_tier(Some("SUPERDS_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("ds_pro")));
...
fn is_max_tier_handles_mixed_case_and_whitespace() {
    assert!(is_max_tier(Some("DsPro_Heavy")));
    assert!(is_max_tier(Some("ds_pro heavy")));
    assert!(is_max_tier(Some("SUPERds HEAVY")));
}

#[test]
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("ds_pro_heav")));
    assert!(!is_max_tier(Some("ds_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_ds_pro() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("ds_pro"), "got: {url}");
    assert!(url.contains("referrer=ds-build"), "got: {url}");
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): DS Build usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("ds_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("ds_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ds_telemetry::events::DsProUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            "Upgrade to DS Heavy",
            "Get the most out of DS Build. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use ds_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option DS Pro
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ds_telemetry::events::DsProUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with DS Pro.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to DS Pro");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to DS Heavy");

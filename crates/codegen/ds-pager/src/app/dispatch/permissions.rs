...
///
/// Special case for [`ds_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.ds/config.toml`, and fire the
/// `ds.cli/yolo_mode_changed` ACP notification. See the option-id constant
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == ds_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != ds_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(ds_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())

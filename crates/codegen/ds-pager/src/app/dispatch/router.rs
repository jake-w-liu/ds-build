...
use super::auth::{
    dispatch_cancel_login, dispatch_chatgpt_login, dispatch_chatgpt_logout, dispatch_login,
    dispatch_logout, dispatch_submit_auth_code, dispatch_switch_account,
};
use super::billing::dispatch_open_ds_pro_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(ds_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, ds_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(ds_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            ...
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(ds_telemetry::events::AnnouncementCtaClicked {
                    id: promo.id.clone(),
                    source: surface,
                });
        ...
        Action::ChatgptLogin => dispatch_chatgpt_login(app),
        Action::ChatgptLogout => dispatch_chatgpt_logout(app),
        Action::SwitchAccount => dispatch_switch_account(app),
        Action::CheckSubscription => vec![Effect::CheckSubscription { verify: None }],
        Action::OpenDsUrl => dispatch_open_ds_pro_url(app),
        Action::OpenUrl(url) => {
            use crate::terminal::hyperlinks::SchemeFilter;
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(ds_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let ds_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {

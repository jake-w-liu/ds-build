...
/// Show session info: fetch via ds.cli/session/info and display in scrollback.
///
...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    if app.is_zdr {
        ...
        lines.push("  Usage and code data may be used by DeepSeek to improve the product.");
        lines.push("");
        lines.push("  Use /privacy opt-out to enable privacy mode.");
    }

    lines.push("");
    lines.push("  Learn more: https://www.deepseek.com/legal");
    let text = lines.join("\n");
...
/// (persists via ACP ext-request, NOT `~/.ds/config.toml`).
...
/// Show context info: fetch via ds.cli/session/info and display rich breakdown.
///
...
///
/// When the remote settings `ds_build_usage_redirect_url` flag is set (delivered via

...
//! `ds.cli/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `ds.cli/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `ds.cli/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`ds.cli/session/interjection`) is

...
use ds_telemetry::events::{DsProUpsell, DsProUpsellClicked};
...
/// Whether the user is at the highest subscription tier (DS Heavy).
///
...
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://www.deepseek.com/ds_pro?referrer=ds-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://www.deepseek.com?_s=usage";
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(ds_shell::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free DS Build usage limit for now. Get DS Pro for much higher limits, or try again later: https://www.deepseek.com/ds_pro?referrer=ds-build";
...
///
/// **`max_tier = true`** (positively identified as DS Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        ds_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using DS Build",
            CreditLimitCardAction::PurchaseCredits,
            ds_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            ds_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(ds_telemetry::events::CreditLimitUpsellShown {
            surface: ds_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(ds_telemetry::events::CreditLimitUpsellShown {
        surface: ds_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            ds_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `DsProUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_ds_pro_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the DS Pro upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_ds_pro_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the DS Pro upsell modal. Controls the heading
...
fn open_ds_pro_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            DsProUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with DS Pro.",
            DsProUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(ds_telemetry::events::DsProUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to DS Pro".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to DS Heavy".into(),
            description: "Get the most out of DS Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /ds_pro page lists
...
/// `ds.cli/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<ds_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<ds_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_ds_pro_url(app: &mut AppView) -> Vec<Effect> {
    log_event(DsProUpsellClicked {
        source: DsProUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://www.deepseek.com/ds_pro?referrer=ds-build");
    // Funnel attribution: tag CLI-originated DS Pro upsell clicks
    // with `referrer=ds-build`, matching the OAuth consent flow and
    // ds.cli/cli marketing links. Applied even when the URL came from
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "ds-build");

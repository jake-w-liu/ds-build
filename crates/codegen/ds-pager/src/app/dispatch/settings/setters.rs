...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = ds_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == ds_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = ds_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
/// `DS_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut ds_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = ds_shell::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn set_auto_dark_theme(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        // No prior config: fall back to DsNight (the default).
        .unwrap_or_else(|| crate::theme::ThemeKind::DsNight.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
...
pub(in crate::app::dispatch) fn set_auto_light_theme(
    ...
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        .unwrap_or_else(|| crate::theme::ThemeKind::DsDay.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
        Some(c) if c != crate::theme::ThemeKind::Auto.display_name() => c,
        _ => {
            tracing::error!(
...
pub(in crate::app::dispatch) fn set_default_model_inner(
    ...
    // the next `ds.cli/models/update` roundtrip.
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "DS Build")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / DS_CHAT_MODE) catalogs use opaque `/rest/modes`
...
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = ds_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();

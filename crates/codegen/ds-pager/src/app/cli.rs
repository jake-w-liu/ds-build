...
pub enum Command {
    /// Run DS without the interactive UI
    Agent(Box<AgentArgs>),
    /// Manage DeepSeek API key credentials (CodeWhale-style)
    Auth {
        #[command(subcommand)]
        command: AuthCommand,
    },
    ...
    Login {
        ...
        #[arg(long, hide = true)]
        legacy: bool,
        /// Use DS OAuth via api.deepseek.com.
...
Examples:
  ds wrap docker exec -it my-container bash
  ds wrap kubectl exec -it my-pod -- bash

See ~/.ds/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `DS_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.ds/config.toml` or when the `DS_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}

/// Subcommands for `ds auth` (DeepSeek API key management).
#[derive(Debug, Clone, Subcommand)]
pub enum AuthCommand {
    /// Save a DeepSeek API key to `~/.ds/config.toml` (and auth.json).
    ///
    /// Reads from `--api-key`, `--api-key-stdin`, or prompts on the TTY when
    ...
    Set {
        ...
        #[arg(long = "api-key-stdin", conflicts_with = "api_key")]
        api_key_stdin: bool,
    },
    /// Show whether a DeepSeek API key is configured (never prints the secret).
    Status {
        /// Emit machine-readable JSON.
        #[arg(long)]
        json: bool,
    },
    /// Report set / not set (alias of status without JSON).
    Get,
    /// Delete the saved key from config.toml and auth.json.
    Clear,
}
...
/// Targets a running leader process by PID (used by `ds workspace`).
...
pub struct AgentArgs {
    ...
    /// Override the public DeepSeek API base URL.
    #[arg(long = "ds-api-base-url")]
    pub ds_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "ds: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("ds: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the DS WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "ds-ws-origin")]
    pub ds_ws_origin: Option<String>,
    #[arg(long = "ds-ws-url")]
    pub ds_ws_url: Option<String>,
}
...
pub struct LeaderArgs {
    ...
    /// Defer the ds.com relay WebSocket until the first headless IPC client
...
///
/// Delegates entirely to [`ds_version::display_cli_version`] so semver and
/// git commit cannot drift from other entry points (`ds version --json`,
/// startup banner).
fn version_with_channel() -> &'static str {
    use std::sync::OnceLock;
    static V: OnceLock<String> = OnceLock::new();
    V.get_or_init(|| ds_version::display_cli_version(ds_update::channel_label()))
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.ds/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `DS_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            ds_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `ds -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "DS_SANDBOX", value_name = "PROFILE")]
    ...
    /// Sticky: records `[ui] screen_mode = "minimal"` in ~/.ds/config.toml
    /// so future plain `ds` invocations open in minimal mode too.
    #[arg(long = "minimal")]
    pub minimal: bool,
    /// Open in the standard fullscreen TUI, overriding a sticky minimal
    /// preference. Sticky counterpart of --minimal: records
    /// `[ui] screen_mode = "fullscreen"` in ~/.ds/config.toml so future
    /// plain `ds` invocations open fullscreen again. Fullscreen-vs-inline
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.ds/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "DS_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `ds "fix the bug"` or `ds --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`DS_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                ds_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                ds_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<ds_sandbox::ProfileName>().ok()
                    != s.parse::<ds_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `ds "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["ds", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["ds"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["ds", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["ds", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["ds", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ds"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["ds", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["ds"]).expect("bare ds parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["ds", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["ds", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["ds", "fix the bug"]).expect("positional prompt parses");
        assert_eq!(args.initial_prompt(), Some("fix the bug"));
        assert!(args.command.is_none());
        assert!(args.single.is_none());
    }
    #[test]
    fn bare_ds_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["ds"]).expect("bare ds parses");
        assert_eq!(args.initial_prompt(), None);
    }
    #[test]
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args = PagerArgs::try_parse_from(["ds", "  spaced  "]).expect("padded prompt parses");
        assert_eq!(args.initial_prompt(), Some("spaced"));
        let blank = PagerArgs::try_parse_from(["ds", "   "]).expect("blank prompt parses");
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["ds", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["ds"]).expect("bare ds parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["ds", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["ds", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["ds", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["ds", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["ds", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["ds", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["ds", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["ds", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["ds", "agent", "--effort", "max", "stdio"])

...
//! and DeepSeek session extension notifications (`ds.cli/session_notification` and
//! replay-path `ds.cli/session/update`).
...
use ds_acp_lib::AcpClientMessage;

use super::actions::Effect;
use ds_shell::extensions::notification::{
    SessionNotification, SessionUpdate as DsSessionUpdate, is_reauthable_failure,
};
use ds_shell::tools::todo::todo_item_from_plan_entry;
use ds_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The DeepSeek stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the DeepSeek dedup in
...
/// Handle an DeepSeek extension notification.
///
/// Dispatches on method string:
/// - `ds.cli/session_notification` / `ds.cli/session/update` → per-agent session updates
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "ds.cli/follow_ups" => handle_follow_ups(notif, app),
        "ds.cli/task_backgrounded" => handle_task_backgrounded(notif, app),
        "ds.cli/task_completed" => handle_task_completed(notif, app),
        "ds.cli/models/update" => handle_models_update(notif, app),
        "ds.cli/settings/update" => handle_settings_update(notif, app),
        "ds.cli/sessions/changed" => handle_sessions_changed(notif, app),
        "ds.cli/queue/changed" => handle_queue_changed(notif, app),
        ...
        "ds.cli/session/prompt_complete" => handle_prompt_complete(notif, app),
        "ds.cli/session/interjection" => handle_interjection(notif, app),
        "ds.cli/monitor_event" => handle_monitor_event(notif, app),
        "ds.cli/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "ds.cli/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "ds.cli/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "ds.cli/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "ds.cli/announcements/update" => handle_announcements_update(notif, app),
        "ds.cli/git_head_changed" => handle_git_head_changed(notif, app),
        "ds.cli/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "ds.cli/mcp/tools_changed" | "ds.cli/mcp_initialized" => handle_mcp_tools_changed(notif, app),
        "ds.cli/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "ds.cli/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `ds.cli/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse ds.cli/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `ds.cli/queue/changed` right after it).
...
fn handle_ext_method(ext: ds_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "ds.cli/ask_user_question" => handle_ask_user_question(ext, app),
        "ds.cli/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {

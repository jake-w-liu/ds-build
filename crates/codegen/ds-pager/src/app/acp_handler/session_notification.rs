use super::*;
use ds_shell::sampling::error::rate_limited_user_message;
...
/// Replay gate shared by the ACP and DeepSeek session-update paths. Returns `true`
...
/// Handle `ds.cli/session_notification` and replay-path `ds.cli/session/update`.
///
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match &session_notif.update {
        DsSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        DsSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        DsSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        DsSessionUpdate::ScheduledTaskDeleted { .. } => {
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: ds.cli/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_ds_event_seq,
            "ds.cli/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
        ref update @ (DsSessionUpdate::AutoCompactStarted { .. }
        | DsSessionUpdate::AutoCompactCompleted { .. }
        | DsSessionUpdate::AutoCompactFailed { .. }
        | DsSessionUpdate::AutoCompactCancelled { .. }
        | DsSessionUpdate::RetryState(_)
        | DsSessionUpdate::ImageDropped { .. }
        | DsSessionUpdate::MemoryFlushCompleted { .. }
        | DsSessionUpdate::MemoryDreamCompleted { .. }
        | DsSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let DsSessionUpdate::AutoCompactCompleted { tokens_after, .. } = update {
                refresh_context_used(agent, *tokens_after);
                agent.todo.update_todos(Vec::new());
            }
            changed
        }
        DsSessionUpdate::ImageCompressed {
            ref images,
            ref message,
        } => apply_image_compressed(agent, images, message),
        DsSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            ..
        ...
        DsSessionUpdate::HookAnnotation { message } => {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        ds_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        ds_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        ds_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        DsSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(ds_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        DsSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        DsSessionUpdate::SessionSummaryGenerated { session_summary } => {
            agent.generated_session_title =
                Some(crate::util::decode_html_entities(&session_summary).into_owned());
            true
        }
        DsSessionUpdate::SessionRecap { summary, auto } => {
        ...
        DsSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        DsSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        DsSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if ds_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        DsSessionUpdate::MemoryFiles { files } => {
        ...
        DsSessionUpdate::InteractionResolved { tool_call_id } => {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            && !meta.is_replay
        {
            agent.last_applied_ds_event_seq = Some(seq);
        }
...
/// Handle an DeepSeek session notification that targets a child (subagent) session.
///
...
pub(super) fn handle_child_session_notification(
    ...
    match update {
        DsSessionUpdate::AutoCompactStarted { .. }
        | DsSessionUpdate::AutoCompactCompleted { .. }
        | DsSessionUpdate::AutoCompactFailed { .. }
        | DsSessionUpdate::AutoCompactCancelled { .. }
        | DsSessionUpdate::RetryState(_) => {
            let compact_tokens = match &update {
                DsSessionUpdate::AutoCompactCompleted { tokens_after, .. } => Some(*tokens_after),
                _ => None,
            };
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(ds_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
        ref update @ (DsSessionUpdate::MemoryFlushCompleted { .. }
        | DsSessionUpdate::MemoryDreamCompleted { .. }
        | DsSessionUpdate::MemorySessionSaved { .. }) => {
            if let Some(child_view) = agent.subagent_views.get_mut(child_sid) {
                apply_session_event(
...
pub(super) fn apply_session_event(
    ...
    match update {
        DsSessionUpdate::AutoCompactStarted { percentage, .. } => {
        ...
        DsSessionUpdate::AutoCompactFailed { error } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[ds_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &ds_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                ds_telemetry::session_ctx::log_event(
                    ds_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
    ...
    if is_credit_limit {
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use ds_tools::types::SessionMode;

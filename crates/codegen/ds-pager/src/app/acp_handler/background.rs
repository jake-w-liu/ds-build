...
/// Handle `ds.cli/task_backgrounded` — a bash command transitioned to background.
///
...
pub(super) fn handle_task_backgrounded(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse ds.cli/task_backgrounded");
        return false;
    };
...
/// Handle `ds.cli/monitor_event` — background task or monitor emitted new output.
...
pub(super) fn handle_scheduled_task_fired(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let (task_id, prompt, human_schedule, next_fire_at) = match session_notif.update {
        DsSessionUpdate::ScheduledTaskFired {
            task_id,
            prompt,
            human_schedule,
            next_fire_at,
...
pub(super) fn handle_scheduled_task_deleted(
    ...
    let task_id = match session_notif.update {
        DsSessionUpdate::ScheduledTaskDeleted { task_id } => task_id,
        _ => return false,
    };
    let matched = match find_session_match(app, &session_notif.session_id) {
        Some(m) => m,
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let payload: serde_json::Value = match serde_json::from_str(notif.params.get()) {
        Ok(v) => v,
        Err(e) => {
            tracing::warn!(error = %e, "Failed to parse ds.cli/scheduled_task_inject_prompt");
            return false;
        }
    };
    let Some(session_id) = payload["sessionId"].as_str() else {
        tracing::warn!("ds.cli/scheduled_task_inject_prompt: missing or non-string sessionId");
        return false;
    };
    let Some(prompt) = payload["prompt"].as_str().filter(|s| !s.is_empty()) else {
        tracing::warn!("ds.cli/scheduled_task_inject_prompt: missing or empty prompt");
        return false;
    };
    ...
    // `ds.cli/scheduled_task_inject_prompt` notification is routed by the leader
...
pub(super) fn handle_git_head_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(params) = serde_json::from_str::<ds_workspace::session::git::GitHeadChanged>(
        notif.params.get(),
    ) else {
        return false;
    };
...
pub(super) fn handle_task_completed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse ds.cli/task_completed");
        return false;
    };

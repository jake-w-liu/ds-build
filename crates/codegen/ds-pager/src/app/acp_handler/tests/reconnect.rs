#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let id = AgentId(0);
        {
            ...
            agent.last_applied_ds_event_seq = Some(5);
            agent.begin_session_reload(1);
        }
        ...
        let _ = handle_ext_notification(&ds_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
        ...
        assert_eq!(
            app.agents[&id].last_applied_ds_event_seq,
            Some(30),
            "the live DeepSeek line advanced its highwater in-window"
        );
        ...
        assert_eq!(
            agent.last_applied_ds_event_seq,
            Some(5),
            "the DeepSeek highwater reverts with the transcript — left at 30 it would \
    ...
    /// DeepSeek updates dedup on their OWN per-session `eventId` highwater: a
    ...
    /// DeepSeek arms have no other dedup. Replay stays exempt.
    #[test]
    fn ds_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &ds_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 1);
        assert_eq!(app.agents[&id].last_applied_ds_event_seq, Some(10));
        ...
        assert_eq!(
            ...
            1,
            "a duplicate DeepSeek event must not push a second block"
        );
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
            Some("sess-xdup-10")
        ...
        assert!(handle_ext_notification(
            &ds_model_switch_notif("sess-xdup", "sess-xdup-11"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 2);
        assert_eq!(app.agents[&id].last_applied_ds_event_seq, Some(11));

        // Lower-stale re-delivery (an already-applied lower id re-sent by
        // the cursor tail, e.g. goal mode) is dropped too — `<=`, not just
        // equality.
        assert!(!handle_ext_notification(
            &ds_model_switch_notif("sess-xdup", "sess-xdup-9"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].scrollback.len(),
            2,
            "a stale lower-id DeepSeek event must not push a block"
        );
        assert_eq!(app.agents[&id].last_applied_ds_event_seq, Some(11));
    }

    /// An unhandled DeepSeek kind (the default `_` arm) leaves no trace, so it must
    ...
    #[test]
    fn unhandled_ds_update_does_not_advance_cursor_or_highwater() {
        let mut app = make_app_with_agent("sess-ig");
        let id = AgentId(0);

        assert!(!handle_ext_notification(
            &ds_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
            "an unhandled DeepSeek update must not advance the reconnect cursor"
        );
        assert_eq!(
            app.agents[&id].last_applied_ds_event_seq, None,
            "an unhandled DeepSeek update must not advance the dedup highwater"
        );

        // An applied kind (ModelAutoSwitched) advances both.
        assert!(handle_ext_notification(
            &ds_model_switch_notif("sess-ig", "sess-ig-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
        ...
        assert_eq!(app.agents[&id].last_applied_ds_event_seq, Some(8));
    }

    /// Split-highwater regression: a fresh direct-emitted DeepSeek id must NOT
    /// make a queued lower-id ACP chunk look stale. DeepSeek lines bypass the
    ...
    #[test]
    fn direct_ds_event_does_not_shoot_down_delayed_acp_chunk() {
        let mut app = make_app_with_agent("sess-split");
        let id = AgentId(0);

        // Direct DeepSeek emission stamped N+1 arrives first.
        assert!(handle_ext_notification(
            &ds_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
        assert_eq!(app.agents[&id].last_applied_ds_event_seq, Some(21));
        ...
        assert_eq!(
            app.agents[&id].last_applied_ds_event_seq,
            Some(21),
            "…and the ACP apply must not clobber the DeepSeek highwater either"
        );
    }
    ...
    /// DeepSeek extension session updates: replay-stamped ones are gated like ACP
    /// updates, and applied ones advance the reconnect cursor.
    #[test]
    fn ds_session_update_replay_gating_and_cursor() {
        fn model_switch_notif(meta: Option<serde_json::Value>) -> acp::ExtNotification {
            let payload = SessionNotification {
                session_id: acp::SessionId::new("sess-ds"),
        ...
        let mut app = make_app_with_agent("sess-ds");
        let id = AgentId(0);

        // Replay-stamped with no load in flight → dropped, nothing pushed.
        let replay_meta = serde_json::json!({ "isReplay": true, "eventId": "sess-ds-7" });
        assert!(!handle_ext_notification(
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            assert!(agent.scrollback.is_empty(), "unexpected DeepSeek replay dropped");
        ...
        assert_eq!(
            agent.last_seen_event_id.as_deref(),
            Some("sess-ds-7"),
            "applied DeepSeek updates advance the reconnect cursor"
        );
        assert!(agent.finish_session_reload(1, true));
        assert!(
            !scrollback_has_system_text(agent, "pre-outage content"),
            "an DeepSeek replay line counts as replay for the swap decision"
        );
        assert_eq!(
            agent.scrollback.len(),
            1,
            "the staged DeepSeek block is the new transcript"
        );
    }
    ...
    fn reconnect_finalize_reload_skips_adoption_when_terminal_in_replay() {
        ...
        let _ = handle_ext_notification(
            &ds_turn_completed_notif("sess-1", "p-run", "end_turn", true),
            &mut app,
        );
    ...
    /// Apply-only cursor rule (DeepSeek path): a `ModelChanged` the catalog can't
    ...
    fn ignored_model_changed_does_not_advance_cursor_applied_one_does() {
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            seed_models(agent, "deepseek-v4-flash", &["deepseek-v4-flash", "deepseek-v4-pro"]);
        }

        // Unknown model → ignored → both markers untouched.
        assert!(!handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "ds-99-unknown", "sess-1-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
        ...
        assert!(handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "deepseek-v4-pro", "sess-1-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),

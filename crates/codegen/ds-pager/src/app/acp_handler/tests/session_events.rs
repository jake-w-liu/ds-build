#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_compaction_started_sets_activity() {
        ...
        let update = DsSessionUpdate::AutoCompactStarted {
            tokens_used: 90000,
    ...
    fn apply_image_dropped_pushes_scrollback_block() {
        ...
        let update = DsSessionUpdate::ImageDropped {
            notes: notes.clone(),
        };
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `ds logout` /
    /// `ds login` guidance), not the generic re-auth prompt.
    #[test]
    ...
    fn apply_compaction_completed_defers_message_until_turn_end() {
        ...
        let update = DsSessionUpdate::AutoCompactCompleted {
    ...
    fn apply_unhandled_event_returns_false() {
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        let update = DsSessionUpdate::MemoryFlushStarted;
        assert!(!apply_session_event(&update, &mut session, &mut scrollback, false));
    }
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(ds_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views

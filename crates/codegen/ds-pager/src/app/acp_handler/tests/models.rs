#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: a machine-wide `ds.cli/models/update` broadcast
    ...
    fn models_update_preserves_active_agent_model() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_3 = acp::ModelId::new(std::sync::Arc::from("deepseek-v4-flash"));
        agent
            .session
            .models
            .available
            .insert(id_3.clone(), make_model_info("deepseek-v4-flash"));
        agent.session.models.current = Some(id_3);

        let notif = make_models_update_notif("deepseek-v4-pro", &["deepseek-v4-flash", "deepseek-v4-pro"]);
    ...
    fn models_update_uses_shell_default_when_agent_model_removed() {
        ...
        // ds-3 removed from catalog.
        let notif = make_models_update_notif("deepseek-v4-pro.3", &["deepseek-v4-pro.3", "deepseek-v4-pro.5"]);
        handle_models_update(&notif, &mut app);

        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("deepseek-v4-pro.3"),
    ...
    fn models_update_noop_when_agent_matches_shell_default() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_4 = acp::ModelId::new(std::sync::Arc::from("deepseek-v4-pro"));
        agent
            .session
            .models
            .available
            .insert(id_4.clone(), make_model_info("deepseek-v4-pro"));
        ...
        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("deepseek-v4-pro"),
            "app.models.current must be deepseek-v4-pro when agent and shell agree"
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
            agent
                ...
                .map(|id| id.0.as_ref()),
            Some("deepseek-v4-pro"),
            "agent model must remain deepseek-v4-pro"
        );
    }
    ...
    fn models_update_non_active_agent_uses_shell_fallback_not_active_model() {
...

        {
            ...
            let id_5 = acp::ModelId::new(std::sync::Arc::from("deepseek-v4-pro.5"));
            agent_b
                .session
                .models
                .available
                .insert(id_5.clone(), make_model_info("deepseek-v4-pro.5"));
            agent_b.session.models.current = Some(id_5);
        }

        // ds-5 removed from catalog.
        let notif = make_models_update_notif("deepseek-v4-pro", &["deepseek-v4-flash", "deepseek-v4-pro"]);
        handle_models_update(&notif, &mut app);
        ...
        // B's ds-5 was removed — must fall back to shell's deepseek-v4-pro, not A's ds-3.
    ...
    fn model_changed_updates_state_silently_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "deepseek-v4-pro", None);
    ...
    fn model_changed_dropped_when_model_unknown_to_catalog() {
        ...
        let notif = model_changed_ext("sess-1", "ds-99-unknown", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(
            !changed,
    ...
    fn model_changed_applies_reasoning_effort_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "deepseek-v4-pro", Some("high"));
    ...
    fn model_changed_dropped_for_unknown_session_id() {
        ...
        let notif = model_changed_ext("sess-OTHER", "deepseek-v4-pro", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(!changed);

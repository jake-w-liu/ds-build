...
/// Handle `ds.cli/settings/update` — remote settings refreshed on `/new`.
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(update) = serde_json::from_str::<PagerSettingsUpdate>(notif.params.get()) else {
        tracing::warn!("Failed to parse ds.cli/settings/update");
        return false;
    };
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        ds_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = ds_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = ds_shell::config::load_effective_config().ok();
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/DS Pro
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(ds_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("DS_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(ds_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        ds_shell::config::load_merged_requirements(),
        ds_shell::config::load_from_disk().ok(),
        ds_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = ds_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = ds_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = ds_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use ds_shell::util::config::resolve_tips;
    ...
    tracing::info!("settings updated via ds.cli/settings/update");
    true
}
...
/// `ds.cli/yolo_mode_changed` notification the agent maps to
...
/// Handle `ds.cli/sessions/changed` — the leader broadcasts roster
...
pub(super) fn handle_sessions_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) = serde_json::from_str::<crate::app::roster::RosterChanged>(notif.params.get())
        ...
        tracing::warn!("Failed to parse ds.cli/sessions/changed");
        return false;
    };
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<ds_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `DS_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[ds_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = ds_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = ds_announcements::filter_expired(merged);
    ...
    if ds_announcements::prune_hidden_announcement_ids(
...
) -> Option<ds_announcements::RemoteAnnouncement> {
...
/// Deserialization type for the `ds.cli/settings/update` notification payload.
///
/// This is intentionally a separate struct from `SettingsUpdateNotification` in
/// `ds-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `ds-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    // remote_settings also emits gen-ordered `ds.cli/announcements/update`

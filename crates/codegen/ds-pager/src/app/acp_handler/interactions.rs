...
/// Handle `ds.cli/ask_user_question` ext-method.
///
...
pub(crate) fn handle_ask_user_question(
    ext: ds_acp_lib::AcpArgs<acp::ExtRequest>,
    app: &mut AppView,
) -> bool {
    ...
    if let Some(mut old_qv) = agent.question_view.take() {
        ...
        if let Some(ref kind) = old_qv.local_kind {
            ...
            let cmd = match kind {
                ...
                LocalQuestionKind::FreeUsageUpsell { .. } => "DS Pro upsell",
...
/// Handle an `ds.cli/exit_plan_mode` ext_method request.
///

...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + ds_home OnceLock in the shared lib test binary); run: cargo test -p ds-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<ds_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_ds_home().join("sessions");

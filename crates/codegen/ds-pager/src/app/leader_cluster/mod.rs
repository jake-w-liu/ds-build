...
//!
//! Env sandboxing follows this crate's `serial(DS_HOME)` idiom; note
//! `ds_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_ds_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `DEEPSEEK_API_KEY`,
//! `DS_LEADER_SOCKET`, `DS_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `ds_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p ds-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The ds home the agent actually persisted under: `ds_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_ds_home() -> PathBuf {
    ds_config::ds_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same DS_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(DS_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = ds_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("DS_HOME", ds_home.path()),
            crate::test_util::EnvVarGuard::set("DS_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("DS_DEEPSEEK_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("DEEPSEEK_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("DS_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("DS_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("DS_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (ds_version), not
        ...
        // ds-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: ds-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same DS_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `DS_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    ds_ws_url: String::new(),
                    ds_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
        ...
        if !self.authenticated {
            let _: acp::AuthenticateResponse = bounded(
                "authenticate",
                acp_send(
                    acp::AuthenticateRequest::new(acp::AuthMethodId::new("ds.api_key"))

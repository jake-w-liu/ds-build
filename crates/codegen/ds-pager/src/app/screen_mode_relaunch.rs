...
pub(crate) const DS_SCREEN_MODE_ENV: &str = "DS_SCREEN_MODE";
...
/// `ds "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `ds "fix the bug"`). Must not re-fire
...
///
/// Includes [`DS_SCREEN_MODE_ENV`] so the resume hits the same override path
/// as a successful `exec` — plain `ds --resume <id>` (or even
/// `ds --minimal --resume <id>`) is not enough after `/fullscreen` when
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{DS_SCREEN_MODE_ENV}={mode} ds {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(DS_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));

    let mode_label = screen_mode_env_value(want_minimal);
    eprintln!("Reopening session in {mode_label} mode…");
...
/// Parse a [`DS_SCREEN_MODE_ENV`] value (pure; unit-tested directly).
///
/// | Value | Mode |
/// |---|---|
...
pub(crate) fn parse_screen_mode(value: Option<&str>) -> Option<super::ScreenMode> {
    match value {
        ...
        Some("fullscreen") | Some("full") => Some(super::ScreenMode::Fullscreen),
        _ => None,
    }
}

/// Consume the one-shot screen-mode override env (see [`DS_SCREEN_MODE_ENV`]).
///
...
/// (tool shells, workers, nested `ds` invocations) would inherit a forced
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(DS_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(DS_SCREEN_MODE_ENV) };
    }
    parse_screen_mode(raw.as_deref().and_then(OsStr::to_str))
}

/// The user's *explicit* minimal-vs-fullscreen preference (unit-tested), or
/// `None` when none was expressed so the caller can apply a default (e.g. the
/// JediTerm/Windows mouse-leak auto-minimal). `Some(true)` = minimal.
///
/// Priority: CLI flag > config.toml `[ui] screen_mode` (the sticky value
/// written by `--minimal`/`--fullscreen` and the `/minimal`//`/fullscreen`
/// relaunches) > legacy pager.toml `[terminal] minimal`.
///
/// `config_screen_mode` reuses [`parse_screen_mode`]'s vocabulary
/// (`minimal` / `fullscreen` / `full`); an unset or unparseable value falls
/// through to the legacy pager.toml bool. A `fullscreen` preference means
/// "not minimal" — whether the run lands in alt-screen Fullscreen or Inline
/// stays with the alt-screen policy in [`resolve_screen_mode`], so a sticky
/// preference never permanently overrides the auto-inline environment
/// heuristics (Zellij, tmux control mode, `alt_screen = never`).
...
/// Pure screen-mode resolution used at startup (and unit-tested).
///
/// Priority: one-shot env override > `--minimal`/config minimal > alt-screen policy.
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["ds", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["ds", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `ds --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn keeps_equals_form_and_short_model_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "-m", "deepseek-v4-pro", "--cwd=/tmp/proj", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `ds --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
            args(&["ds", "--resume", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry DS_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "DS_SCREEN_MODE=fullscreen ds --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "DS_SCREEN_MODE=minimal ds --minimal --resume abc-sid"
        );
    }
    ...
    fn preference_cli_flag_beats_config_and_legacy() {
        // `--minimal` wins over a sticky fullscreen and vice versa.
        assert_eq!(
    ...
    fn preference_config_screen_mode_beats_legacy_pager_toml() {
        // Sticky config value applies without flags…
        assert_eq!(
            effective_minimal_preference(false, false, Some("minimal"), false),
            Some(true)
        );
        // …and a sticky "fullscreen" overrides a legacy pager.toml minimal.
        assert_eq!(
            effective_minimal_preference(false, false, Some("fullscreen"), true),
            Some(false)
        );
        // "full" alias accepted (same vocabulary as DS_SCREEN_MODE).
        assert_eq!(
    ...
    #[test]
    fn preference_unset_or_invalid_config_falls_back_to_legacy() {
        assert_eq!(
            effective_minimal_preference(false, false, None, true),
            Some(true)
        );
        // No flag, no config, no legacy bool = no preference (caller defaults).
        assert_eq!(
            effective_minimal_preference(false, false, None, false),
            None
        );
        // Unparseable values are ignored, not treated as fullscreen.
        assert_eq!(

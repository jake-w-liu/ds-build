...
//!
//! - Request/response `ds.cli/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `ds.cli/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `ds.cli/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `ds.cli/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `ds.cli/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `ds-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `ds.cli/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `ds.cli/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> ds_shell::agent::roster::RosterEntry {
        use ds_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `ds.cli/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("deepseek-v4-pro"));
        assert!(e.yolo);
    ...
    /// `ds.cli/sessions/changed` round-trip: serialize the agent's `RosterChanged`

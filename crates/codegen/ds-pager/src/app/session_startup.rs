...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain DS session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: ds_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
/// Build `ds.cli/session/fork` params shared by TUI effects and headless.
///
...
pub fn fork_session_params(
    ...
    let source_cwd = ds_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = ds_shell::util::ds_home::ds_home().join("sessions");
    let encoded = ds_shell::util::ds_home::encode_cwd_dirname(&cwd_str);
...
/// Parse `newSessionId` from an `ds.cli/session/fork` ACP response body.
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    ds_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are ds.com conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = ds_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'ds' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct ds.com calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &ds_shell::agent::config::Config,
) -> std::sync::Arc<ds_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(ds_shell::auth::AuthManager::new(
        &ds_shell::util::ds_home::ds_home(),
        agent_config.ds_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.ds_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if ds_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = ds_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = ds_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = ds_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use ds_shell::agent::session_registry_client::SessionRegistryClient;
    use ds_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = ds_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: ds_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["ds"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["ds", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["ds", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["ds", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["ds", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["ds"])).chat_mode);
    }

...
//!
//! Pager-side cache of what `ds-shell` reports from
//! `ds.cli/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `ds.cli/bundle/status` ACP responses.
...
/// Deserialized response from `ds.cli/bundle/status`.
...
/// Deserialized response from `ds.cli/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]

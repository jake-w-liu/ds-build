...
pub use ds_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
pub use cli::{
    AgentArgs, AgentCmd, AuthCommand, Command, HeadlessArgs, LeaderArgs, LeaderTargetArgs,
...
/// (`[ui] mouse_reporting_toggle` / `DS_MOUSE_REPORTING_TOGGLE`). Seeded once
...
/// Whether voice mode is available (GA default on; remote/`DS_VOICE_MODE` can
...
///
/// Precedence: `env` override (`DS_VOICE_MODE`) > `remote` (`voice_mode_enabled`)
/// > default **on**. Remote `Some(false)` is a kill switch; `None` means GA
/// default (on). Free-tier DS Pro upsell for `/voice` / Ctrl+Space is a
...
///
/// Uses [`ds_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `ds-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`ds-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&ds_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<ds_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<ds_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `ds`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<ds_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ds_tty_utils::redirect_native_stderr();
    ...
    let ds_com_config =
        match ds_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.ds_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                ds_shell::auth::DsComConfig::default()
            }
        };
    let refreshed_auth = ds_shell::auth::try_ensure_fresh_auth(&ds_com_config).await;
    let early_prefetch =
        ds_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    ds_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(ds_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = ds_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("DS_HUNK_TRACKER").ok();
    ...
    let launch_auto = ds_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    if args.minimal || args.fullscreen {
        ...
        if config_screen_mode != Some(want) {
            tokio::spawn(async move {
                if let Err(e) =
                    ds_shell::util::config::set_screen_mode(want.to_string()).await
                {
    ...
    cancel.cancel();
    ds_tty_utils::global_process_scope().kill_all();
    match result {
        Ok(run_result) => {
...
fn print_exit_resume_hint(session_id: &str, minimal: bool, w: &mut impl Write) {
    ...
    if minimal {
        let _ = writeln!(w, "  ds --minimal --resume {session_id}");
    } else {
        let _ = writeln!(w, "  ds --resume {session_id}");
    }
}
...
fn disable_mouse_paste_raw() {
    ds_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(ds_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    ds_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        ds_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(ds_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                ds_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = ds_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = ds_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        ds_crash_handler::disable_terminal_escape_restore();
    })
}
...
/// BEL/ESC (titles can arrive from ds.com conversation metadata) would
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - ds"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "ds");
        assert_eq!(terminal_title_string(""), "ds");
        assert_eq!(terminal_title_string("My chat"), "My chat - ds");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["ds-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["ds-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["ds-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["ds-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ds-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ds-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = ds_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["ds-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["ds-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["ds-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["ds-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["ds-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["ds-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["ds-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["ds-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["ds-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["ds-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["ds-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["ds-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["ds-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["ds-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["ds-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["ds-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_ds() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "ds");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "DS Build TUI",
                "",
                "Usage: ds [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["ds-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["ds-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,

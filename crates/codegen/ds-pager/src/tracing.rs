...
/// firehose (`DS_DEBUG_LOG` / `DS_LOG_FILE`).
pub use ds_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `ds-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use ds_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "ds_shell=info,ds_pager=trace,ds_tools=info,ds_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = ds_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = ds_telemetry::instrumentation::layer();
    let sampling_log_layer = ds_telemetry::sampling_log::layer();
    let hooks_log_layer = ds_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    ds_telemetry::debug_log::install_firehose(registry, "tui");

...
//!   `DS_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!   (`DS_MEMTRACE_THRESHOLD_MB`, default 1 GiB, then doubling), a full
...
//! ## Files
//!
//! `$DS_HOME/memtrace/<start-ts>-<pid>.jsonl` (+ `.1` after 4 MiB
...
//!
//! The lib cannot depend on jemalloc; `ds-pager-bin` installs:
...
/// Whether a trace sink is installed ([`start`] ran and `DS_MEMTRACE` is
...
/// Env: disable with `DS_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("DS_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("DS_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("DS_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$DS_HOME/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `DS_MEMTRACE=0`.
///
...
/// (`ds --version`, `ds trace …`) leave no files behind.

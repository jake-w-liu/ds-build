...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.deepseek.com`
...
use ds_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / ds.com / OIDC session tokens and
/// `DEEPSEEK_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<ds_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        ds_shell::auth::shared_api_key_provider(auth_manager),
    ))
}

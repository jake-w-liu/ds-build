...
pub enum SessionEvent {
    ...
    /// Message comes from agent via DsSessionUpdate::HookAnnotation.
    HookAnnotation {
        /// The hook message
        message: String,
    },
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "deepseek-v4-pro.5".into(),
            new_model_id: "ds-build".into(),
            reason: "Model \"deepseek-v4-pro.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"deepseek-v4-pro.5\" is no longer available. Switched to \"ds-build\"."
        );
    }

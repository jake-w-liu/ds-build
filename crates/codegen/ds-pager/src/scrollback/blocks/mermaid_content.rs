...
//! the generic [`CodeBlockSpan`](ds_markdown::CodeBlockSpan) API) and
...
use ds_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](ds_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](ds_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a ds_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `DsDay` is the only light theme; every other concrete theme (and the
/// `DsNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `ds_mermaid::MermaidTheme`; it
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_dsday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::DsDay),
            "DsDay is the light theme"
        );
        for dark in [
            ThemeKind::DsNight,

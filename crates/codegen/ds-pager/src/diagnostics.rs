...
pub fn summarize_warnings(warnings: &[TerminalWarning]) -> Option<crate::startup::StartupWarning> {
    ...
    let is_ssh = ds_shell::util::clipboard::is_remote_session();
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(ds_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .ds/sandbox.toml\nUser config: ~/.ds/sandbox.toml"
        ),
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && ds_shell::util::clipboard::wayland_data_control_supported(),
        is_wayland && ds_shell::util::clipboard::native_tool_name() == "wl-copy",
    )
}
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart DS.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart DS.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::DSDesktop));
    ...
    fn format_themes_env_line_lists_available_below_truecolor() {
        ...
        for level in [ColorLevel::Ansi256, ColorLevel::Basic, ColorLevel::None] {
            ...
            assert!(line.contains("dsnight") && line.contains("dsday"));

...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("DS_EVENT", event_str)
        .env("DS_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("DS_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'DS_EVENT=%s\\nDS_MESSAGE=%s\\nDS_SESSION_ID=%s\\n' \
             \"$DS_EVENT\" \"$DS_MESSAGE\" \"$DS_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("DS_EVENT=Turn complete"),
            "missing DS_EVENT: {content}"
        );
        assert!(
            content.contains("DS_MESSAGE=hello world"),
            "missing DS_MESSAGE: {content}"
        );
        assert!(
            content.contains("DS_SESSION_ID=sess-42"),
            "missing DS_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("DS_SESSION_ID"),
            "DS_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("DS_EVENT=Turn complete"));
        assert!(content.contains("DS_MESSAGE=test body payload"));
        assert!(content.contains("DS_SESSION_ID=test-session-123"));
    }
}

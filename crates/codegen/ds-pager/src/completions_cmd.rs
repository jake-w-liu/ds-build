//! `ds completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "ds" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("ds");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "ds", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "ds", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `ds worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:ds-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:ds-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("ds");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "ds", &mut buf);
    ...
    // so `ds worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]

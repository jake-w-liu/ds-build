...
use ds_shell::auth::{AuthManager, try_ensure_fresh_auth};
use ds_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom ds_api_base_url.
    // If the user has previously run the interactive `ds` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.ds_com_config).await;
    ...
    let client = ds_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = ds_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use ds_shell::session::merge::REMOTE_TIMEOUT;
            use ds_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = ds_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),

...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "DS 4 Fast").
    pub display: String,
...
pub struct SlashInvocation<'a> {
    /// Command token (e.g., "model" for "/model deepseek-v4-pro").
    pub token: &'a str,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model ds-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "ds-code-fast-1");
    }
    ...
    fn required_arg_command_blocks_without_args() {
        ...
        assert!(!is_command_complete("/model ", &reg));
        assert!(is_command_complete("/model deepseek-v4-pro", &reg));
    }
    ...
    fn cursor_inside_leading_command_with_args_filters_by_prefix() {
        ...
        // Cursor 3 in "/mod deepseek-v4-pro" clamps the query to "mo".
        ctrl.refresh(&state, "/mod deepseek-v4-pro", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn cursor_inside_recognized_command_with_args_opens_menu() {
        ...
        ctrl.refresh(&state, "/model deepseek-v4-pro", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    /// though `get()` blocks execution — invoking one shows the DS Pro

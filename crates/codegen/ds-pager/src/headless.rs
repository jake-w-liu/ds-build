//! Headless single-turn mode (`ds -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_ds_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use ds_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use ds_shell::agent::auth_method::AuthMethodKind;
use ds_shell::agent::config::Config as AgentConfig;
use ds_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use ds_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use ds_shell::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use ds_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_ds_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<ds_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use ds_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<ds_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = ds_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut ds_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    ds_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Save a DeepSeek API key with:\n  \
         ds auth set\n\n\
         Or set DEEPSEEK_API_KEY / paste api_key under [model.*] in ~/.ds/config.toml.\n\
         (OAuth: `ds login` or `ds login --device-code` if no browser.)"
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         ds auth set\n\
         # or:  printf 'sk-…' | ds auth set --api-key-stdin\n\
         # or:  export DEEPSEEK_API_KEY=sk-…\n\n\
         OAuth alternative: `ds login --device-code`."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !ds_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !ds_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &ds_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("ds.cli/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'ds models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    ds_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = ds_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `ds workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = ds_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        ds_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_ds_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("DS_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = ds_shell::active_sessions::register(
            ds_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(ds_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(ds_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Pending background work: bash/monitor via ds.cli/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on ds.cli/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.ds can't hang exit.
        let _ = ds_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = ds_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "ds.cli/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "ds.cli/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "ds.cli/session_notification" | "ds.cli/session/update" => {}
    ...
    let Ok(ds_notif) = serde_json::from_str::<DsNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match ds_notif.update {
        DsUpdate::AutoCompactStarted { percentage } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        DsUpdate::AutoCompactCompleted {} => match format {
            OutputFormat::StreamingJson => {
        ...
        DsUpdate::AutoCompactFailed { error } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        DsUpdate::AutoCompactCancelled {} => match format {
            OutputFormat::StreamingJson => {
        ...
        DsUpdate::AutoContinueCompleted { total_tokens } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        DsUpdate::ImageCompressed { message } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        DsUpdate::SubagentSpawned { subagent_id } => {
            return ExtEvent::SubagentSpawned { subagent_id };
        }
        DsUpdate::SubagentFinished { subagent_id, .. } => {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "ds.cli/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "ds.cli/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use ds_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            ds_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> ds_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `ds.cli/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "ds.cli/task_backgrounded",
            serde_json::json!({
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = ds_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("ds.cli/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(

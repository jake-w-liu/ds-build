...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "deepseek-v4-pro.5").
    ...
    /// not offer (e.g. `none` on deepseek-v4-pro.5) are rejected so we fail in the TUI
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("ds-build"));
        let mut state = ModelState::default();
        state.available.insert(
            id.clone(),

...
//!
//! Simplified to only support DsShell (in-process) mode.
...
/// Spawn a DsShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_ds_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<ds_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.ds/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        ds_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)

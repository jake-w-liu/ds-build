//! Release-safe FPS readout — `/debug fps`, `DS_FPS` on release builds.
//!
//! The full frame profiler (`render::frame_metrics`, `DS_FPS`) is compiled
...
//!
//! `DS_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` as
...
//! HUD from startup, so `DS_FPS=1` is never a silent no-op
//! ([`HONORS_DS_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `DS_FPS` env gate: only where the dev
/// `FrameMetrics` overlay is compiled out. In debug/dev builds the env keeps
/// feeding that overlay alone.
const HONORS_DS_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD. `DS_FPS` enables it at startup on
/// release binaries ([`HONORS_DS_FPS_ENV`]); `/debug fps` toggles it
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("DS_FPS").ok())
    }
    /// `env` is the raw `DS_FPS` value; the truthiness rule (nonempty and
    /// not `"0"`) matches `FrameMetrics` and `DS_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_DS_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `DS_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {

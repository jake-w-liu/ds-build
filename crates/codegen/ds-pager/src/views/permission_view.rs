...
use ds_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: ds_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `ds-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell DS what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        assert!(
            text.contains("\"body\": \"hello from ds\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p ds-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/ds-pager

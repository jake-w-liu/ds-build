...
use ds_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &ds_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &ds_hooks_plugins_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[ds_hooks_plugins_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<ds_hooks_plugins_types::PluginOrigin>,
) -> ds_hooks_plugins_types::PluginInfo {
    ds_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: ds_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: ds_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: ds_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a ds_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &ds_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use ds_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectDs) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserDs) => PluginGroup::new(2, "origin:user", "User"),
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [ds_hooks_plugins_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a ds_hooks_plugins_types::MarketplaceScanResult,
        plugin: &'a ds_hooks_plugins_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [ds_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[ds_hooks_plugins_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> ds_telemetry::events::ExtensionsModalTab {
        use ds_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(ds_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(ds_hooks_plugins_types::PluginsAction),
    ...
    AddMcpServer {
        name: String,
        config: Box<ds_shell::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open ds.com connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(ds_hooks_plugins_types::MarketplaceAction),
...
pub enum ModalMessage {
    ...
    MarketplaceConfirmation {
        message: String,
        action: ds_hooks_plugins_types::MarketplaceAction,
    },
}
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use ds_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use ds_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            ds_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use ds_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<ds_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<ds_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<ds_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<ds_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[ds_hooks_plugins_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let ds = ds_config::ds_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user ds home (DS_HOME-aware)
    // or a project-scoped `{cwd}/.ds/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User ds home (DS_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(ds.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.ds/<subdir>/<name>` anywhere in the path.
    ...
    // Project hooks
    if source_dir.ends_with("/.ds/hooks") || source_dir.contains("/.ds/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&ds) {
            let prefix = crate::util::display_ds_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            ds_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_ds_home(path) {
                    crate::util::display_user_ds_path("skills")
            ...
            ds_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            ds_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &ds_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use ds_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &ds_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[ds_hooks_plugins_types::ComponentItem]); 6] {
    use ds_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &ds_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &ds_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.ds/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // ds-home branch is DS_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.ds/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.ds/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("ds_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by ds.com")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "ds_com_linear"),
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> ds_tools::implementations::skills::types::SkillInfo {
        ds_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: ds_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = ds_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(ds_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> ds_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: ds_hooks_plugins_types::PluginOrigin,
    ) -> ds_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> ds_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(ds_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    ds_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    ds_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                ds_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> ds_hooks_plugins_types::HookInfo {
        ds_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: ds_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: ds_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> ds_hooks_plugins_types::MarketplaceScanResult {
        ds_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> ds_hooks_plugins_types::MarketplaceScanResult {
        ds_hooks_plugins_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.ds/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<ds_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> ds_hooks_plugins_types::MarketplacePluginEntry {
            ...
            ds_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                ds_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                ds_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(ds_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = ds_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> ds_hooks_plugins_types::ComponentItem {
        ds_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> ds_hooks_plugins_types::PluginComponents {
        ds_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(ds_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = ds_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<ds_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(ds_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use ds_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectDs, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserDs, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("DeepSeek Official".into()),
                    git_url: Some("https://example.com/r.git".into()),
                },
                5,
                "ds-mp:DeepSeek Official",
                "DeepSeek Official",
            ),
            (
                PluginOrigin::MarketplaceInstall {
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = ds_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = ds_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = ds_hooks_plugins_types::PluginScope::Config;
        assert_eq!(plugin_group(&config).key, "origin:config");

        let mut mp = make_plugin("mp-tool");
        mp.marketplace_source = Some("DeepSeek Official".into());
        let group = plugin_group(&mp);
        assert_eq!(group.key, "ds-mp:DeepSeek Official");
        assert_eq!(group.label, "DeepSeek Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            ds_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("DeepSeek Official".into());
        assert_eq!(plugin_group(&unknown).key, "ds-mp:DeepSeek Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserDs),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use ds_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserDs),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use ds_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserDs);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "DeepSeek Official (1 plugin)"), 1);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(ds_hooks_plugins_types::PluginComponents::default());

...
use ds_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use ds_shell::agent::config::AgentSelectionConfig;
use ds_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.ds` vs `{cwd}/.ds`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `DS_AGENT`,
    /// model `agentType`, then `ds-build`).
...
/// Skips internal variants (DsBuildConcise, DsBuildPlan,
/// DsBuildPlanNoSubagents, DsBuildAskUser, Codex, Opencode,
/// CursorExtended, DsBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = ds_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.ds/personas` and `{cwd}/.ds/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = ds_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".ds").join("personas")),
        (ConfigFileScope::User, ds_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match ds_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => ds_config::ds_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".ds").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.ds/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.ds/{subdir}` or `{cwd}/.ds/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let ds_home = ds_config::ds_home();
    let in_user = dunce::canonicalize(ds_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".ds").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    ds_shell::config::load_effective_config()
        .ok()
        .and_then(|root| ds_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in ds-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    ds_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        ds_agent::config::PromptMode::Extend => "extend",
        ds_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use ds_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".ds").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".ds").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.ds/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".ds").join("personas").join("gone.toml");

...
//! `ds.cli/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `ds.cli/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `ds.cli/suggest` response.
...
// (e.g. ds-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `ds.cli/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `ds.cli/suggest` response.
...
/// Wire `limit` for `ds.cli/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `DS_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `DS_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `ds.cli/suggest` request.
    /// Resolved at construction from `DS_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: ds_config::env_bool("DS_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: ds_config::env_bool("DS_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("DS_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `ds.cli/suggest` response arrives, with the text and

...
use ds_acp_lib::AcpResult;
use ds_markdown::StreamingMarkdownRenderer;
pub use ds_tools::implementations::ds_build::ask_user_question::{
...
/// driven by an ACP `ds.cli/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<ds_telemetry::events::CreditLimitChoice>,
    },
    /// DS Pro upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `DsProUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: ds_telemetry::events::DsProUpsell,
    },
    ...
    AgentTypeMismatch {
        ...
        effort: Option<ds_shell::sampling::types::ReasoningEffort>,
    },
}
...
///
/// Created when an `ds.cli/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `ds.cli/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `ds.cli/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the DS Pro upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> ds_tools::implementations::ds_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: ds_tools::implementations::ds_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the DS Pro upsell) have no "Other"

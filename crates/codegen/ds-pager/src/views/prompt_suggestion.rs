...
/// Env override for the whole feature: `DS_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "DS_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `DS_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "DS_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "ds-build-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `DS_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| ds_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `ds-build-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `ds-build-0.1` default): the shell

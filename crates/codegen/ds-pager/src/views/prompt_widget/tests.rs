#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/ds-test-image.png"));
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/ds-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod deepseek-v4-pro");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        let model_id = agent_client_protocol::ModelId::new(Arc::from("deepseek-v4-pro.5"));
        ...
        // Accept arg completion → should replace "gr" with "DS 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("DS 4.5"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: ds_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> ds_workspace::file_system::FuzzyMatchResult {
        ds_workspace::file_system::FuzzyMatchResult {
    ...
    /// after a turn ends with `ds.cli/suggestPrompt` resolved.

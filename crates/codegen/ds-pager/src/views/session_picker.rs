...
/// only one normal component (e.g., `/ds`), returns that component alone.
/// Does not perform tilde expansion — callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` → `"fw-1"`, `/ds` → `"ds"`, `/` → `"/"`.
///
...
impl SourceFilter {
    ...
    ///
    /// ds.com conversations carry `source == "conversation"` and live remotely,
...
pub(crate) fn capture_picker_selection(
    ...
    content_results: Option<&[ds_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[ds_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/ds"), "user-ds");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/ds"), "ds");
    }
    ...
    fn source_filter_matches() {
        ...
        // ds.com conversations are remote: visible under All + Remote, not Local.

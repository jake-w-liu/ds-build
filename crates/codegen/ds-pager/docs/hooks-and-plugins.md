...
DS Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.ds/hooks/`
- **Project hooks** — from `.ds/hooks/` in your repo
...
Plugin hooks automatically receive `DS_PLUGIN_ROOT` and `DS_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.ds/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `ds plugin marketplace add <source>`)
...
Sources land in `~/.ds/config.toml`:
...
Or in `~/.ds/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../ds-hooks/examples/README.md) — ready-to-use sample hooks

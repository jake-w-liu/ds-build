...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `DS_PLUGIN_ROOT` and `DS_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
- **MCP servers** -- a `.mcp.json` file of server configurations
- **LSP servers** -- a `.lsp.json` file of language server configurations

If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, DS discovers the components above from their standard directories.
...
|----------------------|-------------|
| `DS_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `DS_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

DS sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (DS also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

DS discovers plugins from these locations, in priority order:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session -- loaded for that session only | Trusted automatically |
| `--plugin-dir` (CLI flag, `ds agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.ds/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.ds/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

DS also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `DsOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `ds agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
ds plugin list [--json] [--available]   # List installed plugins (--available requires --json)
ds plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
ds plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
ds plugin update [<name>]               # Omit the name to update all plugins
ds plugin enable <name>
ds plugin disable <name>
ds plugin details <name>                # Show the plugin's component inventory
ds plugin validate [<path>]             # Validate plugin.json (default: current directory)
ds plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `ds plugin install <source>` without `--trust` and DS prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
ds plugin marketplace list [--json]
ds plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
ds plugin marketplace remove <url>      # Git URL or local path of a configured source
ds plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
ds plugin marketplace add my-org/team-plugins
ds plugin marketplace list
ds plugin install my-org/team-plugins --trust
ds plugin list
ds plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `ds plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.ds/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `ds plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.ds/pager.toml`:
...
Place this file at `~/.ds/settings.json` or `~/.claude/settings.json`.
...
DS trusts plugins from `~/.ds/plugins/` automatically. Project plugins in `.ds/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
ds plugin install <source> --trust
```
...
Run `ds inspect` to see every discovered plugin and what it provides:

```bash
ds inspect          # Show plugins with their skills, agents, hooks, and MCP servers
ds inspect --json   # Emit machine-readable JSON
```

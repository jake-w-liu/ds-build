...
Agent mode runs DS as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`ds -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `ds agent` command and apply to every mode. Pass them before the mode name, for example `ds agent --model ds-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `ds-build`).                    |
...
```bash
ds agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `DS_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, DS defines extension methods under the `ds.cli/` prefix for DeepSeek-specific functionality. These cover:

| Category                   | Prefix               | Examples                                         |
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `ds.cli/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `ds.cli/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `ds.cli/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
| **Search**                 | `ds.cli/search/*`      | `fuzzy/open`, `fuzzy/change`, `content`          |
| **Terminal**               | `ds.cli/terminal/*`    | `create`, `kill`, `output`, `wait_for_exit`      |
| **Session Management**     | `ds.cli/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
| **Conversation & History** | `ds.cli/*`             | `prompt_history`, `rewind/*`, `compact_conversation` |
| **Authentication**         | `ds.cli/auth/*`        | `get_url`, `submit_code`                         |
| **Feedback & Telemetry**   | `ds.cli/*`             | `feedback`, `telemetry/*`                        |

The tables here show representative methods in each category. The `ds.cli/*` set is DeepSeek-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| -------------------------- | ------------------------------------ |
| `ds.cli/search/fuzzy/status` | Fuzzy search results update          |
| `ds.cli/git/worktree/status` | Worktree creation progress           |
| `ds.cli/fs_notify`           | Filesystem change notification       |
| `ds.cli/fs/index`            | Full file index update               |
| `ds.cli/fs/index/delta`      | Incremental file index update        |
| `ds.cli/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `ds.cli/session/update`      | Session update (tool calls, content) |

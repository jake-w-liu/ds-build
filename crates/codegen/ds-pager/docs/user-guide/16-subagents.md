...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.ds/agents/` or `~/.ds/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.ds/personas/`; applied during subagent resolution |
...
| **Examples** | `ds-build`, `explore`, `plan` | `researcher`, `concise` |
...
DS Build discovers file-based personas from these locations, in priority order:

- `.ds/personas/*.toml` (project)
- `~/.ds/personas/*.toml` (user)
...
> **Note:** DS Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, DS Build resolves the effective model and reasoning effort in this order, highest priority first:
...
DS Build manages worktrees through the `ds.cli/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "ds-build"               # route explore to a specific model
```
...
# instructions_file = ".ds/personas/concise.md"  # or load from a file
```

DS Build also discovers roles from `.ds/roles/*.toml` and personas from `.ds/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

DS Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · ds-3) — Thinking`

...
Memory lets DS recall facts, decisions, and patterns from earlier sessions. DS indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each DS session starts fresh: the model knows nothing about previous sessions. When you enable memory, DS can:
...
3. `DS_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.ds/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.ds/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.ds/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

DS suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, DS saves a structured metadata summary to that session's daily log. The summary contains:
...
DS builds the summary from conversation metadata without an LLM call, without added latency. DS skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask DS to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.ds/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

DS records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, DS opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, DS shows `Memory saved to ~/.ds/memory/MEMORY.md`.

### Forget

Ask DS to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.ds/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what DS remembers:

```
> what do you remember?
```

DS searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.ds/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, DS confirms in the scrollback:

```
Memory saved to ~/.ds/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what DS has stored.
...
Dream also runs automatically. By default, DS checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, DS automatically searches memory for content relevant to the current project and injects it as context. This means DS starts with knowledge from previous sessions without a reminder.
...
DS searches memory automatically, but you can also trigger searches manually in the chat:
...
The `ds memory` command manages memory from the shell. It has one subcommand, `clear`:
...
ds memory clear --workspace

# Clear the global MEMORY.md
ds memory clear --global
...
To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.ds/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.ds/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, DS applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, DS uses the session's primary model. |
...
When a session memory is old, DS attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, DS watches `~/.ds/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `ds inspect` output.
2. Check the flag: `ds --experimental-memory` or `DS_MEMORY=1`.
3. Check for `--no-memory` or `DS_MEMORY=0` overriding your config.
...
```bash
ls ~/.ds/memory/
cat ~/.ds/memory/MEMORY.md
$EDITOR ~/.ds/memory/MEMORY.md
```

...
DS connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, DS uses models hosted by DeepSeek, and new sessions start with `ds-build`. Default models require no configuration. Authenticate with `ds login` or an API key, then start a session.
...
Set a persistent default in `~/.ds/config.toml`:
...
DS supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
Add custom model endpoints in `~/.ds/config.toml` under `[model.<name>]` sections:
...
env_key = "DEEPSEEK_API_KEY"                   # Env var holding the API key (optional; string or array)
...
DS resolves the API key in this order:
...
3. Your signed-in session token (from `ds login`), for a model with no `api_key`/`env_key` of its own
4. The `DEEPSEEK_API_KEY` environment variable (global fallback; DS also accepts `DS_CODE_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells DS when to trigger auto-compaction. When you override a known model, DS inherits that model's context window. When you define a new model and omit `context_window`, DS defaults to 200,000 tokens, so set it explicitly to match your provider.
...
When you override a built-in model, DS starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which DS sends verbatim.
...
Point DS at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `DS_MODELS_BASE_URL` | Yes | Base URL for inference. DS fetches the model list from `{base_url}/models`. |
| `DEEPSEEK_API_KEY` | Yes | API key sent as `Authorization: Bearer`. DS also accepts `DS_CODE_API_KEY`. |
| `DS_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, DS inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, DS uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `ds login` -- the API key is enough.
...
```bash
export DS_WEB_SEARCH_MODEL="deepseek-v4-pro"
```

If you point web search at a custom model, you also need a `[model.*]` entry so DS can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:

...
- **Shell builtins** -- handled by the agent backend (ds-shell)
- **Pager builtins** -- handled by the TUI frontend (ds-pager)
...
When the context window fills up, DS auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
choice persists to `[ui].screen_mode` in `config.toml`, so future plain `ds`
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `DS_MEMORY=1`. `/remember` is always available.
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, DS prompts you.
...
Set, manage, or check an autonomous goal. DS works toward the objective across turns and reports progress.
...
- `/docs web` opens https://api-docs.deepseek.com/ in the browser
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.ds/skills/commit/SKILL.md`, you can invoke it with:

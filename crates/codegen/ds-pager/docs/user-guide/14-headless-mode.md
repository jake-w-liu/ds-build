...
Headless mode runs DS non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
DS processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, DS emits an error object (process exit non-zero). Prompt-level
...
DS may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `ds -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
ds -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
ds -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
ds -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
ds -p "What's the secret number?" --resume abc123
```
...
ds -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
ds --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  ds -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
DS's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(ds -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
ds -p "Format all files" --yolo

# Run tests and fix failures
ds -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `DEEPSEEK_API_KEY`        | API key for authentication (required when no browser login)   |
| `DS_HOME`                    | Override config directory (default: `~/.ds`)                |
| `DS_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `DEEPSEEK_API_KEY` with an API key from [platform.deepseek.com](https://platform.deepseek.com):

```bash
export DEEPSEEK_API_KEY="ds-..."
ds -p "Run the test suite" --yolo
```
...
- **`DEEPSEEK_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`ds login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`ds login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `ds -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug ds -p "..." 2> debug.log`.

---

## Project Root Discovery

When DS starts, it discovers the project root by walking upward from `--cwd`
...
DS discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
DS stores data in `~/.ds` (override with `DS_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.ds` read-only:

- Pre-populate `auth.json` or use `DEEPSEEK_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export DEEPSEEK_API_KEY="ds-..."
export DS_DISABLE_AUTOUPDATER=1
ds -p "..." --no-auto-update
```
...
| `DS_DISABLE_AUTOUPDATER=1`    | Process   |
...
- Resume: `ds -p "continue" --resume "<id>"` or `ds -p "continue" --continue`

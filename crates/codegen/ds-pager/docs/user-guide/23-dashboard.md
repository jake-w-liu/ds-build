...
- **`ds dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.ds/config.toml` under `[keybindings]` like every other shortcut.
...
```
 DS Build · Dashboard — 4 agents · 2 awaiting
...
The state icon matches DS Build's sibling views (
`tasks_pane`):
...
Set `DS_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `ds --bg`") would list

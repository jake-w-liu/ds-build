...
DS reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `DEEPSEEK_API_KEY`, `DS_MEMORY`)
3. **config.toml** (`~/.ds/config.toml`)
...
Location: `~/.ds/config.toml`

If the file does not exist, DS uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); DS writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `DS_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). DS writes the change to
`[ui] vim_mode` in `~/.ds/config.toml` immediately and applies it to every
...
preference**: whichever mode you last chose explicitly is what a plain `ds`
...
You normally never edit this key by hand — DS writes it whenever you pass an
explicit `--minimal` / `--fullscreen` flag or run `/minimal` / `/fullscreen`.
A plain `ds` launch only reads it. A CLI flag always wins over the config
...
`DS_SCROLL_SPEED`, `DS_SCROLL_MODE`, `DS_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `DS_SCROLL_LINES`. Precedence: env var →
...
allowed_domains = ["docs.rs", "deepseek.com"]           # override the built-in allowlist
```
...
(`DS_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`DS_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
env_key = "DEEPSEEK_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Credential resolution: `api_key` > `env_key` > signed-in session token > `DEEPSEEK_API_KEY`.
...
MCP servers can also be configured per-project in `.ds/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.ds/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.ds/config.toml` (current dir) > `<repo-root>/.ds/config.toml` > `~/.ds/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `DS_MEMORY=1`).
...
[subagents.models]
explore = "ds-build"               # route to different models
```
...
`ds inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. DS writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.ds/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "ds"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `ds`. |
...
| DS Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, DS detects the terminal brand and selects the best
...
`$DS_EVENT`, `$DS_MESSAGE`, and `$DS_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'DS' -message '$DS_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$DS_MESSAGE' ntfy.sh/my-ds-alerts"
...
fires, try `condition = "always"` as a fallback. DS supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by DS; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= DS_EXTERNAL_OTEL)
...
| `DS_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `DS_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `DS_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `DS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `DS_OIDC_ISSUER` | OIDC issuer URL |
| `DS_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `DS_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `DS_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `DS_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `DS_AGENT` | Custom agent definition path or name |
| `DS_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `DS_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `DS_LOG_FILE` log and headless stderr output |
...
| `DS_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `DS_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `DS_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `DS_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.ds/config.toml` | Main configuration file |
| `~/.ds/pager.toml` | TUI appearance configuration |
...
| `~/.ds/lsp.json` | LSP server configuration (user-scoped) |
| `~/.ds/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.ds/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.ds/` within your repository:

| File | What it configures |
|------|--------------------|
| `.ds/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.ds/config.toml`) |
| `.ds/skills/` | Project-specific skills |
| `.ds/hooks/` | Project-specific lifecycle hooks |
| `.ds/agents/` | Project-specific agent definitions |
| `.ds/lsp.json` | LSP server configuration |
| `.ds/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.ds/lsp.json` | All projects |
| Project | `.ds/lsp.json` | Current repository |

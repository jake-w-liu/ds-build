...
Hooks let you run custom scripts or HTTP requests at key moments during a DS session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.ds/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.ds/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.ds/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match DS's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
DS injects the following variables into every hook process:

- `DS_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `DS_HOOK_NAME` — the full configured name of this hook
- `DS_SESSION_ID` — the current session identifier
- `DS_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `DS_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `DS_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `DS_PLUGIN_ROOT` and `DS_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`DS_PLUGIN_ROOT` and `DS_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/ds-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env DS itself sees).
...
`${DS_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.ds/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.ds/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.ds/logs`).

## More Examples

See the built-in examples in the `ds-hooks` crate:

- [Safe Shell Guard](../../../ds-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../ds-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../ds-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../ds-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.ds/hooks/` and customize.

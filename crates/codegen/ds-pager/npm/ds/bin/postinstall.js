#!/usr/bin/env node
// Runs once after npm install/update. Reads the ds binary from the
// matching per-platform optional dependency (@ds-official/ds-<platform>)
// and installs it to ~/.ds/bin/ using versioned filenames:
//
//   Unix:    ds-<version>  +  ds  (symlink)
//   Windows: ds-<version>.exe  +  ds.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.ds', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@ds-official/ds: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@ds-official/ds-${key}`;
    try {
...
if (!version) {
    console.error('@ds-official/ds: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@ds-official/ds: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@ds-official/ds: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running ds processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid ds-* matching ds-pager-*.
...
if (!platformDir) {
    console.error(`@ds-official/ds: platform package @ds-official/ds-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @ds-official/ds');
    process.exit(0);
}

installBinary('ds', platformDir, `ds${EXE}`);
cleanupOldVersions('ds');
cleanupOldVersions('ds-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.ds');
...
// Persist the npm registry so `ds update` and the trampoline use the same one.
const npmRegistry = process.env.DS_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @ds-official:registry',
...
// Set DS_INSTALL_COMPLETIONS=1 to auto-generate to ~/.ds/completions.
const DS_PATH = path.join(CANONICAL_DIR, `ds${EXE}`);
if (process.env.DS_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.ds', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'ds.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_ds');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(DS_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(DS_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.ds/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: ds completions bash > ~/.local/share/bash-completion/completions/ds');
    console.log('     ds completions zsh  > ~/.zsh/completions/_ds');
}

...
//   1. Brotli-compresses the built binary into `../ds-<platform>/bin/<bin>.br`
...
// (`@ds-official/ds`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw ds binary is
...
const dsRoot = process.env.DS_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'ds-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `ds-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] ds-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'ds',
            envVar: 'DS_DARWIN_ARM64',
            defaultSource: path.join(dsRoot, 'target', 'release', 'ds-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'ds',
            envVar: 'DS_DARWIN_X64',
            defaultSource: path.join(dsRoot, 'target', 'x86_64-apple-darwin', 'release', 'ds-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'ds',
            envVar: 'DS_LINUX_X64',
            defaultSource: path.join(dsRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'ds-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'ds',
            envVar: 'DS_LINUX_ARM64',
            defaultSource: path.join(dsRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'ds-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'ds.exe',
            envVar: 'DS_WIN32_X64',
            defaultSource: path.join(dsRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'ds-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'ds.exe',
            envVar: 'DS_WIN32_ARM64',
            defaultSource: path.join(dsRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'ds-pager.exe'),
        },
    ];

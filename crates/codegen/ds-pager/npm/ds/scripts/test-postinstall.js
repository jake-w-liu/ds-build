...
// postinstall.js and the bin/ds trampoline.
//
...
// ─── Extracted logic (mirrors postinstall.js and bin/ds exactly) ─────

/** Semver-aware descending sort for "ds-X.Y.Z" filenames. */
...
function installVersionedBinary(vendoredBinPath, version, canonicalDir) {
    const canonicalPath = path.join(canonicalDir, 'ds');
    fs.mkdirSync(canonicalDir, { recursive: true });

    const versionedName = `ds-${version}`;
...
function cleanupOldVersions(canonicalDir, currentVersionedName) {
    ...
    const versionedBinaries = entries
        .filter(e => e.startsWith('ds-') && !e.includes('.tmp.') && !e.includes('.link.') && e !== currentVersionedName)
...
/** Bootstrap canonical from vendored (same as bin/ds trampoline). */
...
test('creates versioned binary and symlink on fresh install', () => {
    ...
    try {
        const vendored = path.join(dir, 'vendored-ds');
        ...
        assert.strictEqual(target, 'ds-0.1.140');
...
test('upgrade swaps symlink and preserves old binary', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ds-0.1.141');
        ...
        const oldBinary = path.join(binDir, 'ds-0.1.140');
...
test('idempotent: reinstalling same version does not re-copy', () => {
    ...
    try {
        ...
        const versionedPath = path.join(binDir, 'ds-0.1.140');
...
test('symlink swap is atomic (no intermediate missing state)', () => {
    ...
    try {
        const binDir = path.join(dir, 'bin');
        const canonicalPath = path.join(binDir, 'ds');
...
test('handles upgrade from old-style regular file to versioned symlink', () => {
    ...
    try {
        ...
        // Simulate old installation: ds is a regular file
...
test('handles broken symlink (target deleted externally)', () => {
    ...
    try {
        ...
        const canonicalPath = path.join(binDir, 'ds');
        fs.symlinkSync('ds-0.1.99', canonicalPath);
...
test('three sequential upgrades: v1 -> v2 -> v3 all coexist', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.3');

        // All three versioned binaries still exist (no cleanup yet)
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.1')));
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.2')));
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.3')));
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup keeps N-1 version and removes older ones', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ds-0.1.138'), 'v138');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.139'), 'v139');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.140'), 'v140');
        // ds-0.1.141 is the current version (excluded from cleanup)
        fs.writeFileSync(path.join(binDir, 'ds-0.1.141'), 'v141');

        cleanupOldVersions(binDir, 'ds-0.1.141');

        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'N-1 should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.139')), 'N-2 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.138')), 'N-3 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup with only one old version keeps it', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'single old version should be kept');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'current should exist');
    } finally {
        cleanup(dir);
    }
...
test('cleanup with no old versions is a no-op', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'current should still exist');
        const entries = fs.readdirSync(binDir).filter(e => e.startsWith('ds-'));
...
test('cleanup ignores .tmp. and .link. files', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ds-0.1.141'), 'current');
        // Leftover temp files from a crashed install
        fs.writeFileSync(path.join(binDir, 'ds-0.1.140.tmp.12345'), 'crashed-tmp');
        fs.writeFileSync(path.join(binDir, 'ds.link.12345'), 'crashed-link');

        cleanupOldVersions(binDir, 'ds-0.1.141');

        // Temp files should not be touched by cleanup (they're filtered out)
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140.tmp.12345')), 'tmp file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'ds.link.12345')), 'link file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: 0.1.9 vs 0.1.10 (digit boundary)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ds-0.1.8'), 'v8');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.9'), 'v9');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.10'), 'v10');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.11'), 'v11');

        cleanupOldVersions(binDir, 'ds-0.1.11');

        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.11')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.10')), '0.1.10 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.9')), '0.1.9 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.8')), '0.1.8 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: major version boundary (0.x vs 1.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ds-0.9.99'), 'old');
        fs.writeFileSync(path.join(binDir, 'ds-1.0.0'), 'v1');
        fs.writeFileSync(path.join(binDir, 'ds-1.0.1'), 'current');

        cleanupOldVersions(binDir, 'ds-1.0.1');

        assert.ok(fs.existsSync(path.join(binDir, 'ds-1.0.0')), '1.0.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.9.99')), '0.9.99 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: minor version boundary (0.1.x vs 0.2.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ds-0.1.999'), 'old');
        fs.writeFileSync(path.join(binDir, 'ds-0.2.0'), 'v2');
        fs.writeFileSync(path.join(binDir, 'ds-0.2.1'), 'current');

        cleanupOldVersions(binDir, 'ds-0.2.1');

        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.2.0')), '0.2.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.999')), '0.1.999 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semverSortDescending: unit test comparator directly', () => {
    const input = ['ds-0.1.9', 'ds-0.1.10', 'ds-0.1.2', 'ds-1.0.0', 'ds-0.2.0'];
...
test('bootstrapCanonical creates versioned binary from vendored', () => {
    ...
    try {
        ...
        assert.strictEqual(result, path.join(binDir, 'ds'));
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'versioned binary should exist');
...
test('bootstrapCanonical is idempotent', () => {
    ...
    try {
        ...
        assert.strictEqual(
            fs.readFileSync(path.join(binDir, 'ds-0.1.140'), 'utf8'),
            'original-content',
...
test('bootstrapCanonical works when canonical already exists (different version)', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result), 'ds-0.1.141');
        // v1 should still exist
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'old version should still exist');
    } finally {
        cleanup(dir);
    }
});
...
test('full lifecycle: install, upgrade, cleanup', () => {
    ...
    try {
        ...
        cleanupOldVersions(binDir, 'ds-0.1.142');

        // Current (v3) + N-1 (v2) should exist; v1 removed
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.142')), 'v3 should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'v2 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'v1 should be removed');

        // Canonical symlink points to v3
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.142');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ds'), 'utf8'), 'v3');
    } finally {
        cleanup(dir);
    }
});
...
test('downgrade: installing older version than current', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.140');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ds'), 'utf8'), 'v1');

        // v2 should still exist (never delete old binaries during install)
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'v2 should still exist');
    } finally {
        cleanup(dir);
    }
});

test('non-ds files in bin dir are not touched by cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // DS versions
        fs.writeFileSync(path.join(binDir, 'ds-0.1.138'), 'old1');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.139'), 'old2');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.140'), 'current');

        cleanupOldVersions(binDir, 'ds-0.1.140');

        assert.ok(fs.existsSync(path.join(binDir, 'other-tool')), 'non-ds file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'README.md')), 'non-ds file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
// ═══════════════════════════════════════════════════════════════════════
// ds vs ds-pager Isolation Tests
// ═══════════════════════════════════════════════════════════════════════

console.log('\nds isolation isolation tests\n');

/**
 * Cleanup for a named binary (mirrors postinstall.js cleanupOldVersions).
 * Uses prefix + leading digit to avoid ds-* matching ds-pager-*.
 */
...
test('installing both ds and ds-pager creates independent symlinks', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'ds-binary');
        const vendoredPager = path.join(dir, 'vendored-pager');
        fs.writeFileSync(vendoredPager, 'pager-binary');

        installNamedBinary(vendored, 'ds', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'ds-pager', '0.1.141', binDir);

        // Both symlinks exist and point to correct targets
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.141');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds-pager')), 'ds-pager-0.1.141');

        // Both versioned files exist with correct content
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ds-0.1.141'), 'utf8'), 'ds-binary');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ds-pager-0.1.141'), 'utf8'), 'pager-binary');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of ds-* does not remove ds-pager-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Old ds versions
        fs.writeFileSync(path.join(binDir, 'ds-0.1.138'), 'old-ds-1');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.139'), 'old-ds-2');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.140'), 'old-ds-3');
        // Current ds
        fs.writeFileSync(path.join(binDir, 'ds-0.1.141'), 'current-ds');

        // ds-pager versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'ds', '0.1.141');

        // ds cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'current ds should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.140')), 'N-1 ds should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.139')), 'N-2 ds should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.138')), 'N-3 ds should be removed');

        // ALL ds-pager versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.138')), 'ds-pager-0.1.138 must survive ds cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.139')), 'ds-pager-0.1.139 must survive ds cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.141')), 'ds-pager-0.1.141 must survive ds cleanup');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of ds-pager-* does not remove ds-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });

        // ds versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'ds-0.1.138'), 'old-ds-1');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.139'), 'old-ds-2');
        fs.writeFileSync(path.join(binDir, 'ds-0.1.141'), 'current-ds');

        // Old ds-pager versions
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.140'), 'old-pager-3');
        // Current pager
        fs.writeFileSync(path.join(binDir, 'ds-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'ds-pager', '0.1.141');

        // ds-pager cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.141')), 'current pager should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.140')), 'N-1 pager should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-pager-0.1.139')), 'N-2 pager should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-pager-0.1.138')), 'N-3 pager should be removed');

        // ALL ds versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.138')), 'ds-0.1.138 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.139')), 'ds-0.1.139 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')), 'ds-0.1.141 must survive pager cleanup');
    } finally {
        cleanup(dir);
    }
});
...
test('full dual-binary lifecycle: install, upgrade, cleanup both', () => {
    ...
    try {
        ...
        // v1
        fs.writeFileSync(vendored, 'ds-v1');
        fs.writeFileSync(vendoredPager, 'pager-v1');
        installNamedBinary(vendored, 'ds', '0.1.140', binDir);
        installNamedBinary(vendoredPager, 'ds-pager', '0.1.140', binDir);

        // v2
        fs.writeFileSync(vendored, 'ds-v2');
        fs.writeFileSync(vendoredPager, 'pager-v2');
        installNamedBinary(vendored, 'ds', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'ds-pager', '0.1.141', binDir);

        // v3
        fs.writeFileSync(vendored, 'ds-v3');
        fs.writeFileSync(vendoredPager, 'pager-v3');
        installNamedBinary(vendored, 'ds', '0.1.142', binDir);
        installNamedBinary(vendoredPager, 'ds-pager', '0.1.142', binDir);

        // Cleanup both independently
        cleanupOldVersionsNamed(binDir, 'ds', '0.1.142');
        cleanupOldVersionsNamed(binDir, 'ds-pager', '0.1.142');

        // Current + N-1 for each
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-0.1.140')));

        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'ds-pager-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'ds-pager-0.1.140')));

        // Symlinks correct
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.142');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds-pager')), 'ds-pager-0.1.142');
    } finally {
        cleanup(dir);
    }
});
...
test('ds installs normally regardless of platform key', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored-ds');
        ...
        for (const platform of ['darwin-arm64', 'linux-x64', 'linux-arm64']) {
            const result = installNamedBinary(vendored, 'ds', '0.1.150', binDir);
            assert.ok(fs.existsSync(result.versionedPath), `ds should install for ${platform}`);
            assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ds-0.1.150');
        }
    } finally {
        cleanup(dir);
    }
...
test('ds-pager installs when vendored binary exists (darwin-arm64 path)', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendoredPager = path.join(dir, 'vendored-pager');
        ...
        const result = installNamedBinary(vendoredPager, 'ds-pager', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'pager versioned binary should exist');
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ds-pager-0.1.150');
...
test('Linux pager vendor files are not required for ds install', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        ...
        fs.writeFileSync(path.join(vendorBase, 'darwin-arm64', 'ds-pager'), 'mac-pager');
        ...
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-x64', 'ds-pager')));
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-arm64', 'ds-pager')));

        // ds install should succeed independently
        const dsVendored = path.join(dir, 'vendored-ds');
        fs.writeFileSync(dsVendored, 'ds-linux');
        const result = installNamedBinary(dsVendored, 'ds', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'ds should install without Linux pager');
    } finally {
        cleanup(dir);
    }
});

test('skipping pager install on Linux does not affect ds cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');

        // Install ds across two versions
        fs.writeFileSync(vendored, 'ds-v1');
        installNamedBinary(vendored, 'ds', '0.1.149', binDir);
        fs.writeFileSync(vendored, 'ds-v2');
        installNamedBinary(vendored, 'ds', '0.1.150', binDir);

        // Simulate Linux: only run ds cleanup, skip pager entirely
        cleanupOldVersionsNamed(binDir, 'ds', '0.1.150');

        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.150')), 'current ds should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ds-0.1.149')), 'N-1 ds should be kept');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ds')), 'ds-0.1.150');
...
test('canonical pager from non-npm install is preserved on Linux', () => {
    ...
    try {
        ...
        // Simulate pager installed by install-ds.sh (not npm)
        const pagerVersioned = path.join(binDir, 'ds-pager-0.1.150');
        fs.writeFileSync(pagerVersioned, 'installer-pager-binary');
        fs.chmodSync(pagerVersioned, 0o755);
        const pagerCanonical = path.join(binDir, 'ds-pager');
        fs.symlinkSync('ds-pager-0.1.150', pagerCanonical);

        // Run ds-only install + cleanup (simulating Linux postinstall)
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'ds-binary');
        installNamedBinary(vendored, 'ds', '0.1.150', binDir);
        cleanupOldVersionsNamed(binDir, 'ds', '0.1.150');
        ...
        assert.strictEqual(fs.readlinkSync(pagerCanonical), 'ds-pager-0.1.150');
    } finally {
        cleanup(dir);
    }
});

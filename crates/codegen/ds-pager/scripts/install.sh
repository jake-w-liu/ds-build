#!/bin/bash
#
# DS CLI installer — https://www.deepseek.com/cli/install.sh
#
# Auth: DS_DEPLOYMENT_KEY (takes precedence) or ~/.ds/auth.json from `ds login`.
# Env: DS_CHANNEL (stable|alpha|enterprise, default: stable), DS_BIN_DIR, DS_PROXY_URL
#
# Usage:
#   curl -fsSL https://www.deepseek.com/cli/install.sh | bash            # latest stable
#   curl -fsSL https://www.deepseek.com/cli/install.sh | bash -s 0.1.42  # specific version
#   DS_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://www.deepseek.com/cli/install.sh)
#
...
# Read a token from ~/.ds/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_ds_token() {
    local auth_file="$HOME/.ds/auth.json"
    local scope="$1"
...
# Resolve auth: DS_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://api.deepseek.com::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.ds.cli/sign-in"
AUTH_SOURCE=""

if [ -n "$DS_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_ds_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_ds_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.ds/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.ds/auth.json." >&2
    fi
fi
...
BASE_URL_PRIMARY="https://www.deepseek.com/cli"
BASE_URL_FALLBACK="https://storage.googleapis.com/ds-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.ds/downloads"
BIN_DIR="${DS_BIN_DIR:-$HOME/.ds/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${DS_CHANNEL:-stable}"

# Pick a working BASE_URL: try Cloudflare-fronted deepseek.com first, fall back to
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing DS $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing DS $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/ds-$platform"
artifact_base="${BASE_URL}/ds-${version}-${platform}"
    ...
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: DS is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in ds.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/ds.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded ds failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.ds/bin and ~/.ds/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/ds"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/ds and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.ds/completions/bash" "$HOME/.ds/completions/zsh"
"$BIN_DIR/ds" completions bash > "$HOME/.ds/completions/bash/ds.bash" 2>/dev/null || true
"$BIN_DIR/ds" completions zsh  > "$HOME/.ds/completions/zsh/_ds"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/ds" completions fish > "$HOME/.config/fish/completions/ds.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.ds/config.toml"
...
if [ -n "$DS_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${DS_PROXY_URL:-https://api.deepseek.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$DS_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.ds/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.ds/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.ds/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.ds/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "DS $version installed to $BIN_DIR/ds.exe" >&2
else
    echo "DS $version installed to $BIN_DIR/ds" >&2
fi
...
# Try to symlink into a directory already on PATH so ds works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/ds" "$candidate/ds"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/ds -> $BIN_DIR/ds" >&2
...
# Also update shell config so ~/.ds/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> ds installer >>>
fish_add_path $HOME/.ds/bin
# <<< ds installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> ds installer >>>
export PATH="$HOME/.ds/bin:$PATH"
fpath=(~/.ds/completions/zsh $fpath)
...
# <<< ds installer <<<'
    ...
    if grep -qs "ds installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> ds installer >>>/ { skip=1; next }
            /# <<< ds installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'ds' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'ds' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'ds' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.ds/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use ds from cmd.exe or PowerShell, add %USERPROFILE%\\.ds\\bin to your PATH." >&2
fi

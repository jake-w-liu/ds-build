...
//!
//! These tests exercise the real `ds-pager` binary through a PTY using
...
/// enterprise deploy report: with `DS_GOAL=1`, `/goal` must
...
/// explicitly off (`DS_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `DS_FOLDER_TRUST=1` and a git repo that
...
/// also pins `DS_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning

...
//!
//! Uses `ds-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.ds/auth.json` from `DS_AUTH_JSON` (path) or the
//! developer's `~/.ds/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p ds-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use ds_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.ds/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("DS_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.ds/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("DS_HOME".into(), ds.display().to_string()),
        ...
        ("TMUX".into(), "".into()),
        // Do not set DEEPSEEK_API_KEY — prefer OIDC entry in auth.json (pty_e2e pattern).
    ];
    // Pin the feature gate explicitly so the cycle is deterministic regardless
    // of the developer's shell. `DS_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (ds_config::
...
fn require_auth() -> bool {
    std::env::var("DS_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (ds-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "DS_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );

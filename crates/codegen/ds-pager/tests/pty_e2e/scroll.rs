...
use ds_pager_pty_harness::PtyHarness;
pub(crate) use ds_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`ds-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `ds-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `DS_SCROLL_SPEED`) appended to the controller's env.

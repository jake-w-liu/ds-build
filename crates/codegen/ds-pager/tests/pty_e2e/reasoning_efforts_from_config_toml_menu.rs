...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("deepseek-v4-pro.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.ds/config.toml` with a per-model reasoning-effort menu.
    let ds_home = content.home().join(".ds");
    std::fs::create_dir_all(&ds_home).expect("create .ds");
    // Quote the dotted model id: bare `[model.ds-4.5]` is TOML key-path syntax (model.ds-4.5), not the id "deepseek-v4-pro.5".
    std::fs::write(

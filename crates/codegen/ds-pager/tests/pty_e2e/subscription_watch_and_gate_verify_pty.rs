...
const PAID_TIER: &str = "DsProPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "DS Pro Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `ds_heavy` = live
/// `/user` string `DsProPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `DS_LOCAL_AUTH` dev issuer
/// (`http://localhost:22255`). Two reasons: `is_ds_oauth2_issuer()` accepts
...
/// call to api.deepseek.com (hermetic, no CI-network flake). Pair with
/// `DS_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let ds_home = content.home().join(".ds");
    std::fs::create_dir_all(&ds_home).expect("create temp .ds");
    std::fs::write(
...
#[ignore = "PTY e2e; run with cargo test -p ds-pager --test pty_e2e -- --ignored"]
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("deepseek-v4-flash")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("DS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
    ...
    content.server().set_models(vec![
        MockModel::new("deepseek-v4-flash"),
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://www.deepseek.com/ds_pro?referrer=ds-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("DS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );

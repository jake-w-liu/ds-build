//! End-to-end tests for the built `ds-pager` binary, driven through a PTY.
//!
//! Mirrors `ds-shell/tests/test_built_binary_e2e.rs`: every test is
...
//! 2. `DS_BINARY` env var (shared convention with the shell crate)
//! 3. `CARGO_BIN_EXE_ds-pager` (set by `cargo test` for in-tree runs)
...
//! lives in the [`ds_pager_pty_harness`] crate, not here, so its deps
//! never land in the production pager binary.
//!
//! Run locally:
//! ```bash
//! cargo test -p ds-pager --test pty_e2e -- --ignored --nocapture
//! ```
//!
//! Run against a pre-built CI artifact:
//! ```bash
//! PAGER_BINARY=./artifacts/ds-${VERSION}-linux-x86_64 \
//!   cargo test -p ds-pager --test pty_e2e -- --ignored --nocapture
//! ```

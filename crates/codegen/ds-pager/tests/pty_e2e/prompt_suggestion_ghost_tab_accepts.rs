...
/// turn and the turn-end `ds.cli/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`ds-build-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "DS_PROMPT_SUGGESTIONS");
    env.push(("DS_PROMPT_SUGGESTIONS".into(), "true".into()));

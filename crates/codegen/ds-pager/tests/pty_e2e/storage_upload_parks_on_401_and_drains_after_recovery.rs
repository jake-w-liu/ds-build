...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party DeepSeek OAuth (`is_ds_auth()`:
    // AuthMode::Oidc + DeepSeek issuer); the harness's DEEPSEEK_API_KEY is ApiKey mode
    ...
    env_refs.retain(|(k, _)| *k != "DS_TRACE_UPLOAD");
    env_refs.push(("DS_TRACE_UPLOAD", "true"));
    env_refs.push(("DS_TELEMETRY_TRACE_UPLOAD", "true"));
    env_refs.push(("DS_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"));
    ...
    //
    // Accounting (ds-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `DS_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`

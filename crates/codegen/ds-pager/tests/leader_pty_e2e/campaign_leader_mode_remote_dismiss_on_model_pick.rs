...
#[ignore = "PTY e2e; run with cargo test -p ds-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared DS_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let ds_home = content.home().join(".ds");
    std::fs::create_dir_all(&ds_home).expect("create DS_HOME");
    std::fs::write(
    ...
    // Session (OAuth) auth, not the harness's default DEEPSEEK_API_KEY: the
    ...
    let state_path = ds_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(ds_home.join("config.toml")).expect("read config.toml");
    assert!(

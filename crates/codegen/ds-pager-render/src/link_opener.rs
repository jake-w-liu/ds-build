...
//!
//! Extracted from the `OpenDsUrl` dispatch handler so that any
...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("DS_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "DS_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    ds_tools::util::detach_std_command(&mut command);
...
///
/// The returned command is TTY-guarded via [`ds_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    ds_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the DS Pro upsell flow to attribute clicks to `referrer=ds-build`,
/// matching the OAuth consent screen and ds.cli/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/ds session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://www.deepseek.com/ds_pro", "referrer", "ds-build");
        assert_eq!(out, "https://www.deepseek.com/ds_pro?referrer=ds-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://www.deepseek.com/ds_pro?referrer=other",
            "referrer",
            "ds-build",
        );
        assert_eq!(out, "https://www.deepseek.com/ds_pro?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://www.deepseek.com/ds_pro?heavy=1",
            "referrer",
            "ds-build",
        );
        assert_eq!(
            out,
            "https://www.deepseek.com/ds_pro?heavy=1&referrer=ds-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`ds.com/#ds_pro`); we still want the referrer attached.
        let out = ensure_query_param("https://www.deepseek.com/#ds_pro", "referrer", "ds-build");
        assert_eq!(out, "https://www.deepseek.com/?referrer=ds-build#ds_pro");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "ds-build");

...
//! `ds-ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
//! Multi-fire writes (native / tmux / OSC 52); user-facing success is [`trust`].

mod trust;

pub use ds_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(ds_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(ds_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `ds wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `ds-pager`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `ds` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `DS_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_DS_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `ds`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("DS_OSC52_SINK").is_some()
            || std::env::var_os("LC_DS_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if ds_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
pub fn resolve_clipboard_route(ctx: &TerminalContext) -> ClipboardRoute {
    ...
    ClipboardRoute {
        ...
        // upstream `ds wrap` sink is capturing our output and will forward
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = ds_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        let mut cmd = Command::new("tmux");
        cmd.args(["load-buffer", "-"])
            .stdin(Stdio::from(stdin))
            .stdout(Stdio::null())
            .stderr(Stdio::null());
        ds_tty_utils::detach_std_command(&mut cmd);
        let mut child = cmd.spawn()?;
        // Bounded wait: a wedged tmux server must not freeze the UI thread.
        let status = ds_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`ds_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        ds_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = ds_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match ds_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
/// Write text and return a toast; emits `ds-shell-clipboard_copy` when enabled.
...
fn log_clipboard_copy_event(
    ...
    if !ds_telemetry::client::is_enabled() {
        return;
    }
    ds_telemetry::session_ctx::log_event(ds_telemetry::events::ClipboardCopy {
...
#[cfg(any(test, feature = "test-support"))]
    ...
    ds_shared::clipboard::get_text().map_err(|error| {
...
#[cfg(any(test, feature = "test-support"))]
    ...
    if !ds_shared::clipboard::x11_display_env_present() {
        return None;
    }
    ds_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && ds_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    ds_telemetry::session_ctx::log_event(
        ds_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    ds_telemetry::session_ctx::log_event(ds_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match ds_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use ds_shared::clipboard::ImageData;
...
#[cfg(any(test, feature = "test-support"))]
    ...
    ds_shared::clipboard::clipboard_image_snapshot()
}
...
#[cfg(any(test, feature = "test-support"))]
    ...
    ds_shared::clipboard::clipboard_change_count()
}
...
#[cfg(any(test, feature = "test-support"))]
    ...
    ds_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(ds_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match ds_shared::clipboard::get_image() {
        Ok(img) => {

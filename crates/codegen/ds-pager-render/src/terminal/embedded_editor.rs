//! Detects whether ds is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` ds is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `ds-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).

...
pub enum TerminalName {
    ...
    JetBrains,
    /// DS Desktop (Electron app).
    #[strum(to_string = "DS Desktop")]
    DSDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` ds is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches ds, which
    ...
    pub fn telemetry_snapshot(&self) -> ds_telemetry::events::TerminalTelemetry {
        ...
        ds_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: ds_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> ds_shared::session::FeedbackTerminalInfo {
        use ds_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(ds_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `ds ssh` can
    // forward env vars.
...
/// `ds-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "dsdesktop" => Some(TerminalName::DSDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.ds/pager.toml` and

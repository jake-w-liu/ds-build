...
//! The default theme is DsNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! DsNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::DsNight, ThemeKind::DsDay];
    ...
    /// when quantized to 256 or 16 colors. DsNight uses neutral grays
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "dsnight" | "ds-night" | "dark" => Some(Self::DsNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "dsday" | "ds-day" | "light" | "day" => Some(Self::DsDay),
...
/// `"dsnight"` → `"DS Night"`). Falls back to `value` verbatim.
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.ds/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::DsNight => Self::dsnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::DsDay => Self::dsday(),
            ...
            // if reached, fall back to DsNight.
            ThemeKind::Auto => Self::dsnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `DS_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on dsnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., DsNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // DsNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    ds_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::DsNight.is_auto());
        assert!(!ThemeKind::DsDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::dsday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::dsnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::dsday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. DsNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::DsNight => Theme::dsnight(),
                ThemeKind::DsDay => Theme::dsday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_dsnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark DsNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::dsnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("dsnight"),
            Some(ThemeKind::DsNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::DsNight));
        assert_eq!(ThemeKind::from_name("dsday"), Some(ThemeKind::DsDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::DsDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("dsnight", ThemeKind::DsNight),
            ("ds-night", ThemeKind::DsNight),
            ("dark", ThemeKind::DsNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("dsday", ThemeKind::DsDay),
            ("ds-day", ThemeKind::DsDay),
            ("light", ThemeKind::DsDay),
            ("day", ThemeKind::DsDay),

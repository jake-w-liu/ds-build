...
/// in `config.toml`. When `None`, defaults to `DsNight` / `DsDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::DsDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::DsNight),
    }
}

...
pub use ds_config::ds_home;

/// Path to `$DS_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    ds_home().join("pager.toml")
}

/// User-facing label for the user ds directory (``~/.ds`` or ``$DS_HOME``).
///
/// Derived from resolved [`ds_home()`] vs `ds_config::default_ds_home()`,
/// not from whether `DS_HOME` is set in the environment.
pub fn display_ds_home_prefix() -> String {
    if ds_home() == ds_config::default_ds_home() {
        "~/.ds".to_string()
    } else {
        "$DS_HOME".to_string()
    }
...
/// User-facing path under [`ds_home()`], e.g. ``~/.ds/config.toml``.
pub fn display_user_ds_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_ds_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`ds_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let ds = ds_home();
    if let Ok(rest) = path_buf.strip_prefix(&ds) {
        let prefix = display_ds_home_prefix();
...
/// True when `path` is under user [`ds_home()`] (not project `{cwd}/.ds`).
pub fn is_under_user_ds_home(path: &Path) -> bool {
    path.starts_with(ds_home())
}
...
mod tests {
    ...
    #[test]
    fn display_ds_home_prefix_default_install() {
        if std::env::var("DS_HOME").is_ok() {
            return;
        }
        assert_eq!(display_ds_home_prefix(), "~/.ds");
    }

    #[test]
    fn display_user_ds_path_joins_relative() {
        let path = display_user_ds_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".ds") || path.contains("$DS_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_ds() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.ds/memory/MEMORY.md");

...
impl S3StorageClient {
    ...
    pub async fn batch_upload(
        ...
        let client = &self.client;
        let bucket = &*self.bucket;
        let results: Vec<ds_cli_proxy_types::BatchUploadResult> = futures::stream::iter(
            files,
        )
        .map(|(path, content, content_type)| async move {
            ...
            match upload_result {
                Ok(_) => ds_cli_proxy_types::BatchUploadResult {
                    path,
                    bucket: Some(bucket_owned),
                    status: ds_cli_proxy_types::BatchUploadStatus::Ok,
                    size: Some(size),
                    generation: None,
                    error: None,
                },
                Err(e) => {
                    ...
                    ds_cli_proxy_types::BatchUploadResult {
                        path,
                        bucket: Some(bucket_owned),
                        status: ds_cli_proxy_types::BatchUploadStatus::Error,
                        size: None,
                        generation: None,
...
mod tests {
    ...
    async fn batch_upload_all_succeed() {
        ...
        for r in &results {
            assert_eq!(
                r.status,
                ds_cli_proxy_types::BatchUploadStatus::Ok
            );
    ...
    async fn batch_upload_partial_failure() {
        ...
        let by_path: HashMap<&str, &ds_cli_proxy_types::BatchUploadResult> =
        ...
        assert_eq!(
            fail.status,
            ds_cli_proxy_types::BatchUploadStatus::Error
        );

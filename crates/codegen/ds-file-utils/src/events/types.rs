...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `ds-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `ds-shell::session::events`.
    ...
    /// discriminator constants in `ds-shell::session::events`.
    ...
    /// `ds-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `ds-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `ds.cli/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `ds.cli/interject`, or "Send now" on a
...
/// Telemetry mirror of `ds-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `ds-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `ds-shell`'s `GoalPauseReason`. The two types
...
mod tests {
    ...
    fn goal_strategist_fired_serializes_cadence_field() {
        ...
        assert_eq!(v["every"], 3);
        assert_eq!(v["model_id"], "deepseek-v4-pro");
    }

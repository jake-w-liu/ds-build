...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".ds", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn ds_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".ds")));
                assert!(!is_project_dir(&home.join(".ds/bin")));
            }
        }

        #[test]
        fn ds_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".ds-proxy-work")));
            }
        }

//! HTTP client for the DeepSeek sampling APIs.
//!
...
pub use ds_sampling_types::ApiBackend;

/// Process-level fallback for the `x-ds-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "ds-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "ds-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-ds-*` headers. Optional fields are skipped when empty/`None`.
struct DsRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl DsRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-ds-conv-id", self.conv_id)
            .header("x-ds-req-id", self.req_id)
            .header("x-ds-model-override", self.model_id)
            .header("x-ds-session-id", self.session_id)
            .header("x-ds-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-ds-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ds-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ds-user-id", id);
        }
        b
    }
}
...
/// Deserialize a Responses API SSE event, with a fallback for DeepSeek-specific
...
/// when the API emits the DeepSeek-specific `context_details` field.
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = ds_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "ds.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-ds-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-ds-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-ds-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-ds-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-ds-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-ds-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-ds-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    fn format_request_headers(
        ...
        req_headers.push(Self::format_header("x-ds-conv-id", x_ds_conv_id));
        req_headers.push(Self::format_header("x-ds-req-id", x_ds_req_id));
        req_headers.push(Self::format_header("x-ds-model-override", model_id));
        if include_accept {
    ...
    pub async fn chat_completion(
        ...
        let x_ds_conv_id = &payload.x_ds_conv_id.clone().unwrap_or_default();
        let x_ds_req_id = &payload.x_ds_req_id.clone().unwrap_or_default();
        ...
        let ds_headers = DsRequestHeaders {
            conv_id: x_ds_conv_id,
            req_id: x_ds_req_id,
            model_id: &model_id,
            session_id: payload.x_ds_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_ds_turn_idx.as_deref(),
            agent_id: payload.x_ds_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_ds_deployment_id.as_deref(),
            user_id: payload.x_ds_user_id.as_deref(),
        };
    ...
    pub async fn chat_completion_stream(
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_ds_conv_id, x_ds_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_ds_conv_id = request.x_ds_conv_id.as_deref().unwrap_or_default();
        let x_ds_req_id = request.x_ds_req_id.as_deref().unwrap_or_default();
        ...
        let ds_headers = DsRequestHeaders {
            ...
            req_id: x_ds_req_id,
            model_id: &model_id,
            session_id: request.x_ds_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_ds_turn_idx.as_deref(),
            agent_id: request.x_ds_agent_id.as_deref().unwrap_or_default(),
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_ds_conv_id, x_ds_req_id, &model_id, false);
            let server_message = parse_error_bytes(bytes.as_ref());
    ...
    /// gate that adds the opt-in `x-ds-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject DeepSeek-specific fields not in async-openai's CreateResponse type.
        ...
        let mut http_request = ds_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_ds_conv_id = request.x_ds_conv_id.clone();
        let x_ds_req_id = request.x_ds_req_id.clone();
        let x_ds_session_id = request.x_ds_session_id.clone();
...
mod tests {
    ...
    use ds_sampling_types::types::ChatRequestMessage;
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-DEEPSEEK-token-auth".to_string(), "ds-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the ds-shell agent product.
        assert!(ua.starts_with("my-client ds-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = ds_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "ds-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,

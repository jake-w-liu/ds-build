...
//! it" buckets.
//!
//! `ds-sampler` is intentionally decoupled from `ds-shell`
...
/// tools live in `ds-tools`
/// (`crates/codegen/ds-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `ds_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `ds_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `ds-shell/src/auth/manager.rs` to match, otherwise the

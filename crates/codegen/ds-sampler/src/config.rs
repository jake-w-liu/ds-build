...
//! `ds_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `ds_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-DS-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-ds-doom-loop-check` header on

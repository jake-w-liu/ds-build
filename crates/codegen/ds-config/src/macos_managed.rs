...
//! preference domain `ai.ds.cli` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "ai.ds.cli";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "ai.ds.cli:requirements_toml_base64";
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.ds.cli`)
...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("DS_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("DS_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${DS_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("DS_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("DS_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded

//! Filesystem locations for ds config files and binaries.
...
/// The default user ds directory (`~/.ds`, canonicalized) used when
/// `DS_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`ds_home()`] is the default without duplicating the computation.
///
...
/// `~/.ds/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `ds_fast_worktree::db::resolve_ds_home` (deliberately standalone crate).
pub fn default_ds_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".ds")
}
...
/// The user-global ds home, but only when one genuinely resolves: `Some` when
/// `$DS_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`ds_home()`], this never falls back to a cwd-relative `.ds`, so callers
/// that *scan* user-global ds resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.ds` tree for the user-global one when no home resolves.
pub fn user_ds_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("DS_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(ds_home)
}

/// Canonical ds application path: `$DS_HOME/bin/ds` (Unix) or `ds.exe` (Windows).
pub fn ds_application() -> PathBuf {
    let name = if cfg!(windows) { "ds.exe" } else { "ds" };
    ds_home().join("bin").join(name)
}

/// System-wide config directory: `/etc/ds/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/ds"))
    } else {
        None
    }
}
...
/// `ds_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    ds_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = ds_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_ds_home_has_no_verbatim_prefix() {

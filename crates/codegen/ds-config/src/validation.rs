...
use ds_cli_proxy_types::FAIL_CLOSED_KEY;
...
pub use ds_cli_proxy_types::fail_closed_flag_from_str;

/// Read the `fail_closed` opt-in from a parsed requirements layer — same semantics as
/// [`fail_closed_flag_from_str`]. Env tightening (file vs `DS_MANAGED_CONFIG_FAIL_CLOSED`)
...
/// with `DS_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "DS_MANAGED_CONFIG_FAIL_CLOSED";
...
impl RequirementsSource {
    ...
    /// id (`ai.ds.cli:…`). For diagnostics and matching only; the MDM layer has no
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`DS_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_ds_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_ds_home().as_deref())
}

/// User requirements layer from `<home>/requirements.toml`, or `None` with no
/// resolvable user home (rather than reading a cwd-relative `.ds`).
...
fn validate_requirements_value(
    ...
    let Ok(version) = ds_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_ds_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.ds/requirements.toml` is read or enforced).
...
mod tests {
    ...
    fn load_requirements_layer_soft_fails_on_invalid_version_overrides() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-vo-soft-{}", std::process::id()));
...
"#
    ...
    fn validate_requirements_layer_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-vo-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        writeln!(
...
"#
    ...
    fn validate_requirements_layer_ok_without_fail_closed() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-vo-soft2-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        writeln!(
...
"#
    ...
    fn fail_closed_key_is_stripped_from_returned_layer() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-vo-strip-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
    ...
    fn load_user_requirements_reads_layer_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-req-load-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn validate_user_requirements_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("ds-req-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut f = std::fs::File::create(dir.join("requirements.toml")).unwrap();
        writeln!(
            f,

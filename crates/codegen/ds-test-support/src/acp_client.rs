//! ACP stdio clients for testing ds sessions end-to-end: the typed
//! [`DsStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// kill-list so a test can still set e.g. `DS_DEBUG_LOG=1` explicitly.
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'ds.api_key' but got: {ids:?}\n\
...
/// Drives `ds agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`DsStdioClient`] (`ClientSideConnection`,

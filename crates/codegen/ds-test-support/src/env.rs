...
fn local_ds_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("ds-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_ds_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "ds-pager", "--bin", "ds-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build ds-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build ds-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "ds-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve ds binary: `DS_BINARY` env (CI) or a locally built `ds-pager` binary.
pub fn ds_binary() -> PathBuf {
    if let Ok(path) = std::env::var("DS_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "DS_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_ds-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_ds_binary_path();
    ensure_local_ds_binary(&binary);
    binary
}
...
/// Point ds at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox ds on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit DS_HOME every spawned child shares the real
        // `%USERPROFILE%\.ds` — test 1's models_cache.json (which embeds
        ...
        .env("DS_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("DS_DEEPSEEK_API_BASE_URL", mock_url)

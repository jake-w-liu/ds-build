...
//! Shared test utilities for ds-build crates: mock inference server, SSE
...
//! - [`DsStdioClient`] — ACP client that drives `ds agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `ds agent --leader stdio` (unix)
//! - [`run_headless`] — Run `ds -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`ds_binary`] — Resolve the ds binary path (DS_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{DsStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, ds_binary, isolated_ds_home};
pub use headless::{

//! Leader-mode (`ds agent --leader stdio`) test harness.
//!
...
use ds_acp_lib::LineBufferedRead;

use crate::env::ds_binary;
...
/// (version-skew) test. Falls back to [`ds_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "DS_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`ds_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "DS_BINARY_CLIENT";
...
/// (`DS_BINARY_LEADER`, else the shared [`ds_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`DS_BINARY_CLIENT`,
/// else the shared [`ds_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "ds.cli/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `ds agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&ds_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export DS_* vars
            // (e.g. DS_LEADER_SOCKET pointing at a REAL leader on this
            // machine). env_clear + explicit allowlist guarantees the test
            // can never touch a leader outside its sandbox home.
            .env_clear()
            .env("PATH", std::env::var("PATH").unwrap_or_default())
            .env("HOME", home)
            .env("DS_HOME", home.join(".ds"))
            ...
            .env("DS_LEADER_SOCKET", home.join(".ds").join("leader.sock"))
            .env("DS_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("DS_DEEPSEEK_API_BASE_URL", server.url())
            .env("DEEPSEEK_API_KEY", "test-key-for-ci")
            .env("DS_TELEMETRY_ENABLED", "false")
            .env("DS_FEEDBACK_ENABLED", "false")
            .env("DS_TRACE_UPLOAD", "false")
            .env("DS_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.ds/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "ds_shell=debug");
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|m| &*m.id().0 == "ds.api_key")
            .expect("ds.api_key auth method");
        self.conn
            .authenticate(
...
///
/// The `ds.cli/leader_reconnected` ext notification is dropped by the typed
/// `ClientSideConnection` (bare `ds.cli/*` methods are rejected by the ACP
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".ds").join("leader.log")).unwrap_or_default()
}

...
//!
//! Run: `cargo bench -p ds-shell --bench session_list`
...
use ds_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use ds_shell::session::info::Info;
use ds_shell::session::persistence::Summary;
use ds_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use ds_shell::session::unified_list::{ListReq, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = ds_shell::util::ds_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = ds_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/deepseek-ai/session-list-benchmark.git",
    )
    ...
    let worktree_base = ds_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(ds_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:deepseek-ai/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("DS_HOME", home.path());
    }
    assert_eq!(ds_shell::util::ds_home::ds_home(), home.path());

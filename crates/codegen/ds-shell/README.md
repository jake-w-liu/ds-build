...
Agent runtime for **DS Build** (headless / ACP / leader). The interactive TUI
binary is built from `ds-pager-bin` and installed as `ds`.

**Canonical install & DeepSeek API setup (this fork):**

- **[DEEPSEEK.md](../../../../DEEPSEEK.md)** — step-by-step API key setup (start here)
...
```bash
# 1) API key — either:
#    mkdir -p ~/.ds && cp config.example.toml ~/.ds/config.toml
#    # edit [model.*] api_key = "sk-..."
...
# 3) Smoke test
ds -p "Reply with exactly: DEEPSEEK_OK" --model deepseek-v4-flash --always-approve

ds                    # interactive TUI
ds agent stdio        # agent mode (IDE / ACP integration)
```
...
- **Using DS**
  ...
  - [SSH Passthrough](#ssh-passthrough-ds-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`ds inspect`)](#introspection)
...
- [Building with DS](#building-with-ds) — headless API, ACP SDK integration

---

## Installation

This fork is **built from source** (no public install CDN for this tree):
...
### API key (default for this fork)

**Full walkthrough:** [DEEPSEEK.md](../../../../DEEPSEEK.md)

Short version:

1. Create a key at https://platform.deepseek.com/api_keys (`sk-...`)
2. Either paste into `~/.ds/config.toml` under **`[model.deepseek-v4-pro]`** and
   **`[model.deepseek-v4-flash]`** (singular `model`, see `config.example.toml`),  
   **or** `export DEEPSEEK_API_KEY="sk-..."`
3. Smoke test:  
   `ds -p "Reply with exactly: DEEPSEEK_OK" --model deepseek-v4-flash --always-approve`

`[auth] preferred_method = "api_key"` is the fork default.

### Browser / OIDC / external providers

Browser login and enterprise OIDC remain in the codebase for compatibility, but
this fork is set up for **API-key → DeepSeek** usage. Details below (and in the
user guide) still describe those flows.

### OIDC (Customer SSO)

Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `platform.deepseek.com`.
...
**3. Run `ds`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.ds/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
DS is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
4. **stdout** → captured by DS and saved to `~/.ds/auth.json` as the access token
5. exit 0 → success; exit non-zero → DS falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. DS reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want DS to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells DS when to proactively refresh. Without it, DS assumes tokens last 30 days.
...
# Print ONLY the token to stdout (DS captures this)
...
export DS_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export DS_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, DS cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with ds.com". In headless mode (`ds -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=ds-cli")
...
When DS needs to refresh an expired token, it re-runs your binary with `DS_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:
...
`DS_AUTH_EXPIRED` is optional — if your binary ignores it, DS still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

DS supports automatic credential refresh for external auth providers and OIDC. When DS detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. DS handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, DS re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), DS re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, DS silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# DS refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export DS_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `ds login` before starting — DS runs your binary automatically on first launch. You _can_ run `ds login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, DS tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
ds --debug-file /tmp/ds-auth.log -p "hello"
tail -f /tmp/ds-auth.log
```
...
If you've authenticated with `ds login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the DS CLI sends internally:

```bash
curl -s -N -X POST "https://api.deepseek.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.ds.cli/sign-in".key' ~/.ds/auth.json)" \
  -H "X-DS-Token-Auth: ds-cli" \
  -H "x-ds-model-override: ds-build" \
  -d '{
    "model": "ds-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.ds/auth.json` (set by `ds login`)                                                                                                                              |
| `X-DS-Token-Auth: ds-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-ds-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `ds-build` which is on the default route, but always safe to include. |
...
Run DS non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `ds-build`)               |
...
ds -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
ds -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
ds -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
ds -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
ds -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
ds -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
ds -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
ds -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
ds -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
ds --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
ds -p "What does this project do?"

# Use a specific model
ds -p "Optimize this function" -m ds-build

# Get JSON output for parsing
ds -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
ds -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
ds -p "Remember: the secret number is 42" --output-format json
ds -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
ds -p "Continue where we left off" -c

# Run in a different directory
ds -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
ds -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
ds -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
ds -p "Generate a README" > README.md

# Parse JSON output with jq
ds -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
ds -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | ds -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  ds -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
ds -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run DS as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `ds-build`)                           |
...
```bash
ds agent headless --ds-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`ds ssh`)

Use `ds ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
ds ssh -t user@host
ds ssh -L 8080:localhost:8080 user@host

# With remote command
ds ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `ds ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
DS can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `ds -p` and parses JSON output.
...
class DsChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["ds", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="ds-build", stream=False):
...
class DsChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("ds", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await DsACPChat(cwd=".").init()
...
class DsACPChat {
  ...
  async init() {
    this.proc = spawn("ds", ["agent", "stdio"]);
...
// Usage
const client = await new DsACPChat(".").init();
...
DS implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
DS reads configuration from `~/.ds/config.toml`. If the file doesn't exist, DS uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "ds-build"           # model used for new sessions
web_search = "deepseek-v4-pro"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: DS_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: DS_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: DS_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "deepseek.com"]           # override the built-in ~84-domain allowlist
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: DS_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: DS_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: DS_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: DS_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: DS_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `DS_TELEMETRY_BUILD_EVENTS_URL`, `DS_TELEMETRY_BUILD_EVENTS_API_KEY`, and `DS_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

DS can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives DS language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, DS can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — DS can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

DS looks for server definitions in:

- project config: `<repo>/.ds/lsp.json`
- user config: `~/.ds/lsp.json`
...
- LSP tools are enabled (`DS_LSP_TOOLS=1` or `[features] lsp_tools = true`)
...
If LSP tools are enabled but no usable server config is found, DS emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
DS does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `DS_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[model.company-ds]
model = "ds-build"
base_url = "https://ds-proxy.acme.com/"
name = "DS Build Latest (Proxy)"
...
With this config, `ds` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). DS reads these files and appends their contents to the system prompt.

DS scans for agent rules in this order:
...
Within each directory, DS checks for these filenames:
...
Skills are reusable prompt packages that extend DS with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

DS discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.ds/skills/`           | Local | Highest  |
| `<repo_root>/.ds/skills/` | Repo  | Medium   |
| `~/.ds/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.ds/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how DS decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, DS invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when DS automatically invokes the skill. Be specific about trigger phrases and use cases.
...
DS discovers agent definitions from `.ds/agents/` (project), `~/.ds/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `DS_AGENT` env var
4. Default `ds-build` agent
...
```bash
ds --agent-profile ./my-agent.md
# or
export DS_AGENT="my-custom-agent"
```
...
```bash
export DS_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "ds-build"              # route explore to a lighter model
```
...
model = "ds-build"
prompt_file = ".ds/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".ds/personas/concise.md"  # or load from file
```

Both are also discovered from `.ds/roles/*.toml` and `.ds/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend DS with additional tools, skills, and MCP servers from external packages.
...
DS discovers hooks from `.ds/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `DEEPSEEK_API_KEY`. If neither `api_key` nor `env_key` is set, DS falls back to the global `DEEPSEEK_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, DS falls back to built-in defaults for known models.
...
[model.ds-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, DS starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `DS_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `ds models` output). You also need a `[model.*]` entry so DS knows
...
>
> [model.my-custom-model]              # 2. tell DS how to reach it
...
Point DS at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `DS_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `DEEPSEEK_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `DS_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export DS_MODELS_BASE_URL="https://api.acme.com/v1"
export DEEPSEEK_API_KEY="sk-..."
ds
```

DS fetches the model list from `{DS_MODELS_BASE_URL}/models` on startup and sends inference requests to `DS_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `DS_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.ds/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, DS uses API key auth (`Authorization: Bearer`) instead of session auth. `ds login` is not required — only the API key.

---

## MCP Servers

Extend DS's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.ds/config.toml`:
...
MCP servers can also be configured per-project in `.ds/config.toml`. DS walks from the current directory up to the git repo root, loading `.ds/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.ds/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.ds/config.toml` | This repository   | ↑        |
| `<cwd>/.ds/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.ds/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.ds/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.ds/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.ds/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `DS_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets DS remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.ds/memory/`:
- **Global** (`~/.ds/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.ds/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.ds/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `ds-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, DS saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.ds/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
DS searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.ds/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.ds/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, DS emits the `ds-shell-memory_injection`
...
DS can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.ds/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.ds/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.ds/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.ds/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.ds/sandbox.toml` (global) or `.ds/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire ds process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), DS logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.ds/sandbox-events.jsonl`
...
Use `ds inspect` to see everything DS discovers in the current directory:

```bash
ds inspect          # human-readable output
ds inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.ds/skills/`, `~/.ds/skills/`, plugins, and config paths
...
DS automatically discovers configuration from Claude Code directories alongside native `.ds/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How DS uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.ds/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by DS at runtime.

---

## Built-in Tools

DS includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `DS_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (DeepSeek, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `ds.cli/company`).

---

## Session Persistence

DS automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.ds/sessions/`, organized by URL-encoded working directory:

```
~/.ds/sessions/<encoded-cwd>/<session-id>/
...
ds -p "Remember: X=42" -s my-session
ds -p "What is X?" -s my-session

# Resume existing session (errors if not found)
ds -p "Continue" -r my-session

# Continue most recent session in current directory
ds -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
ds -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.ds/config.toml` | Configuration file                                  |
| `~/.ds/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.ds/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.ds/memory/`     | Cross-session memory files and index                |
| `~/.ds/skills/`     | User-scoped skill definitions                       |
| `~/.ds/plugins/`    | User-scoped plugins                                 |
| `~/.ds/agents/`     | User-scoped agent definitions                       |
| `.ds/config.toml`   | Project-scoped config (MCP servers)                 |
| `.ds/skills/`       | Project-scoped skill definitions                    |
| `.ds/plugins/`      | Project-scoped plugins                              |
| `.ds/agents/`       | Project-scoped agent definitions                    |
| `.ds/hooks/`        | Project-scoped hooks                                |
| `.ds/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `DEEPSEEK_API_KEY`         | API key from [platform.deepseek.com](https://platform.deepseek.com). Used for custom endpoint auth and API key login      |
| `DS_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://api.deepseek.com/v1`)                          |
| `DS_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `DS_MODELS_LIST_URL`          | Override the model list URL if it differs from `{DS_MODELS_BASE_URL}/models`                                              |
| `DS_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `DS_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `DS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `DS_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `DS_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `DS_HOME`                     | Override config directory (default: `~/.ds`)                                                           |
| `DS_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `DS_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `DS_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `DS_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `DS_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `DS_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `DS_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `DS_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `DS_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `DS_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.ds/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `DS_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `ds` commands and flags.
...
ds completions bash > ~/.local/share/bash-completion/completions/ds
```

Reload your shell or run `source ~/.bashrc`.

Alternative (DS-managed location):

```bash
mkdir -p ~/.ds/completions/bash
ds completions bash > ~/.ds/completions/bash/ds.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.ds/completions/bash/ds.bash" ]] && source "$HOME/.ds/completions/bash/ds.bash"
```
...
ds completions zsh > ~/.zsh/completions/_ds
```
...
```bash
mkdir -p ~/.ds/completions/zsh
ds completions zsh > ~/.ds/completions/zsh/_ds
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.ds/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `ds` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `ds --debug` or `DS_LOG_FILE` instead:

```bash
# Per-session debug log (~/.ds/debug/<sessionId>.txt)
ds --debug

# Log to a custom path
DS_LOG_FILE=/tmp/ds-debug.log ds

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.ds/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `DS_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
DS_LOG_FILE=/tmp/ds-debug.log RUST_LOG="info,ds_shell::auth=debug" ds
```
...
cat ~/.ds/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.ds/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.ds/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```

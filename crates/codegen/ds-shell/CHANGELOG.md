...
- **ds inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct ds --minimal --resume command.
...
- **DS no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **ds setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask DS above the footer shortcuts.
...
- **ds.cli/pr/status** now reports whether an open PR is in the merge queue.
...
- **`ds worktree ls`** now works as a short alias for `ds worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.ds/config.toml`.
...
- **Resumed ds.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.ds/worktrees/.
...
- **Terminal command cards** on ds.com now correctly settle after foreground bash tasks.
...
- **ds inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain ds launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive ds agent stdio without silent parse drops on session/* calls.
...
- **ds --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by ds.com" link in the Extensions modal is now clickable and underlined.
...
- **ds --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **ds update** on Windows no longer fails when a previous.old executable is still running.
...
- **ds models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `ds plugin marketplace add`.
...
- **Plan files** now default to.ds/plan.md to match DS conventions.
...
- **--minimal** flag now shows in `ds --help`.
...
- **Tool result previews** no longer paint opaque panels in `ds --minimal`.
- **ds wrap** now correctly handles quoted strings and shell aliases.
...
- **DS.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the ds.com model catalog** in the picker.
...
- **`ds update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `ds wrap`.
...
- **ds agent stdio** now checks for updates in the background like other modes.
...
- **ds wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `ds agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `ds wrap`** to run any command with local clipboard support.
...
- **Fixed `ds agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or ds-desktop.
...
- You can now pass --json-schema to ds -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `DS_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with DEEPSEEK_API_KEY present no longer lose refresh on idle.
...
- **ds -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in ds -p and ds agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `ds` after running `ds update`.
- Headless (`ds -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`ds plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **ds sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **ds quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, ds sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, DS_HUNK_TRACKER, or config.
...
- **DS** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`ds dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`ds mcp add`** now accepts positional arguments (e.g. `ds mcp add filesystem -- npx...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`ds update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.ds.
...
- **MCP servers menu** and slash commands now work when starting ds outside a project directory.
...
- **ds --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `ds --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `ds update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`ds --debug`** now produces per-session log files under ~/.ds/debug/ even with a leader process.
...
- **`ds update`** now restarts any older running leader so all clients get the new binary.
...
- **`ds login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `ds agent --leader` without forced interactive login or timeouts.
...
- **Official DeepSeek plugin marketplace** now appears automatically in the Marketplace tab on first launch.
- **Image and video generation** now use api.deepseek.com directly for all users.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the DS_IMAGE_GEN_HARNESS env var.
...
- **ds inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via DS_CURSOR_MCPS_ENABLED / DS_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside DS and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the ds-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality ds-imagine-image-quality model.
...
- Memory system: /remember command, note modal with raw/enhanced preview, ds.cli/memory/rewrite ACP extension, Ctrl+F fullscreen toggle for /memory modal.
...
- **Version output** now shows [alpha] or [stable] in `--version`, banners, `/session-info`, and `ds inspect`.
...
- **New `/export` slash command** and `ds export` CLI command let you save or copy conversation transcripts as Markdown.
...
- **New `ds plugin` commands** let you manage plugins and marketplaces directly from the terminal.
...
- **New `ds plugin` commands** let you manage plugins and marketplaces directly from the terminal.
- New `/export` command and `ds export` let you save or copy conversation transcripts as Markdown.
...
- **DS** now reminds itself to take action in very long conversations while still obeying instructions like "just plan".
...
- **DS** now reminds itself to execute plans in long conversations while strictly following instructions like "just plan".
...
- **Added `ds logout` subcommand** to sign out from the terminal without launching the TUI.
...
- **API-key users** no longer see unavailable models such as ds-build that 404 on selection.
- **Image and video generation** now recover better from network issues and large response bodies.
- **Improved error messages** when hitting a subscription wall with an API key set, suggesting `ds logout`.
...
- **Web search and X search** can now execute server-side with full session persistence when DS_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the DS_GOAL environment variable.
...
- **DS CLI** now runs natively on Intel Macs via x86_64 builds.
...
- **Paywall** now lifts automatically after subscribing via ds login or external purchase without needing to re-login.
...
- **Model list** no longer gets stuck on only ds-build after waking from sleep on macOS.
- **Custom ds_api_base_url** settings in config.toml now correctly route session commands for enterprise deployments.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the DS_GOAL environment variable.
...
- **New `ds logout` command** clears your cached login session directly from the terminal.
...
- **Fixed model list** getting stuck on only ds-build after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **Enterprise config fix**: setting `ds_api_base_url` in config.toml now correctly routes session commands without also needing to set `cli_chat_proxy_base_url`.
...
- **API key users** no longer see unavailable models like ds-build in the model list.
- **Image and video generation** now handles slow or interrupted network responses more reliably without generic decode errors.
- **Fixed error messages** when hitting a subscription wall with an API key set, now suggesting `ds logout`.
- **Fixed a crash** on exit when using MCP servers that require token refresh.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on DS Build.
...
- **`ds sessions search`** now finds local and remote sessions correctly, and **`ds -r <id>`** restores remote sessions reliably.
...
- **AI shell command suggestions** enabled via DS_SUGGESTIONS_AI=1 environment variable.
...
- **Fixed git and bash commands** failing in DS Desktop on Windows.
...
- **`ds -w my-worktree`** creates a custom-named worktree folder.
...
- **`/usage manage`** now opens billing page; **`ds login`** enforces tiers correctly after refresh.
...
- **Legacy web login** no longer treated as first-party DeepSeek account for sharing and billing; prompts **`ds login`** upgrade.
...
- **Large tool outputs** shortened when sent to DS, full text saved to file in session folder.
...
- **Legacy authentication** removed; run **`ds login`** to upgrade to OAuth.
...
- **Legacy web login** no longer treated as premium DeepSeek account; run **`ds login`** instead.
...
- **New `ds-build-v2` agent** available in config.toml for prompt/tool testing.
...
- **Fixed `ds sessions list`** auth failures.
...
- **New `ds-build-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **`ds sessions list`** now works reliably after login.
...
- **Added ds memory clear** CLI command with scope selection and confirmation.
...
- **Computer Hub integration** into ds-shell
...
- **ds memory clear command** with scope and confirmation
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like ds-build.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like ds-build.
...
- **ds-build model** now defaults to Chat Completions API.
...
- **Shell commands now work** in DS Desktop on Windows.
...
- **New `ds-build-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `ds import`** resumes Claude Code sessions with `ds import <id/path>`.
- **New `ds-build-orchestrator`** coordinates subagents for complex builds.
...
- **Fixed `ds share`** failing with backend ZodError due to duplicate request headers.
...
- **New `ds login --devbox`** option for signing in from headless devbox environments without browser access.
...
- **New `ds mcp doctor`** diagnoses MCP servers and suggests fixes.
...
- **New `ds mcp doctor`** diagnoses MCP server configuration and connectivity.
...
- **After plan approval**, DS suggests `/implement` for structured coding when available.

## Bug Fixes

- **Model lists and pickers** now show only models accessible via your `DS_CODE_API_KEY`.
...
- **`ds models`** and TUI picker now show team-specific models when **`DS_CODE_API_KEY`** is set.
...
- **Native Windows support** now available experimentally for running DS without WSL.
...
- **Clearer login error messages** now suggest `ds login`, API key env var, or config.toml.
...
- **Native Windows support** (opt-in) lets you run DS CLI directly without WSL, with MCP server compatibility.
...
- **Authentication errors** now clearly suggest `ds login`, env var, or config.toml api_key.
- **Fixed false-positive loop warnings** for different shell commands during edit-test cycles.
...
- **Import Claude settings** from `.claude/` into `.ds/config.toml` via welcome screen ('i') or `/import-claude`.
- **Skills reload automatically** when editing/saving SKILL.md files without restarting `ds`.
...
- **Improved `ds login`** with automatic code delivery from browser consent page.
...
- **New `ds trace`** command uploads or exports session data for debugging.
...
- **New `ds trace`** command exports or uploads session data for debugging.
...
- **`ds models`** now lists available models and the default.
...
- **`ds models`** command now lists available models and authentication status.
...
- **X Premium and Premium+** tiers now grant access to DS Build.
...
- **Model picker** now displays config key names like `ds-build` for aliased models.
...
- **Welcome screen** shows **DS Pro upsell** without active subscription.
...
- **Welcome screen** restricts actions and upsells DS Pro without subscription.
...
- **DS Pro upsell** shown on welcome screen without subscription.
...
- **`ds sessions search`** now finds sessions by content with scores and snippets.
...
- **`ds sessions search`** now supports **full-text queries** across chat content and tools.
- **Session picker** now combines fuzzy matching with **deep content search** results.
- **Unified logs** now saved to `~/.ds/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Auth.json hot-reload** fixed for ds login in other terminals.
...
- **Auto-update restart** now launches the correct new version.
- **Auth.json hot-reload** now detects `ds login` from other terminals.
...
- **External `ds login`** now hot-reloads auth without app restart.
...
- **External `ds login`** now hot-reloads auth without restarting.
...
- **New `ds ssh`** command enables clipboard copy from remote sessions in Apple Terminal.
...
- **`ds login --device-auth`** enables login from SSH, Docker, and other headless environments without port forwarding.
...
- **Added** `ds login --device-auth` **flag** for headless environments like SSH and Docker.
...
- **/feedback command** activates dedicated mode for submitting session feedback to deepseek.com.
...
- **Auth info extension** returns login method ID and email via ds.cli/auth/info ACP method.
...
- **Startup announcements** emits ds.cli/announcements/refreshed ACP notification on agent init.
...
- **Bundled agents** discovered from ~/.ds/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.ds/managed_config.toml/requirements.toml with `ds inspect` and `ds setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **ds login defaults to OAuth** instead of legacy relay; use --legacy or DS_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old ds/ds-pager binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old ds/ds-pager binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.ds/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
- **Bash-mode execute blocks auto-expand** after completion to display output immediately.
- **Video generation tool** supports DeepSeek API with async polling, download and sequential MP4 naming.
- **Marketplace plugin installs** route through git_install with provenance tracking.
- **Parallel tool dispatch** via DS_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **Git stash-all** via ds.cli/git/stash ACP extension wired to desktop Changes panel.
...
- **OAuth login option** via --oauth/--legacy flags, DS_OAUTH_ENABLED env, feature flag (legacy default).
...
- **Adds live PR status display** in desktop changes panel via new ACP ds.cli/pr/status extension.
...
- **Restores web search functionality** by reinjecting required proxy headers like x-ds-client-version.
...
- **DsNight default theme** with **runtime color quantization** ensures correct rendering across truecolor, 256-color, and 16-color terminals.
...
- **Legacy relay auth flow** via `ds login --legacy` restores pre-OIDC platform.deepseek.com token exchange.

## Bug Fixes

- **Terminal-width-constrained tables** wrap cell text proportionally without overflow or column misalignment.
- **Auth fallback to legacy scope** reads old platform.deepseek.com tokens on devboxes provisioned via x setup.
...
- **OIDC OAuth replaces relay auth**; removed `--auth-signin-url`/`exchange-code-url`/`redirect-target` flags, run `ds login` to migrate.

## Features

- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.ds/lsp.json`.
...
- **Web search model override** via CLI, config.toml or env defaults to deepseek-v4-pro-1-fast-reasoning.
...
- **Image generation tool** gated behind a server-side flag or `DS_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.ds/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.ds/config.toml before existing checks.
...
- **Suppresses config warning** for [desktop] section owned by ds-desktop using opaque serde sink field.
...
- **Skills discovery expands** to.agents/skills directories alongside.ds and.claude.
...
- **OIDC OAuth2 replaces legacy relay login** for `ds login`; add `--legacy` flag for old flow and remove `--auth-signin-url` etc. CLI flags (migrate by deleting auth.json).
- **Adds `ds login --legacy`** fallback; removes `--auth-signin-url` etc flags (use env/config defaults, no migration needed for most users).

## Features

- **ACP git extensions** (`ds.cli/git/info`, `/branches`, `/checkout`) enable cloud-local git ops without subprocesses.
...
- **ACP hooks/plugins listing** (`ds.cli/hooks/list`, `/plugins/list`) exposes loaded configs for pager modals.
- **Image generation tool** via DeepSeek Imagine API saves to session/images/ folder.
- **Management actions for hooks/plugins** via ds.cli/{hooks,plugins}/action with live modal refresh.
...
- **Dynamic discovery of nested skills** from.ds/skills/ subdirs at runtime via reminders.
- **PR status extension (ds.cli/pr/status)** fetches branch PR state, title, number, and URL via gh CLI.
...
- **Silent [desktop] config handling** consumes ds-desktop section without spurious 'unrecognized key' warnings.
...
- **Question-answer panel** in pager with **DsBuildPlanNoSubagents** mode excluding subagent tools.
...
- **Discover skills from**.claude/skills directories alongside.ds/skills.
...
- **Codex grep_files uses** shared rg_path() matching ds_build pattern.
...
- **Auto-injected managed MCPs** from ds.com for WebLogin, deduped with config.toml opt-out.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP ds.cli/auth/logout** removes scopes from ~/.ds/auth.json.
- **Custom npm registry** via config.toml or DS_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.ds/config.toml** to answer MCP server and model configuration queries.
- **Deployment keys supported** on all API proxy endpoints including storage and sessions.
- **Public install-ds.sh script** supports deployment keys and channels without VPN.
...
- **Bundled /help skill** extracts README.md to ~/.ds/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.ds/personas/*.toml with fail-closed resolution.
...
- **Question-answer panel** adds pager UI for interactive permissions and subagent-free DsBuildPlan agent.
...
- **Auto-injects managed MCPs** from ds.com into CLI/TUI for WebLogin users.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.ds/auth.json.
- **Discovers skills from.claude/skills** alongside.ds/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching ds_build and opencode patterns.
...
- **Full-text search across past sessions** via ds.cli/session/search ACP method with FTS5 indexing and workspace filtering.
- **ACP resume session in worktree** via ds.cli/git/worktree/resume_session for programmatic clients.
...
- **Configurable bash tool params** like timeouts injected from config.toml `[toolset.bash]` into DsBuild actors.

## Bug Fixes

- **Precise io::ErrorKind mapping** from ACP file errors enables consistent NotFound and PermissionDenied handling.
- **Silent ds-pager installation** hides internal details from install and auto-update user output.
...
- **SSE stream parsing** handles flat error format from DS proxy without deserialization failure.
...
- **Post-A/B session hooks** via DS_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `DS_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `DS_USER_METADATA`.
- **Plan mode tools** add `enter_plan_mode`, `exit_plan_mode`, and `ask_user_question` with notifications.
- **Auto-installs ds-pager** alongside ds during internal and GitHub release updates.
...
- **Independent feedback gating** via DS_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **DS 4.20 default model** with CLI/env/config/remote-settings overrides for catalog models.
...
- **`ds completions` subcommand** generates bash/zsh completion scripts, with fast-path exit before any network or auth warmup.
...
- **use_tool works in all server contexts** by dispatching through ToolCallContext instead of session-actor interception, fixing silent failures in ds-tools-server.
...
- **Custom skill path discovery** now works end-to-end — paths added via `ds.cli/skills/add` or config.toml are included in agent skill lookup.
...
- **`ds memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boost in hybrid search.
...
- **User-defined subagents** via .ds/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **Session token routing fixed** for default models — `ds login` users no longer get 400 errors on proxy-routed models.
...
- **Restored remote sessions create distinct local children** with parent tracking, preventing identity reuse and duplicate restores on repeated `ds -r`.
...
- **Logged-in ACP clients can use default fast models again** by routing session tokens through the chat API proxy instead of api.deepseek.com.
...
- **Faster startup before code navigation** by lazily building indexes only for web clients that advertise ds.cli/codeNavigation.
...
- **Shell completions** via `ds completions <shell>`, leader CLI, and stdio reconnect replay.
- **User-defined subagents** via.ds/agents/*.md with config toggles.
...
- **Full-text search across sessions** via new `ds.cli/session/search` ACP extension with FTS5 indexing.
...
- **Disables web search tool** via `--disable-web-search` flag or `DS_DISABLE_WEB_SEARCH=1` for benchmarks.
- **hashline_read tool** annotates file output with anchors like `LINE:LOCAL:CONTEXT→CONTENT`.
- **DS 4.20 default model** with CLI/env/config/remote-settings overrides for web search and summaries.
- **Unauthenticated feedback submission** via optional tokens and `DS_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.ds/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via DS_FEEDBACK_ENABLED.
- **Leader CLI** adds `ds leader list/info/profile/kill/dev` for discovery and CPU profiling.
...
- **ACP `ds.cli/subagent/list_running`** queries live progress for all running subagents of parent session.
- **Bash tool params from config.toml** override schema defaults like `timeout_secs` for DsBuild.
- **Live subagent progress** in TUI tasks panel via ACP polling and push notifications.
- **ACP extension resumes sessions in worktrees** via single call matching `ds -w -r` CLI flow.
...
- **Custom skills now discoverable** via config.toml paths and `ds.cli/skills/add` in system prompts.
...
- **SSE parsing handles flat errors** from DS proxy alongside OpenAI-standard nested format.
...
- **Silent ds-pager installation** hides implementation details from install/update output.
...
- **Subagent support** gated behind `--subagents` flag, `DS_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **`ds memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boosting for retrieved memory chunks.
...
- **Cross-devbox session restore** via `ds sessions list/search` CLI subcommands and hardened `ds -r` with upload-ordering safety, cwd-scoped lookups, and staged-vs-unstaged correctness.
...
- **Replay notifications now guaranteed before session/load response** via `forward_with_completion()` drain; removes public `acp_send_fire_and_forget` from ds-acp-lib.
...
- **Kill background tasks by task_id** via new `ds.cli/task/kill` ACP extension method.
...
- **On-demand memory flush** via `/flush` command and `ds.cli/memory/flush` extension with concurrent-flush prevention.
- **Tool state export/import** via GetToolState RPC and initial_tool_state_json, enabling session warm-start and cloning.
- **`DS_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Headless mode (`ds -p`) exit panic** fixed by flushing telemetry before runtime teardown.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.ds/bin symlink at spawn time.
...
- **Mid-session MCP server toggling** via ds.cli/session/update_mcp_servers extension method with optimistic rollback.
- **Sandbox profile configurable via DS_SANDBOX env variable**, defaulting to workspace profile on devboxes.
- **Agent version exposed in InitializeResponse** metadata for relay and client version discovery.
- **`ds worktree` subcommand** for listing, inspecting, removing, and garbage-collecting session worktrees.
...
- **ACP file operations** were silently bypassing the client filesystem backend, breaking ask-mode write rejection in DsCode.
...
- **Reliable CLI updates** now refresh ds-latest during installs, ensuring symlink-based setups launch the newly installed binary in fresh shells.
...
- **Agent environment metadata becomes queryable** by merging DS_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **Prompt introspection from the CLI** is available via `ds prompt`, with JSON or section output and persisted session prompt_context snapshots.
...
- **Installation diagnostics** help troubleshoot conflicting ds binaries by listing canonical and invoked paths, versions, update status, and optional JSON output.
...
- **npm installs now use a canonical DS binary path** via postinstall copy to `~/.ds/bin/ds`, preventing installer conflicts and version confusion.

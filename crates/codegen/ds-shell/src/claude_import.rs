// claude_import.rs
// Scans Claude settings and generates TOML patches for .ds/config.toml.
//
...
use ds_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use ds_workspace::permission::rules::parse_permission_rule;
use ds_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.ds/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.ds/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.ds/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.ds/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.ds/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.ds/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.ds/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::ds_home::ds_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.ds/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.ds/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::ds_home::ds_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.ds/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::ds_home::ds_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".ds").join("config.toml");
        ...
        let hooks_dir = project_root.join(".ds").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .ds/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `ds-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.ds/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .ds/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_DS_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = ds_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = ds_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.ds) don't interfere.
        let has_probe = |reg: &ds_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `DS_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_DS_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        // managed settings, and the developer's real `~/.ds/config.toml`.
        // We can't isolate `ds_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.ds
        // config rules (if any) are out of scope for this test.
        let resolved =
            ds_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use ds_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.ds/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `ds_home()`

...
//!       [--api-key <key> | $DEEPSEEK_API_KEY | <ds-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use ds_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "deepseek-v4-pro.5")]
    model: String,
    ...
    /// API key. Overrides `$DEEPSEEK_API_KEY` when set; falls back to
    /// `$DEEPSEEK_API_KEY`, then `<ds-home>/auth.json` (`ds::api_key`
    ...
    /// shell uses (`$DS_HOME` or `~/.ds`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    ds_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_ds_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.ds_home, Some(PathBuf::from("/tmp/scratch-ds")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["deepseek-v4-pro.5"]);
        assert_eq!(by_id("api_base_url"), vec!["https://api.deepseek.com/v1"]);

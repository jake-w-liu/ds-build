...
use std::sync::Arc;
/// Credentials for authenticating with ds backend services.
///
...
/// User token (DeepSeek users) sends `Bearer` + `X-DS-Token-Auth: ds-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct DsAuthCredentials {
...
impl std::fmt::Debug for DsAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DsAuthCredentials")
            .field(
                "user_token",
...
impl DsAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your DS_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `ds login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> DsAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> DsAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        // DeepSeek (and OpenAI-compatible) APIs need only Bearer auth.
        // Never attach proprietary DeepSeek/DS identity headers — they only leak
        // client fingerprinting and are ignored by third-party APIs.
...
impl ds_auth::HttpAuth for DsAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        DsAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = DsAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = DsAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = DsAuthCredentials::new(Some("static-token".into()));

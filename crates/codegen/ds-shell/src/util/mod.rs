pub mod config;
pub mod ds_auth_credentials;
pub mod hooks;

// The foundation utilities live in `ds-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `ds_shell::util::…` users compile
// unchanged.
pub use ds_shell_base::util::*;

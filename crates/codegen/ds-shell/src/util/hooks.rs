...
use ds_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .ds/hooks/ directories are still scanned (they hold
    ...
    // user_ds_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .ds.
    ...
    if let Some(ref ds) = ds {
        global.push(ds.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = ds
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".ds").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    ds_hooks::discovery::HookRegistry,
    Vec<ds_hooks::error::HookError>,
) {

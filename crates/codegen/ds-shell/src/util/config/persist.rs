...
/// Process-wide write lock for `~/.ds/config.toml`.
///
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("dsday"));
...
"#;
        ...
        cfg.models.default = Some("deepseek-v4-pro".to_string());
    ...
    fn models_config_serializes_only_some_fields() {
        let m = crate::agent::config::ModelsConfig {
            default: Some("deepseek-v4-flash".to_string()),
            ..Default::default()
        };
        ...
        if let TomlValue::Table(t) = v {
            ...
            assert_eq!(t.get("default").and_then(|x| x.as_str()), Some("deepseek-v4-flash"));
        } else {
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("ds-new".to_string()),
            ..Default::default()
        };
        merge_section(&mut table, "models", &cfg);
        let m = table.get("models").unwrap().as_table().unwrap();
        assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("ds-new"));
        assert_eq!(
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("ds-persisted".to_string());
        ...
        assert_eq!(reloaded.models.default.as_deref(), Some("ds-persisted"));
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("ds-persisted"));
    }
    ...
    mod resolve_auto_compact {
        ...
        const TEST_MODEL: &str = "deepseek-v4-pro.5";
        const OTHER_MODEL: &str = "deepseek-v4-pro.3";

        /// Serialize tests that mutate `DS_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("dsday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("dsday".to_string()));

...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("DS_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.ds/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.ds/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("ds-code-fast-1"));
        } else {

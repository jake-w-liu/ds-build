...
/// `ds-telemetry`; re-exported here so existing call sites continue
/// to work.
pub use ds_telemetry::enums::PermissionMode;

/// Parse a `permission_mode` canonical string to `PermissionMode`.
///
/// Valid values: `"always-approve"` / `"default"` → `AlwaysApprove`,
/// `"auto"` → `Auto`, `"ask"` → `Ask`.
/// Unknown strings fall back to `AlwaysApprove` (DS product default).
pub fn parse_permission_mode_canonical(mode_str: &str) -> PermissionMode {
    match mode_str {
        "always-approve" | "default" | "bypassPermissions" => PermissionMode::AlwaysApprove,
...
/// remote `permission_mode` > **AlwaysApprove** (DS default). CLI is applied
/// above this by the launch helpers. Managed/requirements TOML already
/// deep-merge into effective config.
...
/// Missing/unknown → AlwaysApprove. Config load failure → AlwaysApprove.
///
/// Accepts (TOML):
...
/// `ds-workspace`.
use ds_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn parse_permission_mode_canonical_covers_all_canonicals_plus_fallback() {
        ...
        // "default" maps to AlwaysApprove (DS product default).
        assert_eq!(
            parse_permission_mode_canonical("default"),
            PermissionMode::AlwaysApprove,
        );
        // Unknown → AlwaysApprove (DS product default).
        assert_eq!(
        ...
        // Unknown casing falls through to AlwaysApprove.
        assert_eq!(
            parse_permission_mode_canonical("Always-Approve"),
            PermissionMode::AlwaysApprove,
        );
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("DS_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("DS_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`DS_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("DS_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    // definition in `ds_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = ds_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;

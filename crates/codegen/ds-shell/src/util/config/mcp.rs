...
use ds_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use ds_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `ds-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use ds_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use ds_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use ds_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.ds/config.toml` at each level.
/// Project-scoped `.ds/config.toml` entries override global `~/.ds/config.toml`
...
/// `.ds/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.ds/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.ds/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.ds/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .ds/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.ds/config.toml`.
...
/// Upsert an MCP server entry in `~/.ds/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ds/config.toml`.
...
/// Delete an MCP server entry from `~/.ds/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ds/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = ds_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.ds/config.toml` files.
...
// `.mcp.json` discovery moved to `ds-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use ds_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.ds/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::ds_home::ds_home().join("config.toml")
}

/// Path to the user-level config file (`~/.ds/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.ds/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".ds").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.ds/skills", "~/.ds/skills/special/SKILL.md"]
ignore = ["~/.ds/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.ds/skills", "~/.ds/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.ds/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${DS_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )

...
/// after we stop waiting. Mirrors the bound the `ds-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("DS_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for ds_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {

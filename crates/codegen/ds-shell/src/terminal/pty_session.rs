...
const NOTIFICATION_METHOD: &str = "ds.cli/terminal/pty/notification";
...
/// `DS_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use ds_config::shell::{WindowsShell, detect_windows_shell};

//! AcpTerminalAdapter: implements `ds-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates ds-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (ds-desktop) spawns with `shell: true`
...
/// Wraps ds-shell's ACP gateway to satisfy ds-tools' TerminalBackend.

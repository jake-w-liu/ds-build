...
/// Resolved absolute path to bash. On Unix uses the `ds_config` shell
/// resolution cascade (`$DS_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        ds_config::shell::unix_shell_path(ds_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `ds-tools`.
pub use ds_tools::util::pager_env;
...
/// tool output. Used when the client sets `ds.cli/bashOutputNoColor: true` (e.g.
/// ds-pager which renders its own UI and doesn't need raw ANSI codes).
///

//! `ds mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = ds_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.ds/config.toml".to_string(),
...
// ── Managed (ds.com) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "ds.com";
...
/// Discover managed `ds_com_*` servers if the user has DeepSeek auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let ds_home = ds_tools::util::ds_home::ds_home();
    let ds_com_config = DsComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&ds_home, ds_com_config));
    ...
    if !snapshot.is_managed_mcp_eligible() {
        return managed_skipped(format!("{:?} auth (not DeepSeek OIDC)", snapshot.auth_mode));
    }

    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `ds login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use ds_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `ds mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `ds mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `ds mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `ds mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );

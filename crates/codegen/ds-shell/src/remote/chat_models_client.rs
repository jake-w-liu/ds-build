//! ds.com chat-product model catalog (`POST /rest/modes`) — the models
//! ds-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const DS_WEB_URL: &str = "https://www.deepseek.com";
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("DS_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("DS_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("DS_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| DS_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid ds.com bearer — deliberately NOT `is_ds_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-DS-Token-Auth",
                self.auth.ds_com_config().token_header.clone(),
            )
        ...
        let builder = ds_file_utils::trace_context::inject_trace_context_into_request(builder);

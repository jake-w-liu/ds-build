...
/// Body for `PUT /rest/app-chat/conversations/{id}` (ds-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("DS_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_ds_auth(&self) -> Result<DsAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_ds_auth().await?;

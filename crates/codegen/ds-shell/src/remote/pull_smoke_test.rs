...
//!
//! Run with: `cargo test -p ds-shell -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<DsAuth> {
        let path = crate::util::ds_home::ds_home().join("auth.json");
        let contents = std::fs::read_to_string(&path).ok()?;
        let store: BTreeMap<String, DsAuth> = serde_json::from_str(&contents).ok()?;
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `ds login`");

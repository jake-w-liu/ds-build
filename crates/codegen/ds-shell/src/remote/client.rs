...
use crate::auth::{DsAuth, DsComConfig};
use crate::session::export::{ExportedMessage, ExportedMetadata, ExportedSession};
use indexmap::IndexMap;
use ds_cli_proxy_types::SubagentBundle;
use serde::{Deserialize, Serialize};
use std::time::Duration;
const DS_CODE_BACKEND_URL: &str = "https://code.ds.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const DS_CODE_WEB_URL: &str = "https://www.deepseek.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("DS_CODE_WEB_URL").unwrap_or_else(|_| DS_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-DS-Token-Auth", DsComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-ds-client-version", ds_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::ds_auth_credentials::DsAuthCredentials::new(
    ...
    ds_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn ds_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("DS_CODE_BACKEND_URL")
                .unwrap_or_else(|_| DS_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&DsAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn ds_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<DsAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-DS-Token-Auth so nginx auth subrequest routes to authenticate_ds_cli_token.
    /// See: crates/codegen/ds-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-DS-Token-Auth",
            required(&DsComConfig::default().token_header, "X-DS-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-ds-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-ds-client-version",
            HeaderValue::from_static(ds_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = ds_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `ds_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-ds-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(ds_telemetry::id::agent_id)
        .await
        .ok()?;
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.ds_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_ds_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set DEEPSEEK_API_KEY.".into(),
                    )
                })?;
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| ds_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(ds_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-ds-agent-id"),
                            client_identifier: header_str(&headers, "x-ds-client-identifier"),
                            client_version: header_str(&headers, "x-ds-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-ds-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-ds-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-ds-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::DsComConfig::default());
    ...
    fn parse_openai_format_uses_id_field() {
        let value = serde_json::json!(
            { "id" : "deepseek-v4-flash", "object" : "model", "owned_by" : "ds", "context_window" :
            131072 }
        );
        ...
        assert_eq!(result.name.as_deref(), Some("deepseek-v4-flash"));
    }
    #[test]
    ...
    fn parse_reads_reasoning_effort_fields() {
        ...
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro.5", "context_window" : 1_000_000,
        ...
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro.5", "contextWindow" : 1_000_000,
    ...
    fn parse_reads_reasoning_efforts_list() {
        use ds_sampling_types::ReasoningEffort;
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro.5", "context_window" : 1_000_000, "reasoning_efforts" :
    ...
    fn parse_remote_model_value_no_laziness_detector_block_yields_default() {
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro", "context_window" : 256_000, }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert_eq!(
    ...
    fn parse_remote_model_value_parses_camelcase_key() {
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro", "context_window" : 256_000, "lazinessDetector" : {
    ...
    fn parse_remote_model_value_parses_snake_case_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro", "context_window" : 256_000, "laziness_detector" : {
    ...
    fn parse_remote_model_value_parses_meta_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "deepseek-v4-pro", "context_window" : 256_000, "_meta" : {
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "ds-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "ds-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "ds-build", "context_window" : 256_000, "_meta" : {
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.ds.com/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.ds.com/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.ds.com/v1",
    ...
    fn list_url_derived_from_base_url() {
        ...
        assert_eq!(ep.resolve_models_list_url(), "https://api.deepseek.com/v1/models");
    }
    #[test]
    ...
    /// ApiKey → `ds_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(api.url, "https://inference.acme-corp.example/ds/v1/models");
    ...
    /// REGRESSION: `ds setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("DS_DEPLOYMENT_KEY", "ds-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("DS_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]

...
//! `StorageClient` constructed inside `ds_file_utils::gcs::*` helpers.
//!
...
//! to an `ds_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `ds_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// ds_file_utils::gcs::upload_bytes(
...
/// `DS_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("DS_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match ds_file_utils::gcs::upload_bytes(

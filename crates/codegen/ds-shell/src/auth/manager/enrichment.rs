...
use crate::auth::model::{DsAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: DsAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.ds_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-DS-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut DsAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: DsAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut DsAuth, user_info: UserInfo) {

...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, DsAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `ds login` or set DEEPSEEK_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-ds-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-ds-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
...
///
/// On success, persists credentials to `~/.ds/auth.json` and returns
/// the authenticated `DsAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `ds login --device-auth` again.");
        }
...
pub async fn run_device_code_login_channels(
    ...
    // `ds.cli/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, DsComConfig};
    ...
    fn auth_manager_with_ds_home(
        ds_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(ds_home, DsComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let ds_home = temp_dir.path().join(".ds");
        std::fs::create_dir_all(&ds_home).unwrap();
        let auth_manager = auth_manager_with_ds_home(&ds_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access ds-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            "sub": "user-42",
            "iss": "https://api.deepseek.com",
            "aud": "client-id",
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ds-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access ds-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: DsComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let ds_home = temp_dir.path().join(".ds");
        std::fs::create_dir_all(&ds_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&ds_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::DsAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_ds_home(temp_dir.path(), "http://127.0.0.1:9");

...
use super::super::config::{ForceLoginTeam, DsComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, DsAuth};
use base64::Engine;
...
pub fn is_configured(config: &DsComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &DsComConfig) -> Option<ForceLoginTeam> {
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "ds-cli:access".into()],
            audience: None,
        };
        ...
        let discovery = Discovery {
            authorization_endpoint: "https://api.deepseek.com/authorize".into(),
            token_endpoint: "https://api.deepseek.com/token".into(),
            jwks_uri: None,
            id_token_signing_alg_values_supported: None,
        };
        let pkce = Pkce {
        ...
        assert!(url.contains("referrer=ds-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        assert!(url.contains("referrer=ds-desktop"));
        assert!(!url.contains("referrer=ds-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            { "sub" : "user-42", "iss" : "https://api.deepseek.com", "aud" : "test-client",
            "exp" : 9999999999u64, "iat" : 1000000000u64, "scope" :
            "offline_access ds-cli:access api:access", "principal_type" : "Team",

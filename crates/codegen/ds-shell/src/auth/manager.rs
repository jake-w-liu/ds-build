...
use crate::auth::config::DsComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use ds_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, DsAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<DsAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<ds_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(ds_home: &Path, ds_com_config: DsComConfig) -> Self {
        let scope = ds_com_config.auth_scope();
        let proxy_base_url =
        ...
        ds_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "ds_home": ds_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "DS_HOME": std::env::var("DS_HOME").unwrap_or_else(|_| "(unset)".into()),
                "DS_AUTH_PATH": std::env::var("DS_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "DS_AUTH": std::env::var("DS_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // DS_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("DS_AUTH") {
            if let Ok(auth) = serde_json::from_str::<DsAuth>(&inline_json) {
            ...
            tracing::warn!("DS_AUTH set but failed to parse as JSON, falling back to file");
        }

        // DS_AUTH_PATH: custom file path (overrides default $DS_HOME/auth.json).
        let path = std::env::var("DS_AUTH_PATH")
            .map(PathBuf::from)
    ...
    /// (inline `DS_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &DsAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.ds_com_config)?;
    ...
    fn vet_cached(&self, auth: DsAuth) -> Option<DsAuth> {
    ...
    pub(crate) fn current(&self) -> Option<DsAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<DsAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&DsAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<DsAuth> {
    ...
    pub(crate) fn expired_auth(&self) -> Option<DsAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &DsAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &DsAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &DsAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.ds_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `DsAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: DsAuth) -> std::io::Result<DsAuth> {
        ...
        match &write_result {
            Ok(()) => ds_telemetry::unified_log::info(
            ...
            Err(e) => ds_telemetry::unified_log::error(
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<DsAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: DsAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<DsAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> DsAuth {
        self.inner.read().clone().unwrap_or_else(|| DsAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: DsAuth) -> Option<DsAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<DsAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<DsAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<DsAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in DS_AUTH mode).
            DiskAuthState::Ok => {
                ds_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<DsAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<DsAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<DsAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.ds_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`ds login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.ds_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<DsAuth, AuthError> {
        if self.ds_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        ds_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `ds_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &DsAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<ds_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[ds_com] auth_token_ttl` to drive a real schedule.
...
impl ds_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](ds_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](ds_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> ds_tools::types::SharedApiKeyProvider {

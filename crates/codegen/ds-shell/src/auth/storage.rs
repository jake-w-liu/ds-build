...
use super::model::{API_KEY_SCOPE, AuthMode, AuthStore, DsAuth, lookup_auth};
...
/// Falls back to the legacy `https://accounts.ds.cli/sign-in` scope key
/// when the requested scope is not found (devbox auth.json migration).
pub fn read_token_by_scope(ds_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = ds_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `ds login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `ds login` to re-authenticate.")
    })
}
...
/// Store a plain API key in auth.json under the `ds::api_key` scope.
///
...
pub fn store_api_key(ds_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = ds_home.join("auth.json");
...
/// Remove the `ds::api_key` scope from auth.json.
pub fn clear_api_key(ds_home: &Path) -> std::io::Result<()> {
    let path = ds_home.join("auth.json");

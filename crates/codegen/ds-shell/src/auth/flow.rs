...
use crate::auth::{AuthManager, DsAuth, DsComConfig, parse_output};
use crate::util::ds_home;
...
/// session.
fn is_cached_credential_compatible(auth: &DsAuth, ds_com_config: &DsComConfig) -> bool {
    let expected_issuer = ds_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| ds_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(ds_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("DS_LOGIN_DEVICE_FLOW")
...
/// DeepSeek OAuth2 provider supports it. Enterprise OIDC (`oidc=Some`) always uses
...
/// Whether interactive DeepSeek OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `DS_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `ds_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("DS_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `ds.cli/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `ds.cli/auth/get_url` ACP response.
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.ds_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = ds_com_config.oidc.is_some(),
        has_oauth2 = ds_com_config.oauth2.is_some(),
        has_external_auth = ds_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if reauth {
        auth_manager.clear()?;
        // Also remove the legacy platform.deepseek.com scope so stale tokens
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, ds_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, ds_com_config)
        }) {
            ds_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = ds_com_config.auth_provider_command {
    ...
    // `ds login --devbox` uses run_devbox_login and is not gated here.
    if !ds_com_config.blocks_automatic_oidc()
    ...
    // DeepSeek OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / DS_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(ds_com_config) {
        return crate::auth::oidc::run_login_flow(ds_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = ds_com_config.oauth2 {
    ...
    tracing::error!(
        "auth: no OAuth2 configuration available (neither enterprise OIDC nor DeepSeek OAuth2 configured)"
    );
    anyhow::bail!(
        "No OAuth2 configuration available. Run `ds login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(ds_com_config: &DsComConfig) -> Option<DsAuth> {
    let ds_home = ds_home::ds_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&ds_home, ds_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(ds_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(ds_com_config).await {
        return Some(auth);
    }
    let ds_home = ds_home::ds_home();
    let auth_manager = Arc::new(AuthManager::new(&ds_home, ds_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, ds_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<DsAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_ds_auth() && a.refresh_token.is_some())
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = ds_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: DsAuth) -> DsAuth {
...
fn report_signed_in(auth: &DsAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<DsAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(ds_com_config).await)
    } else {
        ensure_authenticated(ds_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `ds login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `DS_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // oauth2=None) always uses the loopback flow; only the DeepSeek OAuth2 provider
    // supports the device flow. Without this guard, `ds login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.ds_com_config, login_override).await {
        if config.ds_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!("Sign-in is not available for this deployment. Set DEEPSEEK_API_KEY instead.");
        }
        let ds_home = ds_home::ds_home();
        let auth_manager = Arc::new(AuthManager::new(&ds_home, config.ds_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.ds_com_config) {
            eprintln!(
    ...
    // config was actually applied; `ds setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `DEEPSEEK_API_KEY` / `DS_CODE_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        ds_telemetry::external::set_identity(
            ds_telemetry::external::IdentityAttrs::default(),
        );
        ds_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_ds_api_key_env(),
    })
}

/// `ds logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let ds_home = ds_home::ds_home();
    let auth_manager = AuthManager::new(&ds_home, config.ds_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via DEEPSEEK_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("DEEPSEEK_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::DS_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `DS_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("DS_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("DS_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("DS_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A ds.com first-party (deepseek-issuer) OIDC session — `is_ds_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> DsAuth {
        DsAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(DS_OAUTH2_ISSUER.to_string()),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), DsComConfig::default());
        ...
        // Third-party external (no deepseek issuer) stays excluded.
        mgr.hot_swap(DsAuth {
            oidc_issuer: None,
            auth_mode: AuthMode::External,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only ds_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), DsComConfig::default()));
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = DsComConfig {
            auth_provider_command: Some("printf '%s' ds-ext-token".to_string()),
            ..DsComConfig::default()
        };
        let mgr = Arc::new(
        ...
        assert_eq!(auth.map(|a| a.key), Some("ds-ext-token".to_string()));
    }
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), DsComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("DS_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("DS_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `ds login` must use loopback, not device —
        ...
        // The DeepSeek OAuth2 provider (oidc=None, oauth2=Some) does use device.
        let ds = DsComConfig::default();
        assert!(ds.oauth2.is_some() && ds.oidc.is_none());
        assert!(cli_should_use_device(&ds, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> DsAuth {
        DsAuth {
    ...
    fn pinned_cfg(team: &str) -> DsComConfig {
        DsComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = DsAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(DS_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = DsComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = DsAuth {
        ...
        // DS_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into DS: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `ds login` passes `on_stderr=None`; stderr must be inherited so

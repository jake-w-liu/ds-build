...
pub(super) const LEGACY_SCOPE: &str = "https://accounts.ds.cli/sign-in";

/// auth.json scope key for plain API key auth (desktop login, `ds login --api-key`).
pub const API_KEY_SCOPE: &str = "ds::api_key";
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from ds-desktop login or `ds login --api-key`)
    ApiKey,
}
...
pub struct DsAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_ds_code_access: Option<bool>,
    ...
    /// provider's `issuer` claim. In both modes an deepseek issuer marks the
    /// credential first-party (`is_ds_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for DsAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DsAuth")
...
impl DsAuth {
    ...
    /// `true` when the token comes from a first-party DeepSeek account —
    /// either an OIDC login against https://api.deepseek.com (or the local-dev
    /// equivalent), or an external auth provider that declared an DeepSeek
    ...
    pub fn is_ds_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_ds_oauth2_issuer),
            AuthMode::ApiKey | AuthMode::WebLogin => false,
        }
    }

    /// `true` when this auth can access ds.com managed MCP connectors.
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_ds_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_ds_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_ds_auth(),
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &DsAuth) {
...
#[cfg(test)]
impl DsAuth {
    /// Returns a `DsAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// DsAuth { key: "my-key".into(), ..DsAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, DsAuth>;
...
/// header prefix (`eyJ0edsOiJh…`); the tail (signature bytes) is
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `ds login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<DsAuth> {
...
/// Early-invalidation buffer. Override with `DS_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("DS_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &DsAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &DsAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> DsAuth {
        DsAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_ds_auth_matrix() {
        use crate::auth::DS_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| DsAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an deepseek issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(DS_OAUTH2_ISSUER)).is_ds_auth());
        assert!(with_issuer(AuthMode::External, Some(DS_OAUTH2_ISSUER)).is_ds_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_ds_auth());
        assert!(!with_issuer(AuthMode::External, None).is_ds_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_ds_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_ds_auth());

        // ApiKey / WebLogin stay false even with an deepseek issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(DS_OAUTH2_ISSUER)).is_ds_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(DS_OAUTH2_ISSUER)).is_ds_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(DS_OAUTH2_ISSUER)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(DS_OAUTH2_ISSUER)).is_session_auth());
    }
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "DsProPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("DsProPro"));
    }

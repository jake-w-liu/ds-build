...
use crate::util::ds_auth_credentials::DsAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::DsAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| ds_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: DsAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = DsAuthCredentials::new(None);
...
///
/// - `"ds-shell"`   — classic DS CLI / TUI (ds-shell)
/// - `"ds-pager"`   — new DS Pager / TUI (ds-pager)
/// - `"ds-desktop"` — DS Desktop app
/// - `"ds-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-ds-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-ds-client-identifier`
///
/// to the underlying `ds-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-DS-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> ds_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn ds_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        ds_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(ds_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = DsAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in ds-file-utils)
...
impl ds_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = DsAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        ds_telemetry::external::set_identity(
            ds_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `ds-pager` and `ds-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> ds_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let ds_com_config = crate::auth::DsComConfig::default();
    let exporter = ds_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = ds_com_config.token_header.clone();
    let ds_home = crate::util::ds_home::ds_home();
    let bootstrap = Arc::new(AuthManager::new(&ds_home, ds_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    ds_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::DsComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use ds_auth::AuthCredentialProvider;
    /// Serializes tests that pin `DS_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `DS_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("DS_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("DS_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("DS_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("DS_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> DsAuth {
        DsAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<DsAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::DsComConfig::default(),
        ));
        mgr.hot_swap(DsAuth {
            key: "stale".into(),
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(DsAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use ds_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("ds-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("ds-token-EX").as_str())
        );
    ...
    /// user token, matching the precedence in `DsAuthCredentials::apply`.

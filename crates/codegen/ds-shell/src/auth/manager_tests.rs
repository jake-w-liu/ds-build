...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> DsAuth {
    DsAuth {
...
fn auth_scope_uses_oauth2_when_present() {
    let cfg = DsComConfig::default();
    // Default config always has oauth2 set to the DeepSeek defaults.
    assert_eq!(
        cfg.auth_scope(),
        format!(
            "{}::{}",
            crate::auth::config::DS_OAUTH2_ISSUER,
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older ds version).
...
/// legacy `https://accounts.ds.cli/sign-in` entry from auth.json.
#[test]
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(DsAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `ds login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `ds logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `ds login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_ds_code_access`. The `&mut DsAuth` signature already
...
/// contributor adding a token-like field to both `DsAuth` and
...
fn apply_user_info_enrichment_preserves_token_fields() {
    ...
    assert_eq!(disk.has_ds_code_access, snapshot.has_ds_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    use ds_tools::types::ApiKeyProvider;
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), DsComConfig::default()).with_proxy_base_url(&base);
...
fn oidc_session_for_team(principal_id: &str) -> DsAuth {
    DsAuth {
        key: team_jwt(principal_id),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        oidc_issuer: Some(crate::auth::config::DS_OAUTH2_ISSUER.to_string()),
        oidc_client_id: Some("client".into()),
        ..DsAuth::test_default()
    }
}
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || DsAuth {
        key: "ds-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..DsAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), DsComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("ds-cached-key".to_string()),
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use ds_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use ds_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, DsComConfig::default()));

...
pub(crate) struct ExternalAuthOutput {
    ...
    /// Token issuer. An DeepSeek issuer marks the credential as first-party;
    /// see [`DsAuth::is_ds_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `DsAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<DsAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<DsAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &DsAuth) -> Option<DsAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_ds_auth() {
        ...
        // deepseek issuer claim → first-party session (relay-eligible).
        let auth = parse_output(&ok(
            r#"{"access_token":"t","expires_in":900,"issuer":"https://api.deepseek.com"}"#,
        ))
        .unwrap();
        assert_eq!(auth.oidc_issuer.as_deref(), Some("https://api.deepseek.com"));
        assert!(auth.is_ds_auth());

        // Non-deepseek issuer is stored but stays third-party.
        let auth = parse_output(&ok(
    ...
    #[test]
    fn sync_sets_ds_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $DS_AUTH_EXPIRED", true).unwrap();

...
//!
//! 1. [`ds_telemetry::unified_log::warn`] for the local
//!    `~/.ds/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`ds_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`ds_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`ds_sampler::SamplerConfig::attribution_callback`] at every
...
use ds_sampler::{Auth401AttributionCallback, SamplingConsumer};
use ds_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`ds_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `ds_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn ds_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `ds_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `ds-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `ds-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.ds/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `DsAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `DsAuth` (or
    ...
    // `ds_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.ds/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `ds_tools::Auth401AttributionCallback`

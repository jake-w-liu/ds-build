...
use crate::auth::model::DsAuth;
use crate::auth::token_type::TokenType;
use ds_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&DsAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        ds_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<DsAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<DsAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<DsAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.ds_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<DsAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            ds_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<DsAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<DsAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &DsAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, DsAuth};
    ...
    fn rejected_cred() -> Option<DsAuth> {
        Some(DsAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), DsComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<DsAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.ds_com_config().auth_scope();
        let fresh = DsAuth {

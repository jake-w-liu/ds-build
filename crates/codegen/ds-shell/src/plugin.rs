...
use ds_agent::plugins::discovery::PluginScope;
use ds_agent::plugins::git_install::{self, UpdateStatus};
use ds_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use ds_plugin_marketplace::git::{self, SourceCacheLease};
use ds_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.ds/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &ds_agent::plugins::install_registry::MarketplaceProvenance,
...
pub fn update_plugins_by_selector(
    ...
    for (repo_key, repo) in &repos_to_update {
        let outcome = if repo.marketplace.is_some() {
            ...
            match git_install::update_repo(repo_key, repo, false) {
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `ds plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `ds plugin install ./{name}`, or add a \
                     source with `ds plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `ds plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `ds plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &ds_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.ds/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user ds via user_ds_home() (None when no home resolves) and
    // home separately, so removal still runs from $DS_HOME when no home dir
    // exists, and never touches a cwd-relative .ds.
    let home = dirs::home_dir();
    let ds = ds_config::user_ds_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref ds) = ds {
        settings_candidates.push(ds.join("settings.local.json"));
        settings_candidates.push(ds.join("settings.json"));
    }
    ...
    if let Some(ref ds) = ds {
        known_candidates.push(ds.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "DeepSeek Official",
                "https://github.com/deepseek-ai/plugin-marketplace.git"
            )),
            "DeepSeek Official (deepseek-ai/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "DeepSeek Official (pin: sentry@deepseek-ai/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "DeepSeek Official (deepseek-ai/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - DeepSeek Official (deepseek-ai/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "deepseek-ai/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("ds plugin install ./sentry"), "{msg}");
        assert!(msg.contains("ds plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["DeepSeek Official (pin: sentry@deepseek-ai/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - DeepSeek Official (pin: sentry@deepseek-ai/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("ds plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/deepseek-ai/plugin-marketplace.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("DeepSeek Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "DeepSeek Official (deepseek-ai/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:deepseek-ai/plugin-marketplace.git"),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("deepseek-ai/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "deepseek-ai/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("DeepSeek Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("deepseek-ai/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "DeepSeek Official");
            }
    ...
    fn plan_install_bare_name_official_match_proceeds_despite_skip() {
        ...
        let plan = plan_install(&sources, "sentry", None, |source| {
            if source.name == "DeepSeek Official" {
                Ok(vec![mp_entry("sentry")])
            } else {
                Err("sync failed".to_string())
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> ds_workspace::permission::resolution::MarketplaceAllowlist {
        ds_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = ds_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "deepseek-ai/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "DeepSeek Official");
    }

...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        ds_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
/// would just 401).
fn eligible_team_principal(auth: DsAuth) -> Option<DsAuth> {
...
/// config is a ds.com feature with one ds.com auth.
fn read_active_team_auth() -> Option<DsAuth> {
    let home = crate::util::ds_home::ds_home();
...
/// files (TUI tick vs `ds login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `ds setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `DS_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("DS_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `ds-http` classifier.
...
/// Override with `DS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("DS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** DsBuildDeployment UUID (the id
...
/// Resolve deployment key from `DS_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (ds setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("DS_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.ds/`, trying the
...
struct SyncOutcome {
    ...
    /// `ds setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// read-only `ds setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if ds_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(ds_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        ds_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
/// Post-login hook for `ds login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<DsAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `ds setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(DsAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// `ds setup`); ceasing to serve `fail_closed` rolls back.
pub fn managed_policy_gate() -> Result<(), String> {
    // Skip under the lib unit-test build only: `bootstrap` reaches this without a staged
    // `DS_HOME` and would flake on the dev machine's real marker/auth. The pure decision
...
/// self-healing class as deleting the user-writable files outright; /etc/ds and MDM are unaffected.
...
/// Outcome of the `ds setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.ds`.
    Installed,
...
/// Result of `ds setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `ds setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `ds setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `ds setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `ds setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && ds_config::signed_policy::verification_active()
...
/// Run the `ds setup` sync for the current principal. The caller must check

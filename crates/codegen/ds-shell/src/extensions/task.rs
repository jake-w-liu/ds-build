...
use ds_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `ds.cli/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ds-pager) build
...
/// Wire DTO for the `ds.cli/task/kill` ext response payload (nested under
...
/// Wire DTO for the `ds.cli/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ds-pager) build
...
/// Wire DTO for the `ds.cli/subagent/cancel` response payload (under `result` in
...
/// Handle `ds.cli/scheduler/*` extension methods.
...
/// Handle `ds.cli/subagent/*` extension methods.
...
mod tests {
    ...
    // ── ds.cli/subagent/cancel outcome wire DTO ──────────────────────────

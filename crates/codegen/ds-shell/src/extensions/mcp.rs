...
//!
//! - `ds.cli/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `ds.cli/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `ds.cli/mcp/servers_updated` — notification pushed when managed configs resolve
//! - `ds.cli/mcp/server_status` — per-server delta pushed by the
...
// rmcp is quarantined in ds-mcp; see that crate's docs.
use ds_mcp::rmcp;
...
/// Agent-only `ds.cli/mcp/*` ACP method/notification names.
///
/// Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in
/// `ds_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "ds.cli/mcp/";

    pub const LIST: &str = "ds.cli/mcp/list";
    pub const READ_RESOURCE: &str = "ds.cli/mcp/read_resource";
    pub const AUTH_STATUS: &str = "ds.cli/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "ds.cli/mcp/auth_trigger";
    pub const TOGGLE: &str = "ds.cli/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "ds.cli/mcp/toggle_tool";
    pub const UPSERT: &str = "ds.cli/mcp/upsert";
    pub const DELETE: &str = "ds.cli/mcp/delete";

    pub const SERVERS_UPDATED: &str = "ds.cli/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "ds.cli/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "ds.cli/mcp/init_progress";
}
...
// Re-export the `ds.cli/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `ds.cli/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    gateway: &ds_acp_lib::AcpAgentGatewaySender,
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent ds.cli/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `ds.cli/mcp/*` methods this agent services, resolved from the wire string.
///
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<ds_tools::bridge::ToolBridge>,
    event_writer: Option<&ds_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(ds_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = ds_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from ds-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `ds-mcp` directly.
...
impl ds_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<ds_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(ds_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<ds_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(ds_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(ds_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(ds_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    /// The emit-only reverse method (`ds.cli/mcp/sdk_call`) shares the `ds.cli/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound ds.cli/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    fn test_mcp_list_response_serialization() {
            ...
            servers: vec![
                McpServerEntry {
                    ...
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("DS CLI".to_string()),
                    },
                    source_label: None,
                    session: None,
                },
        ...
        assert_eq!(json["servers"][0]["scopeName"], "DS CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("ds_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["ds_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "ds_com_slack");

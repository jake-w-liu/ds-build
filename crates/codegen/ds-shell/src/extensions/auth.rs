//! `ds.cli/auth/*` and legacy `ds.cli/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ds.cli/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "ds.cli/getApiKey" => handle_get_api_key(),
        "ds.cli/setApiKey" => handle_set_api_key(args),
        "ds.cli/auth/submit_code" => handle_submit_code(agent, args),
        "ds.cli/auth/get_url" => handle_get_url(agent).await,
        "ds.cli/auth/logout" => handle_logout(agent, args).await,
        "ds.cli/auth/info" => handle_info(agent),
        "ds.cli/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_ds_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            ...
            crate::auth::store_api_key(&ds_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("DEEPSEEK_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&ds_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `ds-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a ds-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("ds-asset:///{key}")),
        None => None,
    };

...
mod tests {
    ...
    fn test_auth() -> crate::auth::DsAuth {
        crate::auth::DsAuth {
            key: "token".to_string(),
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".ds/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".ds/roles")).unwrap();
        std::fs::write(
            project_root.join(".ds/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".ds/roles/reviewer.toml"),

...
//! using the ds-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `ds.cli/code/goto-definition` | Definition location(s) for symbol at position |
//! | `ds.cli/code/goto-references` | Reference location(s) for symbol at position |
//! | `ds.cli/code/find-definitions` | All definitions of a symbol by name |
//! | `ds.cli/code/find-references` | All references to a symbol by name |
//! | `ds.cli/code/status` | Indexing status |
...
/// Reason string for the `ds.cli/code/status` response.
///
...
pub async fn handle(
    ...
    use ds_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "ds.cli/code/goto-definition" => {
...
fn to_code_nav_ext_response(resp: ds_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for ds-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise ds.cli/codeNavigation.enabled to use code navigation"
        }

//! `ds.cli/session/repair` — out-of-band recovery for sessions bricked by
...
//!
//! Repairs via [`ds_chat_state::compaction_utils::repair_history`]. Resident
...
use ds_chat_state::compaction_utils::HistoryRepairReport;
...
/// Response payload for `ds.cli/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ds.cli/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `ds_root` is injectable for tests.
async fn repair_on_disk(ds_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &ds_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(ds_root.to_path_buf());
    ...
    let report = ds_chat_state::compaction_utils::repair_history(&mut chat_history);

//! ACP extension handler for bulk session updates (`ds.cli/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_ds.cli/session/update"` for
//! DeepSeek extensions) and extract the notification payload from `params`.
...
/// Send updates as chunked `_ds.cli/session/updates/chunk` notifications.
...
mod tests {
    ...
    fn dummy_gateway() -> ds_acp_lib::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("ds.cli/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_ds.cli/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    fn ds_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_ds.cli/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }

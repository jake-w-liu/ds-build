//! `ds.cli/memory/flush`, `ds.cli/memory/rewrite`, and `ds.cli/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("ds.cli/compact_conversation") => handle_compact(agent, args).await,
        "ds.cli/memory/flush" => handle_flush(agent, args).await,
        "ds.cli/memory/rewrite" => handle_rewrite(agent, args).await,

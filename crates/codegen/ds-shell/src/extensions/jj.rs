//! Jujutsu extension handlers — delegates to [`ds_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use ds_workspace::session::git::{CommitData, StageData};
use ds_workspace::session::jj;

/// Handle a `ds.cli/git/*` method for a jj-colocated repo.
///
...
pub async fn try_handle(
    ...
    match method {
        "ds.cli/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "ds.cli/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report
        // the working-copy commit (`@`), consistent with `status`/`info`.
        "ds.cli/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "ds.cli/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area — stage/unstage are no-ops
        "ds.cli/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "ds.cli/git/stage/content" | "ds.cli/git/unstage" => Some(to_ext_response(Ok(Empty {}))),
        ...
        "ds.cli/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "ds.cli/git/stash" => Some(Err(acp::Error::invalid_params()

//! `ds.cli/feedback`, `ds.cli/feedback/dismiss`, `ds.cli/btw`, and `ds.cli/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ds.cli/feedback" | "ds.cli/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        m if m.starts_with("ds.cli/review") => {
...
/// Handle `ds.cli/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set DS_FEEDBACK_ENABLED=true or \
                ...
                match serde_json::from_str::<ClientFeedbackInput>(args.params.get()) {
                    ...
                    Err(_) => {
                        ...
                        ClientFeedbackInput {
                            ...
                            client_type:
                                ds_cli_proxy_types::feedback_types::ClientType::Tui,
                            rating_type: None,
                            rating_value: None,
...
///
/// Methods:
/// - `ds.cli/review/comment`: record a new inline code comment to cloud storage
/// - `ds.cli/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ds.cli/review/comment" => {

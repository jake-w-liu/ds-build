//! `ds.cli/plugins/*` extension handlers.
//!
...
pub fn loaded_plugin_to_info(plugin: &ds_agent::plugins::LoadedPlugin) -> PluginInfo {
    use ds_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &ds_agent::plugins::PluginOrigin) -> PluginOrigin {
    use ds_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectDs => PluginOrigin::ProjectDs,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserDs => PluginOrigin::UserDs,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ds.cli/plugins/action" => {
            let req: ds_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use ds_agent::plugins::PluginOrigin as AgentOrigin;
    use ds_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> ds_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        ds_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("DeepSeek Official".to_string()),
        ...
        assert_eq!(info.marketplace_source.as_deref(), Some("DeepSeek Official"));
    }

...
//! lifecycle:
//!
//! - `ds.cli/session/rename`                  rename a session locally + remote
//! - `ds.cli/session/delete`                  delete a session locally + remote
//! - `ds.cli/session/update_mcp_servers`      mid-session MCP server swap
//! - `ds.cli/session/fork`                    fork a session into a new one
//! - `ds.cli/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `ds.cli/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `ds.cli/internal/reload_skills`          skills file watcher fan-out
//! - `ds.cli/internal/reload_models`          model list hot-reload from config.toml
//! - `ds.cli/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `ds.cli/internal/auth_cleared`           auth hot-clear cleanup
//! - `ds.cli/plugins/reload`                  rebuild shared plugin registry
//! - `ds.cli/commands/list`                   list slash commands
...
use ds_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ds.cli/session/rename" => handle_session_rename(agent, args).await,
        "ds.cli/session/delete" => handle_session_delete(agent, args).await,
        "ds.cli/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "ds.cli/session/fork" => handle_session_fork(agent, args).await,
        "ds.cli/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "ds.cli/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "ds.cli/internal/reload_skills" => handle_reload_skills(agent),
        "ds.cli/internal/reload_models" => handle_reload_models(agent),
        "ds.cli/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "ds.cli/internal/auth_cleared" => handle_auth_cleared(agent),
        "ds.cli/plugins/reload" => handle_plugins_reload(agent).await,
        "ds.cli/commands/list" => handle_commands_list(agent, args).await,
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires DeepSeek OAuth credentials"),
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `ds sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires DeepSeek OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.ds/config.toml`,
...
/// Hot-reload the model catalog from `~/.ds/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `ds.cli/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `ds.cli/commands/list` (the pull used by ds-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .ds/plugins under it, plus

...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "DS_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$DS_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${DS_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$DS_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$DS_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the DS_MEMORY env var.
...
/// Run `f` with DS_MEMORY explicitly unset.
fn without_ds_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("DS_MEMORY", None, f)
}
/// Run `f` with DS_MEMORY set to a specific value.
fn with_ds_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("DS_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_ds_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "DS_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_ds_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "DS_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_ds_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "DS_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_ds_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "DS_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_ds_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override DS_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_ds_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, DS_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_ds_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override DS_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("deepseek-v4-pro"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$DS_TEST_USER $DS_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the DS_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with DS_SUBAGENTS explicitly unset.
fn without_ds_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("DS_SUBAGENTS", None, f)
}
/// Run `f` with DS_SUBAGENTS set to a specific value.
fn with_ds_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("DS_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_ds_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn subagents_config_cli_flag_enables() {
    without_ds_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
        let sa = SubagentsConfig::resolve(Some(true), &config, None);
        assert!(sa.enabled);
    });
}
#[test]
fn subagents_config_cli_no_subagents_force_disables() {
    without_ds_subagents(|| {
        // Config defaults to enabled; CLI Some(false) must win.
        let config = toml::Value::Table(toml::map::Map::new());
        let sa = SubagentsConfig::resolve(Some(false), &config, None);
        assert!(!sa.enabled, " --no-subagents must force-disable");
    });
}
#[test]
fn subagents_config_cli_no_subagents_overrides_config_enabled() {
    without_ds_subagents(|| {
        let config: toml::Value = toml::from_str("[subagents]\nenabled = true\n").unwrap();
        let sa = SubagentsConfig::resolve(Some(false), &config, None);
        assert!(!sa.enabled, "CLI force-disable beats config enabled=true");
    });
}
#[test]
    ...
    with_ds_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(None, &config, None);
            assert!(! sa.enabled, "DS_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_ds_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(None, &config, None);
            assert!(
                ! sa.enabled, "DS_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_ds_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(Some(true), &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override DS_SUBAGENTS=0");
        },
    );
}
#[test]
fn subagents_config_models_parsed() {
    without_ds_subagents(|| {
        ...
        assert_eq!(sa.models.get("plan").unwrap(), "deepseek-v4-pro.5");
    });
}
#[test]
    ...
    with_ds_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(None, &config, None);
            assert!(sa.enabled, "DS_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "ds-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "DS_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("DS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    with_env_var_opt(
        ...
        ws,
        || with_env_var_opt(
            "DS_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "DS_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("DS_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_ds_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_ds_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "DS_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("DS_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_ds_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_ds_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_ds_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_ds_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "DS_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".ds/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("deepseek-v4-flash"));
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".ds/prompts/impl.md"));
}
#[test]
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".ds").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".ds/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".ds/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".ds").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".ds").join("bundled");
    std::fs::create_dir_all(workspace.join(".ds").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".ds").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".ds").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".ds").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".ds/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".ds/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ds/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ds/personas/reviewer.toml"),
    ...
    with_env_var(
        ...
        || {
            ...
            >(
                ...
                .unwrap();
            let resolved = SubagentsConfig::resolve(Some(true), &config, Some(&workspace));
            assert_eq!(
    ...
    std::fs::remove_file(workspace.join(".ds/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".ds/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
    ...
    std::fs::remove_file(home.join(".ds/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".ds/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.ds/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn roles_coexist_with_models_and_toggle() {
    ...
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.enabled);
    assert_eq!(cfg.models.get("explore").map(| s | s.as_str()), Some("ds-fast"));
...
[model.ds-build]
base_url = "https://inference.acme-corp.example/ds/v1"
...
[endpoints]
deployment_key = "ds-token-ENTERPRISE"
ds_api_base_url = "https://inference.acme-corp.example/ds/v1"
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!("https://managed-api.example/v1", cfg.endpoints.ds_api_base_url);
    assert_eq!(
...
#[test]
fn validate_hooks_path_rejects_outside_ds_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("must be under ~/.ds/"),
        "should mention ~/.ds/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let ds_home = crate::util::ds_home::ds_home();
    let traversal = format!("{}/../evil", ds_home.display());
...
#[test]
fn validate_hooks_path_accepts_ds_hooks_subdir() {
    let ds_home = crate::util::ds_home::ds_home();
    let valid_path = ds_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.ds/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use ds_workspace::permission::resolution::ManagedSettingsFeatures;
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own ds yolo.
#[test]
...
fn simulate_release_build() -> ds_test_support::EnvGuard {
    ds_test_support::EnvGuard::set(ds_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). DS_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores DS_HOME even on panic). No user-global
/// `$DS_HOME/config.toml` is seeded: `ds_home()` is `OnceLock`-cached,
...
/// in `ds-agent`. An ABSOLUTE plugin path is used so the merged
...
/// DS_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use ds_agent::plugins::{TrustStore, discover_plugins};
    use ds_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("DS_HOME", home.path());
    let _flag = EnvGuard::unset("DS_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. DS_HOME-isolated (empty
/// store); DS_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let ds = repo.path().join(".ds");
    std::fs::create_dir_all(&ds).unwrap();
    std::fs::write(ds.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {

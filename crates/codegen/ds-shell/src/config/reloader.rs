...
use crate::auth::{DsAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.ds/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.ds/config.toml`,
    ...
    /// (`<cwd>/.ds/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// ds process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.ds/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.ds/models_cache.json` was rewritten on disk (possibly by another
    ...
    ModelsCacheChanged,
    /// Updated UI settings — agent broadcasts `ds.cli/config_changed` to IPC clients.
    Ui {
        theme: Option<String>,
        yolo: bool,
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.ds_home.join("auth.json");
            ...
            // same tick (e.g. `~/.ds/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.ds_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.ds/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.ds/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.ds/config.toml` | `<cwd>`           |
...
/// for `.ds/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.ds/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `ds inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> ds_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> ds_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<DsAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.ds/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".ds").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .ds/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.ds/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.ds/skills".to_string()]);
...
theme = "dark"
yolo = true
fork_secondary_model = "deepseek-v4-pro.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("deepseek-v4-pro.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.ds/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.ds/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.ds/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.ds/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(

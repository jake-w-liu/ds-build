...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.ds/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** ds process
    /// sharing the same `~/.ds` (the writer may also be this process;
...
/// Watches `~/.ds/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.ds/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.ds/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let ds_home_buf = ds_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(ds_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(ds_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(ds_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(ds_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %ds_home.display(),
                    error = %e,
                    "failed to watch ds home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.ds/config.toml` so the loop above
        // watches `<cwd>/.ds/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.ds/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.ds/config.toml` trigger a
    ...
    /// `<cwd>/.ds/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.ds/` does not yet
/// exist at session-open time, the `.ds/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.ds/` followed by a
/// write to `<cwd>/.ds/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ds_dir = cwd.join(".ds");
    if let Err(e) = debouncer
        .watcher()
        .watch(&ds_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .ds directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ds_dir = cwd.join(".ds");
    if let Err(e) = debouncer.watcher().unwatch(&ds_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .ds directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.ds/` hasn't
...
/// Watches skill directories (`~/.ds/skills/`, `<repo>/.ds/skills/`, etc.)
...
/// recursively: `ds_home` (`~/.ds`, or `$DS_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.ds` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, ds_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, ds_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".ds", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, ds_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(ds_home) {
        return true;
    }
...
/// `~/.ds` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](ds_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = ds_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &ds_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &ds_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only ds_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_ds_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let ds_home = home.join(".ds");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &ds_home, Some(home));

        // ds_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&ds_home));
        ...
        assert!(!g(&home.join("repo").join(".ds")));
    }

    /// Regression for the ~/.ds inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.ds/worktrees/`) must not
    ...
    /// A write to `<ds_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another ds process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.ds/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let ds_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_ds = cwd.path().join(".ds");
        fs::create_dir_all(&project_ds).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_ds.join("config.toml"), "").unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.ds/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.ds/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_ds = new_cwd.path().join(".ds");
        fs::create_dir_all(&project_ds).unwrap();
        fs::write(project_ds.join("config.toml"), "").unwrap();

...
pub enum ClientMode {
    /// Headless mode (ds agent, ds agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (ds agent stdio, ds -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `ds.cli/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `ds update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `ds update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `ds update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["ds-a".into(), "ds-b".into()],
            pid: 4242,
        };

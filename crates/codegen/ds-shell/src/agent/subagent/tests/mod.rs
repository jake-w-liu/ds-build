...
async fn subagent_inherits_session_cli_overrides() {
    use ds_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use ds_agent::config::PermissionMode;
    const PIN: &str = ds_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let persisted_id = match cmd_rx.try_recv().expect("persist hop must fire") {
        SessionCommand::DsSessionNotification { notification } => {
            notification
                .meta
                .as_ref()
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/ds/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/ds/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn ds_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = ds_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
#[test]
fn evict_stale_completed_caps_map_size() {
    let mut coordinator = SubagentCoordinator::new();
    // Insert more than MAX_COMPLETED_RETAINED with distinct completion ages.
    for i in 0..80 {
        let id = format!("sub-{i}");
        let age_secs = 80 - i; // lower i = older completion
        coordinator.completed.insert(
            ...
            CompletedSubagent {
                ...
                persona: None,
                started_at: std::time::Instant::now()
                    - std::time::Duration::from_secs(age_secs + 1),
                completed_at: std::time::Instant::now()
    ...
    assert_eq!(coordinator.completed.len(), 80);
    coordinator.evict_stale_completed();
    assert!(
        coordinator.completed.len() <= SubagentCoordinator::MAX_COMPLETED_RETAINED,
        "cap completed map: got {}",
        coordinator.completed.len()
    );
    // Newest completions (highest i) should remain.
    assert!(coordinator.completed.contains_key("sub-79"));
...
fn move_to_completed_evicts_immediately() {
    ...
    for i in 0..SubagentCoordinator::MAX_COMPLETED_RETAINED {
        let id = format!("old-{i}");
        coordinator.completed.insert(
            id.clone(),
            CompletedSubagent {
                ...
                completed_at: std::time::Instant::now()
                    - std::time::Duration::from_secs(
                        (SubagentCoordinator::MAX_COMPLETED_RETAINED - i) as u64 + 1,
                    ),
                result: SubagentResult {
                    success: true,
                    ..Default::default()
    ...
    assert_eq!(
        coordinator.completed.len(),
        SubagentCoordinator::MAX_COMPLETED_RETAINED
    );
    coordinator.insert(dummy_tracker("fresh", "parent", "explore", "fresh desc"));
    let _ = coordinator.move_to_completed(
        "fresh",
        "fresh desc".into(),
        "explore".into(),
        SubagentResult {
    ...
    assert!(
        coordinator.completed.len() <= SubagentCoordinator::MAX_COMPLETED_RETAINED,
        "completed map must stay capped after move_to_completed, got {}",
        coordinator.completed.len()
    );
    assert!(
        coordinator.completed.contains_key("fresh"),
        "newest completion must be retained"
    );
    assert!(
        !coordinator.completed.contains_key("old-0"),
        "oldest completion must be evicted"
    );
}

#[test]
fn push_pending_completion_caps_buffer() {
    let mut coordinator = SubagentCoordinator::new();
    for i in 0..(SubagentCoordinator::MAX_PENDING_COMPLETIONS + 10) {
        coordinator.push_pending_completion(SubagentCompletionSummary {
            subagent_id: format!("sub-{i}"),
            subagent_type: "explore".into(),
            description: "d".into(),
            success: true,
            duration_ms: 1,
            tool_calls: 0,
            turns: 1,
            output: std::sync::Arc::from(format!("out-{i}")),
        });
    }
    assert_eq!(
        coordinator.drain_pending_completions().len(),
        SubagentCoordinator::MAX_PENDING_COMPLETIONS
    );
}
...
fn dummy_tracker(
    ...
    let cwd = ds_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn ds_workspace::file_system::AsyncFileSystem> = Arc::new(
        ds_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
    ...
    let handle = SessionHandle {
        ...
        max_turns: None,
        hunk_tracker_handle: ds_hunk_tracker::HunkTrackerHandle::noop(),
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(ds_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = ds_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(ds_tool_types::SubagentCapabilityMode::All)
    );
}
#[test]
fn role_default_used_when_no_explicit_override() {
    ...
    assert_eq!(
        resolved.capability_mode, Some(ds_tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(ds_tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = ds_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = ds_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
    ...
    if let ConversationItem::User(ref u) = ctx.conversation[1] {
        let text: String = u
            ...
            .filter_map(|p| match p {
                ds_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        ds_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use ds_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use ds_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("deepseek-v4-pro.5");
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    ds_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
async fn handle_subagent_request_rejects_file_as_cwd() {
    ...
    let tmp_file = tmp_dir.path().join("ds-test-cwd-file");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("DS_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("DS_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("DS_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("DS_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("DS_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("DsBuild:read_file");
...
/// Regression: on the DEFAULT ds-build host —
...
async fn background_unknown_type_emits_subagent_finished_notification() {
    ...
    while let Ok(cmd) = cmd_rx.try_recv() {
        if let SessionCommand::DsSessionNotification { notification } = cmd
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        if let ds_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            ...
            assert_eq!(req.method.as_ref(), "ds.cli/session_notification");
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ds_test_utils::require_git!();
    use ds_test_utils::git::{git_commit_all, init_git_repo};
    ...
    ds_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    ds_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    use crate::agent::auth_method::{CACHED_TOKEN_AUTH_METHOD_ID, DEEPSEEK_API_KEY_METHOD_ID};
    use ds_chat_state::AuthType;
    let session = acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID);
    let api_key = acp::AuthMethodId::new(DEEPSEEK_API_KEY_METHOD_ID);
    let byok = byok_model_entry("ds-byok");
    let plain = test_model_entry("ds-plain");
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("deepseek-v4-flash".to_string(), test_model_entry("ds-3-2025-02-15"));
    assert!(
        super::handle_request::task_model_override_error(Some("deepseek-v4-flash"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
        "key lookup should succeed"
    );
    assert!(
        super::handle_request::task_model_override_error(Some("ds-3-2025-02-15"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("ds models"));
    }
    assert!(
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = ds_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> ds_sampling_types::SamplingConfig {
    use std::num::NonZeroU64;
    ds_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> ds_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = ds_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    ds_chat_state::ChatStateActor::spawn(
        vec![],

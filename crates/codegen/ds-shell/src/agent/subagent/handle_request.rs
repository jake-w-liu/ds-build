...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use ds_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(ds_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = ds_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != ds_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != ds_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("ds-subagent-worktrees").join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: ds_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = ds_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        ds_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(ds_fast_worktree::WorktreeKind::Subagent)
    ...
    {
        use ds_tools::implementations::ds_build::task::MAX_SUBAGENT_DEPTH;
    ...
    {} else if context_source != InitialContextSource::Resumed && !verbatim_mirror_fork {
        if let Some(ref pi) = effective_runtime.persona_instructions {
            let reminder = ds_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = ds_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
    ...
    let parent_traceparent = ds_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = ds_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<ds_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = ds_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<ds_tools::registry::types::ToolConfig> = vec![
            (& ds_build::ReadFileTool).into(), (& ds_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = ds_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == ds_hooks::event::HookEventName::Stop {
                            s.event = ds_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    ds_agent::config::McpServerRef::Named(name) => {
                    ...
                    ds_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            ds_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    ds_telemetry::session_ctx::log_event(ds_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            ds_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            ds_chat_state::CompactionMode::Summary,
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    ds_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                ds_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    ds_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
    ...
    let outcome = if result.success {
        ds_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        ds_telemetry::events::Outcome::Cancelled
    } else {
        ds_telemetry::events::Outcome::Error
    };
    ds_telemetry::session_ctx::log_event(ds_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/ds/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);

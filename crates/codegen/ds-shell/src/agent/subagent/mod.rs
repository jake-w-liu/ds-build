...
use ds_workspace::file_system::AsyncFileSystem;
use ds_hunk_tracker::HunkTrackerHandle;
...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<ds_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn ds_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: ds_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::DsAuth>,
    ...
    pub parent_terminal_backend: Option<Arc<dyn ds_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<ds_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<ds_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: ds_tools::implementations::ds_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: ds_tools::implementations::ds_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: ds_tools::implementations::ds_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<ds_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<ds_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<ds_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<ds_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<ds_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<ds_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: ds_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<ds_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "ds-build").
    ...
    pub parent_tool_snapshot: Option<Vec<ds_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<ds_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: ds_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: ds_tools::types::compat::CompatConfig,
    /// Shared set of IDs delivered via auto-wake synthetic prompts.
    pub auto_wake_delivered:
        Option<ds_tools::reminders::task_completion::AutoWakeDeliveredIds>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut ds_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("DS_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("DS_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use ds_subagent_resolution::ResumeSourceData;
...
use ds_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &ds_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ds_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &ds_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ds_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &ds_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &ds_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = ds_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(ds_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[ds_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<ds_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<ds_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        ds_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[ds_sampling_types::conversation::ConversationItem],
) -> bool {
    use ds_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<ds_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = ds_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        ds_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = ds_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &ds_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use ds_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<ds_agent::config::AgentDefinition> {
    let mut def = ds_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<ds_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || ds_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = ds_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// agent definition.
///
/// The harness flavor (alternate vs ds-build) normally follows the PARENT
/// agent: `DsBuildOrchestrator` parents give children
...
/// (so a ds-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut ds_agent::config::AgentDefinition,
) {
...
fn summarize_tool_config(
    config: &ds_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
    use std::collections::HashMap;
...
fn resolve_subagent_permission_mode(
    requested: ds_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> ds_agent::config::PermissionMode {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    ds_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<ds_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<ds_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = ds_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = ds_tools::reminders::wrap_reminder(&message);
...
fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::DsSessionNotification {
...
/// `ds.cli/subagent/list_running` on reconnect.
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::DsSessionNotification { notification });
            }
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("ds.cli/session_notification", params.into());

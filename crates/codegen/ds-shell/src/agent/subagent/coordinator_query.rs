...
impl SubagentCoordinator {
    ...
    /// Max retained completed subagent snapshots after TTL eviction.
    ...
    pub(crate) const MAX_PENDING_COMPLETIONS: usize = 64;

    /// How long completed entries stay queryable in-memory before TTL eviction.
    ...
    /// then cap the map to [`Self::MAX_COMPLETED_RETAINED`] (oldest
    /// `completed_at` first) so a high-churn session cannot grow unbounded.
    ///
    /// Call on every completion path as well as on query — not only on query —
    /// so memory is released even when nothing polls `get_task_output`.
    pub fn evict_stale_completed(&mut self) {
        let cutoff = Self::COMPLETED_TTL;
        self.completed
            .retain(|_, entry| entry.completed_at.elapsed() < cutoff);
        let excess = self
            .completed
            .len()
            .saturating_sub(Self::MAX_COMPLETED_RETAINED);
        if excess == 0 {
            return;
        }
        let mut by_age: Vec<(String, std::time::Instant)> = self
            .completed
            .iter()
            .map(|(id, e)| (id.clone(), e.completed_at))
            .collect();
        // Oldest completions first (Instant is monotonic within the process).
    ...
    /// Push a between-turn completion summary, dropping the oldest when
    /// over [`Self::MAX_PENDING_COMPLETIONS`].
    pub(crate) fn push_pending_completion(&mut self, summary: SubagentCompletionSummary) {
        if self.pending_completions.len() >= Self::MAX_PENDING_COMPLETIONS {
            let excess = self
                .pending_completions
                .len()
                .saturating_sub(Self::MAX_PENDING_COMPLETIONS - 1);
            self.pending_completions.drain(0..excess);
        }
        self.pending_completions.push(summary);
    }
    ...
    /// parent session ID. Callers (e.g. the `ds.cli/subagent/list_running`

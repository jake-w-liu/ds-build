...
const CLIENT_VERSION_HEADER: &str = "x-ds-client-version";

// ============================================================================
// Turn delta wire types (local to ds-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::ds_auth_credentials::DsAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::ds_auth_credentials::DsAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(ds_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::ds_auth_credentials::DsAuthCredentials,
    ) -> Arc<dyn ds_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(ds_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, ds_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-DS-Token-Auth", "ds-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = ds_file_utils::trace_context::inject_trace_context_into_request(request);
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["deepseek-v4-flash".to_string()],
            primary_model_id: Some("deepseek-v4-flash".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("deepseek-v4-flash".to_string()));

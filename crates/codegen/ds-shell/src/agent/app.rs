...
use crate::auth::{AuthManager, AuthMode, DsAuth, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (ds.com
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(ds_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
...
/// `ds.cli/...` extension method, for injection into the agent's inbound ACP
/// stream by the leader's own watcher tasks (config hot-reload, skills).
///
/// The wire method is written **`_`-prefixed** (`_ds.cli/internal/...`):
...
/// Start a skills file watcher and wire it to inject `ds.cli/internal/reload_skills`
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = ds_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`ds_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    ds_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    ds_telemetry::unified_log::set_version(ds_version::VERSION);
    ...
    ds_file_utils::queue::cleanup_orphaned_uploads(
        &ds_home::ds_home(),
        ds_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by ds-desktop when spawning `ds agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("DS_CLIENT_VERSION") {
    ...
    // blocking stdin reads (see `ds_acp_lib::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `ds agent stdio` is a local/interactive entrypoint (spawned by
            // ds-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    ds_telemetry::unified_log::set_version(ds_version::VERSION);
    // `ds agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `ds agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a ds.com session. \
        Run `ds login` to sign in, or use `ds agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.ds_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'ds login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `ds login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&ds_home::ds_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
        ...
        let auth_manager = Arc::new(AuthManager::new(&ds_home::ds_home(), ctx.clone()));
        if crate::agent::auth_method::has_ds_api_key_env()
    ...
    // Capture the ds build URL for the first-connection callback
    let ds_code_url = format!("{}/build", ctx.ds_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open DS Build: {} (press Enter to open in browser)",
                ds_code_url
            );
            eprintln!();
            let url_for_open = ds_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&DsAuth>, session: &DsAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's ds.com relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `ds agent leader`
...
///   duplication of mirroring every agent message to ds.com.
///
...
/// When a ds.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the ds.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: ds_version::VERSION.to_string(),
    })
    ...
    let auth: Option<DsAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<DsAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<DsAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from ds.com relay)
            ...
            // Start (or arm) the ds.com relay. Eager by default — a bare
            // `ds agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no ds.com session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.ds_com_config.auth_scope();
            // Gated on user_ds_home() so a cwd-relative .ds/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = ds_config::user_ds_home()
                    ...
                    match update {
                        ...
                        ConfigUpdate::ProjectMcpServersChanged { cwd } => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "ds.cli/internal/reload_project_mcp_servers",
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.ds/models_cache.json
                            // (another ds process fetched a fresher /v1/models
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "ds.cli/internal/reload_models_cache",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> DsAuth {
        DsAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::DS_OAUTH2_ISSUER.to_string()),
    ...
    /// constructor (`for_session`) with a relay-eligible DeepSeek OIDC session.
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::DsComConfig {
            ds_ws_url: format!("ws://{addr}"),
            ds_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        crate::agent::relay::RelayConfig::for_session(&auth, &cfg, None, None)
            .expect("DeepSeek OIDC session must be relay-eligible")
    }
    ...
    /// `ds agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the ds.com relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `ds agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
    ...
    fn internal_reload_request_line_uses_wire_ext_prefix() {
        ...
        assert_eq!(
            msg["method"], "_ds.cli/internal/reload_models",
        ...
        let msg: serde_json::Value = serde_json::from_str(line.trim_end()).unwrap();
        assert_eq!(msg["method"], "_ds.cli/internal/auth_cleared");
    }

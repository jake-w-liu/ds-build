...
pub fn bootstrap(
    ...
    match crate::chatgpt::cached_model_entries() {
        Ok(models) if !models.is_empty() => models_manager.install_chatgpt_models(models),
        Ok(_) => {}
        Err(error) => {
            tracing::warn!(%error, "could not load cached ChatGPT model catalog");
        }
    }
...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.ds_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (ds -p, subagents).
    ...
    // Writeback talks to the code backend; requires ds.com auth.
    if cfg.storage_mode == StorageMode::Writeback
        && !auth_manager.current().is_some_and(|a| a.is_ds_auth())
    {
        tracing::info!("Writeback is disabled: requires auth with ds.com");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_bundled_files(&ds_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&ds_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let ds_auth = auth_manager.current().filter(|a| a.is_ds_auth());
    let user_id = ds_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = ds_auth.as_ref().and_then(|a| a.team_id.clone());

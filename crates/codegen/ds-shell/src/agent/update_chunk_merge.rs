...
impl ReplayBuffer {
    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Ds(prev)), SessionNotification::Ds(new)) => {
                let (force, first, second) = merge_ds_chunks(*prev, *new);
                (
                    force,
                    SessionNotification::Ds(Box::new(first)),
                    second.map(|s| SessionNotification::Ds(Box::new(s))),
                )
            }
...
/// Merge two consecutive DeepSeek notifications.
fn merge_ds_chunks(
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::Ds(n) => match &n.update {
...
mod tests {
    ...
    #[test]
    fn ds_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn ds_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        assert!(matches!(flushed, SessionNotification::Ds(_)));
        assert!(rest.is_some());
        assert!(buf.pending.is_none());
    }

    #[test]
    fn ds_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn ds_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let ds_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(ds_chunk).expect("should force-flush");

        // Both emitted immediately — different kinds can't merge.
        assert!(matches!(flushed, SessionNotification::Acp(_)));
        assert!(matches!(rest, Some(SessionNotification::Ds(_))));
        assert!(buf.pending.is_none());
    }
}

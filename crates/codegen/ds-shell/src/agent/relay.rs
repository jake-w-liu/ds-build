...
//! connection to the ds.com relay server with automatic reconnection.
...
/// Config for the ds.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a ds.com first-party session
    /// (`is_ds_auth`: deepseek-issuer OIDC or external credential) with a
    /// non-empty bearer. BYOK/ApiKey, non-deepseek issuers (enterprise OIDC,
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_ds_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.ds_ws_url.clone(),
            ws_origin: ctx.ds_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to ds.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `ds login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from DS WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        "x-ds-client-version",
        axum::http::header::HeaderValue::from_static(ds_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                ds_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test DsAuth with the given key.
    fn test_auth(key: &str) -> DsAuth {
        DsAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_ds_issuer() {
        use crate::auth::DS_OAUTH2_ISSUER;
        let cfg = DsComConfig::default();
        let builds = |a: &DsAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let ds = DsAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(DS_OAUTH2_ISSUER.to_string()),
            ..test_auth("ds-bearer")
        };
        assert!(ds.is_ds_auth(), "precondition: is_ds_auth");
        assert!(builds(&ds));
        let external_ds = DsAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(DS_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_ds.is_ds_auth(), "precondition: is_ds_auth");
        assert!(builds(&external_ds));
        assert!(!builds(&DsAuth {
            key: String::new(),
            ..ds.clone()
        }));
        assert!(!builds(&DsAuth {
    ...
    /// Helper: write a DsAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &DsAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        impl TokenRefresher for CountingRefresher {
            async fn refresh(
                ...
                RefreshOutcome::Success(Box::new(DsAuth {
        ...
        let mut config = RelayConfig::for_session(&expired_session, &cfg, None, Some(am.clone()))
            .expect("DeepSeek OIDC session is relay-eligible");
        let cancel = CancellationToken::new();

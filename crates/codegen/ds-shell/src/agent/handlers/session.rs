...
/// Router for ds.cli/session/* and ds.cli/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "ds.cli/session/info" => handle_session_info(agent, args).await,
        "ds.cli/session/close" => handle_session_close(agent, args).await,
        "ds.cli/session/list" => handle_session_list(agent, args).await,
        "ds.cli/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("ds.cli/session_summaries/") => {
...
/// `ds.cli/sessions/list` — the FleetView roster. Returns every
...
/// `ds.cli/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via ds.cli/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "ds.cli/session_summaries/session_list" => {
        ...
        "ds.cli/session_summaries/workspace_list" => {
            tracing::debug!("ds/session_summaries/workspace_list is working");
        ...
        "ds.cli/session_summaries/workspace_list_recent" => {

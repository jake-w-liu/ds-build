...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("ds-build", "DS Build"),
            model("deepseek-v4-pro.5", "DS 4.5"),
        ];
        let current = acp::ModelId::from("ds-build");
        ...
        assert_eq!(selected_models[0].id, "ds-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("ds-build", "DS Build")];
        let current = acp::ModelId::from("ds-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("ds-build", "")];
        let current = acp::ModelId::from("ds-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "ds-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "ds-build".to_string(),
            category: "model".to_string(),
            label: "DS Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "ds-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "DS Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn ds_session_detail_serializes_camel_case() {
        let detail = DsSessionDetail::build(
            "sess-1".to_string(),
            "/Users/me/ds".to_string(),
            "ds-build".to_string(),
            None,
        );
        ...
        assert_eq!(v["cwd"], "/Users/me/ds");
        assert_eq!(v["currentModelId"], "ds-build");

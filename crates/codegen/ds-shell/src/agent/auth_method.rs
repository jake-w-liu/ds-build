...
/// Preferred DeepSeek API key env var.
pub const DEEPSEEK_API_KEY_ENV_VAR: &str = "DEEPSEEK_API_KEY";

/// Alternate product-scoped API key env var.
pub const DS_API_KEY_ENV_VAR: &str = "DS_API_KEY";

/// Legacy env var name. Checked as a fallback when preferred keys are not set.
pub const LEGACY_DEEPSEEK_API_KEY_ENV_VAR: &str = "DS_CODE_API_KEY";

/// Read the API key from the environment or `~/.ds/config.toml`.
///
/// Priority:
/// 1. `DEEPSEEK_API_KEY`
/// 2. `DS_API_KEY`
/// 3. `DS_CODE_API_KEY`
/// 4. Top-level `api_key` in `~/.ds/config.toml` (CodeWhale-style paste)
/// 5. First non-empty `api_key` under `[model.*]` / `[models.*]` in user or
///    project config
/// 6. `ds::api_key` scope in `~/.ds/auth.json` (from `ds auth set` / ACP)
pub fn read_ds_api_key_env() -> Result<String, std::env::VarError> {
    std::env::var(DEEPSEEK_API_KEY_ENV_VAR)
        .or_else(|_| std::env::var(DS_API_KEY_ENV_VAR))
        .or_else(|_| std::env::var(LEGACY_DEEPSEEK_API_KEY_ENV_VAR))
        .or_else(|_| {
            read_api_key_from_user_config().ok_or(std::env::VarError::NotPresent)
        })
}

/// Scan user/project config (and auth.json) for a pasted API key (BYOK).
fn read_api_key_from_user_config() -> Option<String> {
    // Prefer the shared helper (top-level + models + auth.json under ds_home).
    if let Some(key) = crate::auth::api_key_setup::read_api_key_from_config_or_auth_json() {
        return Some(key);
    }
    ...
    if let Ok(cwd) = std::env::current_dir() {
        roots.push(cwd.join(".ds").join("config.toml"));
    }
    roots.sort();
    roots.dedup();
    for path in roots {
        ...
        // toml 0.9: `str::parse::<Value>()` only accepts a single value
        // expression, not a full document — use `toml::from_str`.
        ...
        if let Some(key) = first_model_api_key(&value) {
            return Some(key);
        }
    }
    None
...
/// True for empty keys and example placeholders like `sk-PASTE_YOUR_…`.
/// Intentionally does **not** match the substring `PASTE` alone — config
/// comments often say "PASTE YOUR KEY HERE" and must not poison real keys.
...
fn first_model_api_key(root: &toml::Value) -> Option<String> {
    // Per-model overrides live under `[model.<id>]` (singular), not `[models]`.
    let models = root
        .get("model")
...
/// Returns `true` if any supported API key source is set (env or config).
pub fn has_ds_api_key_env() -> bool {
    read_ds_api_key_env().is_ok()
}

/// Whether `ds.api_key` should be advertised (and pushed FIRST) when building
/// the `auth_methods` list at `initialize()` time.
///
/// Regression: `ds.api_key` must stay first when only per-model credentials
/// exist (no global `DEEPSEEK_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// `disable_api_key_auth` (`[ds_com_config] disable_api_key_auth` /
/// `DS_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `DEEPSEEK_API_KEY` can't bypass a deployment's forced IdP login.
pub fn should_advertise_ds_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_ds_api_key_env() || models.into_iter().any(ModelEntry::has_own_credentials)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    /// True if `ds.api_key` should be advertised AT ALL. Caller computes via
    /// [`should_advertise_ds_api_key`]. When `preferred_method` is `Oidc`,
    ...
    /// Optional display label for the login method (`ds.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `ds_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `ds.com` method).
...
pub struct BuiltAuthMethods {
    ...
    /// `cached_token` wins over `ds.api_key` when both are present. When
...
/// `has_external_api_key` is true, the **first** entry MUST be `ds.api_key`.
...
/// 1. `ds.api_key`     (if `has_external_api_key`)
...
/// - `ds.api_key`  else if `has_external_api_key`
/// - `None`         otherwise
///
/// Pinned (`preferred_method`):
/// - `ApiKey`: only `ds.api_key` if available; else empty list + `None` (fail).
/// - `Oidc`: `cached_token` (if any) + interactive login; never `ds.api_key`.
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        methods: vec![ds_api_key_auth_method()],
        default_auth_method_id: Some(acp::AuthMethodId::new(DEEPSEEK_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(
    ...
    if has_external_api_key {
        methods.push(ds_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(DEEPSEEK_API_KEY_METHOD_ID));
    }

    if has_cached_token {
        methods.push(cached_token_auth_method());
        // cached_token wins over ds.api_key for default_auth_method_id so
        // is_session_based_auth() returns true and OIDC refresh stays alive.
        let overrode_api_key = default_auth_method_id.is_some();
        default_auth_method_id = Some(acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID));
        if overrode_api_key {
            ds_telemetry::unified_log::info(
                "auth method priority: cached_token overrides ds.api_key for default_auth_method_id",
                None,
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.ds_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.ds_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(ds_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            DEEPSEEK_API_KEY_METHOD_ID => Self::DsApiKey,
            CACHED_TOKEN_AUTH_METHOD_ID => Self::CachedToken,
            DS_COM_METHOD_ID => Self::DsCom,
    ...
    pub fn is_api_key(self) -> bool {
        matches!(self, Self::DsApiKey)
    }

    /// `true` for session-based methods (cached_token, ds.com, oidc).
    pub fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::DsCom | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::DsCom | Self::Oidc)
    }
...
/// `true` for session-based ACP methods (cached_token, ds.com, oidc).
...
/// `DEEPSEEK_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `ds login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Set DEEPSEEK_API_KEY (or DS_API_KEY), or add api_key to ~/.ds/config.toml.";

/// Next ACP method id when `cached_token` cannot proceed (missing / expired /
/// legacy WebLogin), or `None` when fallthrough is forbidden.
///
/// Unpinned: prefer non-interactive `ds.api_key` when advertiseable, else
/// interactive `ds.com`.
///
...
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set DEEPSEEK_API_KEY / DS_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `ds login` to authenticate.";

pub const DEEPSEEK_API_KEY_METHOD_ID: &str = "ds.api_key";
pub fn ds_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(DEEPSEEK_API_KEY_METHOD_ID),
            "ds.api_key".to_string(),
        )
        .description(Some(
            "DEEPSEEK_API_KEY / DS_API_KEY or api_key/env_key in ~/.ds/config.toml".to_string(),
        )),
    )
}
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.ds/auth.json".to_string())),
    )
}

pub const DS_COM_METHOD_ID: &str = "ds.com";

/// DeepSeek OAuth2/OIDC auth. Method id `"ds.com"` kept for ACP wire-compat.
pub fn ds_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("DS");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(DS_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// `cached_token` to non-interactive `ds.api_key` (not browser OAuth).
    ...
    /// `ds.api_key` first, while `default_auth_method_id` prefers session;
    /// after session fails, this helper must still pick `ds.api_key`.
    #[test]
    ...
    /// No advertiseable API-key credentials → interactive `ds.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_ds_com_without_api_key() {
        assert_eq!(
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(DEEPSEEK_API_KEY_METHOD_ID);
    ...
    // Reordering so `ds.api_key` is after login methods must fail the tests below.

    /// BYOK with only per-model `env_key` must list `ds.api_key` first.
    #[test]
    fn enterprise_byok_first_method_is_ds_api_key() {
        ...
        assert_eq!(
            ...
            Some(AuthMethodKind::DsApiKey),
            "BYOK enterprise-style: auth_methods.first() MUST be ds.api_key \
        ...
        assert!(
            ...
            "first method MUST NOT need interactive login when ds.api_key is available",
        );
    }

    /// BYOK + cached session token: ds.api_key stays first in the methods
    /// list (skips login screen), but `default_auth_method_id` is
    /// `cached_token` (keeps OIDC refresh alive).
    #[test]
    fn byok_with_cached_token_keeps_ds_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::DsApiKey),
            "ds.api_key MUST precede cached_token in advertised order",
        );
    ...
    /// `ds.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `ds.com` is
    ...
    #[test]
    fn fresh_user_only_advertises_ds_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::DsCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `ds.com` (mutually exclusive). ds.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_ds_com_but_ds_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::DsApiKey));
        assert!(
            built
                .methods
                .iter()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::DsCom),
            "ds.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `ds.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let ds = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::DsCom)
            .expect("ds.com must be advertised");
        assert_eq!(ds.name(), "Acme Corp");
        let meta = ds.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.ds/config.toml` skeleton from the bug report, walks it through
    /// the same predicate (`should_advertise_ds_api_key`) and the same
    /// list-builder (`build_auth_methods`) that `MvpAgent::initialize()` uses
    /// in production, and asserts that `auth_methods.first()` is `ds.api_key`
    /// (which causes the pager to skip the login screen).
    ///
    /// This is the test that *would have caught* that regression -- if you mentally
    /// re-introduce that bug (push ds.api_key LAST when has_external_api_key
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // login method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_ds_api_key(false, models.values());
            ...
            assert_ne!(
                first_kind(&built.methods),
                Some(AuthMethodKind::DsApiKey),
                "without env_key resolved, ds.api_key must NOT be advertised first",
            );
        }

        // With the env var present (the actual enterprise scenario), the predicate
        // returns true and the builder MUST put `ds.api_key` first so the
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // ds.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::DsApiKey),
                "BYOK: ds.api_key must be auth_methods.first(); deferred-to-last \
    ...
    /// `DEEPSEEK_API_KEY` alone (no per-model creds) also triggers
    /// advertising `ds.api_key` as the first method. Historical "external
    /// key" path; covered here so the predicate keeps treating env-var-only
    /// users the same as per-model users.
    #[test]
    #[serial]
    fn global_external_api_key_advertises_ds_api_key_first() {
        let _set = EnvGuard::set(DEEPSEEK_API_KEY_ENV_VAR, "ds-external-key");
    ...
    /// AND per-model env_key), so the builder never advertises `ds.api_key`
    /// and the pager sends the user to the deployment's login method instead.
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_ds_api_key_method() {
        let _set = EnvGuard::set(DEEPSEEK_API_KEY_ENV_VAR, "ds-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_ds_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_ds_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::DsApiKey),
            "ds.api_key must not be advertised when disable_api_key_auth is set",
        );
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::DsCom),
    ...
    /// Legacy `DS_CODE_API_KEY` env var is accepted as a fallback
    /// when `DEEPSEEK_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_ds_api_key() {
        let _unset_new = EnvGuard::unset(DEEPSEEK_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_DEEPSEEK_API_KEY_ENV_VAR, "ds-legacy-key");
        assert!(has_ds_api_key_env());
        assert_eq!(read_ds_api_key_env().unwrap(), "ds-legacy-key");
    ...
    /// When both `DEEPSEEK_API_KEY` and `DS_CODE_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(DEEPSEEK_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_DEEPSEEK_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_ds_api_key_env().unwrap(), "new-key");
    }

    // -- ds login --legacy regression coverage ------------------------
    //
    // `ds login --legacy` produces a DsAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `DS_AUTH` env var (or via legacy
    ...
    // to-end. A regression in DS_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `DS_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn ds_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, DsAuth, DsComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("DS_AUTH_PATH");
        let _g2 = EnvGuard::unset(DEEPSEEK_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `ds login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = DsAuth {
        ...
        // Provide it via DS_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("DS_AUTH", &legacy_json);

        // AuthManager picks it up from the env var directly (no file needed).
        let dir = tempfile::tempdir().unwrap();
        let cfg = DsComConfig::default();
        let mgr = AuthManager::new(dir.path(), cfg);
        let current = mgr.current();
        assert!(
            current.is_some(),
            "legacy token in DS_AUTH env MUST be loaded directly -- if this fails, \
        ...
        // With only this legacy token (no ds api key), first method must be
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("DS_AUTH");
        let _g2 = EnvGuard::unset("DS_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::DsCom),
            "no cached token AND no api key: pager must show login (ds.com first)",
        );
    }
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![DEEPSEEK_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(DEEPSEEK_API_KEY_METHOD_ID));
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, DS_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![DS_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}

...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = ds_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                ds_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    ds_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                ds_workspace::permission::AccessKind::Read(Some(".env".into())),
                ds_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        ds_workspace::permission::Decision::PolicyDeny(_)
                    ),

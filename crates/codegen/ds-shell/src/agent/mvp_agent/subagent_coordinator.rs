                ...
                while let Some(event) = rx.recv().await {
                    match event {
                        ...
                        SubagentEvent::Completions(request) => {
                            ...
                            let mut completions = {
                                let mut coord = this.subagent_coordinator.borrow_mut();
                                let drained = coord.drain_pending_completions();
                                // Also age out completed snapshots while we hold the lock.
                                coord.evict_stale_completed();
                                drained
                            };
        ...
        let (
            ...
            (
                ...
                ps.map(|h| h.tool_context.hunk_tracker_handle.clone())
                    .unwrap_or_else(ds_hunk_tracker::HunkTrackerHandle::noop),
                ...
                ps.map(|h| h.tool_context.fs.inner().clone())
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .unwrap_or_default();
                        std::sync::Arc::new(ds_workspace::file_system::LocalFs::new(cwd))
                    }),
        ...
        Some(crate::agent::subagent::SubagentSpawnContext {
            ...
            task_output_tool_name: {
                ...
                sessions
                    ...
                    .unwrap_or_else(|| {
                        ds_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL
                            .to_string()
                    })
            },

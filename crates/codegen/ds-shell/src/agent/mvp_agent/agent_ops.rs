...
impl MvpAgent {
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_ds_auth())
    }
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::DsAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// Requires feature flag AND DeepSeek authentication (OIDC or legacy WebLogin).
    ...
    /// must not block the ACP response (ds-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled ds-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_ds_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_ds_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            ds_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    ///
    /// Used by the `ds.cli/commands/list` ext method and the
    ...
    /// Return the current DsAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::DsAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &ds_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on ds-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<ds_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    ds_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    ds_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match ds_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => ds_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     `ds.com`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`ds.api_key`) or
    ...
    pub(crate) fn auth_type(&self) -> ds_chat_state::AuthType {
    ...
    /// When `cached_token` cannot proceed, prefer non-interactive `ds.api_key`
    /// iff `should_advertise_ds_api_key`; otherwise `ds.com`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().ds_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_ds_api_key(
                self.cfg.borrow().ds_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `ds.com`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::DS_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::DsAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_ds_event = auth.is_ds_auth();
        ...
        let (
            ...
            let ds_user_id = is_ds_event.then(|| user_id.clone());
            let ds_team_id = is_ds_event.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<ds_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// funnels through here. Emits `ds.cli/announcements/update` and advances
    ...
    pub(super) fn emit_announcements(&self, mode: AnnouncementsPushMode) {
        ...
        let accepted = self
            ...
            .forward_fire_and_forget(
                acp::ExtNotification::new("ds.cli/announcements/update", params.into()),
            );
        if !accepted {
            return;
        }
    ...
    /// Next generation for an `ds.cli/announcements/update` push. Strictly
    ...
    pub(super) async fn send_model_auto_switched(
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&notification) {
            let _ = self
                .gateway
                .ext_notification(
                    acp::ExtNotification::new("ds.cli/session_notification", params.into()),
                )
                .await;
        }
    }
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == ds_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = ds_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == ds_chat_state::AuthType::ApiKey
    ...
    /// Whether the current session is a personal ds.com account on a gated
    ...
    /// DS Pro upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_ds_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `ds_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> ds_tools::implementations::ds_build::image_gen::ImageGenConfig {
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.ds_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| ds_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("ds-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        &self,
    ) -> ds_tools::implementations::ds_build::deploy_app::AppBuilderDeployerConfig {
    ...
    pub(super) fn prepare_video_gen_config(
        ...
        use ds_tools::implementations::ds_build::video_gen::VideoGenConfig;
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `DS_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `DS_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> ds_tools::implementations::ds_build::web_fetch::WebFetchConfig {
        use ds_tools::implementations::ds_build::web_fetch::WebFetchConfig;
        let cfg = self.cfg.borrow();
    ...
    pub fn with_models(
        ...
        let has_ds_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_ds_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_ds_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[ds] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_ds_auth {
            tracing::info!(
                "[ds] Relay sync: DISABLED (no auth - run 'ds login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: ds_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: ds_tools::implementations::ds_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().ds_com_config.auth_provider_command.clone(),
    ...
    /// Handle `ds.cli/internal/evict_sessions` — the leader server tells us a
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<ds_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// `ds.cli/subagent/cancel`. Active/pending → cancelled (a finish follows);
    /// already-finished → its terminal status; unknown id → `NotFound`.
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> ds_tools::implementations::ds_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<ds_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or DS_RELAY_SYNC_ENABLED)
    ...
    /// Get hooks list for a session (for `ds.cli/hooks/list` extension).
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<ds_hooks_plugins_types::HooksListResponse> {
        let handle = self.get_session_handle(session_id)?;
        handle.get_hooks_list().await
    }
    /// Execute a hooks management action (for `ds.cli/hooks/action`).
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: ds_hooks_plugins_types::HooksAction,
    ) -> Option<ds_hooks_plugins_types::ActionOutcome> {
        if matches!(action, ds_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&ds_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
        handle.execute_hooks_action(action).await
    }
    /// Execute a plugins management action (for `ds.cli/plugins/action`).
    pub async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: ds_hooks_plugins_types::PluginsAction,
    ) -> Option<ds_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, ds_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(ds_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    /// Get a snapshot of the shared plugin registry (for `ds.cli/plugins/list`).
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<ds_agent::plugins::PluginRegistry>> {
    ...
    /// Build the `ds.cli/sessionConfig` and `ds.cli/sessionDetail` `_meta` values
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::DsSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<ds_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<ds_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, ds_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let capture = ds_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                ds_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let ds_home = crate::util::ds_home::ds_home();
    ...
    ///
    /// `DS_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`ds_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<ds_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> ds_agent::AgentDefinition {
        use ds_agent::AgentDefinition;
        let ds_agent_env_set = std::env::var("DS_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(ds_agent::config::is_strict_harness_agent_type);
        if !ds_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = ds_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = ds_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("DS_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("ds-build-concise") | Some("ds_build_concise") => {
                AgentDefinition::ds_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::ds_build_plan()
                    }
                }
            }
            Some(name) => {
                ds_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::ds_build_plan)
            }
            None => AgentDefinition::ds_build_plan(),
        };
        if !ds_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = ds_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs_notify_config = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/fs_notify"))
            .and_then(|v| {
                use crate::session::{ClientFsConfig, ClientFsMode};
                use ds_fsnotify::FsConfig;
        ...
        let fs: Arc<dyn ds_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let hunk_plan = plan_hunk_tracking(
            init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ds.cli/hunkTracker"))
        ...
        let incremental_bash_output = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/incrementalBashOutput"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let no_color = init
            .client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/bashOutputNoColor"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (ds_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_ds_auth = self.auth_manager.current().is_some_and(|a| a.is_ds_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_ds_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = ds_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = ds_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
        ...
        let mut session_env = ds_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                ds_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            ds_agent::config::ModelOverride::Override(id) => {
            ...
            ds_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            if servers.is_empty() {
                let user_path = ds_tools::util::ds_home::ds_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".ds").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = ds_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                ds_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = ds_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
            ...
            let git_head_changed = init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ds.cli/gitHeadChanged"))
                .and_then(|v| v.as_bool());
            ...
            spawn_session_on_thread(
                    ...
                    ds_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        ds_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == ds_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        ds_hunk_tracker::LocAggregate::LinesReverted {

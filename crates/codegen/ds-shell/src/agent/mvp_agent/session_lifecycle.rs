...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`ds.cli/session/close`). It must
    ...
    /// (`ds.cli/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `ds.cli/sessions/changed` notification with the session in `removed` so
    ...
    /// `ds.cli/sessions/changed` notification with the current entry in
    ...
    /// Emit an `ds.cli/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `ds.cli/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `ds.cli/sessions/changed` special-case in `leader/server.rs`).

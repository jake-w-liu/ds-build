...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        ds_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::ds_home::ds_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    ds_workspace::permission::cleanup_stale_permission_state(
        ...
        ds_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_ds_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("ds-web") => client_type = ClientType::DsWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("ds-code-extension") => client_type = ClientType::Extension,
                Some("ds-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first ds.cli/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().ds_com_config.api_key_auth_disabled()
            && auth_method::read_ds_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::ds_home::ds_home(),
            )
        {
            unsafe { std::env::set_var("DEEPSEEK_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (ds::api_key scope)");
            ds_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (ds::api_key scope)",
                None,
                None,
            );
        }
        ...
        let has_external_api_key = auth_method::should_advertise_ds_api_key(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.ds_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.ds_com_config.auth_provider_label.clone(),
                cfg.ds_com_config.auth_provider_command.is_some(),
                cfg.ds_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising ds.com auth method",
            );
        }
        let preferred_method = self.cfg.borrow().ds_com_config.preferred_method;
        ...
        ds_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "ds_home" : crate ::util::ds_home::ds_home().display()
        ...
        debug_assert!(
            ...
            Some(auth_method::AuthMethodKind::DsApiKey)),
            "BYOK invariant violated: ds.api_key MUST be auth_methods.first() \
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                .agent_capabilities(
                    acp::AgentCapabilities::new()
                        ...
                        .meta(
                            serde_json::json!(
                                { "ds.cli/fs_notify" : true, "ds.cli/hooks" : { "blockingEvents"
                                : [ds_hooks::event::HookEventName::PreToolUse],
                                "decisions" : ["deny"], }, }
                            )
                                .as_object()
                                .cloned(),
                ...
                .meta({
                    let metadata = parse_json_object_env("DS_AGENT_METADATA");
                    serde_json::json!(
                        { "dsShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (ds_mcp::wire::MCP_SDK) :
                        ...
                        ds_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().ds_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::DEEPSEEK_API_KEY_METHOD_ID => {
                if self.cfg.borrow().ds_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_ds_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set DEEPSEEK_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(ds_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().ds_com_config.preferred_method, Some(crate
            ...
            auth_method::DS_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let ds_ctx = self.auth_manager.ds_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_ds_oauth(cli_oauth);
                tracing::debug!(
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: ds.com/oidc handler set api_key (SessionToken)"
                    );
                    ds_telemetry::unified_log::debug(
                        "auth: ds.com/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        ds_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if !self.is_data_collection_disabled()
            || ds_telemetry::external::is_active()
        {
            ...
            let perm = if session_yolo_mode {
                ds_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                ds_telemetry::enums::PermissionMode::Auto
            } else {
                ds_telemetry::enums::PermissionMode::Ask
            };
            let internal_gate_open = !self.is_data_collection_disabled();
            tokio::spawn(async move {
                let git = ds_telemetry::context::collect_git_context(&cwd_str);
                let ev = ds_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                ds_telemetry::session_ctx::log_event_dual(internal_gate_open, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match ds_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("ds.cli/sessionConfig".to_string(), session_config_value);
            obj.insert("ds.cli/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/persist"))
            .cloned();
        ...
        let git_root = ds_workspace::session::git::find_git_root_from_path(
                cwd.as_path(),
            )
            .ok();
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        ds_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        ds_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        ds_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => ds_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            ds_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = ds_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : ds_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a ds worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            ds_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use ds_workspace::session::git::RestoreKind;
            let outcome = ds_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("ds.cli/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = ds_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("ds.cli/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let is_ds_build = persisted_model.0.starts_with("ds-build");
        let same_family_fallback = if is_ds_build {
            available.keys().find(|id| id.0.starts_with("ds-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("ds-build")).cloned()
        };
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("ds.cli/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    ds_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = ds_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = ds_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "ds.cli/runningPromptId".to_string(),
        ...
        response_meta_map.insert("ds.cli/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("ds.cli/sessionDetail".to_string(), session_detail_value);
        ...
        if !self.is_data_collection_disabled() {
            log_event(ds_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            ds_file_utils::trace_context::link_current_span_to_meta(
        ...
        if let Some(ctx) = trace_context.clone() {
            ...
            let mut prompt_metadata = PromptMetadata {
                ...
                shell_version: Some(ds_version::VERSION.to_string()),
                workspace_type: None,
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: ds_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                respond_to: tx,
                persist_ack: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "ds.cli/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<ds_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = ds_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = ds_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = ds_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            ds_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            ...
            "ds.cli/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "ds.cli/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "ds.cli/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "ds.cli/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "ds.cli/session/resolve_local_for_worktree_resume"
            | "ds.cli/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "ds.cli/session/rename" | "ds.cli/session/delete"
            | "ds.cli/session/update_mcp_servers" | "ds.cli/session/fork"
            | "ds.cli/internal/reload_all_mcp_servers"
            | "ds.cli/internal/reload_project_mcp_servers" | "ds.cli/internal/reload_skills"
            | "ds.cli/internal/reload_models" | "ds.cli/internal/reload_models_cache"
            | "ds.cli/internal/auth_cleared" | "ds.cli/plugins/reload"
            | "ds.cli/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "ds.cli/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "ds.cli/memory/flush" | "ds.cli/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "ds.cli/skills/refresh-baseline" => {
                self.refresh_skill_baseline_for_all_sessions();
                crate::extensions::to_ext_response(
                    Ok(serde_json::json!({ "ok" : true })),
                )
            }
            "ds.cli/interject" => crate::extensions::interject::handle(self, &args).await,
            "ds.cli/feedback" | "ds.cli/feedback/dismiss" | "ds.cli/btw" => {
                crate::extensions::feedback::handle(self, &args).await
            }
            "ds.cli/recap" => crate::extensions::recap::handle(self, &args).await,
            "ds.cli/cloud/terminate" => {
                crate::extensions::auth_gate::require_ds_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `ds login` to authenticate.",
                )?;
            ...
            "ds.cli/billing" => crate::extensions::billing::handle(self, &args).await,
            "ds.cli/auto-topup-rule" => {
                crate::extensions::billing::handle(self, &args).await
            }
            "ds.cli/share_session" => crate::extensions::share::handle(self, &args).await,
            "ds.cli/privacy/setCodingDataRetention" => {
                crate::extensions::privacy::handle(self, &args).await
            }
            "ds.cli/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "ds.cli/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "ds.cli/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "ds.cli/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("ds.cli/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("ds.cli/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("ds.cli/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("ds.cli/pr/") => {
            ...
            s if s.starts_with("ds.cli/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("ds.cli/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "ds.cli/yolo_mode_changed"
        ...
        if args.method.as_ref() == "ds.cli/permissions/reset" {
        ...
        if args.method.as_ref() == "ds.cli/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "ds.cli/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(), "ds.cli/queue/remove" | "ds.cli/queue/reorder" |
            "ds.cli/queue/clear" | "ds.cli/queue/edit" | "ds.cli/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "ds.cli/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_ds.cli/session/update" {
            if let Ok(notification) = serde_json::from_str::<
                SessionNotification,
            >(args.params.get()) {
                tracing::info!(
                    "Storing DeepSeek session notification: session_id={}", notification
                    .session_id.0
                );
                    ...
                    .get(&notification.session_id)
                {
                    let _ = handle
                        .cmd_tx
                        .send(crate::session::SessionCommand::DsSessionNotification {
                            notification,
                        });
                } else {
                    tracing::warn!(
                        "Received DeepSeek session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse DeepSeek session notification params");
            }
        }
        if args.method.as_ref() == "ds.cli/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                ds_telemetry::session_ctx::log_event(ds_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "ds.cli/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ds_telemetry::session_ctx::log_event(ds_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| ds_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "ds.cli/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ds_telemetry::session_ctx::log_event(ds_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "ds.cli/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ds_telemetry::session_ctx::log_event(ds_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| ds_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == ds_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                ds_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            ds_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }

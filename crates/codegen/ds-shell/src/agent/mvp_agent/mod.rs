...
use ds_acp_lib::AcpAgentGatewaySender as GatewaySender;
...
use ds_telemetry::id::{agent_id, agent_instance_id};
use ds_telemetry::session_ctx::log_event;
use ds_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use ds_workspace::permission::{ClientType, PermissionEvent};
...
use ds_sampler::SamplerConfig as SamplingConfig;
...
use ds_workspace::session::git::GitDiscoveryResult;
use ds_hunk_tracker::HunkTrackerActor;
/// Hard-error message for legacy Direct hub-bind sessions (`ds.cli/cloud_server_id`).
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("ds.cli/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `ds.cli/persist`.
...
/// `/user` already reports a newly qualifying tier (e.g. `DsProPro`). In
...
fn parse_session_kind(
    ...
    meta.and_then(|m| m.get("ds.cli/session"))
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "ds.cli/pluginDirs";
...
/// Used to stamp `ds.cli/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("ds.cli/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a ds_sampling_types::TokenUsage>,
...
/// Typed payload for the `ds.cli/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[ds_announcements::RemoteAnnouncement]>,
    last_emitted: &[ds_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<ds_announcements::RemoteAnnouncement>> {
    let current = ds_announcements::filter_expired_at(
...
/// Override with `DS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("DS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `ds.cli/code/status` responses and to generate
...
pub enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `ds.cli/codeNavigation.enabled`.
...
pub struct MvpAgent {
    ...
    /// ds.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// Whether the current client advertised `ds.cli/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `ds.cli/folderTrust.interactive` (it
    ...
    /// list. Set by `enforce_ds_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.ds/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<ds_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    monitor_event_buffer: ds_tools::implementations::ds_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        ds_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        ds_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: ds_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled ds-desktop's first
    ...
    workspace_ops: RefCell<Option<ds_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `ds.cli/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<ds_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &ds_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<ds_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !ds_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = ds_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `ds-build` →
/// `ds-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `ds-build` →
/// `ds-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use ds_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-ds-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-DS-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-ds-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            .entry("X-DS-Token-Auth".to_string())
            .or_insert_with(|| "ds-cli".to_string());
        headers
...
/// Parse the client-advertised `ds.cli/hunkTracker.mode` string. Case-insensitive
/// and trimmed. Absent/blank/`off`/`disabled` => `None`; unknown => `AllDirty`.
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<ds_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(ds_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<ds_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - DeepSeek updates (`"_ds.cli/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<ds_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_ds_event = method == "_ds.cli/session/update";
        if is_ds_event {
            if target_client_id.is_none() && !mark_replay {
                ...
                >(raw_params.get()) else {
                    tracing::debug!(
                        "replay: skipping DeepSeek update with unparseable params"
                    );
                    return;
                };
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("ds.cli/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("ds.cli/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "ds.cli/leaderClientId", tid);
            }
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<ds_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = ds_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-DeepSeek auth (API keys, enterprise) always passes. For DeepSeek OAuth2
    /// users, reads `allow_access` from remote settings. Defaults to
    /// `false` (blocked) when remote settings are unavailable.
    pub(super) async fn enforce_ds_code_access(&self, auth: &crate::auth::DsAuth) {
        if !auth.is_ds_auth() {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings ds_build_access_gate)"
            );
            self.retry_subscription_check().await;
        }
    }
    /// Single-shot subscription check called by the pager's "Check
    /// subscription" button (`ds.cli/auth/check_subscription`). The pager
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://www.deepseek.com/ds_pro?referrer=ds-build".to_string(),
                        ),
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_ds_auth = auth.is_ds_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_ds_auth {
    ...
    /// Fire-and-forget `ds.cli/settings/update` from the current remote snapshot.
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("ds.cli/settings/update", params.into()),
                );
        }
    }
    ...
    /// cached_token, ds.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.ds/bundled/` and the manifest.
...
/// absent — matching the `ds_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_ds_code_access` (initial login gate) and

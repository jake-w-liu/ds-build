//! ds.com chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching ds-web's refetch cadence.
...
pub const DS_CHAT_MODE_ENV: &str = "DS_CHAT_MODE";
...
pub fn process_chat_mode_enabled() -> bool {
    ...
    match std::env::var(DS_CHAT_MODE_ENV) {
        Ok(v) => {
            let v = v.trim();
...
impl ChatModesManager {
    ...
    /// The active ds.com identity, or `None` when unauthenticated. Modes are
...
/// Maps ds.com modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        let mut m = available("deepseek-v4-pro.5", "");
        m.title = "   ".to_owned();
        let resp = ListModesResponse {
            modes: vec![m],
            default_mode_id: String::new(),
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "deepseek-v4-pro.5");
    }
}

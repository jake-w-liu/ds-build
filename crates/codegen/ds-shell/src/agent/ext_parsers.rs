...
/// Parse a `ds.cli/queue/{remove,reorder,clear,edit,interject}` ext-notification's
...
pub(super) fn parse_queue_edit_command(
    ...
    match method {
        ...
        "ds.cli/queue/clear" => Some(SessionCommand::ClearQueue { owner }),
...
mod tests {
    ...
    /// Each `ds.cli/queue/*` ext-notification maps to the
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("ds.cli/queue/remove", &p, Some("ds-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("ds-tui"));
            }
        ...
        match parse_queue_edit_command("ds.cli/queue/remove", &p, None) {
            Some(SessionCommand::RemoveQueuedPrompt {
                expected_version, ..
        ...
        match parse_queue_edit_command("ds.cli/queue/reorder", &p, None) {
            Some(SessionCommand::ReorderQueue { ordered_ids }) => {
        ...
        match parse_queue_edit_command("ds.cli/queue/edit", &p, Some("ds-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("ds-vscode"));
            }
        ...
        match parse_queue_edit_command("ds.cli/queue/interject", &p, Some("ds-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
        ...
        match parse_queue_edit_command("ds.cli/queue/interject", &p, None) {
        ...
        assert!(
            parse_queue_edit_command("ds.cli/queue/interject", &serde_json::json!({}), None)
                .is_none()
        );

        // unknown method → None.
        assert!(
            parse_queue_edit_command("ds.cli/queue/bogus", &serde_json::json!({}), None).is_none()
        );
        // remove without id → None (can't target an entry).
        assert!(
            parse_queue_edit_command("ds.cli/queue/remove", &serde_json::json!({}), None).is_none()
        );
    }
}

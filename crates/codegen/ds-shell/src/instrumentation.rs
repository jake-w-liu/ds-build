//! Shim — see `ds_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `ds_shell::instrumentation_timer!`). Keeping the macro here
...
pub use ds_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `ds_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `ds_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `ds_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `ds_telemetry::instrumentation`.
#[macro_export]

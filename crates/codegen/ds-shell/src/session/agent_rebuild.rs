...
//! [`ds_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`ds_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`ds_agent::Agent`] owns an [`ds_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`ds_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use ds_agent::config::AgentDefinition;
use ds_agent::error::AgentBuildError;
use ds_agent::prompt::context::PromptAudience;
use ds_agent::prompt::skills::SkillsConfig;
use ds_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use ds_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use ds_tools::implementations::ds_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<ds_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<ds_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        agent
            .tool_bridge()
            .update_resource(ds_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(ds_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use ds_tools::implementations::ds_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            ds_tools::computer::local::LocalTerminalBackend::new_local(
                ds_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: ds_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(ds_tools::types::tool::ToolKind::Task)
            .expect("DsBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("DsBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_ds_build())
                    .await

...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: ds_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(ds_chat_state::compaction_transcript::COMPACTION_DIR);
...
async fn test_jsonl_round_trip() {
    ...
    let new_model = acp::ModelId::new("deepseek-v4-pro.3");
...
async fn load_session_without_updates_defers_rewind_points() {
    use ds_workspace::session::file_state::RewindPoint;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use ds_paths::RelPathBuf;
    use ds_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use ds_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
#[tokio::test]
async fn test_ds_session_update_round_trip() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as DsSessionNotification,
        SessionUpdate as DsSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let ds_notification = DsSessionNotification {
        session_id: acp::SessionId::new("test-session-123"),
        update: DsSessionUpdateType::DiffReview {
            content: vec![
                DiffContent { diff :
    ...
    adapter
        .append_update(&info, &SessionUpdate::Ds(Box::new(ds_notification.clone())))
        .await
        .unwrap();
    ...
    adapter
        ...
        .unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2, "Should have 2 updates (1 DeepSeek + 1 ACP)");
    match &loaded.updates[0] {
        SessionUpdate::Ds(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "test-session-123");
            match &notification.update {
                DsSessionUpdateType::DiffReview { content } => {
        ...
        _ => panic!("Expected DeepSeek update as first item"),
    }
...
async fn test_subagent_notifications_round_trip() {
    use crate::extensions::notification::{
        SessionNotification as DsSessionNotification,
        SessionUpdate as DsSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let spawned = DsSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: DsSessionUpdateType::SubagentSpawned {
    ...
    adapter.append_update(&info, &SessionUpdate::Ds(Box::new(spawned))).await.unwrap();
    let finished = DsSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: DsSessionUpdateType::SubagentFinished {
            subagent_id: "child-001".to_string(),
            child_session_id: "child-001".to_string(),
    ...
    adapter.append_update(&info, &SessionUpdate::Ds(Box::new(finished))).await.unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2);
    match &loaded.updates[0] {
        SessionUpdate::Ds(notification) => {
            match &notification.update {
                DsSessionUpdateType::SubagentSpawned {
                    subagent_id,
                    child_session_id,
                    description,
                    subagent_type,
        ...
        other => panic!("Expected Ds update, got {other:?}"),
    }
    match &loaded.updates[1] {
        SessionUpdate::Ds(notification) => {
            match &notification.update {
                DsSessionUpdateType::SubagentFinished {
                    subagent_id,
                    status,
                    tool_calls,
                    turns,
    ...
    assert_eq!(spawned_json["method"], "_ds.cli/session/update");
    ...
    assert_eq!(finished_json["method"], "_ds.cli/session/update");
...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(ds_chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
#[tokio::test]
async fn test_copy_session_data_transforms_ds_updates() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as DsSessionNotification,
        SessionUpdate as DsSessionUpdateType,
    };
    ...
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let source_info = Info {
        id: acp::SessionId::new("source-ds"),
        cwd: "/source".to_string(),
    };
    adapter.init_session(&source_info, default_model_id()).await.unwrap();
    let ds_notification = DsSessionNotification {
        session_id: acp::SessionId::new("source-ds"),
        update: DsSessionUpdateType::DiffReview {
            content: vec![
                DiffContent { diff :
    ...
    adapter
        .append_update(&source_info, &SessionUpdate::Ds(Box::new(ds_notification)))
        .await
        .unwrap();
    let target_info = Info {
        id: acp::SessionId::new("fork-source-ds-abcd1234"),
    ...
    match &loaded.updates[0] {
        SessionUpdate::Ds(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "fork-source-ds-abcd1234");
        }
        _ => panic!("Expected DeepSeek update"),
    }
}
#[tokio::test]
...
async fn test_copy_session_data_with_model_override() {
    ...
    let options = CopySessionOptions {
        ...
        new_model_id: Some("deepseek-v4-flash".to_string()),
        target_prompt_index: None,
        ..Default::default()
    };
    adapter.copy_session_data(&source_info, &target_info, options).await.unwrap();
    let loaded = adapter.load_session(&target_info).await.unwrap();
    assert_eq!(loaded.summary.current_model_id.0.as_ref(), "deepseek-v4-flash");
...
async fn test_load_prompts_only_applies_rewind_truncation() {
    use crate::extensions::notification::{
        SessionNotification as DsNotification, SessionUpdate as DsSessionUpdate,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = Info {
    ...
    let rewind = DsNotification {
        session_id: info.id.clone(),
        update: DsSessionUpdate::RewindMarker {
    ...
    for update in [
        ...
        SessionUpdate::Ds(Box::new(rewind)),
...
async fn test_append_feedback_creates_file_and_persists() {
    ...
    use ds_cli_proxy_types::feedback_types::{
    ...
    let user_entry = LocalFeedbackEntry::UserFeedback(UserFeedbackEntry {
        ...
        submission: Some(FeedbackSubmission {
            ...
            message_id: None,
            model_id: Some("ds-3-fast".into()),
            resolved_model_id: Some("deepseek-v4-pro.5".into()),
...
async fn test_copy_session_data_copies_tool_state() {
    ...
    let tool_state_json = serde_json::json!(
        { "state" : { "ds_build.TodoState" : { "todos" : [] } } }
    );
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    ds_sandbox::set_configured_profile("workspace");
    let expected = ds_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item("mid")),
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::ds_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::ds_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::ds_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ds_home::encode_cwd_dirname("/project");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ds_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::ds_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ds_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::ds_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::ds_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// path end-to-end (loader + serde + ds_sampling_types::
...
/// the legacy ds-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"ds-build"}"#,
        ],
    );
    assert_eq!(
        items.len(), 5,
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let ds_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"ds-build"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"ds-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    assert_eq!(
        legacy_assistant.model_id.as_deref(), Some("ds-build"),
    ...
    let ds_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"ds-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"ds-build"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();

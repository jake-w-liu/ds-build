...
/// Method name for DeepSeek extension session/update notifications.
pub(crate) const DS_SESSION_UPDATE_METHOD: &str = "_ds.cli/session/update";

/// A unified session update that can be either an ACP notification or an DeepSeek extension notification.
...
pub enum SessionUpdate {
    ...
    /// DeepSeek extension session notification (e.g., diff_review)
    Ds(Box<SessionNotification>),
}
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            SessionUpdate::Ds(notification) => {
                map.serialize_entry("method", DS_SESSION_UPDATE_METHOD)?;
                map.serialize_entry("params", notification)?;
            }
        }
        map.end()
...
pub(crate) struct SessionUpdateEnvelope {
    ...
    /// Either "session/update" for ACP or "_ds.cli/session/update" for DeepSeek extensions.
    pub method: String,
...
impl SessionUpdateEnvelope {
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        ...
        match update {
            ...
            SessionUpdate::Ds(notification) => Ok(Self {
                timestamp,
                method: DS_SESSION_UPDATE_METHOD.to_string(),
                params: serde_json::to_value(notification)?,
            }),
        }
    }
    ...
    pub(crate) fn into_update(self) -> Result<SessionUpdate, serde_json::Error> {
        if self.method == DS_SESSION_UPDATE_METHOD {
            let notification: SessionNotification = serde_json::from_value(self.params)?;
            Ok(SessionUpdate::Ds(Box::new(notification)))
        } else {
    ...
    pub(crate) fn from_str(line: &str) -> Result<SessionUpdate, serde_json::Error> {
        ...
        if let Ok(envelope) = serde_json::from_str::<BorrowedEnvelope<'_>>(line) {
            ...
            return if envelope.method == Some(DS_SESSION_UPDATE_METHOD) {
...
pub struct PersistedData {
    ...
    /// All session updates (ACP updates and DeepSeek extension updates) in chronological order
...
pub trait StorageAdapter: Send + Sync {
    ...
    /// Append a session update (ACP update or DeepSeek extension update) and increment counter
...
pub(crate) fn filter_rewind_lines<'a>(lines: Vec<&'a str>) -> Vec<&'a str> {
    ...
    for line in &lines {
        let (raw_params, is_ds_event) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(line) {
            let raw = env.params.map(|p| p.get()).unwrap_or(line);
            let ds = env.method == Some(DS_SESSION_UPDATE_METHOD);
            (raw, ds)
        } else {
            (*line, false)
        };
...
pub fn filter_rewind_updates(updates: Vec<SessionUpdate>) -> Vec<SessionUpdate> {
    ...
    for update in updates {
        ...
        if let SessionUpdate::Ds(ref n) = update
...
/// Returns only `SessionUpdate::Acp` updates (DeepSeek-extension updates like
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific ds home.
pub fn load_updates_for_replay_at(
    session_id: &str,
    ds_home: &std::path::Path,
) -> std::io::Result<Option<Vec<acp::SessionUpdate>>> {
    let sessions_root = ds_home.join("sessions");
...
pub enum PromptExtractEvent {
    ...
    /// A `RewindMarker` DeepSeek update: truncate accumulated prompts to this index.
    ///
...
/// - DeepSeek `"rewind_marker"`      → `update.target_prompt_index`
...
/// Peek inside ACP or DeepSeek `params` to read the `update.sessionUpdate` tag and
...
pub(crate) fn parse_prompt_extract_event(line: &str) -> PromptExtractEvent {
    ...
    if !is_ds_event && tag == *USER_MESSAGE_CHUNK {
    ...
    if is_ds_event && tag == *REWIND_MARKER {
...
mod tests {
    ...
    /// Wrap a DeepSeek notification as the envelope stored in updates.jsonl.
    fn ds_envelope(session_update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_ds.cli/session/update","params":{{"sessionId":"s","update":{session_update_json}}}}}"#
        )
    }
    ...
    #[test]
    fn ds_rewind_marker_yields_rewind_to() {
        let line = ds_envelope(
    ...
    #[test]
    fn ds_rewind_to_zero_yields_rewind_to_zero() {
        let line = ds_envelope(
    ...
    #[test]
    fn ds_diff_review_yields_not_user() {
        let line = ds_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    /// A valid JSON object that has no recognisable ACP/DeepSeek shape — NotUserMessage.
    #[test]
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        // DeepSeek-style line persisted by an older binary: no _meta at all.
        let old_ds = r#"{"timestamp":2,"method":"_ds.cli/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
        let raw = format!("{a1}\n{old_ds}\n");
        ...
        let new_ds = r#"{"timestamp":2,"method":"_ds.cli/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
        let raw = format!("{a1}\n{new_ds}\n");
        let prepared = prepare_replay_lines(&raw, Some("ev1"));
        assert!(!prepared.mark_replay);
        assert_eq!(prepared.lines.len(), 1);
    ...
    fn fast_reject_rewind_marker_in_content() {
        ...
        // (b) an ACP (non-ds) update carrying rewind_marker in content is NOT a
        // real ds rewind_marker → NotUserMessage (no RewindTo).
    ...
    fn prepare_replay_reports_spawn_without_finish() {
        let spawn = |id: &str, child: &str| {
            format!(
                r#"{{"method":"_ds.cli/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
            )
        };
        let finish = |id: &str| {
            format!(
                r#"{{"method":"_ds.cli/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
            )
        };
    ...
    #[test]
    fn from_str_unknown_ds_variant_deserializes_via_envelope() {
        ...
        let line = ds_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Ds(notif) => {
                assert_eq!(
                    notif.update,
                    crate::extensions::notification::SessionUpdate::Unknown
                );
            }
            SessionUpdate::Acp(_) => panic!("expected Ds variant"),
        }
    }

    #[test]
    fn from_str_known_ds_variant_still_works() {
        let line = ds_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Ds(notif) => {
                assert_eq!(

...
//!
//! Receives [`ds_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`ds_mcp::liveness`]),
//! - the [`ds_mcp::servers::DsClientHandler`] (server-pushed
...
//! `ds.cli/mcp/server_status` and the payload schema defined by
...
pub const SERVER_STATUS_METHOD: &str = "ds.cli/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`ds_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `ds_com_` prefix)
    /// or `local` (user `.ds/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`ds_mcp::servers::ClientStateKind::Ready`]
...
/// [`ds_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit ds.com"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && ds_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///   `ConfigRemoved` keys,
/// - emit one ACP `ds.cli/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`ds_acp_lib::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `ds.cli/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    /// Contract: managed server names (starting with `ds_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("ds_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use ds_mcp::servers::{McpClient, McpState};
    ...
    fn discard_gateway() -> ds_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        ds_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = ds_acp_lib::AcpAgentGatewaySender::new(gw_tx);

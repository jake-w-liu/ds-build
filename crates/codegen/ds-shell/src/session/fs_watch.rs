//! Session-level fs-watch policy over [`ds_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `ds_fsnotify`. This module
...
use ds_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
/// Dedup key for `ds.cli/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> ds_workspace::file_system::FileIndexDelta {
    use ds_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `ds.cli/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `ds.cli/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = ds_workspace::session::git::get_branch(&self.cwd).await;
        let commit = ds_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = ds_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = ds_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: ds_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                ds_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || ds_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = ds_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(ds_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: ds_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {

...
/// JSON-RPC wrapper for DeepSeek extension notifications.
#[derive(Debug, Serialize)]
struct DsJsonRpcNotification<'a> {
    method: &'static str,
    params: &'a crate::extensions::notification::SessionNotification,
}

const ACP_SESSION_UPDATE_METHOD: &str = "session/update";
const DS_SESSION_UPDATE_METHOD: &str = "_ds.cli/session/update";
...
impl ExportedMessage {
    ...
    pub fn from_ds_notification(
        notification: &crate::extensions::notification::SessionNotification,
    ) -> Self {
        let wrapper = DsJsonRpcNotification {
            method: DS_SESSION_UPDATE_METHOD,
            params: notification,
        };
        let content = serde_json::to_string(&wrapper).unwrap_or_else(|_| "{}".to_string());
        ...
        updates
            ...
            .map(|update| match update {
                ...
                SessionUpdate::Ds(notification) => {
                    ExportedMessage::from_ds_notification(notification)
                }
            })
            .collect()
    }

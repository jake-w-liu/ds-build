...
/// DeepSeek `SessionUpdate::RewindMarker` discriminant. Appears verbatim in compact
/// JSON, so it doubles as a cheap substring pre-filter.
pub(crate) static REWIND_MARKER: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&DsSessionUpdate::RewindMarker {
        target_prompt_index: 0,
        created_at: String::new(),
    })
});

/// DeepSeek `SessionUpdate::TaskBackgrounded` discriminant.
pub(crate) static TASK_BACKGROUNDED: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&DsSessionUpdate::TaskBackgrounded {
...
/// DeepSeek `SessionUpdate::TaskCompleted` discriminant.
pub(crate) static TASK_COMPLETED: LazyLock<String> = LazyLock::new(|| {
    tagged_discriminant(&DsSessionUpdate::TaskCompleted {

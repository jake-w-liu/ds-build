...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: ds_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `deepseek-v4-pro.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-ds-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<ds_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<ds_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.ds/hooks`,
    ...
    /// Out-of-band history repair (`ds.cli/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<ds_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    /// DeepSeek extension session notification - client-side events to store in persistence
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, ds_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `ds.cli/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<ds_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<ds_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<ds_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<ds_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: ds_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<ds_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `ds.cli/plugins/list`.
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<ds_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_ds_notification`).
    ...
    /// serialized mailbox; the rebroadcast of `ds.cli/queue/changed` is the
    ...
    /// `ds.cli/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `ds-build-0.1` when available via
    ...
    /// `ds-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `ds.cli/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<ds_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<ds_sampling_types::conversation::ConversationItem>>>,
    },

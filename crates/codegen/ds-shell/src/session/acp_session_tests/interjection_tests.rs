...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `ds.cli/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let ds_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "ds.cli/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an ds.cli/session/interjection broadcast");
                assert_eq!(

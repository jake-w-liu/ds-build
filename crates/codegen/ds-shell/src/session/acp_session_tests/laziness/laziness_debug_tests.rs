...
fn assistant_with_reasoning_items(
    ...
    if !reasoning_text.is_empty() {
        out.push(ConversationItem::Reasoning(
            ds_sampling_types::rs::ReasoningItem {
                id: String::new(),
                summary: vec![ds_sampling_types::rs::SummaryPart::SummaryText(
                    ds_sampling_types::rs::SummaryTextContent {
...
fn synthetic_user_text(
    text: &str,
    reason: ds_sampling_types::SyntheticReason,
) -> ConversationItem {
    ConversationItem::User(UserItem {
        content: vec![ContentPart::Text { text: text.into() }],
...
fn window_assistant_text_pin_skips_empty_assistant_turns() {
    ...
    let empty_asst = ConversationItem::Assistant(ds_sampling_types::AssistantItem {
        content: String::new().into(),
        tool_calls: vec![ds_sampling_types::ToolCall {
            id: "c".into(),
            name: "read_file".into(),
            arguments: "{}".into(),
        }],
...
fn sample_line() -> LazinessDebugLogLine {
    LazinessDebugLogLine {
        ...
        model_id: "deepseek-v4-pro.5".to_string(),
        items_sent: 28,

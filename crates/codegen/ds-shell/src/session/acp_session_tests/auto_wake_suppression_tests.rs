use super::support::*;
use super::*;
use ds_tools::reminders::task_completion::consumed_completion_ids;
use ds_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use ds_tool_types::{
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use ds_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use ds_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
...
struct OneTaskTerminal {
    tasks: Vec<ds_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl ds_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: ds_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        ds_tools::computer::types::TerminalRunResult,
        ds_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<ds_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> ds_tools::computer::types::KillOutcome {
        ds_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<ds_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<ds_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> ds_tools::computer::types::TaskSnapshot {
    ds_tools::computer::types::TaskSnapshot {
        task_id: id.into(),

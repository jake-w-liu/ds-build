...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), DsComConfig::default()));
    am.hot_swap(DsAuth {
...
/// attribution.
fn auth_error() -> ds_sampler::SamplingErrorInfo {
    ds_sampler::SamplingErrorInfo {
        kind: ds_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: ds_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        ds_chat_state::AuthType::SessionToken => "cached_token",
        ds_chat_state::AuthType::ApiKey => "ds.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `DEEPSEEK_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(ds_chat_state::Credentials {
...
/// `DEEPSEEK_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> ds_sampler::SamplingErrorInfo {
    ds_sampler::SamplingErrorInfo {
            kind: ds_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model ds-build does not exist or your team does not have access".into(),
...
async fn legacy_auth_hint_on_404_model_not_found() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                msg.contains("ds logout"),
                "hint must mention `ds logout`, got: {msg}"
            );
            assert!(
                msg.contains("ds login"),
                "hint must mention `ds login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> ds_sampler::SamplingErrorInfo {
    ds_sampler::SamplingErrorInfo {
            kind: ds_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://api.deepseek.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_ds_token_auth=ds-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
// transiently collapsed to `ApiKey` (session-token cache miss + `DEEPSEEK_API_KEY`)
...
fn session_token_auth_gate_truth_table() {
    ...
    // Session method + Unknown BYOK: refresh only against a first-party DeepSeek
...
/// Negative: a genuine `ds.api_key` method keeps its configured key on the
...
/// End-to-end for the frozen-gate bug: a session born on `ds.api_key` (gate
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = ds_sampler::SamplerConfig {

...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> ds_computer_hub_sdk::ObservabilityBridge {
    ds_computer_hub_sdk::ObservabilityBridge::new(
        None,
        ds_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> ds_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> ds_agent::Agent {
    use ds_tools::implementations::ds_build::update_goal::UpdateGoalTool;
    use ds_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// DS-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_ds_build_agent_with_todo() -> ds_agent::Agent {
    use ds_tools::implementations::ds_build::todo::TodoWriteTool;
    use ds_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> ds_agent::Agent {
    use ds_tools::implementations::ds_build::enter_plan_mode::EnterPlanModeTool;
    use ds_tools::implementations::ds_build::exit_plan_mode::ExitPlanModeTool;
    use ds_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<ds_tools::registry::types::ToolConfig>,
) -> ds_agent::Agent {
    test_agent_from_config(
        ds_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        ds_agent::AgentDefinition::default_ds_build(),
        std::sync::Arc::new(ds_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: ds_agent::AgentDefinition,
    backend: std::sync::Arc<dyn ds_tools::computer::types::TerminalBackend>,
) -> ds_agent::Agent {
    use ds_tools::computer::local::LocalFs;
    use ds_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("ds-test"),
    ...
    ds_agent::Agent::new(
        definition,
        ds_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        ds_agent::ReminderPolicy::default(),
        ds_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<ds_acp_lib::AcpClientMessage>,
    ...
    let actor = SessionActor {
        ...
        permissions: ds_workspace::permission::PermissionHandle::allow_all(),
        tool_context,

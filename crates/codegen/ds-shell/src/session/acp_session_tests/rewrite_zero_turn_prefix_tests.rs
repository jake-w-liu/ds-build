...
/// Local skill is always listable, so it renders into the ds markdown skill
/// catalog when the pending baseline is drained for a different agent.
fn regression_skill() -> ds_tools::implementations::skills::types::SkillInfo {
...
/// Regression: a zero-turn agent rebuild INTO a ds/Default agent
/// must re-inject the baseline skill `<system-reminder>`. `initialize()`
/// is otherwise the only place skills are surfaced for the ds agent, so

...
//!
//! Tests mutate `DS_GOAL_CLASSIFIER` so they carry `serial`.
...
use ds_tools::implementations::ds_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<ds_tools::implementations::ds_build::task::types::SubagentRequest>,
) {
    match behaviour {
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "the infrastructure-failure path must be recorded",
            );
            assert_eq!(
                counters.summarizer_spawns.load(SeqOrd::SeqCst),
                0,

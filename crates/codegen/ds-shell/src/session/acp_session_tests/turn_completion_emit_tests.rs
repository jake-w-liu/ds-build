...
//! notification the real `send_ds_notification` persists — not a
...
fn is_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        m,
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ds(n))
            if matches!(n.update, DsSessionUpdate::TurnCompleted { .. })
    )
}
...
fn turn_completed_fields(msgs: &[PersistenceMsg]) -> Option<(String, String, Option<String>)> {
    msgs.iter().find_map(|m| match m {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ds(n)) => match &n.update {
            ...
            *actor
            ...
            while let Ok(msg) = gateway_rx.try_recv() {
                if let ds_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                    && args.request.method.as_ref() == "ds.cli/session_notification"
                    && let Ok(v) =

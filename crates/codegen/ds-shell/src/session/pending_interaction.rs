...
use crate::extensions::notification::{SessionNotification, SessionUpdate as DsSessionUpdate};
...
pub enum PendingKind {
    ...
    Question,
    /// `ds.cli/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `ds.cli/exit_plan_mode` from a detached task
...
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: DsSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            DsSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                DsSessionUpdate::InteractionResolved {

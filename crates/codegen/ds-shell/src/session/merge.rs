...
//!
//! Used by both the ACP `ds.cli/session/list` handler and the `ds sessions`
...
pub async fn fetch_merged(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            ds_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
...
mod tests {
    ...
    fn all_metadata_fields_populated_together() {
        ...
        assert_eq!(merged[0].repo_name.as_deref(), Some("ds"));

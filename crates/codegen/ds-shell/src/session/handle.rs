...
use ds_file_utils::queue::UploadQueue;
...
pub struct SessionHandle {
    ...
    pub chat_state_handle: ds_chat_state::ChatStateHandle,
    ...
    /// `ds.cli/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `ds.cli/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `ds.cli/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: ds_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn ds_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> ds_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            ds_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<ds_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<ds_hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<ds_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx

...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(ds_version::VERSION.to_string());
    s
}

/// Pipeline: persist → strip → submit → telemetry (telemetry fields captured
/// before strip). Callers merge `DS_USER_METADATA` and set
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `DS_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `DS_TELEMETRY_ENABLED`. These are analytics data that
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<ds_file_utils::queue::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub fn set_upload_queue_stats(&self, stats: Arc<ds_file_utils::queue::UploadQueueStats>) {
    ...
    pub fn client_type(&self) -> ds_cli_proxy_types::feedback_types::ClientType {
    ...
    pub(crate) async fn submit_text_feedback(
        ...
        if let Some(user_meta) =
            crate::agent::mvp_agent::parse_json_object_env("DS_USER_METADATA")
        {
    ...
    /// `ds.cli/debug/trigger_feedback` ACP extension method to exercise
    ...
    pub async fn shutdown(&self, queue: Option<&ds_file_utils::queue::UploadQueue>) {
...
mod tests {
    ...
    async fn test_shutdown_with_upload_queue_drains() {
        ...
        use std::sync::Arc;
        use ds_file_utils::queue::{TraceExportSource, UploadQueue, UploadRetryPolicy};

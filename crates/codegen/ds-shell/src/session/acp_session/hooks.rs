...
//!
//! Hooks registered at `session/new` (`_meta["ds.cli/hooks"]`) come in two flavors,
//! both matched by the agent ([`ds_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `ds.cli/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `ds.cli/hooks/event`,
...
use ds_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use ds_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "ds.cli/hooks/event";
const HOOK_RUN_METHOD: &str = "ds.cli/hooks/run";

/// Default per-callback bound for a client's `ds.cli/hooks/run` reply; on timeout the gate
...
/// Outcome of the `ds.cli/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "ds.cli/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed ds.cli/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "ds.cli/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("ds.cli/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::HookBlocked {
    ...
    /// `ds.cli/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = ds_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    ds_telemetry::session_ctx::log_event(
                        ds_telemetry::events::ClientHookGate {
    ...
    /// Issue one `ds.cli/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `ds.cli/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    ds_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));

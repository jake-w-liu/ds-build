...
pub fn conversations_lane_enabled() -> bool {
    ...
    std::env::var("DS_SESSION_LIST_CONVERSATIONS")
        .ok()
        .is_some_and(|v| {
            !matches!(
...
/// Env lane (desktop `DS_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`DS_CHAT_MODE`); hard-off in release builds.
...
/// Parse `ds.cli/session/list` params and, under process-wide chat mode, force
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("ds.cli/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
...
pub struct ExtListResponseMeta {
    #[serde(rename = "ds.cli/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "ds.cli/partial")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            cwd: "/Users/me/ds".into(),
            hostname: Some("devbox".into()),
            source: "local".into(),
            model_id: Some("ds-build".into()),
            num_messages: 7,
            last_active_at: Some(updated_at.into()),
            branch: Some("main".into()),
            repo_name: Some("ds".into()),
            worktree_label: Some("wt".into()),
            git_root_dir: Some("/Users/me/ds".into()),
            git_remotes: vec!["git@github.com:example/repo.git".into()],
            source_workspace_dir: Some("/Users/me/ds-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["ds.cli/session"]["kind"], "build");
        assert_eq!(value["gitRootDir"], "/Users/me/ds");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/ds-src");
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(r.facets.get(CWD_FACET_KEY),
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/ds"));
    }
    #[test]
    fn bare_session_info_is_minimal_plus_meta() {
        ...
        assert_eq!(value["sessionId"], "s1");
        assert_eq!(value["cwd"], "/Users/me/ds");
        assert_eq!(value["title"], "a summary");
        assert_eq!(value["_meta"]["ds.cli/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!(
            { "ds.cli/facetFilters" : { "kind" : ["build"], "starred" : true },
            "ds.cli/query" : "antelope", "ds.cli/limit" : 5, }
        );
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "ds.cli/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!(
                { "ds.cli/facetFilters" : { "kind" : ["build"], "starred" : [true],
                "workspace" : ["w1"] }, "ds.cli/query" : "antelope", "ds.cli/limit" : 5,
                }
            )),
            ..ListReq::default()
        };
    ...
    fn ds_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::DsComConfig::default(),
        ));
        am.hot_swap(crate::auth::DsAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::ds_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = ds_test_support::EnvGuard::set(
            "DS_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(ds_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "ds.cli/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = ds_test_support::EnvGuard::unset("DS_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = ds_test_support::EnvGuard::set("DS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = ds_test_support::EnvGuard::set("DS_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::DS_CHAT_MODE_ENV;
        let _chat_off = ds_test_support::EnvGuard::unset(DS_CHAT_MODE_ENV);
        let _desktop_off =
            ds_test_support::EnvGuard::unset("DS_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                ds_test_support::EnvGuard::set("DS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = ds_test_support::EnvGuard::set(DS_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!(
            { "_meta" : { "ds.cli/facetFilters" : { "kind" : ["build"], "starred" : [true]
            } }, }
        )
        .to_string();
        {
            let _off = ds_test_support::EnvGuard::unset(DS_CHAT_MODE_ENV);
        ...
        {
            let _on = ds_test_support::EnvGuard::set(DS_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    /// Wire pin for the cross-crate `ds.cli/partial` envelope the pager parses:

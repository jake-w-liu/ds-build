...
use std::sync::Arc;
use ds_chat_state::compaction_utils::{
...
use ds_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `DS_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("DS_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("DS_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        ds_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("DS_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG DS_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(ds_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`ds-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                ds_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
        ...
        self.send_ds_notification(DsSessionUpdate::AutoCompactCompleted {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            ds_telemetry::session_ctx::log_event(
                ds_telemetry::events::AutoCompactSuppressed {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    ds_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    ds_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if ds_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: ds_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            ds_telemetry::events::CompactionTrigger::Manual => "manual",
            ds_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = ds_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            ds_hooks::event::HookEventName::PreCompact,
            ds_hooks::event::HookPayload::PreCompact {
        ...
        let simplified_messages = if verbatim_input_enabled {
            ds_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
        ...
        let effective_tool_defs: Vec<ds_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            ds_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<ds_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(ds_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<ds_sampling_types::HostedTool> =
            if use_backend_search {
        ...
        let estimated_input_tokens =
            ds_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, ds_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = ds_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match ds_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(ds_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(ds_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(ds_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            ds_telemetry::session_ctx::log_event(
                                ds_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = ds_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                ds_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                ds_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use ds_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn ds_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn ds_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : ds_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        ds_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            ds_hooks::event::HookEventName::PostCompact,
            ds_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if ds_token_estimation::exceeds_threshold(
            ...
            let percentage = ds_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &ds_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = ds_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_ds_notification(DsSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                ds_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if self
                    ...
                    == SUPPRESS_NONE
                {
                    self.send_ds_notification(DsSessionUpdate::AutoCompactFailed {
                        error: String::new(),
                    })
                    .await;
                }
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<ds_sampling_types::ToolSpec>,
        user_context: Option<&str>,
    ...
    fn persist_compaction_checkpoint(
        ...
        use crate::extensions::notification::{
            CompactionCheckpointFile, CompactionCheckpointInfo, SessionUpdate as DsSessionUpdate,
        };
        ...
        self.persist_ds_update_only(DsSessionUpdate::CompactionCheckpoint(Box::new(info)));
        tracing::info!(
            prompt_index_at_compaction,
...
mod inline_auto_compact_flow_tests {
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<ds_acp_lib::AcpClientMessage>,
        ...
        let chat_state_handle = ds_chat_state::ChatStateActor::spawn(
            vec![],
            ...
            Box::new(ds_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: ds_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            vcs_kind: ds_workspace::session::git::VcsKind::Git,
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<ds_acp_lib::AcpClientMessage>();
                ...
                async fn notification_for(reason: SuppressReason) -> String {
                    ...
                    while let Ok(msg) = persistence_rx.try_recv() {
                        if let PersistenceMsg::Update(
                            crate::session::storage::SessionUpdate::Ds(notif),
                        ) = msg
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use ds_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    async fn forked_release_still_over_threshold_suppresses_auto() {
        ...
        local
            .run_until(async {
                ...
                while let Ok(msg) = persistence_rx.try_recv() {
                    if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ds(
                        notif,
                    )) = msg
                        && matches!(
                            &notif.update,
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): DS Build usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("DS Build usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> ds_sampler::SamplingErrorInfo {
        ds_sampler::SamplingErrorInfo {
            kind: ds_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = ds_sampler::SamplingErrorInfo {
                    kind: ds_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = ds_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = ds_chat_state::CompactionMode::Summary;

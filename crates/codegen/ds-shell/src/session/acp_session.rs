...
use crate::extensions::notification::SessionUpdate as DsSessionUpdate;
use crate::extensions::notification::{
    RetryState, SessionNotification as DsSessionNotification, is_reauthable_failure,
};
...
use ds_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use ds_agent::prompt::skills::SkillsConfig;
use ds_sampler::SamplerConfig as SamplingConfig;
use ds_sampling_types::truncate_bytes;
...
use ds_tools::types::compat::CompatConfig;
use ds_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use ds_workspace::file_system::CodebaseIndexManager;
use ds_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use ds_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "ds_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `ds.cli/hooks/run` reverse request).
mod hooks;
...
impl ds_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        ds_tools::types::resources::ManagedGatewayToolCallResponse,
        ds_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| ds_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                ds_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                ds_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = ds_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| ds_tool_protocol::ToolId::new("use_tool").expect("valid"));
                ds_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            ds_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            ds_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, ds_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, ds_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, ds_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, ds_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `ds-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<ds_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: ds_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<ds_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<ds_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-ds-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<ds_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<ds_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<ds_agent::Agent>,
    /// Dedup slot for `ds.cli/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `ds.cli/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Kill-switch (`DS_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<ds_tools::implementations::ds_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, ds_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: ds_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<ds_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["ds.cli/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<ds_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<ds_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: ds_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `ds-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: ds_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`ds_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`ds_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: ds_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: ds_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == ds_tools::implementations::ds_build::SCHEDULER_CREATE_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_ds_notification(DsSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<ds_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `ds prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &ds_agent::PromptContext) {
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use ds_tools::types::output::{MCPOutput, ToolOutput};
    use ds_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl ds_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl ds_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> ds_tool_protocol::ToolId {
            ds_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ds_tool_runtime::ListToolsContext,
        ) -> ds_tool_types::ToolDescription {
            ds_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
...
mod tool_meta_stamp_tests {
    //! Pin the `ds.cli/tool` stamps on the harness emission paths: the early
    ...
    use super::*;
    use tokio::sync::mpsc;
    use ds_tools::registry::types::ToolConfig;
    use ds_tools::tool_taxonomy::TOOL_META_KEY;
    use ds_workspace::permission::PermissionCommand;
    ...
    /// The `ds.cli/tool` object from an event's `_meta`, if present.
    ...
    async fn prepare_tool_call_stamps_early_tool_call_and_refinement() {
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries ds.cli/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "ds_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries ds.cli/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries ds.cli/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
mod managed_gateway_tool_tests {
    ...
    use ds_tools::types::output::{MCPOutput, ToolOutput};
    use ds_tools::types::tool::{ToolKind, ToolNamespace};
    use ds_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<ds_tools::types::resources::ManagedGatewayToolCatalog>()
            .await

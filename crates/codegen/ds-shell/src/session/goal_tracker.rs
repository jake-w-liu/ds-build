...
/// scratch root (`<temp_dir>/ds-goal-<id>`) and the verifier
...
/// Private per-goal scratch root: `<temp_dir>/ds-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("ds-goal-{verifier_id}"))
}
...
mod tests {
    ...
    /// `ds-goal-<12 hex chars>` scratch-root format (and the
    ...
    /// `temp_dir()` + `verifier_id`: a `ds-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("ds-goal-vid123"));
        assert_eq!(

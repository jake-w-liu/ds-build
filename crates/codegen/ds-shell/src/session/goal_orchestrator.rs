...
use crate::extensions::notification::{
    SessionNotification as DsSessionNotification, SessionUpdate as DsSessionUpdate,
};
...
impl GoalNotifySender {
    ...
    /// latch updates that must not run the `send_ds_notification`
    /// rewind-window-close side effect.
    pub(crate) fn send_update(&self, update: DsSessionUpdate) {
    ...
    fn dispatch_update(&self, update: DsSessionUpdate, persist: bool) {
...
/// drop its goal state.
pub(crate) fn build_goal_cleared() -> DsSessionUpdate {
    DsSessionUpdate::GoalUpdated {
        goal_id: String::new(),
...
mod tests {
    ...
    fn planning_field(update: &DsSessionUpdate) -> Option<bool> {
        match update {
            DsSessionUpdate::GoalUpdated { planning, .. } => *planning,
            _ => panic!("expected GoalUpdated"),
        }
    }
    ...
    fn build_goal_updated_verifying_flag_reflects_latch() {
        ...
        let mut o = make_base_orchestration();
        let verifying = |u: &DsSessionUpdate| match u {
            DsSessionUpdate::GoalUpdated {
                verifying_completion,
                ..
            } => *verifying_completion,
    ...
    fn build_goal_updated_gates_per_model_breakdown_on_two_or_more_models() {
        ...
        let mut o = make_base_orchestration();
        o.live_tokens_by_model = vec![("deepseek-v4-pro".into(), 5_000)];
        ...
        o.live_tokens_by_model = vec![("deepseek-v4-pro".into(), 5_000), ("deepseek-v4-flash".into(), 3_000)];
        match build_goal_updated(&o, 0, 0) {
            DsSessionUpdate::GoalUpdated {
                live_tokens_by_model,
                ..
                ...
                live_tokens_by_model,
                vec![("deepseek-v4-pro".to_owned(), 5_000), ("deepseek-v4-flash".to_owned(), 3_000)]
            ),
            _ => panic!("expected GoalUpdated"),
        }
    }

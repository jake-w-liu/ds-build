...
//! `ds-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use ds_file_utils::events::tracker::EventTracker;
pub(crate) use ds_file_utils::events::types::{
...
) -> Option<ds_sampling_types::PriorTurnInterrupt> {
    use ds_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run ds-build flavor. Distinct from
...
/// (`ds-file-utils`). The two enums are intentional mirrors and live in

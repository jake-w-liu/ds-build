...
/// Prefix on `x_ds_req_id` for laziness-classifier sampler calls.
...
pub(crate) const LAZINESS_REQ_ID_PREFIX: &str = "ds-laziness-";
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let ds_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = ds_sampling_types::reasoning_item_text(r);
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<ds_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text = ds_tools::util::truncate_str_with_marker(
                        &text,
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args = ds_tools::util::truncate_str_with_marker(
                        &args,
                        CLASSIFIER_TURN_MAX_LEN,
                    )
                    .into_owned();

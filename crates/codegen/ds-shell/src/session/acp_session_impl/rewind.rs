...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &ds_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            ds_workspace::session::file_state::FlexiblePath,
            Option<String>,
        ...
        if wants_conversation_rewind {
            ...
            self.persist_ds_update_only(DsSessionUpdate::RewindMarker {
                target_prompt_index: target_index,
    ...
    /// Out-of-band history repair (`ds.cli/session/repair`) for a resident
    /// session: run `ds_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<ds_chat_state::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(ds_chat_state::commands::RepairHistoryBlocked);
        }

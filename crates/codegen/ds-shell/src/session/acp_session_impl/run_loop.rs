...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<ds_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<ds_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            ds_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : ds_telemetry::memory_log::TARGET,
            ...
            ds_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(ds_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(ds_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            ds_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(ds_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(ds_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(ds_hooks::event::HookEventName::SessionEnd, None,
            ds_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            ds_hooks::dispatcher::dispatch_non_blocking(& registry,
            ds_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(ds_hooks::event::HookEventName::Stop,
            None, ds_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            ds_hooks::dispatcher::dispatch_non_blocking(& registry,
            ds_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            ds_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            session.send_ds_notification(DsSessionUpdate::MemorySessionSaved { path :
            ...
            ds_telemetry::session_ctx::log_event(ds_telemetry::events::SessionEnded
            ...
            ds_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(ds_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(ds_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .send_ds_notification(DsSessionUpdate::PluginUpdatesInstalled { updates },)
            ...
            .push_capped(ds_tools::implementations::ds_build::task::types::MonitorEventNotification
            ...
            ds_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            SessionCommand::DsSessionNotification { notification } => { session
            .handle_ds_session_notification(notification). await; }
            ...
            .send(ds_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(ds_file_utils::events::Event::McpServerToggled { server_name :
            ...
            .forward_fire_and_forget(acp::ExtNotification::new("ds.cli/mcp/tools_changed",
            ...
            session.fire_hook(ds_hooks::event::HookEventName::SessionStart, None,
            ds_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            ds_hooks::dispatcher::dispatch_non_blocking(& registry,
            ds_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            ds_cli_proxy_types::feedback_types::FeedbackToolOutcome; let
            ...
            .fire_hook(ds_hooks::event::HookEventName::SessionEnd, None,
            ds_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            ds_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(ds_hooks::event::HookEventName::Stop,
            None, ds_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            ds_hooks::dispatcher::dispatch_non_blocking(& registry,
            ds_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[ds_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = ds_chat_state::compaction_utils::extract_user_query(&raw);

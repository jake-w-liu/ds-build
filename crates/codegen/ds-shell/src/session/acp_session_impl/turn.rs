...
impl SessionActor {
    ...
    pub(crate) async fn normalize_images_with_notices(
        ...
        if !norm_result.compressed.is_empty() {
            ...
            let images = norm_result
                ...
                .collect();
            self.send_ds_notification(DsSessionUpdate::ImageCompressed { images, message })
                .await;
        }
        if !norm_result.re_encode_fallbacks.is_empty() {
            ...
            self.send_ds_notification(DsSessionUpdate::ImageCompressed {
                images: vec![],
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = ds_telemetry::events::SlashCommandUsed {
        ...
        self.send_before_turn_event(ds_tool_protocol::turn_hook::BeforeTurnPayload {
            ...
            conversation_message_count: msg_count,
            session_relationship: ds_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = ds_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || ds_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = ds_telemetry::events::PromptSubmitted {
            ...
            ds_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(ds_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            ds_hooks::event::HookEventName::UserPromptSubmit,
            ds_hooks::event::HookPayload::UserPromptSubmit {
        ...
        self.observability_bridge
            .emit(ds_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                self.send_after_turn_event(ds_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: ds_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(ds_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: ds_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(ds_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: ds_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
        ...
        self.dispatch_hook(
            ds_hooks::event::HookEventName::Stop,
            ds_hooks::event::HookPayload::Stop {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&ds_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = ds_agent_lifecycle::TurnAbortInput::new(
                    ds_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = ds_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    /// monitor events only via the structured `ds.cli/monitor_event` channel.
    pub(crate) async fn inject_pending_monitor_events(&self) {
        let Some(buffer) = &self.tool_context.monitor_event_buffer else {
            return;
        };
        let mine = ds_tools::implementations::ds_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = ds_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = ds_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use ds_tools::types::memory_backend::MemoryBackend as _;
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<ds_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                ds_telemetry::session_ctx::log_event(ds_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_ds_session_id = Some(self.session_info.id.to_string());
            request.x_ds_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_ds_agent_id = Some(ds_telemetry::id::agent_id());
            if request.x_ds_deployment_id.is_none() {
            ...
            self.observability_bridge
                .emit(
                    ds_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: ds_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            ds_telemetry::unified_log::info(
            ...
            for item in response.items {
                match item {
                    ds_sampling_types::ConversationItem::Assistant(_) => {
            ...
            self.observability_bridge
                .emit(
                    ds_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: ds_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
            ...
            if let Some(trigger_info) = self.check_preflight_overflow().await {
                if let Err(e) = self.run_compact_only(trigger_info).await {
                    ...
                    // Suppress further auto-compactions for this turn to
                    // prevent an unbounded retry loop when compaction
                    // consistently fails (e.g., compaction model is
                    // unreachable).  The model will be sampled with an
...
mod user_echo_broadcast_tests {
    ...
    /// the text from the `ds.cli/session/interjection` broadcast, so a live

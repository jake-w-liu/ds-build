...
//! its buffered/transient/direct variants, DeepSeek-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, ds_chat_state::UsageTotals)],
    ...
    /// Producer for the **high-frequency streaming path** with an DeepSeek
    /// extension payload. Routes through `event_tx` -> `ReplayBuffer` ->
    /// `emit_buffered` so chunks get merged + debounced + emitted.
    ///
    /// For one-shot DeepSeek events (RetryState, ImageCompressed, HookExecution,
    /// AutoCompactCompleted, etc.), use `send_ds_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_ds_update` vs `send_ds_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_ds_update(&self, update: DsSessionUpdate) {
        self.close_rewind_window().await;
        let notification = DsSessionNotification {
    ...
    /// - **DeepSeek** (`ToolCallDeltaChunk`) -> inlines a gateway
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Ds(n) => {
                self.log_outbound_ds_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    /// Tracing log for buffered DeepSeek notifications emerging from
    /// emit_buffered. Mirrors `log_outbound_notification` for ACP.
    /// Visible with `RUST_LOG=acp_event=info`.
    fn log_outbound_ds_buffered(&self, notification: &DsSessionNotification) {
        if !matches!(
            notification.update,
            DsSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target : "acp_event", event = "ds_buffered_notification_sent", session_id =
            % self.session_info.id, "Sending buffered DeepSeek session notification"
        );
    }
    ...
    /// Handle DeepSeek session notifications - store them in persistence
    ...
    pub(super) async fn handle_ds_session_notification(
        &self,
        mut notification: DsSessionNotification,
    ) {
        if !matches!(
            notification.update,
            DsSessionUpdate::SubagentProgress { .. }
        ) {
            tracing::debug!("storing DeepSeek session notification");
        }
        {
        ...
        match &notification.update {
            DsSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    ds_hooks::event::HookEventName::SubagentStart,
                    None,
                    ds_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = ds_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        ds_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            DsSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    ds_hooks::event::HookEventName::SubagentEnd,
                    None,
                    ds_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
        ...
        let _ = self
            ...
            .persistence_tx
            .send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ds(Box::new(notification)),
            ));
    }
    /// Persist an DeepSeek extension notification to `updates.jsonl` **without** sending it
    ...
    pub(super) fn persist_ds_update_only(&self, update: DsSessionUpdate) {
        let notification = DsSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send DeepSeek update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            ds_hooks::event::HookEventName::Notification,
            None,
            ds_hooks::event::HookPayload::Notification {
    ...
    /// Send an DeepSeek extension notification to the client
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ds_notification(&self, update: DsSessionUpdate) {
        self.send_ds_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_ds_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ds_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod ds_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_ds_event_id(
        prx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>,
    ) -> String {
        loop {
            match prx.try_recv().expect("an DeepSeek line must be persisted") {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ds(notif)) => {
                    return notif
                        ...
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted DeepSeek lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// `send_ds_notification` (own emission) and
    /// `handle_ds_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_ds_lines_carry_event_id() {
        ...
        local
            .run_until(async {
                ...
                actor
                    .send_ds_notification(DsSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_ds_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_ds_session_notification(DsSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: DsSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_ds_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_ds_update_only(DsSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_ds_event_id(&mut prx);

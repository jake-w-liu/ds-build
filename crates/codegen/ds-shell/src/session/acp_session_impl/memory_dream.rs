...
pub(super) struct MemoryFlushSnapshot {
    counts: ds_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &ds_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                ds_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                ds_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        ds_telemetry::session_ctx::log_event(
            ds_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_ds_notification(DsSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_ds_req_id: Some(format!("ds-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: ds_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_ds_notification(DsSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            ds_telemetry::session_ctx::log_event(
                ds_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_ds_conv_id: Some(session_id.clone()),
                x_ds_req_id: Some(format!("ds-flush-{}", uuid::Uuid::new_v4())),
                x_ds_session_id: Some(session_id.clone()),
                x_ds_agent_id: Some(ds_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: ds_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        ds_telemetry::session_ctx::log_event(
            ds_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => ds_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => ds_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => ds_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => ds_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::MemoryFlushed {
        ...
        self.send_ds_notification(DsSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    /// LLM call using the `ds-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    ds_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                ds_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ds_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ds_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                ds_sampler::collect_response(events).await
            }
        };

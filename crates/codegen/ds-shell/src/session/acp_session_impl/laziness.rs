...
impl SessionActor {
    ...
    /// sees "DS is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `deepseek-v4-pro.5` (and
            // other tool-flavoured DS variants) reject the field at
            ...
            x_ds_conv_id: Some(session_id_str.clone()),
            x_ds_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_ds_session_id: Some(session_id_str),

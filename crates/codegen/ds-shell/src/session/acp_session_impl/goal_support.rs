...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: ds_tools::implementations::ds_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`DS_GOAL_PLANNER`) and a plan
...
/// Override with `DS_GOAL_REVERIFY_AFTER` (floored at 1).
...
mod fold_tokens_by_model_tests {
    ...
    fn mixed_models_sum_marginals_sorted_desc() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("deepseek-v4-flash")),
            rec(Some("g1"), 100, 500, Some("deepseek-v4-pro")), // marginal 400
            rec(Some("g1"), 0, 50, Some("deepseek-v4-flash")),    // ds-3 total 150
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(
            out,
            vec![("deepseek-v4-pro".to_owned(), 400), ("deepseek-v4-flash".to_owned(), 150)]
        );
    }
    ...
    fn single_distinct_model_collapses_to_one_entry() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("deepseek-v4-pro")),
            rec(Some("g1"), 0, 200, None), // folds under current = deepseek-v4-pro
        ];
        let out = fold_tokens_by_model(&records, "g1", "deepseek-v4-pro");
        assert_eq!(out, vec![("deepseek-v4-pro".to_owned(), 300)]);
    }
    ...
    fn other_goal_records_excluded() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("deepseek-v4-pro")),
            rec(Some("g2"), 0, 999, Some("deepseek-v4-pro")),
            rec(None, 0, 999, Some("deepseek-v4-pro")),
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(out, vec![("deepseek-v4-pro".to_owned(), 100)]);
    }

    #[test]
    fn last_below_anchor_does_not_underflow() {
        let records = vec![rec(Some("g1"), 500, 100, Some("deepseek-v4-pro"))];
    ...
    fn captured_model_survives_mid_goal_current_model_switch() {
        // A record captured `deepseek-v4-pro` at spawn keeps it even though the
        // current model at aggregation time is `ds-3`.
        let records = vec![rec(Some("g1"), 0, 100, Some("deepseek-v4-pro"))];
        let out = fold_tokens_by_model(&records, "g1", "deepseek-v4-flash");
        assert_eq!(out, vec![("deepseek-v4-pro".to_owned(), 100)]);
    }
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use ds_tools::implementations::ds_build::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                ds_tools::implementations::ds_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}

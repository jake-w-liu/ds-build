...
impl SessionActor {
    ...
    /// Emit `ds.cli/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            ds_workspace::session::git::get_worktree_info(cwd),
            ds_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("ds.cli/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<ds_tools::implementations::ds_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        if became_idle {
            ...
            self.agent
                ...
                .tool_bridge()
                .update_resource(
                    ds_tools::implementations::ds_build::task::types::CurrentPromptIdResource(
                        String::new(),
                    ),
                )
                .await;
        ...
        // replayed `_ds.cli/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_ds_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_ds_notification_with_extra_meta(
    ...
    pub(super) fn classify_install_error(
        err: &ds_agent::plugins::install_registry::InstallError,
    ) -> String {

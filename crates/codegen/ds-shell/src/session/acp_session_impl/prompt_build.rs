...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.ds/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.ds/`, `~/.claude/`,
/// `~/.ds/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<ds_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<ds_agent::prompt::user_message::RuleEntry>,
    Vec<ds_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.ds/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = ds_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for ds ordering), so a failed offload never
...
impl SessionActor {
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: ds_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use ds_agent::prompt::agents_md::read_agents_config_with_paths;
        use ds_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use ds_workspace::file_system::{git_status_short, jj_status};
        use ds_workspace::session::git::VcsKind;
    ...
    ) -> Vec<ds_agent::prompt::user_message::McpServerEntry> {
        use ds_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &ds_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );

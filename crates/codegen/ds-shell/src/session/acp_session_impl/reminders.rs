...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> ds_agent::ReminderPolicy {
    let mut policy = ds_agent::ReminderPolicy::default();
...
/// Wrapped in ds's `<system-reminder>` shape by [`SessionActor::push_system_reminder`].
...
pub(super) fn todo_gate_active(
    policy: &ds_agent::system_reminder::ReminderPolicy,
    audience: ds_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    ...
                    .await;
                let read_tool_name =
                    ds_tools::reminders::task_completion::resolve_read_tool_name(&bridge)
                        .await;
                let reminder = ds_tools::reminders::task_completion::format_between_turn_bash_completions(
                    &bash_completions,
        ...
        let reminder =
            ds_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) fn todo_gate_policy(
        &self,
    ) -> Option<ds_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(ds_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();

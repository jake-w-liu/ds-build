...
use crate::extensions::notification::SessionNotification as DsSessionNotification;
...
///
/// One-shot DeepSeek events (RetryState, ImageCompressed, HookExecution,
/// etc.) take the direct `send_ds_notification` path for per-event hooks and persistence.
#[derive(Debug, Clone)]
pub(crate) enum SessionNotification {
    Acp(Box<AcpSessionNotification>),
    Ds(Box<DsSessionNotification>),
}
...
impl From<DsSessionNotification> for SessionNotification {
    fn from(n: DsSessionNotification) -> Self {
        Self::Ds(Box::new(n))
    }
}
...
impl SessionNotification {
    ...
    pub(crate) fn expect_acp(&self) -> &AcpSessionNotification {
        match self {
            Self::Acp(n) => n,
            Self::Ds(_) => panic!("expected Acp notification, got Ds"),
        }
    }

...
/// Memory log target — matches `ds_telemetry::memory_log::TARGET`.
const LOG: &str = "ds_memory";
...
pub fn should_flush(
    ...
    let should = ds_token_estimation::exceeds_threshold_with_headroom(
        total_tokens,
        context_window,
...
pub fn is_duplicate(content: &str, db_path: &std::path::Path) -> bool {
    ...
    let conn = match ds_sqlite_journal::JournalMode::for_db_path(db_path).open_readonly(db_path) {
        Ok(c) => c,
        Err(_) => {

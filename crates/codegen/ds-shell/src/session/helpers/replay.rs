...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        if env.method != Some("_ds.cli/session/update") {
            continue;
        }
        ...
        >(raw_params.get())
            && let DsSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Ds(notification) => {
                match &notification.update {
                    DsSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    DsSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other DeepSeek notifications are informational — skip them.
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn make_rewind_marker(target: usize) -> SessionUpdate {
        SessionUpdate::Ds(Box::new(DsNotification {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Ds(Box::new(DsNotification {
            session_id: acp::SessionId::new("test"),
            update: DsSessionUpdate::CompactionCheckpoint(Box::new(CompactionCheckpointInfo {

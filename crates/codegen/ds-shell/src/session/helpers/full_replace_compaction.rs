//! ds-build's L5 wiring onto the shared full-replace engine
//! (`ds_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](ds_compaction::sample_full_replace_summary);
//! this module adapts ds-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](ds_compaction::FullReplaceError).
...
use ds_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use ds_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for ds-build's full-replace pass.
///
...
/// else the structured ds-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the ds-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured ds-build), ignoring the shared
...
/// Map ds-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces ds-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    ds_telemetry::session_ctx::log_event(CompactionRetryDegraded {

...
pub use ds_telemetry::enums::PrCreationSource;
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &ds_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: ds_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    ds_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("deepseek-v4-flash");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("deepseek-v4-flash".to_string()));
        assert_eq!(snapshot.models_used, vec!["deepseek-v4-flash".to_string()]);

        // Record additional model usage
        handle.record_model_usage("deepseek-v4-pro");
        handle.record_model_usage("deepseek-v4-flash"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"deepseek-v4-flash".to_string()));
        assert!(snapshot.models_used.contains(&"deepseek-v4-pro".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.record_model_usage("deepseek-v4-flash"); // existing model
        handle.record_model_usage("deepseek-v4-pro.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // ds-3 not duplicated, ds-5 added
        assert!(snapshot.models_used.contains(&"deepseek-v4-pro.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("deepseek-v4-flash");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("deepseek-v4-pro");
        ...
        assert!(restored.models_used.contains(&"deepseek-v4-flash".to_string()));
        assert!(restored.models_used.contains(&"deepseek-v4-pro".to_string()));
        ...
        // ITL stats must survive the restore (regression test for ds-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the ds-critique bug describes: the
        ...
        handle2.record_model_usage("deepseek-v4-flash"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // ds-3 not duplicated

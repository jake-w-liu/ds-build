// Re-export all types from the standalone ds-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use ds_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `ds-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `ds-sampler` crate can reference them without a circular
// dep on `ds-shell`.

// Tests for the types now live in ds-sampling-types crate.

use ds_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `ds-sampling-types` or
/// `ds-tools`) so neither of those crates needs to depend on

...
//!
//! The canonical error types now live in `ds_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a ds-shell dependency).

// Re-export everything from the standalone crate.
pub use ds_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal ds.com upgrades do not raise API
/// team limits; admins purchase credits or a higher spend-based tier.
/// See https://api-docs.deepseek.com/
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str = "You\u{2019}ve hit your team\u{2019}s API rate limit. Ask a team admin to purchase more credits for higher limits, or try again later. See https://api-docs.deepseek.com/";
...
/// for the selected method). Do **not** decide from `has_ds_api_key_env()` alone:
...
/// This stays in ds-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a DS subscription")
                    && crate::agent::auth_method::has_ds_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (DEEPSEEK_API_KEY). \
                         Your cached OAuth session is being used instead. \
                         To use your API key, run `ds logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                ds_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: ds_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != ds_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == ds_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn rate_limited_user_message_oauth_vs_api_key() {
        ...
        assert!(
            RATE_LIMITED_USER_MESSAGE_API_KEY
                .contains("https://api-docs.deepseek.com/")
        );
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        unsafe {
            ...
            std::env::remove_var("DS_CODE_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("DS_CODE_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("ds-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'ds-build' requires a DS subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                ...
                "should suggest ds logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),

...
//! 4. Smoke-validate session JSON under `$DS_HOME/sessions/` after exit.
//!
//! Requires a built `ds` binary (`DS_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p ds-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p ds-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use ds_agent::plugins::SharedPluginRegistryHandle;
use ds_agent::plugins::discovery::DiscoveryConfig;
use ds_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _ds_guard = EnvVarGuard::set("DS_HOME", &ds_home);
    ...
    let mut registry = InstallRegistry::empty(ds_home.join("installed-plugins"));
    ...
    // ds_home() caches the first DS_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        ds_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built ds binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/DS_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via ds_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _ds_guard = EnvVarGuard::set("DS_HOME", &ds_home);
    ...
    // Smoke: session storage under DS_HOME/sessions has JSON artifacts.

//! Integration test for `_ds.cli/session/update` notifications.
//!
//! This test verifies that:
//! 1. DeepSeek session notifications (e.g., diff_review) can be sent via ext_notification
...
use ds_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as DsSessionUpdate,
};
use ds_shell::session::info::Info as SessionInfo;
use ds_shell::session::persistence::default_model_id;
use ds_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that DeepSeek session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_ds_session_notification_storage_roundtrip() {
    ...
    let ds_notification = SessionNotification {
        session_id: session_id.clone(),
        update: DsSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Ds(Box::new(ds_notification.clone())),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.updates.len(),
        2,
        "Should have 2 updates (1 DeepSeek + 1 ACP)"
    );

    // Verify DeepSeek notification
    match &loaded.updates[0] {
        SessionUpdate::Ds(notification) => {
            assert_eq!(notification.session_id, session_id);
            match &notification.update {
                DsSessionUpdate::DiffReview { content } => {
...
/// must survive `updates.jsonl` and reload as a replayable `_ds.cli/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Ds(Box::new(ds_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and DeepSeek notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add DeepSeek notification with totalTokens
    let ds_notification = SessionNotification {
        session_id: session_id.clone(),
        update: DsSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_ds_session_notification_serialization() {

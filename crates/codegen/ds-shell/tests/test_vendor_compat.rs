...
//! `.ds`, `.cursor`, and `.claude` vendor dirs, spawns a real `ds agent
//! stdio` process against the mock inference server (toggling the
//! `DS_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the DS-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p ds-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_DS_SKILL: &str = "ZZ_DS_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".ds", "ds-skill", MARKER_DS_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = DsStdioClient::spawn_with_home_and_env(&server, workdir.path(), home, env).await;
...
/// Defaults (all vendors on): ds + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_DS_SKILL),
            "ds-skill must always be present"
        );
        assert!(
...
/// `DS_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; ds stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_DS_SKILL),
            "ds-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `DS_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; ds stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_DS_SKILL),
            "ds-skill always present"
        );
...
/// `DS_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `DS_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `DS_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `DS_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("DS_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("DS_CURSOR_SKILLS_ENABLED", "false"),
            ("DS_CURSOR_RULES_ENABLED", "false"),
            ("DS_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_DS_SKILL), "ds always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("DS_CLAUDE_SKILLS_ENABLED", "false"),
            ("DS_CLAUDE_RULES_ENABLED", "false"),
            ("DS_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only ds-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary

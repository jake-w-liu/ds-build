...
Launches ds agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/ds-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/ds-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/ds-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    DS_BINARY=/path/to/ds-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a ds agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated ds environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.ds/auth.json   (copied from real home)
      ~/.ds/memory/     (pre-populated by tests)
      ~/.ds/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="ds-memtest-")
        ...
        # Create .ds dirs in fake home
        self.ds_home = os.path.join(self.fake_home, ".ds")
        self.memory_dir = os.path.join(self.ds_home, "memory")
        self.logs_dir = os.path.join(self.ds_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.ds/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.ds_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.ds/config.toml (where the agent reads config)."""
        with open(os.path.join(self.ds_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("DS_BINARY", "ds")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}ds binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("DS_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"DS_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """DS_MEMORY=1 env var enables memory even without config."""
    section("13. DS_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"DS_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with DS_MEMORY=1")
        else:
            fail("no index despite DS_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.ds/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("DS_BINARY", "ds")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["DS_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["DS_NO_LEADER"] = "1"
...
def build_binary():
    """Build ds-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "ds-pager")

    # Find rg for DS_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["DS_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building ds-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "ds-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "DS_BINARY" in os.environ:
        binary = os.environ["DS_BINARY"]
        print(f"Binary: {binary} (from $DS_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["DS_BINARY"] = binary

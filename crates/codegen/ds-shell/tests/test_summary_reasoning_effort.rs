...
//!
//! Each test spawns a real `ds agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p ds-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.ds/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".ds").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let ds_dir = home.path().join(".ds");
        std::fs::create_dir_all(&ds_dir).expect("create .ds dir");
        std::fs::write(
...
"#,
        ...
        let client = DsStdioClient::spawn_with_home(&server, workdir.path(), home).await;

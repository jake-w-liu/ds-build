...
use ds_config::signed_policy::{self, SignedPayload};
...
async fn sync_fail_closed_policy(home: &std::path::Path, kp: &ring::signature::Ed25519KeyPair) {
    ...
    assert!(ds_shell::managed_config::managed_policy_gate().is_ok());
}
...
fn signed_dk_empty_body(kp: &ring::signature::Ed25519KeyPair, deployment_id: &str) -> String {
    let payload = SignedPayload {
        version: ds_cli_proxy_types::SIGNED_PAYLOAD_VERSION,
...
/// A signature-rejected sync surfaces as failure in BOTH `ds setup` and the
...
async fn rejected_signature_surfaces_as_setup_and_login_failure() {
    ...
    assert!(
        matches!(
            outcome,
            ds_shell::managed_config::SetupOutcome::Failed(
                ds_shell::managed_config::ManagedConfigError::SignatureRejected
            )
        ),
        "setup must surface the signature rejection, got {outcome:?}"
    );

    let login = ds_shell::managed_config::post_login_sync(None).await;
    assert_eq!(
        login,
...
async fn withdrawn_requirements_is_deleted_and_covered_by_the_new_sidecar() {
    ...
    assert!(
        !ds_shell::config::is_managed_config_hard_stale_for(&team_identity("team-007")),

...
//!
//! This test spawns a real `ds agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p ds-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // DsStdioClient sets HOME=<temp>; the binary uses <HOME>/.ds as DS_HOME.
        let ds_home = shared_home.path().join(".ds");
        let session_dir = locate_session_dir(&ds_home, session_id.0.as_ref());

...
//! - Session resume
//!
//! Each test spawns a real `ds agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p ds-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "ds-build")
/// - `model-b`: no agent_type (→ "ds-build")
...
/// `ds-build` harness. The system prompt sent to the LLM should contain
/// the ds-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_ds_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("DS") || sys_prompt.contains("ds"),
            "default model should use ds-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_ds =
            original_sys_prompt.contains("DS") || original_sys_prompt.contains("ds");
        let resumed_has_ds =
            resumed_sys_prompt.contains("DS") || resumed_sys_prompt.contains("ds");
        assert_eq!(
            original_has_ds,
            resumed_has_ds,
            "resumed session should use the same harness as the original\n\
             original identity markers: ds={original_has_ds}\n\
             resumed identity markers: ds={resumed_has_ds}\n\
...
/// default to `"ds-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_ds_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("DS") || sys_prompt.contains("ds"),
                "model without agent_type should default to ds-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `DS_AGENT` escape hatch should override the model's agent_type.
/// Setting `DS_AGENT=ds-build` with an alternate-agent model should use
/// ds-build harness.
#[tokio::test]
#[ignore]
async fn test_ds_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            ds_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("DS_AGENT", "ds-build");
            let mut child = cmd.spawn().expect("spawn ds");
            ...
            let incoming = ds_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("DS") || sys_prompt.contains("ds"),
                "DS_AGENT=ds-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;

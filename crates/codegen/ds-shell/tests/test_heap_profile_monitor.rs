...
use ds_shell::auth::{AuthManager, AuthMode, DsAuth, DsComConfig};
use ds_shell::heap_profile::{
...
use ds_test_support::{EnvGuard, MockInferenceServer};
use ds_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = DsComConfig::default().auth_scope();
    let auth = DsAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[ds_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), DsComConfig::default()));

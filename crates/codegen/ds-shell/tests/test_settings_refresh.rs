...
//! Run locally:
//! ```bash
//! cargo test -p ds-shell --test test_settings_refresh
//! ```
...
async fn test_fetch_settings_blocking_round_trip() {
    with_local_set(|| async {
        ...
        let result = tokio::task::spawn_blocking({
            ...
            let auth = auth.clone();
            move || ds_shell::remote::fetch_settings_blocking(&url, &auth, None)
        })
        .await
        .unwrap();
        assert!(
...
async fn test_doom_loop_recovery_settings_round_trip() {
    use ds_shell::util::config::DoomLoopRecoverySettings;

//! `paste-latency` — clipboard paste latency benchmark for `ds-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p ds-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p ds-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/ds-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for ds-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {

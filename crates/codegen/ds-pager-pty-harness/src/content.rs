...
//! [`MockInferenceServer`] from `ds-test-support` and provides the
...
pub use ds_test_support::mock_server::LogEntry;
pub use ds_test_support::mock_server::MockModelEntry as MockModel;
pub use ds_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use ds_test_support::sse;
pub use ds_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.ds
    ...
    ///
    /// Mirrors `ds_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit DS_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // DS_HOME is set in the test runner's env).
            ("DS_HOME".into(), ds_home),
            ("DS_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("DS_DEEPSEEK_API_BASE_URL".into(), self.url()),
            ("DEEPSEEK_API_KEY".into(), "test-key-for-ci".into()),
            ...
            ("DS_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    /// on the DS Pro upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("DS_HOME").as_deref(),
            content.home().join(".ds").to_str()
        );
        assert_eq!(get("DS_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("DS_DEEPSEEK_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("DEEPSEEK_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("DS_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("DS_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("DS_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("DS_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("DS_MAX_RETRIES").as_deref(), Some("0"));

...
/// Seed a fake DeepSeek OAuth entry into the isolated home's `auth.json` so the
/// shell has session auth (the harness's `DEEPSEEK_API_KEY` is ApiKey/BYOK mode
...
/// [`ContentController::env_for_pager`] minus `DEEPSEEK_API_KEY`, so the entry
...
pub fn oauth_env_for_pager(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.retain(|(k, _)| k != "DEEPSEEK_API_KEY");
    env
}

//! Unified PTY harness for ds-pager.
//!
...
//!    `ds-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `DS_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use ds_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through ds's stdout. Used to reproduce the doubled-line class of bugs
    /// where ds's diff renderer never re-asserts a region it didn't write
    ...
    pub fn wait_for_full_text(&mut self, text: &str, timeout: Duration) -> Result<()> {
        if self.screen.full_contains(text) {
            return Ok(());
        }
        let deadline = Instant::now() + timeout;
        loop {
            ...
            if remaining.is_zero() {
                anyhow::bail!(
                    "timed out after {timeout:?} waiting for text in scrollback+screen: \
                     {text:?}\nfull contents:\n{}",
                    self.screen.full_text()
                );
            }

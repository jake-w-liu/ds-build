//! `scroll-matrix` — scroll validation matrix sweep for ds-pager.
//!
...
use ds_pager_pty_harness::pager_binary;
use ds_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against ds-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_ds-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves DS_SCROLL_LOG against ITS cwd (the

//! Minimal (scrollback-native) render mode — `ds --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `ds_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `ds-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`ds_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`ds-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `ds-pager`.
///
...
/// subsequent calls are ignored (see [`ds_pager::minimal_hook`]).
pub fn install() {
    ds_pager::minimal_hook::install(ds_pager::minimal_hook::MinimalHooks { draw });
}

    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (ds_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    ds_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use ds_shell::tools::{TodoItem, TodoPriority};

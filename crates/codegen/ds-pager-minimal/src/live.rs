...
use ds_pager::render::Renderable;
use ds_pager::scrollback::state::ScrollbackState;
use ds_pager::scrollback::wrappers::EntryRenderer;
use ds_pager::theme::Theme;
use ds_pager::views::prompt_widget::PromptStyle;
use ds_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &ds_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(
    appearance: &ds_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    ds_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                ds_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(ds_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a ds_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &ds_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_watchers(agent: &ds_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == ds_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut ds_pager::app::agent_view::AgentView,
) -> Option<ds_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &ds_pager::app::agent_view::AgentView,
    activity: &Option<ds_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let hint = if ds_pager::app::minimal_auto_set_for_mouse_leak() {
...
fn render_prompt_info(
    ...
    use ds_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = ds_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<ds_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> ds_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use ds_pager::app::agent::AgentState;
        use ds_pager::scrollback::RenderBlock;
        use ds_pager::scrollback::entry::ScrollbackEntry;
        use ds_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &ds_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use ds_pager::acp::tracker::TurnActivity;
        use ds_pager::app::agent::AgentState;
        let theme = Theme::current();
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            ds_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(ds_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &ds_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use ds_pager::app::app_view::PendingAction;
        use ds_pager::input::key::KeyShortcut;

...
use ds_pager::app::app_view::{AuthState, TrustState};
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to DS", bold)),
            );
        ...
        MinimalAuthHint::TrustFolder { workspace } => {
            ...
            y = put_line(
                ...
                bottom,
                Line::from(Span::styled(
                    "DS may run or modify contents in this directory,",
                    gray,
                )),
            );
            y = put_line(
...
mod tests {
    ...
    fn device_user_code_parses_verification_url() {
        assert_eq!(
            device_user_code("https://accounts.ds.cli/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH")
        );
        assert_eq!(
            device_user_code("https://accounts.ds.cli/oauth2/device"),
            None
        );
    ...
    fn auth_hint_maps_auth_state() {
        use ds_pager::app::app_view::AuthMode;
        ...
        let st = AuthState::Authenticating {
            request_seq: 1,
            handle: None,
            auth_url: Some("https://accounts.ds.cli/device?user_code=ABCD-EFGH".into()),
        ...
        match minimal_auth_hint(&st, &trust_done, true, false) {
            MinimalAuthHint::SigningIn { url, code } => {
                assert_eq!(
                    url.as_deref(),
                    Some("https://accounts.ds.cli/device?user_code=ABCD-EFGH")
                );
    ...
    fn render_auth_shows_url_and_code() {
        ...
        let hint = MinimalAuthHint::SigningIn {
            url: Some("https://accounts.ds.cli/device?user_code=ABCD-EFGH".into()),
            code: Some("ABCD-EFGH".into()),
        };
        render_auth(&mut buf, area, &theme, &hint);
        let text = buffer_text(&buf, area);
        assert!(text.contains("Sign in to DS"), "header: {text:?}");
        assert!(text.contains("accounts.ds.cli/device"), "url: {text:?}");

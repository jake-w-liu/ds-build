//! Headless markdown analysis sharing DS Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `ds-markdown` so analysis matches what DS
//! Build actually renders 1:1.
//!
//! After parsing, DS applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set DS Build uses to render markdown.
///
...
/// Offset event stream from DS's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with DS Build's options; count elements and flag structural issues.

...
/// `~/.ds/config.toml` (STT URL pieces, language, sample rate, endpointing)
...
/// `DS_VOICE_MODE` override) — there is deliberately no local enable/disable
...
pub struct VoiceConfig {
    ...
    /// `x-ds-client-identifier` / `User-Agent` headers the sampler and imagine
    ...
    ///
    /// `x-ds-client-identifier` value (e.g. `"ds-shell"`).
    #[serde(skip)]
    pub client_identifier: String,
    /// `User-Agent` value (e.g. `"ds-shell/1.2.3 (macos; aarch64)"`).
    #[serde(skip)]
...
mod tests {
    ...
    fn default_stt_ws_uses_wss() {
        ...
        assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.deepseek.com/v1/stt");
    }

    #[test]
    fn scheme_less_api_base_uses_wss() {
        let cfg = VoiceConfig {
            api_base: "api.deepseek.com".into(),
            ..VoiceConfig::default()
        };
        assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.deepseek.com/v1/stt");
    }
    ...
    fn wss_api_base_is_not_doubled() {
        let cfg = VoiceConfig {
            api_base: "wss://api.deepseek.com".into(),
            ..VoiceConfig::default()
        };
        assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.deepseek.com/v1/stt");
    }
    ...
    /// voice gate — default on, remote kill switch / `DS_VOICE_MODE`).
    #[test]

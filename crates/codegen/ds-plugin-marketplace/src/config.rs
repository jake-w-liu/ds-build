//! Parse marketplace sources from `~/.ds/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "DeepSeek Official"
//! git = "https://github.com/deepseek-ai/ds-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// instead of `~/.ds`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "DeepSeek Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("deepseek-ai") && branch.as_deref() == Some("main"))
        );
    }

...
//!
//! Catalog file lookup, in order: `.ds-plugin/marketplace.json` (preferred),
//! `.ds-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.ds-plugin/marketplace.json` (preferred DeepSeek convention)
/// 2. `.ds-plugin/plugin.json`
...
mod tests {
    ...
    fn load_index_valid_ds_dir() {
        ...
        assert_eq!(result.unwrap().name, "ds");
    }

    #[test]
    fn load_index_ds_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".ds-plugin", "ds"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "ds");
    }

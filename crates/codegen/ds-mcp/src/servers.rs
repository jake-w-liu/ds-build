...
use ds_tools::util::ProcessGroup;

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `ds_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use ds_workspace_types::MCP_TOOL_NAME_DELIMITER;
...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"ds_com_linear"`).
...
/// SDK-side id echoed back in `ds.cli/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["ds.cli/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["ds.cli/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `ds.cli/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.ds/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: ds_file_utils::events::EventWriter,
    ...
    /// `ds.cli/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: ds_file_utils::events::EventWriter::noop(),
    ...
    /// `DsClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: ds_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &ds_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `ds-telemetry`; re-exported
...
impl McpError {
    ...
    pub fn error_category(&self) -> ds_file_utils::events::McpErrorCategory {
        use ds_file_utils::events::McpErrorCategory;
        match self {
...
///   the frontend via `ds.cli/mcp/tools_changed` notifications and callable via
///   `ds.cli/mcp/call`.
...
/// `ds_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl ds_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> ds_tool_protocol::ToolId {
        ...
        ds_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| ds_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        ds_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(ds_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(ds_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            ds_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(ds_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(ds_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        ds_telemetry::session_ctx::log_event(ds_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &ds_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, ds_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| ds_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(ds_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(ds_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(ds_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(ds_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(ds_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(ds_file_utils::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`ds.cli/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`DsClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, DsClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`DsClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `ds.cli/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`DsClientHandler`]** constructed by
...
/// and the [`DsClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to ds-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `ds.cli/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: ds_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, DsClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `ds.cli/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("ds-shell-{server_name}"),
                ds_version::VERSION.to_string(),
            ),
        )
    ...
    /// Build the [`DsClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> DsClientHandler {
        DsClientHandler {
    ...
    /// references (the [`DsClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            ds_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = ds_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`ds_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "ds_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("ds-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = ds_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = ds_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        ds_telemetry::session_ctx::log_event(
                            ds_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: ds_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    ds_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            ds_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = ds_telemetry::instrumentation::timer("mcp_start_servers");
...
/// `ds.cli/mcp/server_status` events.
///
...
impl ClientHandler for DsClientHandler {
    ...
    // See the [`DsClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            ds_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("ds_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("DS_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("ds_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("ds_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_ds_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use ds_computer_hub_sdk::LocalRegistry;
        use ds_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = ds_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = ds_file_utils::events::EventWriter::open(tmp.path());
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'ds_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            ds_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }

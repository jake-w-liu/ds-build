...
const MCP_OAUTH_CLIENT_NAME: &str = "DS";
...
// mode disabled, multiple `ds` invocations).
//
// Layer 1 (cross-process): filesystem lock at $DS_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    ds_config::ds_home().join(format!("mcp_auth_{safe}.lock"))
}

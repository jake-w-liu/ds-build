//! Memory-system configuration value types, extracted from ds-shell
...
//! `ds-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.ds/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.ds/memory/` are scanned:

...
use ds_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-ds-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `ds_build_settings`
    ...
    /// OAuth2 provider issuer URL (e.g., "https://api.deepseek.com"). When present
    ...
    /// default (on). Sits below env `DS_FOLDER_TRUST`, user
    ...
    /// remote settings `ds_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `ds_build_settings` remote settings key) to enable it. See
    ...
    /// `ds_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `DS_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `ds_build_settings`).
    ...
    /// remote settings gate for first-run auto-registration of the official DeepSeek
    ...
    /// ghost text), from the `ds_build_settings` remote settings flag. Sits below
    /// env (`DS_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `ds-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `ds-imagine-image`) instead of the default quality model
    /// (`ds-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `DS_VOICE_MODE`. Free-tier DS Pro upsell
    ...
    /// User's subscription tier from remote settings `ds_build_access_gate`.
    /// E.g. "free", "premium", "ds_pro", "ds_heavy".
    ...
    /// Whether the user is allowed to use DS Build. Set by remote settings
    /// `ds_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "DS Pro", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `ds_build_usage_redirect_url`
    ...
    /// `ds_build_settings`. Per-model override on `ModelInfo`
    /// (`ds_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`DS_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`DS_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`DS_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`DS_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `ds workspace` CLI command (Computer Hub
    /// workspace exposure), from `ds_build_settings.workspace_command_enabled`.
...
/// only guaranteed to work with a compatible harness (cursor vs ds-build).
...
pub struct GoalRoleModel {
    ...
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "ds-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-ds-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "ds-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("ds-build"));
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "ds-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("ds-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "deepseek-v4-pro", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "deepseek-v4-pro.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "deepseek-v4-pro", "agent_type": "general-purpose"},
            {"model": "deepseek-v4-flash", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "deepseek-v4-pro", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "deepseek-v4-pro.5", "agent_type": "cursor"},
    ...
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "deepseek-v4-pro", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {

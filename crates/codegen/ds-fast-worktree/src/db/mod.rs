...
use ds_sqlite_journal::JournalMode;
...
impl WorktreeDb {
    /// Open (or create) the DB at `ds_home/worktrees.db`.
    pub fn open(ds_home: &Path) -> Result<Self> {
        Self::open_at(&ds_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.ds/worktrees.db`.
    ///
    /// Discovers ds home via `$DS_HOME`, falling back to the canonicalized
    /// `$HOME/.ds` (matching `ds_config::ds_home`).
...
pub fn resolve_ds_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("DS_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $DS_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .ds
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with ds_config::default_ds_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".ds"))
}

/// Serializes tests that mutate the process-global `DS_HOME` env var so they
...
#[cfg(test)]
static DS_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`DS_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `DS_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `DS_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct DsHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated ds home; pass to `WorktreeDb::open` to read the same DB
...
impl DsHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = DS_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("ds-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via DS_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until DS_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("DS_HOME");
        unsafe { std::env::set_var("DS_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for DsHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("DS_HOME", p),
                None => std::env::remove_var("DS_HOME"),
            }
        }
    }
}

...
//! after every tool call to check for SKILL.md files in `.ds/skills/`,
...
/// Directories that contain skill definitions (`.ds/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".ds", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.ds/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.ds/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.ds/) are outside the git root so the walk breaks early.

...
const BASH_STATE_START_MARKER: &str = "__DS_BASH_STATE_START__";
const BASH_STATE_END_MARKER: &str = "__DS_BASH_STATE_END__";
const ZSH_STATE_START_MARKER: &str = "__DS_ZSH_STATE_START__";
const ZSH_STATE_END_MARKER: &str = "__DS_ZSH_STATE_END__";

/// Marker emitted by the init path to separate login-shell noise (MOTD, etc.)
/// from the actual state dump on stdout.
const INIT_STATE_MARKER: &str = "__DS_INIT_STATE_MARKER__";
...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        // Re-applied last via [`crate::util::apply_ds_agent_marker`] so request
        // env cannot clear it.
        (
            crate::util::DS_AGENT_ENV.to_string(),
            crate::util::DS_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  _emit_encoded() {
    ...
    local var_name="$2"
    if [[ -n "$content" ]]; then
      builtin printf 'ds_snap_%s=$(command base64 -d <<'"'"'DS_SNAP_EOF_%s'"'"'\n' "$var_name" "$var_name"
      command base64 <<<"$content" | command tr -d '\n'
      builtin printf '\nDS_SNAP_EOF_%s\n' "$var_name"
      builtin printf ')\n'
      builtin printf 'eval "$ds_snap_%s"\n' "$var_name"
    fi
  }

  _emit "__DS_BASH_STATE_START__"
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match ds_config::shell::detect_unix_shell_kind() {
            ds_config::shell::UnixShellKind::Bash => Self::Bash,
            ds_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `ds_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => ds_config::shell::UnixShellKind::Bash,
            Self::Zsh => ds_config::shell::UnixShellKind::Zsh,
        };
        ds_config::shell::unix_shell_path(kind)
    }
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // Re-export DS_AGENT=1 after snapshot eval so agent-definition
...
mod tests {
    ...
    fn shell_env_overrides_marks_agent_terminal() {
        ...
        assert_eq!(
            env.get(crate::util::DS_AGENT_ENV).map(String::as_str),
            Some(crate::util::DS_AGENT_ENV_VALUE)
        );
    ...
    fn parse_dump_valid_bash() {
        let raw = "__DS_BASH_STATE_START__\n\
                    /home/user/project\n\
                    export FOO=bar\n\
                    # end of bash state dump\n\
                    __DS_BASH_STATE_END__\n";
    ...
    fn parse_dump_valid_zsh() {
        let raw = "__DS_ZSH_STATE_START__\n\
                    /tmp\n\
                    typeset -x FOO=bar\n\
                    # end of zsh state dump\n\
                    __DS_ZSH_STATE_END__\n";
    ...
    fn parse_after_marker_found() {
        let output = "Welcome to Ubuntu\nMOTD line\n__DS_INIT_STATE_MARKER__\nactual data\n";
        let result = parse_after_marker(output, "__DS_INIT_STATE_MARKER__");
    ...
    fn shell_state_update_from_dump() {
        ...
        let dump = "__DS_BASH_STATE_START__\n\
                     /new/dir\n\
                     export X=1\n\
                     # end of bash state dump\n\
                     __DS_BASH_STATE_END__\n";
    ...
    async fn test_init_bash() {
        ...
        assert!(
            state.snapshot.contains("ds_snap_") || state.snapshot.is_empty(),
    ...
    async fn test_prepare_command_and_roundtrip() {
        ...
        // Run "export DS_TEST_VAR=hello" and capture the new state
        let prep = state
            .prepare_command(
                "export DS_TEST_VAR=hello",
                None,
    ...
    async fn test_gpg_tty_excluded_from_snapshot_bash() {
        ...
        let (code, _) = run_command(&mut state, "export GPG_TTY=/ds-sentinel-tty").await;
        assert_eq!(code, 0);

        let (code, stdout) = run_command(&mut state, "echo \"[$GPG_TTY]\"").await;
        assert_eq!(code, 0);
        assert!(
            !stdout.contains("/ds-sentinel-tty"),

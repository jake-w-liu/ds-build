...
fn output_file_cap_from_env() -> u64 {
    std::env::var("DS_MAX_OUTPUT_FILE_BYTES")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
...
impl LocalTerminalActor {
    ...
    async fn spawn_persistent_command(
        ...
        #[cfg(target_os = "linux")]
        if ds_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| ds_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::ds_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (ds) process retains network for the LLM API.
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = ds_config::shell::shell_command_argv(command);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when ds-agent
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export DS_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $DS_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/ds-sentinel-tty".to_string());

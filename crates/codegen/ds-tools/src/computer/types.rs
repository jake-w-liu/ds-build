...
///
/// Serialized over the wire in the `ds.cli/task/kill` ext response
/// (`ds-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (ds-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in ds-tools, spawns processes)
/// - `AcpTerminalBackend` (in ds-shell, calls ACP protocol)
#[async_trait::async_trait]

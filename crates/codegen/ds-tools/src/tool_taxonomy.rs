...
//! `ToolInput`, proto, or runtime). A future `ds-tool-taxonomy` leaf crate
...
/// object (mirroring `ds.cli/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "ds.cli/tool";
...
impl ToolKind {
    ...
    /// `ds.cli/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   should read `namespace` loosely (as a string) and, on any `ds.cli/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `ds.cli/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::DsBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::DsBuild => ("ds_build", "DsBuild"),
                ToolNamespace::DsBuildConcise => ("ds_build_concise", "DsBuildConcise"),
                ToolNamespace::DsBuildHashline => ("ds_build_hashline", "DsBuildHashline"),
    ...
    fn kind_schema_is_open_string_namespace_stays_closed() {
        ...
        // The schema shape may be a flat `enum` or a `oneOf` of const/enum
        // branches depending on schemars version and per-variant attributes.
        // The invariant is that the namespace is a closed set — no `other`
        // sink or open-string fallback.
        assert!(
            ns.get("enum").is_some() || ns.get("oneOf").is_some(),
            "namespace must be a closed set (enum or oneOf), got {ns}"
        );
        assert!(
            ns.get("type").map_or(true, |t| t != "string"),
            "namespace must not be an open string"
        );
    }
    #[test]
    ...
    fn tool_meta_schema_is_up_to_date() {
        ...
        // Filter out any legacy "cursor" value from the namespace enum.
        // Use `get_mut` instead of `IndexMut` to avoid auto-vivifying
        // a missing `"enum"` key as `null`.
        if let Some(ns_obj) = expected["definitions"]["ToolNamespace"].as_object_mut() {
            if let Some(values) = ns_obj.get_mut("enum").and_then(|v| v.as_array_mut()) {

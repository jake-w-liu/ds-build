//! ToolBridge: adapter that wraps `ds-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the ds_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, ds_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, ds_tool_runtime::ToolError> {
        use crate::implementations::ds_build::scheduler::types::{
        ...
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    ds_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                ds_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx.await.map_err(|_| {
            ds_tool_runtime::ToolError::custom("process_manager", "Scheduler actor dropped reply")
        })
    }
...
mod tests {
    ...
    impl ds_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> ds_tool_protocol::ToolId {
            ds_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::ds_tool_runtime::ListToolsContext,
        ) -> ds_tool_types::ToolDescription {
            ds_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + ds_build's snake_case in one registry

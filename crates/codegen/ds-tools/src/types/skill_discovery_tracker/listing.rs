...
///
/// `"/path/.ds/skills/my-skill/SKILL.md"` -> `"/path/.ds/skills/"`
...
/// Used by the budgeted (ds build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// DS build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the ds
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // ds-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: ds build harness (budgeted XML) ───────────

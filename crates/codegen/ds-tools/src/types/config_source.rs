...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.ds/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.ds/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.ds/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.ds/ or ~/.claude/.
    User { path: PathBuf },

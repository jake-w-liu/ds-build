...
//!
//! `ToolSearchIndex` is defined in `ds-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `ds-shell`
...
/// in `ds-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `ds-shell` after MCP initialization.
#[derive(Clone)]

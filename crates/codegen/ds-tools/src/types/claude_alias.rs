//! Canonical external-settings tool name ↔ DS tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`ds-hooks`) needs the
//! DS tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`ds-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `ds` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to DS, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// DS [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// DS tool names this Claude tool maps to (empty when there is no direct
    /// DS tool — the entry then only contributes a `kind`).
    ds: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, ds: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        ds,
...
const fn match_only(claude: &'static str, ds: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        ds,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (ds-build reads SKILL.md)
...
/// The DS [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The DS tool names a Claude matcher term fires on.
pub fn ds_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.ds)
...
/// The Claude names that map to `ds_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(ds_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.ds.contains(&ds_name))
        .map(|t| t.claude)
}

/// Every distinct DS name the table references, for the `ds-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn ds_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.ds.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a DS name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.ds.is_empty(),
                "dead row: {}",
                t.claude
            );
        }

...
///
/// Use with `#[schemars(with = "DsIntegerSchema")]` on `Option<usize>` fields.
pub struct DsIntegerSchema;
impl schemars::JsonSchema for DsIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "ds_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `ds-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use ds_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;

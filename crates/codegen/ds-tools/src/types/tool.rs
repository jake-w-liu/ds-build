...
//!
//! The tool runtime contract (`Tool` trait) lives in `ds_tool_runtime`.
...
///
/// Serializes to snake_case (`ds_build`, `mcp`, …) for the
...
/// (e.g. `"DsBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    /// Primary DS Build toolset (serde: `ds_build`).
    #[serde(alias = "DsBuild")]
    DsBuild,
    #[serde(alias = "DsBuildConcise")]
    DsBuildConcise,
    #[serde(alias = "DsBuildHashline")]
    DsBuildHashline,
    #[serde(alias = "Codex")]
    Codex,

...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"DsBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"DsBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `DsBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("DsBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("DsBuild:read_file", "legacy-0.4.10"),
            ("DsBuild:search_replace", "legacy-0.4.10"),
            ("DsBuild:get_task_output", "legacy-0.4.10"),
            ("DsBuild:kill_task", "legacy-0.4.10"),
            ("DsBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("DsBuild:run_terminal_cmd", "current"),
            ("DsBuild:read_file", "current"),
            ("DsBuild:search_replace", "current"),
            ("DsBuild:list_dir", "current"),
            ("DsBuild:grep", "current"),
            ("DsBuild:kill_task", "current"),
            ("DsBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "DsBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "DsBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "DsBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "DsBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "DsBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "DsBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "DsBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // DsBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("DsBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "DsBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("DsBuild:run_terminal_cmd"));
        assert!(is_version_managed("DsBuild:read_file"));
        assert!(is_version_managed("DsBuild:search_replace"));
        assert!(is_version_managed("DsBuild:list_dir"));
        assert!(is_version_managed("DsBuild:grep"));
        assert!(is_version_managed("DsBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("DsBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "DsBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "DsBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("DsBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("DsBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "DsBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "DsBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),

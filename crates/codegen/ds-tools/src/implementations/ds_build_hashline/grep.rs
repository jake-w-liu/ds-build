...
use crate::implementations::ds_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl ds_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = ds_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(ds_tool_runtime::Cwd(cwd));
        let mut result = ds_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                ds_tool_runtime::ToolError::execution(
                    ds_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(ds_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");

//! LSP server configuration from `.ds/lsp.json`.
...
/// This is the canonical merge function — both session startup and `ds inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::ds_home::ds_home().join("lsp.json");
    let project_path = cwd.join(".ds").join("lsp.json");
...
/// Load LSP server configs from `~/.ds/lsp.json` and `<cwd>/.ds/lsp.json`.

...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.ds/skills/ or .ds/skills/)
${%- endif %}
</available_skills>"#;
...
impl ds_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, ds_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(ds_tool_runtime::Tool::id(&tool).as_str(), "skill");

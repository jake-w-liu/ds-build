...
//! absolute paths. See the plan document for the full diff vs the ds-build
//! `ListDirTool`.
...
impl ds_tool_runtime::Tool for CodexListDirTool {
    type Args = CodexListDirInput;
    type Output = ListDirOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    ...
    ) -> Result<ListDirOutput, ds_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = ds_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = ds_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {

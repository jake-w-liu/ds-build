...
//! (in `ds-tools`) and the subagent coordinator (in `ds-shell`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"ds-build-plan"`) whose `AgentDefinition` decides the child's harness
...
pub use ds_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `ds-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "DsBuild:run_terminal_cmd" | "DsBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `ds-shell` relies on
...
register_resource!("ds_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-ds-build
...
pub type MonitorEventBuffer = ds_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("ds_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// ds-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in ds-chat-state (via `RunningSubagentSummary`)
...
register_resource!("ds_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("ds_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("ds_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by ds-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by ds-shell at turn start.
...
register_resource!("ds_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("DsBuild:run_terminal_cmd", ToolKind::Execute),
                tc("DsBuild:read_file", ToolKind::Read),
                tc("DsBuild:list_dir", ToolKind::List),
                tc("DsBuild:grep", ToolKind::Search),
                tc("DsBuild:kill_task", ToolKind::KillTaskAction),
                tc("DsBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("DsBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(

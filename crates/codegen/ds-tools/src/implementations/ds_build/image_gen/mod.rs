//! `image_gen` tool — generates images via the DeepSeek Imagine API and saves
...
// Some Imagine models (e.g. `ds-imagine-image`, selectable via `model_override`)
...
pub struct ImageGenClient {
    ...
    /// [`DS_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| DS_IMAGINE_MODEL.to_owned());
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`DS_IMAGINE_MODEL`]). Driven by the remote
...
impl ImageGenConfig {
    ...
    /// default quality model ([`DS_IMAGINE_MODEL`]) is used.
...
impl ds_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ds_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("ds-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("ds-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("ds-imagine-image")), None)
                .unwrap()
                .model,
            "ds-imagine-image"
        );

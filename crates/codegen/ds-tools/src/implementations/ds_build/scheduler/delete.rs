...
impl ds_tool_runtime::ToolOutput for SchedulerDeleteOutput {}
...
Returns success: true if the task was found and removed, false if no task with that ID exists."#
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        Expr::Value(ToolRequirement::Tool {
            ...
            id: ds_tool_runtime::Tool::id(&SchedulerCreateTool).to_string(),
            if_params: None,
        })
    }
}

impl ds_tool_runtime::Tool for SchedulerDeleteTool {
    type Args = SchedulerDeleteInput;
    type Output = SchedulerDeleteOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("scheduler_delete").expect("valid tool id")
    }
    ...
    ) -> Result<SchedulerDeleteOutput, ds_tool_runtime::ToolError> {

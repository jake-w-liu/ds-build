...
use regex::Regex;
use ds_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<ds_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| ds_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(ds_tool_protocol::ToolScope::Write),
        streaming: Some(ds_tool_protocol::StreamingSpec {
...
pub struct BashParams {
    ...
    /// ds-build opts up to 10h via ds-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "ds_build.Bash";
...
impl ds_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<ds_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => ds_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production ds-build sets it to 10h in ds-shell's
...
/// [`ds_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `DS_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("DS_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in ds_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the ds_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`DS_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `DS_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = ds_config::shell::chain_separator();
...
impl ds_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: ds_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> ds_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield ds_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield ds_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield ds_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield ds_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, ds_tool_runtime::ToolError> {
        ...
        let ampersand = ds_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(ds_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background && !background_enabled {
            return Err(ds_tool_runtime::ToolError::invalid_arguments(
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing ds_build callers that never opted in are
                // unaffected.
...
mod tests {
    ...
    fn read_chunk_progress(p: &ds_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            ds_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let ds_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = ds_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, ds_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                ds_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = ds_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = ds_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            ds_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = ds_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(ds_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // ds-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        ) -> ds_tool_runtime::ToolCallContext {
            let mut ctx =
                ds_tool_runtime::ToolCallContext::new(ds_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(ds_tool_runtime::BehaviorVersion(

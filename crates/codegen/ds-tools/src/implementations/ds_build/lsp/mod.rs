...
//! `LspTool` (Tool trait impl) under the `DsBuild` namespace.
...
impl ds_tool_runtime::ToolOutput for LspToolOutput {}
...
impl ds_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, ds_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    ds_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.ds/lsp.json or <cwd>/.ds/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }

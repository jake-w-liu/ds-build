//! `image_edit` tool — edits or transforms images via the DeepSeek Imagine
...
use crate::implementations::ds_build::image_gen::{ImageGenClient, ImageGenResponse};
...
const DS_IMAGINE_MODEL: &str = "ds-imagine-image-quality";
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), ds_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            ds_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
async fn resolve_to_data_url(value: &str) -> Result<String, ds_tool_runtime::ToolError> {
...
impl ds_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ds_tool_runtime::Tool::id(&tool).as_str(), "image_edit");

...
register_resource!("ds_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, ds_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(ds_tool_runtime::ToolError::execution(
                        ds_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl ds_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = ds_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = ds_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),

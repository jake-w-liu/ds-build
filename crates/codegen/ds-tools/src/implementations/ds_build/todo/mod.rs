...
crate::register_resource!("ds_build", "Todo", TodoState);
...
impl ds_tool_runtime::Tool for TodoWriteTool {
    type Args = TodoWriteInput;
    type Output = TodoWriteOutput;

    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
    ...
    ) -> Result<TodoWriteOutput, ds_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(ds_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn replace_mode_creates_items() {
        ...
        let output = expect_success(
            ds_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
                .await
                .unwrap(),
        );
    ...
    async fn replace_clears_previous_state() {
        ...
        ds_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input1)
            .await
            .unwrap();
        ...
        let output = expect_success(
            ds_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input2)
                .await
                .unwrap(),
        );
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("ds_build.Todo").is_some(),
            "TodoState should serialize under 'ds_build.Todo'"
        );

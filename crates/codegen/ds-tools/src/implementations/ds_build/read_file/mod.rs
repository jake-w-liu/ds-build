...
mod versions;
use crate::types::schema::DsIntegerSchema;
...
crate::register_resource!("ds_build", "ReadFile", ReadFileParams);
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<ds_tool_protocol::ToolCapabilities> =
...
async fn handle_pptx(
    file_bytes: Vec<u8>,
    path: &std::path::Path,
) -> Result<ReadFileOutput, ds_tool_runtime::ToolError> {
...
/// an empty window (harness-aligned; not a DS-line clamp).
...
impl ds_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> ds_tool_protocol::ToolId {
        ds_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> ds_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield ds_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
        Box::pin(async_stream::stream! {
            ...
            ds_tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            ds_tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            ds_tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
            ds_tool_runtime::ToolStreamItem::Terminal(Err(e)), }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &ds_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), ds_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = ds_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = ds_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
    ...
    async fn current_read_file_not_found_returns_structured_not_found() {
        ...
        let result = ds_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap();
        match result {
            ReadFileOutput::FileNotFound(msg) => {
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ds/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ds/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        ds_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = ds_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// phantom field only (start past any `split_inclusive` line), so DS

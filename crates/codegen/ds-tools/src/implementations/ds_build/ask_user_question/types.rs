...
//!
//! - **`ds-tools`** — tool blocks on a oneshot, formats the result.
//! - **`ds-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`ds-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `ds-tools`.
...
register_resource!("ds_build", "UserQuestionSender", UserQuestionSender);

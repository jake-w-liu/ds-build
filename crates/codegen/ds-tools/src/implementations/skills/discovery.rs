...
///
/// `skills` is the standard layout (`.ds/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into DS Build sessions.
...
/// name (e.g. `~/.ds/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into DS Build.
...
/// `cwd` (exclusive). At each directory, checks for `.ds/skills/`,
...
/// visits known config roots (`.ds`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.ds` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".ds", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.ds is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.ds/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.ds/ → kept (user content).
        let ds_shell = tmp.path().join(".ds").join("skills").join("shell");
        std::fs::create_dir_all(&ds_shell).unwrap();
        std::fs::write(
            ds_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (ds_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.ds/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .ds skill in an intermediate dir.
        ...
        .unwrap();
        let ds_skill = sub.join(".ds").join("skills").join("ds-dyn");
        std::fs::create_dir_all(&ds_skill).unwrap();
        std::fs::write(ds_skill.join("SKILL.md"), "---\nname: ds-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"ds-dyn"),
            "ds-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only ds-dyn discovered.
        ...
        assert!(
            names_off.contains(&"ds-dyn"),
            "ds-dyn missing: {names_off:?}"
        );
        assert!(

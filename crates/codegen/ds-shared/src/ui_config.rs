...
use ds_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `DS_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A DS STT
    /// catalog code (`en`, `es`, `ja`, … — see DeepSeek STT supported languages) or
    ...
    /// `ds` reopens in whatever mode was last explicitly chosen. Unset keeps
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: ds_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,

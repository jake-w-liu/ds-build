...
pub enum HostedTool {
    ...
    /// This is DeepSeek-specific — not part of the OpenAI Responses API, so it's
...
pub struct ConversationRequest {
    ...
    /// Custom headers for DeepSeek tracking
    pub x_ds_conv_id: Option<String>,
    pub x_ds_req_id: Option<String>,
    pub x_ds_session_id: Option<String>,
    pub x_ds_turn_idx: Option<String>,
    pub x_ds_agent_id: Option<String>,
    pub x_ds_deployment_id: Option<String>,
    pub x_ds_user_id: Option<String>,
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-ds-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the DS Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/ds-compaction` operate over ds-build's
...
impl ds_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> ds_compaction::CompactionRole {
        use ds_compaction::CompactionRole;
        // ds-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // ds-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing ds-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<ds_compaction::CompactionFileRef> {
        // ds-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `DsTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<ds_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl ds_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/ds-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`ds-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/ds-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_ds_turn_idx: req.x_ds_turn_idx,
            x_ds_agent_id: req.x_ds_agent_id,
            x_ds_deployment_id: req.x_ds_deployment_id,
            x_ds_user_id: req.x_ds_user_id,
            trace: None,
        }
    }
}
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch is DeepSeek-specific — not in async_openai's rs::Tool enum.
...
/// Return raw JSON tool definitions for DeepSeek-specific hosted tools that
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_ds_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_ds_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The ds-shell agent does NOT use this path — a
...
/// ([crates/codegen/ds-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use ds_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // ds-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "deepseek-v4-flash");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("deepseek-v4-flash".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("deepseek-v4-flash".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("deepseek-v4-flash".to_string()));
        assert_eq!(
    ...
    fn test_response_reasoning_effort_stamped_on_assistant() {
        ...
        assert!(json.contains(r#""reasoning_effort":"max""#), "{json}");
        let back: ConversationItem = serde_json::from_str(&json).unwrap();
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("deepseek-v4-flash".to_string()));
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in ds-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.ds/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.ds/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.ds/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("deepseek-v4-flash".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_ds_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_ds_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        assert_eq!(item.model_id, Some("deepseek-v4-flash".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("deepseek-v4-flash");
        ...
        let user = ConversationItem::user("Hi").with_model_id("deepseek-v4-flash");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("deepseek-v4-pro.5"));
    ...
    //      (earlier ds-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_ds_build_shape() {

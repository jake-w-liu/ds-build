//! Pure data types for the DeepSeek sampling / chat-completion API layer.
//!
...
//! DeepSeek agent stack.  It intentionally contains **no I/O** (no HTTP clients,
//! no file system access) so it can be depended on by downstream crates
//! (e.g., `ds-chat-state`) without pulling in the full `ds-shell`.

...
// Re-export ToolDefinition and FunctionTool from ds-tools.
...
pub use ds_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// DeepSeek extension: request price in USD ticks (1 USD = 1e10 ticks).
...
/// custom header fields for DeepSeek request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// DeepSeek-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for DeepSeek
...
mod tests {
    ...
    fn reasoning_effort_option_deserializes_bare_string() {
        let opt: ReasoningEffortOption = serde_json::from_value(json!("max")).unwrap();
        assert_eq!(
            opt,
            ReasoningEffortOption {
                id: "max".to_string(),
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "ds" }).as_object().unwrap()))
                .is_none()
        );
    }

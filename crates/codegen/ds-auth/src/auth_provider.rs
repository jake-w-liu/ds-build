...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`ds-desktop`, `ds login`). The `token` field MUST mirror
    ...
    /// Whether `X-DS-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `DsAuthCredentials::apply()` for the wire format contract.

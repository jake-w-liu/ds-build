...
//!
//! This crate defines the wire format for `ds.cli/hooks/*` and `ds.cli/plugins/*`
//! ACP extension methods. It is dependency-free (only `serde`) so both
//! `ds-shell` and `ds-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `ds-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `ds-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// DS's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `ds-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.ds/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}

/// Response for `ds.cli/hooks/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `ds.cli/plugins/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `ds.cli/mcp/list` as consumed by the pager.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `ds.cli/hooks/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `ds.cli/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Shared action response for both `ds.cli/hooks/action` and `ds.cli/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.ds/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.ds/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserDs),
            conflict: None,
        };
...
// ---------------------------------------------------------------------------
// Marketplace types (wire format for ds.cli/marketplace/* ACP endpoints)
// ---------------------------------------------------------------------------

/// Response for `ds.cli/marketplace/list`.
...
/// Request wrapper for `ds.cli/marketplace/action`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]

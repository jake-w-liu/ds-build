...
//!
//! ```
//! use ds_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `DsDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `DsNight`, `TokyoNight`).
    Dark,
}

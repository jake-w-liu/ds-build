...
use ds_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use ds_pager::client_identity::PAGER_CLIENT_VERSION;
use ds_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use ds_shell::agent::config::Config as AgentConfig;
use ds_shell::leader::{
...
use ds_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.ds_ws_origin {
        config.ds_com_config.ds_ws_origin = v.clone();
    }
    if let Some(v) = &args.ds_ws_url {
        config.ds_com_config.ds_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &ds_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.ds_api_base_url {
        config.endpoints.ds_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   DS agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `ds -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use ds_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(ds_telemetry::sampling_log::layer())
        .with(ds_telemetry::instrumentation::layer())
        .with(ds_telemetry::hooks_log::layer())
        .with(ds_telemetry::otel_layer::build_otel_layer(
    ...
    ds_telemetry::debug_log::install_firehose(registry, app_entrypoint);
...
/// `ds setup`: rendering + exit codes only; fetch logic lives in `ds_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use ds_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `ds login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export DS_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:DS_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  ds setup");
        eprintln!();
        eprintln!("Or add the key to ~/.ds/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's DS administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at platform.deepseek.com."
                    );
                }
            }
            Err(e) => {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(ds_shell::env::DsBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, ds_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = ds_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = ds_shell::leader::LeaderClient::connect(
...
/// Env override for the `ds workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "DS_WORKSPACE_COMMAND";
/// Resolution of the `ds workspace` gate. `Unknown` is kept separate from
...
/// The `DS_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&ds_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for ds on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<ds_shell::util::config::RemoteSettings> {
    join_early_prefetch(ds_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`ds workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `ds workspace`. Check your \
             network connection (run `ds login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.ds_com_config.ds_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a ds session, or run `ds workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = ds_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use ds_shell::auth::ensure_authenticated;
    ds_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = ds_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`ds workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.ds/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.ds_com_config,
        false,
        Some("No cached credentials found. Run `ds login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.ds_com_config);
...
struct StdioReplayState {
    ...
    /// `ds.cli/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "ds.cli/session/close" | "_ds.cli/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "ds.cli/leader-replay/session-load";
...
/// `ds.cli/leader_reconnected` with empty params in that case, signalling the
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    ds_telemetry::sentry::flush_on_shutdown();
    ds_telemetry::otel_layer::shutdown_otel();
    ds_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "ds: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<ds_pager::app::AgentArgs>,
    ...
    ds_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => ds_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = ds_shell::agent::models::start_early_prefetch(None);
    ds_shell::agent::mvp_agent::warm_async_http_client();
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(ds_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = ds_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = ds_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&ds_shell::agent::config::RuntimeResolutionContext {
    ...
    if use_leader {
        ...
        let env_urls = ds_shell::leader::LeaderEnvUrls::from(&agent_config.ds_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = ds_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"ds.cli/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = ds_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            ds_shell::agent::run_agent_server(server_config, agent_config).await
        }
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = ds_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `DS_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `ds dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `ds dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `DS_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !ds_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.ds/config.toml and \
             unsetting DS_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("DS_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<ds_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(ds_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<ds_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(ds_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    ds_shell::heap_profile::install(ds_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    ds_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    ds_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        ds_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        ds_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = ds_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    ds_pager::memory_trace::start(
        ds_shell::util::ds_home::ds_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = ds_config::validate_requirements() {
        eprintln!("Couldn't start DS: {e}");
        eprintln!();
        eprintln!(
            "Update DS to a version the policy allows, or ask your administrator \
    ...
    ds_crash_handler::install_terminal_restore_only();
    if ds_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = ds_shell::util::ds_home::ds_home().join("crash");
        if let Some(report) = ds_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("DS crashed during your last session.");
    ...
    let crashed = ds_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    ds_telemetry::debug_log::flush();
    if let Err(e) = result {
        ds_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("DS_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("DS_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(ds_shell::agent::chat_modes::DS_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(ds_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("DS_DEBUG_LOG", path);
            std::env::remove_var("DS_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("DS_DEBUG_LOG", "1");
        set_if_unset("DS_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        ds_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return ds_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        ds_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        ds_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    ds_shell::http::set_client_name(if is_interactive {
        ds_workspace::permission::ClientType::DsPager
    } else {
        ds_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `ds-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                ds_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return ds_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ds_telemetry::otel_layer::otel_guard();
                return ds_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = ds_telemetry::otel_layer::otel_guard();
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ds_pager::models::list_available_models(&agent_config).await;
            }
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ds_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ds_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ds_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return ds_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ds_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            ...
            Command::Auth { command } => {
                init_tracing_simple("cli");
                use ds_pager::app::cli::AuthCommand;
                use ds_shell::auth::{ApiKeyInput, run_auth_clear, run_auth_set, run_auth_status};
                match command {
                    AuthCommand::Set {
                        api_key,
                        api_key_stdin,
                    } => {
                        let input = if let Some(k) = api_key {
                            ApiKeyInput::Inline(k)
                    ...
                    AuthCommand::Status { json } => run_auth_status(json)?,
                    AuthCommand::Get => run_auth_status(false)?,
                    AuthCommand::Clear => run_auth_clear()?,
                }
                return Ok(());
            }
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                ds_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
            ...
            Command::Logout => {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                ds_shell::auth::run_cli_logout(&config)?;
                ds_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = ds_pager::headless::HeadlessPrompt::from_args(
        args.single.as_deref(),
        args.prompt_json.as_deref(),
        args.prompt_file.as_deref(),
    )?;
    // CodeWhale-style first-run: when no API key is configured and stdin is a
    // TTY, prompt once before headless or interactive UI. Non-TTY skips the
    // prompt; auth fails later with a clear `ds auth set` message.
    match ds_shell::auth::ensure_api_key_interactive()? {
        ds_shell::auth::EnsureApiKeyOutcome::AlreadyConfigured
        | ds_shell::auth::EnsureApiKeyOutcome::Saved
        | ds_shell::auth::EnsureApiKeyOutcome::MissingNonInteractive => {}
    }
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(ds_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == ds_pager::headless::OutputFormat::Plain {
                args.output_format = ds_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return ds_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            ds_pager::headless::HeadlessOptions {
    ...
    let result = ds_pager::app::run(args, bg_update_rx).await;
    ds_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `ds` to start.");
            } else {
                eprintln!("Update did not complete. Run `ds update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `ds update` child
...
/// fall back to a fresh blocking `ds update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = ds_shell::env::DsBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                ds_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("DS_NPM_REGISTRY"))
        .ok()
        .or_else(ds_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = ds_shell::config::load_effective_config_disk_only()
        && let Some(ch) = ds_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    if std::env::var_os("DS_DISABLE_AUTOUPDATER").is_some() {
        return false;
    }
    true
}
...
/// `DS_DISABLE_AUTOUPDATER`, debug builds) is layered on separately via
...
/// Handle `ds-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `ds update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in ds_shell::leader::discover_leaders().await {
        if d.classification != ds_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match ds_shell::leader::LeaderClient::connect(
            socket_path,
            "ds-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(ds_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(ds_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            ds_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: ds_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/ds-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            ds_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!ds_shell::heap_profile::prof_available());
            return;
        }
        assert!(ds_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(ds_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(ds_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        ds_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("DS_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("DS_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `ds dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(DS_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["ds", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(DS_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("DS_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("DS_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("DS_WORKSPACE_COMMAND", "off") };
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_ds.cli/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"ds.cli/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `ds.cli/leader_reconnected` with empty params and the external client

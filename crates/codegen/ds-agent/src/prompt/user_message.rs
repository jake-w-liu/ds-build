...
//! - `Default` -- the legacy DS Build prefix (built by the shell layer).
...
use ds_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
///
/// First-user-message rendering strategy for an agent.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum UserMessageTemplate {
    /// Legacy DS Build prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .ds/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `ds_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.ds/, ~/.claude/).
    ...
    /// (`~/.ds/projects/<encoded-cwd>/mcps`). Surfaced in

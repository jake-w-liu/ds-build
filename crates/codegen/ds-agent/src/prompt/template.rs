...
//!
//! Templates are plain Markdown under `templates/` and compiled in with
//! `include_str!` — no obfuscation. The returned `Zeroizing<String>` still
//! clears the heap copy on drop so prompt text is not retained longer than
//! needed in memory after rendering.

use zeroize::Zeroizing;

/// Own a copy of a compile-time template string (zeroed on drop).
fn owned_template(source: &str) -> Zeroizing<String> {
    Zeroizing::new(source.to_owned())
}

/// The base prompt template (from `templates/prompt.md`).
pub(crate) fn base_template() -> Zeroizing<String> {
    owned_template(include_str!("../../templates/prompt.md"))
}
...
pub(crate) fn apply_patch_template() -> Zeroizing<String> {
    owned_template(include_str!("../../templates/apply_patch_prompt.md"))
}
...
/// The subagent-specific base template (from `templates/subagent_prompt.md`).
pub(crate) fn subagent_template() -> Zeroizing<String> {
    owned_template(include_str!("../../templates/subagent_prompt.md"))
}
...
pub const COMPACT_SYSTEM_PROMPT: &str = concat!(
    ...
    "For mathematics and physics, apply five gates to every requested result: contract-closure ",
    "validation (domain, branches, and below, exactly at, and above critical values), ",
    "derivation-integrity validation ",
    "(no correct answer through a false step), evidence-provenance validation (bind each successful ",
    "check to its claim, inputs, output, and tolerance), invariant-ledger validation (units, ",
    "normalization conventions, signs, and coordinate/gauge choices), and state isolation/artifact ",
    "freezing (preserve inputs, ",
    "diff narrow edits, and revalidate every changed dependency). Independently verify residuals, ",
    "admissibility, and boundary conditions. For code, trace ",
...
mod tests {
    ...
    /// Templates are loaded verbatim from `templates/*.md` via `include_str!`.
    #[test]
    fn test_templates_load_from_markdown_sources() {
        let base = base_template();
        let apply_patch = apply_patch_template();
        let subagent = subagent_template();

        assert_eq!(base.as_str(), include_str!("../../templates/prompt.md"));
        assert_eq!(
            apply_patch.as_str(),
        ...
        assert!(
            base.contains("user_query"),
            "base template must reference <user_query>"
        );
        assert!(
            !base.is_empty() && !apply_patch.is_empty() && !subagent.is_empty(),
            "templates must not be empty"
        );
    }

    /// Full and compact prompts must carry the same five math-correctness
    /// gates; model switches must not silently weaken an active derivation.
    #[test]
    fn math_correctness_gates_survive_full_and_compact_prompts() {
        let base = base_template();
        for gate in [
            "Contract-closure validation",
            "Derivation-integrity validation",
            "Evidence-provenance validation",
            "Invariant-ledger validation",
            "State isolation and artifact freezing",
        ] {
            assert!(base.contains(gate), "base prompt missing {gate}");
        }
        for gate in [
            "contract-closure validation",
            "derivation-integrity validation",
            ...
            assert!(
                COMPACT_SYSTEM_PROMPT.contains(gate),
                "compact prompt missing {gate}"
            );
        }
    }

    /// Build a TemplateRenderer with the standard ds-build tool kinds.

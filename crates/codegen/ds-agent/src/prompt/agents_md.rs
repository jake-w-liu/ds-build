...
//!
//! Searches from cwd to repo root, plus `~/.ds/`. Also discovers
//! `*.md` files in `.ds/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.ds/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.ds/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = ds_tools::util::ds_home::ds_home();
    ...
    // Always include ds_home (~/.ds/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `ds-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .ds/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.ds/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            ds_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {

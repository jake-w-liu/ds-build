//! Agent definition types — parsed from `.ds/agents/*.md` files.
...
use ds_tools::implementations::opencode;
...
/// Orchestrator-specific prompt body appended to the standard DsBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&ds_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&ds_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&ds_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&ds_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&ds_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_ds_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_ds_build_toolset() -> ToolServerConfig {
    let mut tools = default_ds_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&ds_build::EnterPlanModeTool).into());
...
/// Toolset for the `ds-computer` (workspace/sandbox) preset.
fn ds_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&ds_build::ReadFileTool).into(),
        (&ds_build::SearchReplaceTool).into(),
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("ds-build", workspace_ds_build_toolset()),
        ("ds-build-concise", ds_build_concise_toolset()),
        ("ds-build-plan", ds_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("ds-computer", ds_computer_toolset()),
    ]
}
...
fn default_ds_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ds_build::ReadFileTool).into(),
            ...
            task_tool_config(),
            (&ds_build::SchedulerCreateTool).into(),
            (&ds_build::SchedulerDeleteTool).into(),
...
fn ds_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&ds_build_concise::BashConciseTool).into(),
            (&ds_build_concise::ReadFileConciseTool).into(),
            (&ds_build_concise::SearchReplaceConciseTool).into(),
            (&ds_build::ListDirTool).into(),
            (&ds_build::GrepTool).into(),
            kill_task_tool_config(),
            (&ds_build::TodoWriteTool).into(),
...
pub fn ds_build_hashline_toolset(
    hashline_tools: Vec<ds_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<ds_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// DS Build + plan mode toolset.
///
/// Extends the default `ds-build` toolset with plan mode tools:
...
fn ds_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ds_build::ReadFileTool).into(),
...
/// DS Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `ds_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn ds_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ds_build::ReadFileTool).into(),
...
/// Default DS Build toolset + `ask_user_question`.
///
/// Same as `default_ds_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn ds_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ds_build::ReadFileTool).into(),
...
/// - `model: ds-3-fast` → `Override("ds-3-fast")`
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::DsBuild => AgentDefinition::default_ds_build(),
            Self::DsBuildConcise => AgentDefinition::ds_build_concise(),
            Self::DsBuildPlan => AgentDefinition::ds_build_plan(),
            Self::DsBuildPlanNoSubagents => AgentDefinition::ds_build_plan_no_subagents(),
            Self::DsBuildAskUser => AgentDefinition::ds_build_ask_user(),
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_ds_build_toolset")]
    ...
    pub capability_mode: Option<ds_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .ds/agents/ (project-level, highest priority)
    Project,
    /// ~/.ds/agents/ (user-level)
    User,
    /// ~/.ds/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_ds_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.ds/agent-memory/<name>/`
    User,
    /// `<project>/.ds/agent-memory/<name>/`
    Project,
    /// `<project>/.ds/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            ...
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".ds/agent-memory").join(agent_name),
...
/// these into `ds_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let ds = ds_config::user_ds_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), ds.as_deref()) {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `ds-build*` agents leave all
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_ds_build_toolset(),
    ...
    pub fn default_ds_build() -> Self {
        Self::base(
            BuiltinAgentName::DsBuild,
            "DS Build agent for software engineering tasks.",
        )
    }
    /// DS Build Concise agent definition — concise output format for SFT/RL.
    pub fn ds_build_concise() -> Self {
        Self {
            tool_config: ds_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::DsBuildConcise,
                "DS Build agent with concise output format.",
            )
        }
    }
    /// DS Build agent with plan mode tools.
    pub fn ds_build_plan() -> Self {
        Self {
            tool_config: ds_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::DsBuildPlan,
                "DS Build agent with plan mode support.",
            )
        }
    }
    /// DS Build + plan mode WITHOUT subagent tools.
    pub fn ds_build_plan_no_subagents() -> Self {
        Self {
            tool_config: ds_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::DsBuildPlanNoSubagents,
                "DS Build agent with plan mode (no subagents).",
            )
        }
    }
    /// Default DS Build agent with the `ask_user_question` tool.
    pub fn ds_build_ask_user() -> Self {
        Self {
            tool_config: ds_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::DsBuildAskUser,
                "DS Build agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: ds_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ds_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ds_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// DS Build Orchestrator — GBL model with full DsBuild tools
    ...
    pub fn ds_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::DsBuildOrchestrator,
                "DsBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_ds_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("ds-build").unwrap();
    ...
    fn ds_computer_exclusive_ids() -> Vec<String> {
        ...
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&ds_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&ds_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn ds_computer_preset_is_curated_ds_build_subset() {
        let gc = toolset_for_preset("ds-computer").unwrap();
        let gb = toolset_for_preset("ds-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = ds_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "ds-computer tool `{}` must also ship in the ds-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "ds-computer should be a curated subset of ds-build"
        );
    }
    #[test]
    fn ds_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("ds-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&ds_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&ds_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&ds_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ds_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ds_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&ds_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&ds_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The ds-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the ds-build
    ...
    #[test]
    fn ds_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("ds-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "ds-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn ds_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("ds-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&ds_build::LspTool).id,
            ToolConfig::from(&ds_build::EnterPlanModeTool).id,
            ToolConfig::from(&ds_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "ds-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_ds_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_ds_build_toolset must ship `{present}`"
            );
        }
    }
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("ds-3-fast".to_string()).to_string(),
            "ds-3-fast"
        );
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.ds/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.ds/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_ds_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have ds_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"ds-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("ds-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("ds-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"ds-3-fast\"");
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "ds-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("ds-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("ds-build", BuiltinAgentName::DsBuild),
            ("ds-build-concise", BuiltinAgentName::DsBuildConcise),
            ("ds-build-ask-user", BuiltinAgentName::DsBuildAskUser),

...
//! primitive consumed cross-crate by `ds-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.ds/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.ds`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `ds-workspace`, which depends on THIS crate) to keep the dep edge

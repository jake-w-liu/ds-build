...
/// Default install directory name under `~/.ds/`.
...
/// Lives here (not in ds-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/ds-code-review").
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("DS_DS_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {

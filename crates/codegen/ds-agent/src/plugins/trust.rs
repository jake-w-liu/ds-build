...
//!
//! Plugins from project directories (`.ds/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.ds/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.ds/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.ds/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_ds_home() so a project's `.ds/trusted-plugins` is never
        // read as the user trust store when neither DS_HOME nor a home dir resolves.
        let Some(ds) = ds_config::user_ds_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = ds.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.ds/trusted-plugins`.
    ...
    /// rewrites `~/.ds/trusted-plugins` without the revoked entry.

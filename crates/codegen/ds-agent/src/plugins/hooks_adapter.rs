...
//! `ds-hooks` runtime.  It pre-filters unsupported events from plugin
...
//! `ds-hooks` crate's parser and runtime.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use ds_hooks::config::{HookSpec, parse_hook_file};
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("DS_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("DS_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    /// `${CLAUDE_PLUGIN_ROOT}` (or its `DS_PLUGIN_ROOT` alias) must be
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${DS_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${DS_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    /// `ds-hooks::runner::command`, which can fail for hooks
    ...
    /// (CLAUDE_PLUGIN_ROOT, DS_PLUGIN_ROOT, CLAUDE_PLUGIN_DATA,
    /// DS_PLUGIN_DATA). This preserves the plugin contract while still
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "DS_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "DS_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        // overridden. CLAUDE_PLUGIN_ROOT and DS_PLUGIN_ROOT both map
        // to plugin_root; CLAUDE_PLUGIN_DATA and DS_PLUGIN_DATA both
        // map to plugin_data.
        for (key, expected) in [
            ("CLAUDE_PLUGIN_ROOT", "/actual/plugin/root"),
            ("DS_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("DS_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(

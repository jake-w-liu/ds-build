...
//!
//! Searches `.ds/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.ds/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.ds/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".ds/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.ds/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.ds/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), ds.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, ds_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.ds/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.ds/agents/` walking from `cwd` up to repo root
/// 2. `~/.ds/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.ds/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user ds agents, `.claude`
/// compat agents, then bundled. `.ds` dirs resolve from `ds_home`
/// (DS_HOME-aware) plus the legacy literal `~/.ds` when DS_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    ds_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.ds, included only when it differs from ds_home
    // (i.e. DS_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_ds = home
        .map(|h| h.join(".ds"))
        .filter(|legacy| ds_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = ds_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_ds {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let ds = ds_config::user_ds_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), ds.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, ds_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let ds = ds_config::user_ds_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), ds.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, ds_home);
...
///
/// Project-level `.ds/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let ds = ds_config::user_ds_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), ds.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, ds_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, ds_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the DS
...
/// Load project agent definitions from every `.ds/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectDs,
            PluginScope::User => PluginOrigin::UserDs,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_ds_when_ds_home_differs() {
        let home = Path::new("/home/u");
        let ds = Path::new("/custom/dshome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(ds))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&ds.join("agents")));
        assert!(paths.contains(&home.join(".ds").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&ds.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".ds").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_ds_home_is_dot_ds() {
        let home = Path::new("/home/u");
        let ds = home.join(".ds");
        let count = user_agent_dirs(Some(home), Some(&ds))
            .into_iter()
            .filter(|(p, _)| *p == ds.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.ds/agents when ds_home == ~/.ds"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_ds_build() {
        let def = by_name("ds-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "ds-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".ds").join("agents");
        let agents_dir_2 = inner_dir.join(".ds").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".ds").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".ds")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".ds")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".ds").join("agents");
        let bundled_dir = home.join(".ds").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".ds")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".ds")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".ds").join("agents");
        let bundled_dir = home.join(".ds").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "ds-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "ds-build.md",
            "ds-build",
        ...
        let def = by_name_in_cwd("ds-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "ds-build");
        assert_eq!(def.description, "Custom ds-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .ds/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("ds-build-orchestrator")
            .expect("from_str must resolve ds-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::DsBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "ds-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("ds-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find ds-build-orchestrator");
        assert_eq!(def.name, "ds-build-orchestrator");
    ...
    fn test_merge_returns_3_builtins_when_no_user_agents() {
        ...
        assert_eq!(entries.len(), 6); // 6 built-ins
    ...
    fn test_merge_filters_toggled_off_builtins() {
        ...
        assert_eq!(entries.len(), 5); // 6 built-ins minus toggled-off plan
        let names: Vec<&str> = entries.iter().map(|e| e.name.as_str()).collect();
        assert!(names.contains(&"general-purpose"));
        assert!(names.contains(&"explore"));
        assert!(!names.contains(&"plan"));
    ...
    fn test_merge_includes_user_defined_agents() {
        ...
        assert_eq!(entries.len(), 7); // 6 built-ins + 1 user
    ...
    fn test_merge_filters_toggled_off_user_agents() {
        ...
        assert_eq!(entries.len(), 6); // only built-ins
    ...
    fn test_merge_project_agent_shadows_builtin() {
        ...
        let entries = merge_subagents(discovered, &HashMap::new());
        assert_eq!(entries.len(), 6); // still 6 — replaced, not appended
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.ds/agents/) agent named "explore" should NOT shadow
        ...
        let entries = merge_subagents(discovered, &HashMap::new());
        assert_eq!(entries.len(), 6); // still 6 built-ins
        let explore = entries.iter().find(|e| e.name == "explore").unwrap();
    ...
    fn test_merge_user_unique_name_appended() {
        ...
        assert!(matches!(&entries[5].source, SubagentSource::Builtin(_)));
        assert_eq!(entries[6].name, "migration-helper");
        assert_eq!(
            entries[6].source,
            SubagentSource::UserDefined {
    ...
    fn test_merge_bundled_unique_name_appended() {
        ...
        let entries = merge_subagents(discovered, &HashMap::new());
        assert_eq!(entries.len(), 7); // 6 built-ins + 1 bundled
        assert_eq!(entries[6].name, "bundled-helper");
        assert_eq!(
            entries[6].source,
            SubagentSource::UserDefined {
    ...
    fn test_merge_all_toggled_off_returns_empty() {
        let toggle = HashMap::from([
            ...
            ("attacker-code".to_string(), false),
            ("attacker-math".to_string(), false),
            ("attacker-research".to_string(), false),
        ]);
        let entries = merge_subagents(vec![], &toggle);

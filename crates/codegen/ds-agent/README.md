...
This crate extracts a first-class `Agent` type from `ds-shell`.
...
`ds-shell`, another in-process host, or a headless batch runner.
...
in `.ds/agents/` (project-level) or `~/.ds/agents/` (user-level).

```rust
use ds_agent::{AgentDefinition, AgentBuilder};
use ds_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".ds/agents/code-reviewer.md")?;
...
```rust
use ds_agent::discovery;

// Find all .md files in .ds/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  ds-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`ds-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `ds-agent` depends on it for tool setup.
- **`ds-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `ds-agent` (AGENTS.md
...
|---|---|---|
| `ds-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p ds-agent

# Test
cargo test -p ds-agent

# Clippy
cargo clippy -p ds-agent --fix --allow-dirty

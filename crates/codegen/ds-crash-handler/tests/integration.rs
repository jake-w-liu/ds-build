//! Integration tests for ds-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = ds_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    ds_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob = ds_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report =
        ds_crash_handler::check_previous_crash(tmp.path()).expect("should produce a crash report");
...
fn clean_exit_does_not_produce_crash_report() {
    ...
    let report = ds_crash_handler::check_previous_crash(tmp.path());

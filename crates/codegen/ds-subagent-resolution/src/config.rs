...
//! `ds_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `ds-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("deepseek-v4-flash"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".ds/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(
            ...
            Some(".ds/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("ds-3-fast"));

...
//!
//! Extracted from `ds-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `ds-chat-state::compaction_utils::strip_system_tags` which
...
/// `ds-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use ds_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(ds_sampling_types::synthesized_reasoning_item(text))
    }

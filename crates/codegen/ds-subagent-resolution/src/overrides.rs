...
//!
//! Extracted from `ds-shell/src/agent/subagent/` `resolve_effective_overrides()`.
...
mod tests {
    use super::*;
    use ds_tools::implementations::ds_build::task::types::ModelOverrideProvenance;
    ...
    fn explicit_model_overrides_role() {
        let overrides = make_overrides(Some("ds-light"), None, None, None, None);
        ...
        assert_eq!(result.model.as_deref(), Some("ds-light"));
    }
    ...
    fn persona_file_error_returns_early_with_defaults() {
        ...
        let overrides = make_overrides(Some("deepseek-v4-flash"), Some("broken"), None, None, None);
        let dir = tempfile::tempdir().unwrap();
        let mut personas = HashMap::new();
        personas.insert(
            "broken".to_string(),

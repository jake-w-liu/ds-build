...
//!
//! Reads OIDC credentials from `~/.ds/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use ds_workspace::config::WorkspaceServerMetadata;
use ds_workspace::daemonize;
use ds_workspace::diag_server;
use ds_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_ds_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<ds_tool_protocol::ServerId>()
        .err()
        .map(|e| format!("{INVALID_SERVER_ID_MARKER} {id:?}: {e}"))
}
#[derive(Parser)]
#[command(name = "ds-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.ds.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `DS_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "DS_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Confine `ds.cli/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `DS_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "DS_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/ds-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = ds_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("DS_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match ds_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use ds_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("DS_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized DS_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if ds_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via DS_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = ds_sandbox::is_active();
            ...
            tracing::info!(
                ...
                ds_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = ds_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("DS_SESSION_ID").ok();
    ...
    ds_workspace::init_metrics();
    let ws_handle = ds_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = ds_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            ds_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    ds_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["ds-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["ds-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["ds-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["ds-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["ds-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://www.deepseek.com/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://www.deepseek.com/preview-auth")
        );

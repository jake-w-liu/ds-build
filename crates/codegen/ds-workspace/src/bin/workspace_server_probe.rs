...
//! access token from `~/.ds/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use ds_computer_hub_sdk::pool::HubConnectionPool;
use ds_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &ds_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<ds_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "DsBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "DsBuild:read_file"},
        ],
    });
    let tools = harness

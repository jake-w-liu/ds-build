...
/// Directory (under `<cwd>/.ds`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.ds/rewind-checkpoints/<session_id>`.
    dir: PathBuf,

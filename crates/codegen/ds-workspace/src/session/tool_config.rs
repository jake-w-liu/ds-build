...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<ds_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<ds_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<ds_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<ds_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn ds_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            ds_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`DS_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("DS_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = ds_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `DS_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: ds_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<ds_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$DS_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: ds_computer_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
    ...
    /// (`$DS_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(ds_tools::computer::local::LocalFs)
            as Arc<dyn ds_tools::computer::types::AsyncFileSystem>;
        let notification_handle = ds_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    ds_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
...
fn build_web_fetch_config() -> ds_tools::implementations::ds_build::web_fetch::WebFetchConfig {
    ...
    if let Ok(proxy) = std::env::var("DS_WEB_FETCH_PROXY") {
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("DsBuild:search_replace", Some(ToolKind::Edit)),
                tc("DsBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("DsBuild:search_replace".to_owned(), ToolKind::Edit),
            ("DsBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("DsBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("DsBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("DsBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("DsBuild:read_file", None),
                test_support::tc("DsBuild:grep", None),
                test_support::tc("DsBuild:list_dir", None),
                test_support::tc("DsBuild:search_replace", None),
                test_support::tc("DsBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("DsBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"DsBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "DS_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use ds_tools::types::resources::{State, WebCitationCounter};

...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "ds-shell", alias = "ds_shell")]
    Generic,
    /// DS TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "ds-tui", alias = "ds_tui")]
    DsTUI,
    /// DS Web client - identified by clientIdentifier "ds-web"
    #[serde(rename = "ds_web")]
    DsWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "ds-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// DS Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to DsTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"ds-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"ds_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "ds-pager", alias = "ds_pager")]
    DsPager,
    /// DS Desktop (Electron) client - identified by clientIdentifier "ds-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "ds_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `ds-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "ds-shell",
            Self::DsTUI => "ds-tui",
            Self::DsWeb => "ds-web",
            Self::Nebula => "nebula",
            Self::Extension => "ds-code-extension",
            Self::DsPager => "ds-pager",
            Self::Desktop => "ds-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"ds-web"`, `"ds-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("ds-web") => Self::DsWeb,
            Some("nebula") => Self::Nebula,
            Some("ds-code-extension") => Self::Extension,
            Some("ds-desktop") => Self::Desktop,
            Some("ds-pager") => Self::DsPager,
            _ => Self::Generic,
        }
    }
...
impl From<&ds_tools::types::ToolInput> for AccessKind {
    fn from(input: &ds_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.ds/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use ds_tools::implementations::ds_build_hashline::edit::types::HashlineEditInput;
        use ds_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use ds_tools::implementations::ds_build::bash::BashToolInput;
        use ds_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use ds_tools::implementations::use_tool::UseToolInput;
        use ds_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use ds_tools::implementations::ds_build::monitor::types::MonitorInput;
        use ds_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use ds_tools::implementations::ds_build::web_fetch::WebFetchInput;
        use ds_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use ds_tools::implementations::ds_build::web_search::WebSearchInput;
        use ds_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_ds_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("ds-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("ds_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(

...
//!
//! Port of common agent auto-permission classifier semantics adapted to DS's
...
/// mapped to DS access kinds + known names).
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    ds_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}

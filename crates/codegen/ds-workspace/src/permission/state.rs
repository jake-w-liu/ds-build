...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"ds_com_notion__notion-fetch"`)
    ...
    /// e.g. `"ds_com_notion"`) for which the user has granted
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    ds_config::sessions_cwd_dir(cwd.as_str())
}
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = ds_home().join("sessions");
...
mod tests {
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("ds_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("ds_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        assert!(restored.allowed_mcp_servers.contains("ds_com_slack"));
...
"#;
    ...
    async fn persist_and_load_roundtrip() {
        ...
        // We can't easily override ds_home(), so instead test

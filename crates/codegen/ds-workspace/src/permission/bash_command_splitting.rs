...
///
/// ```bash
/// DEEPSEEK_API_KEY='ds-some-key' cargo run --bin ds-pager
/// ```
///
...
///   prefix: ["DEEPSEEK_API_KEY=ds-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "ds-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "DEEPSEEK_API_KEY='ds-some-key' cargo run --bin ds-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["DEEPSEEK_API_KEY=ds-some-key".to_owned()],
        ...
        let redirection_command = "cargo build --bin ds-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package ds-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(

...
use ds_computer_hub_sdk::ToolServerHandler;
use ds_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
use ds_tool_runtime::{
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ds_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ds_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ds_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "ds_workspace_rpc_duration_seconds",
...
) -> Vec<ds_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use ds_tools::computer::types::TaskKind;
    use ds_tools::types::resources::Terminal;
    use ds_tools::types::tool::ToolKind;
    use ds_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &ds_tools::registry::types::FinalizedToolset,
) -> Vec<ds_workspace_types::rpc::workspace::TodoSummaryWire> {
    use ds_tools::implementations::ds_build::todo::{TodoState, TodoStatus};
    use ds_tools::types::resources::State;
    use ds_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use ds_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use ds_workspace_types::rpc::search::FuzzyStatusReq;
        use ds_workspace_types::rpc::skills::DiscoverPluginsReq;
        ...
        use ds_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<ds_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use ds_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use ds_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<ds_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use ds_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use ds_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                "DsBuild:read_file",
                Some(ds_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(ds_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            ds_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "ds_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use ds_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();

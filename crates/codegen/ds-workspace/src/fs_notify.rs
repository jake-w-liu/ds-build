...
//! FsNotify adapter functions bridging [`ds_fsnotify`] events to
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> ds_codebase_graph::FileEvent {
    use ds_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`ds_fsnotify::FsEventKind`] to the wire-type
/// [`ds_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> ds_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => ds_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => ds_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => ds_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => ds_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => ds_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](ds_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<ds_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<ds_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    ds_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<ds_codebase_graph::FileEvent> {
    let mut parts = line.splitn(3, '\t');
    let status = parts.next()?.trim();
    let path = parts.next()?;

    match status.chars().next()? {
        'A' => Some(ds_codebase_graph::FileEvent::created(repo_root.join(path))),
        'D' => Some(ds_codebase_graph::FileEvent::removed(repo_root.join(path))),
        'R' | 'C' => {
            let new_path = parts.next()?;
            Some(ds_codebase_graph::FileEvent::renamed(
                repo_root.join(path),
                repo_root.join(new_path),
            ))
        }
        _ => Some(ds_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &ds_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<ds_workspace_types::WorkspaceEvent>,
) {
    ...
    ds_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(ds_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            ds_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: ds_workspace_types::FsEventKind,
) -> ds_codebase_graph::FileEvent {
    use ds_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        ds_workspace_types::FsEventKind::Created => FileEventKind::Created,
        ds_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        ds_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        ds_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.ds/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use ds_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use ds_codebase_graph::FileEventKind;

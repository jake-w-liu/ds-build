...
//! for the shell's `ds.cli/fs/*` ACP extension methods.
//!
//! These mirror the pure functions that previously lived only in the
//! shell (`ds-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `ds.cli/fs/*` request executes on the *remote* workspace server
...
use ds_workspace_types::rpc::fs::FsReadEncoding;

...
use ds_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use ds_tools::util::ds_home::ds_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.ds/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `ds.cli/code/status` reporting `reason: notStarted`.
    ...
    /// In `ds.cli/code/status`, `resolve_index_handle()` calls

//! AcpFsAdapter: implements `ds-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates ds-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use ds_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps ds-shell's ACP gateway to satisfy ds-tools' AsyncFileSystem.
///

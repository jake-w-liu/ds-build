...
//!
//! Persists per-folder trust decisions to `~/.ds/trusted_folders.toml`.
...
//!
//! The store is rooted at [`ds_config::user_ds_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.ds`) when
//! neither `$DS_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.ds/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.ds/`.
...
impl TrustStore {
    /// Load the trust store from `<user_ds_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_ds_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`ds_config::user_ds_home`], never
    /// [`ds_config::ds_home`], so it never falls back to a cwd-relative
    /// `./.ds` — that fallback would let an untrusted cloned repo's `.ds`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(ds_config::user_ds_home())
    }

    /// Map a resolved user-ds-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_ds_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_ds_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user ds home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A ds-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.ds/worktrees.db` registry), so every `ds -w` worktree
...
/// `ds -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `ds -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A ds-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir ds -w was launched from (parity with
...
/// sharing `~/.ds/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.ds/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = ds_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global ds-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match ds_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.ds");
        assert_eq!(
        ...
        // cwd-relative `./.ds` that ds_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.ds/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_ds_home() {
        // Thin source-pin: the production accessor reads user_ds_home()
        // (Option, no cwd fallback), not ds_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            ds_config::user_ds_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `ds -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (ds-managed worktrees) ─────────
    ...
    /// Point `DS_HOME` at an isolated tempdir and register one ds-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `DS_HOME` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_ds_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use ds_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_ds_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_ds_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone ds worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_ds_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<ds_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under ds HOME but not
        ...
        let (_env, _wt) = register_ds_worktree(&temp, "wt", &source_repo, "standalone");

        // Under ds HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root.join("ds-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <ds_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,

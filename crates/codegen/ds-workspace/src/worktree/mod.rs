...
//!
//! Moved from `ds-shell/src/session/worktree.rs` into the workspace
...
use ds_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in ds-workspace-types; re-exported for existing paths.
...
const WORKTREE_LOG: &str = "ds_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> ds_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => ds_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => ds_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => ds_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// Resolve the ds home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`ds_fast_worktree::resolve_ds_home`), so checkout dirs and
/// the metadata DB always live under the same `.ds` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `ds_config::ds_home()`,
/// so worktree paths also agree with trust/hooks and other ds-home paths.
fn ds_home() -> std::path::PathBuf {
    ds_fast_worktree::resolve_ds_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.ds/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    ds_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.ds/worktrees/<repo_name>`)
/// for a given source path, correctly handling ds-managed worktrees.
///
/// When `source_path` is already under `~/.ds/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the ds worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = ds_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.ds/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the ds-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.ds/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to ds_fast_worktree::WorkingTreeMode
    ...
    // Use ds-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(ds_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        ds_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        ds_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        ds_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        ds_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        ds_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.ds/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        ds_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use ds-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(ds_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `ds -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `ds.cli/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `ds.cli/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use ds_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord,
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_ds_home,
};
...
pub fn worktree_db_rebuild() -> Result<ds_fast_worktree::RebuildReport> {
    let home = resolve_ds_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = ds_fast_worktree::ListFilter {
...
/// Scan `~/.ds/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[ds_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/ds/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/ds/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/ds/subagents/standalone-1";
    ...
    /// Point `DS_HOME` at an isolated tempdir (`resolve_ds_home` re-reads
    ...
    /// and restores `DS_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("DS_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: ds_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (DsHomeFixture is not visible across crates).

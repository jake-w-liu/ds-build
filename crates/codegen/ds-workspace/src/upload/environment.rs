...
impl WorkspaceIdentity {
    ...
    /// `DsAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
/// [`AuthIdentity`](ds_computer_hub_sdk::AuthIdentity). The two types carry the
...
impl From<ds_computer_hub_sdk::AuthIdentity> for WorkspaceIdentity {
    fn from(id: ds_computer_hub_sdk::AuthIdentity) -> Self {
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `ds-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$DS_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("DS_SANDBOX_PROFILE").ok(),
            ds_sandbox::is_inside_bwrap(),
...
mod tests {
    ...
    fn assemble_populates_every_field() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("git@github.com:deepseek-ai/example.git".to_string()),
        );
        ...
        assert_eq!(
            ...
            Some("git@github.com:deepseek-ai/example.git")
        );
    ...
    fn serializes_with_all_fields_present() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("host-1".to_string()),
            Some("/work/repo".to_string()),
            Some("https://github.com/deepseek-ai/example".to_string()),
        );
    ...
    fn git_repo_facts_strips_remote_url_credentials() {
        ...
        assert_eq!(url, "https://github.com/deepseek-ai/example.git");
    }
}

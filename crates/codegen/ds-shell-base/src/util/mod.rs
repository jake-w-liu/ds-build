...
pub mod tips;
pub mod uname;
pub use ds_shared::clipboard;
pub use ds_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True for first-party DeepSeek endpoints (`*.deepseek.com`, cli-chat-proxy, and optional
...
/// exempt. Safe against invalid URLs and suffix attacks (`evil-deepseek.com.example`).
pub fn is_first_party_ds_url(url: &str) -> bool {
...
/// True if `pid` is a ds process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_ds_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("ds"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    #[test]
    fn test_is_first_party_ds_url() {
        assert!(is_first_party_ds_url("https://api.deepseek.com/v1"));
        assert!(is_first_party_ds_url(
            "https://api.deepseek.com/v1/chat/completions"
        ));
        assert!(is_first_party_ds_url("https://www.deepseek.com"));
        assert!(is_first_party_ds_url(
            "https://api.deepseek.com/v1/chat/completions"
        ));
        assert!(!is_first_party_ds_url("https://api.openai.com/v1"));
        assert!(!is_first_party_ds_url("https://api.anthropic.com/v1"));
        assert!(!is_first_party_ds_url(
    ...
    #[test]
    fn is_ds_process_self_true_impossible_pid_false() {
        assert!(is_ds_process(std::process::id()));
        assert!(!is_ds_process(u32::MAX));
    }
}

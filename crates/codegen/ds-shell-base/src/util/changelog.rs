...
//! are published per-version to the CDN at `ds.cli/cli/changelogs/`.
//!
...
const CHANGELOG_BASE: &str = "https://www.deepseek.com/cli/changelogs";
...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$DS_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `ds_home()` OnceLock). A seeded `$DS_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("DS_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::ds_home::ds_home);
        Self {
    ...
    ///
    /// When `DS_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$DS_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`DS_HOME` /
    /// `DS_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$DS_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("DS_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$DS_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("ds-home-fallback");
        std::fs::create_dir_all(&home).unwrap();

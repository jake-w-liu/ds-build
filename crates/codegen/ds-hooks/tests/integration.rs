//! Integration tests for ds-hooks.
//!
...
use ds_hooks::discovery::load_hooks;
use ds_hooks::dispatcher;
use ds_hooks::event::*;
use ds_hooks::result::HookDecision;
use ds_hooks::runner::RunContext;
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$DS_HOOK_EVENT" > {f}; echo "NAME=$DS_HOOK_NAME" >> {f}; echo "SESSION=$DS_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`DS_HOOK_EVENT`, `DS_HOOK_NAME`,
/// `DS_SESSION_ID`, `DS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$DS_HOOK_EVENT" > {f}; echo "NAME=$DS_HOOK_NAME" >> {f}; echo "SESSION=$DS_SESSION_ID" >> {f}; echo "ROOT=$DS_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "DS_HOOK_EVENT": "spoofed_event",
                                "DS_HOOK_NAME": "spoofed_name",
                                "DS_SESSION_ID": "spoofed_session",
                                "DS_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "DS_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed DS_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "DS_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed DS_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "DS_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed DS_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(

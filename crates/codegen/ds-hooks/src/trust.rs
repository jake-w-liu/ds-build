...
// (`~/.ds/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_ds_home>/trusted-hook-projects`), or `None` when no user ds home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(ds_config::user_ds_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user ds home (set $DS_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$DS_HOME/disabled-hooks`, or `None` when no user ds
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(ds_config::user_ds_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let ds_dir = dir.join(".ds");
        std::fs::create_dir_all(&ds_dir).unwrap();
        ds_dir.join("trusted-hook-projects")
    }

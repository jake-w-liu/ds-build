...
Sample hooks for DS. Copy to `~/.ds/hooks/` to enable globally, or to `<project>/.ds/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.ds/hooks/bin
cp examples/hooks/safe-shell.json ~/.ds/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.ds/hooks/bin/
chmod +x ~/.ds/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.ds/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.ds/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.ds/hooks/bin/
chmod +x ~/.ds/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.ds/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.ds/hooks/bin
cp examples/hooks/session-log.json ~/.ds/hooks/
cp examples/hooks/bin/session-log.sh ~/.ds/hooks/bin/
chmod +x ~/.ds/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.ds/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.ds/hooks/bin
cp examples/hooks/tool-logger.json ~/.ds/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.ds/hooks/bin/
chmod +x ~/.ds/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match DS names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.ds/hooks/`. The hook stops running on the next session.
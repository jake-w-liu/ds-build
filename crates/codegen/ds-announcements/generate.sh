...
# are the single source of truth for the `ds.cli/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/ds-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the ds-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done

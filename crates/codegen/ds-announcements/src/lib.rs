//! Shared announcement types, persistence, and formatting for DS CLI apps.
//!
//! This crate provides the common logic used by `ds-shell` and
//! `ds-pager` for handling announcements (banner notifications).
...
/// Payload for `ds.cli/announcements/update` ACP notification.
...
/// Read hidden announcement ids from `~/.ds/announcements.json`.
...
/// Write hidden announcement ids to `~/.ds/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    ds_tools::util::ds_home::ds_home().join("announcements.json")
}
...
///
/// Precedence: `DS_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("DS_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid DS_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("DS_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("DS_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get DS Pro","url":"https://www.deepseek.com/ds","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get DS Pro"));
        assert_eq!(cta.url.as_deref(), Some("https://www.deepseek.com/ds"));

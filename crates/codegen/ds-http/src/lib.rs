...
//! `ds_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `DS_POOL_*` /
//! `DS_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `DS_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `ds-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `ds-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `ds-telemetry` consumes
// the same type via `ds_sampler::OriginClientInfo`. The shell-specific
...
/// `DS_CLIENT_VERSION` env vars. Returns `None` when
/// `DS_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`DS_CLIENT_NAME` env var, default `"ds-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("DS_CLIENT_NAME").unwrap_or_else(|_| "ds-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`ds -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-ds-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `ds -p`, `run_headless_inner` for
/// `ds agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn ds_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(ds_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("ds-desktop/1.2.3 ds-shell/"));
        ...
        assert!(without_version.starts_with("ds-web ds-shell/"));
        assert!(!without_version.starts_with("ds-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "ds-shell/0.1.171 (macos; aarch64)");
    }
}

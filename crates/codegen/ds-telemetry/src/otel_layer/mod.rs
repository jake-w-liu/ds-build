...
//!
//! `ds-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "DS_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `DsComConfig`).
///
/// Built by the binaries (`ds-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `ds_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-DS-Token-Auth` header (typically `"ds-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`ds-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://api.deepseek.com/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"ds-tui"`, `"ds-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-ds-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("ds-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid DS_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-DS-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &ds_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-DS-Token-Auth".to_string(), value.to_string());
    }
...
mod tests {
    ...
    use std::sync::Mutex;
    use ds_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("ds-cli"),

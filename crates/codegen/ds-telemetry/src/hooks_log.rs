...
//! file logger that writes to `~/.ds/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `ds_hooks` or `ds_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! DS_HOOKS_LOG=1 ds              # enable, write to ~/.ds/logs/hooks.log
//! DS_HOOKS_LOG=/tmp/h.log ds     # write to custom path
//! DS_HOOKS_LOG=0 ds              # explicitly disable
//! tail -f ~/.ds/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "DS_HOOKS_LOG";
...
///
/// Writes to `~/.ds/logs/hooks.log` (or custom path via `DS_HOOKS_LOG`).
/// Filters to hooks (`ds_hooks`) and plugins (`ds_agent::plugins`) targets.
/// Set `DS_HOOKS_LOG=0` to disable, `DS_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "ds_hooks=debug,ds_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || ds_home().join("logs").join("hooks.log");

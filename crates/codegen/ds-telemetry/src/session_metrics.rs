...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No ds.com auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via ds.com auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<ds_file_utils::UploadMethod>) -> Self {
        match method {
            Some(ds_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(ds_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(ds_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use ds_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `ds-shell-doom_loop_recovery` Mixpanel event's name and

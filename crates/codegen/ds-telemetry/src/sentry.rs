#![allow(dead_code)] // scrubbers retained for optional re-enable; network path is forced off
...
pub struct Config {
    /// Sentry tag `client`, e.g. `"ds-pager"`.
...
pub fn init(config: Config) -> ClientInitGuard {
    // DS builds never phone home to Sentry (or any crash reporter), regardless
    // of SENTRY_DSN / compile-time DSN. Keep a no-op guard so call sites work.
    let _ = CONFIG.get_or_init(|| Config {
        disabled: true,
        ..config
    });
    let _ = (TRACES_SAMPLE_RATE, FLUSH_TIMEOUT);
...
impl Scrubber {
    ...
    fn scrub(&self, s: &str) -> String {
        let out = ds_secrets::redact_secrets(s);
    ...
    fn scrub_value(&self, val: &mut Value) {
        ds_secrets::walk_json_strings(val, &mut |s| *s = self.scrub(s));
    }
}

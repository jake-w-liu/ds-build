...
//!
//! These structs were extracted from `ds-shell` so they can be
...
/// A login method was chosen from the picker. `method` is "ds" or "api_key";
...
/// A login flow completed successfully. `method` is "ds" or "api_key";
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = ds_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: ds_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from ds-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when DS Build is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct DsProUpsellShown {
    pub source: DsProUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct DsProUpsellClicked {
    pub source: DsProUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`DsProUpsell`]; lets the funnel attribute the click to the
...
/// is lifted — i.e. they subscribed (externally on ds.com) and came back.
/// This is the actual conversion signal for DS Heavy subscriptions
/// attributed to DS Build: the user saw the gate in DS Build, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to DS Build's upsell surface.
...
/// `ds-shell-manual_auth` (the `manual_auth` binding gets the `ds-shell-`
...
telemetry_event!(DsProUpsellShown, "ds_pro_upsell_shown");
telemetry_event!(DsProUpsellClicked, "ds_pro_upsell_clicked");

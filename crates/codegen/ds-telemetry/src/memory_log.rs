...
//!
//! Provides a dedicated tracing target (`ds_memory`) with an optional
//! file logger that writes to `~/.ds/logs/memory.log`.
//!
//! ## When to use
//!
...
//! DS_MEMORY_LOG=0 ds                # disable even when enabled
//! tail -f ~/.ds/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "ds_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "DS_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.ds/logs/memory.log`. Filters to `ds_memory=trace`.
    /// Set `DS_MEMORY_LOG=0` to disable, `DS_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || ds_home().join("logs").join("memory.log");

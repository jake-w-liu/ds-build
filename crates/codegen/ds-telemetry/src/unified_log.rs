...
//! over ACP (`ds.cli/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "ds.cli/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "ds-pager")]
    #[serde(rename = "ds-pager")]
    DsPager,
    #[strum(serialize = "ds-desktop")]
    #[serde(rename = "ds-desktop")]
    DSDesktop,
}
...
/// Wire format for the `ds.cli/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    ds_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Called by the `ds.cli/log` notification handler. Entries from

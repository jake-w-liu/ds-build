...
//!
//! Extracted from `ds-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`ds-shell-` /
/// `ds-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("DS Heavy")
/// and JWT-derived keys ("ds_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "DS Heavy" | "ds_heavy" => "ds_heavy",
        "DS Pro" | "ds_pro" => "ds_pro",
        "DS Lite" | "ds_lite" => "ds_lite",
...
/// Returns `true` when product telemetry is allowed to leave the machine.
///
/// DS builds default to **off**. Set `DS_TELEMETRY_OPT_IN=1` *and* configure
/// a non-DeepSeek events endpoint if you intentionally want analytics.
...
/// Returns `true` when session-metrics lifecycle events may be emitted.
/// Hard-disabled unless `DS_TELEMETRY_OPT_IN=1`.
...
fn telemetry_opt_in() -> bool {
    matches!(
        std::env::var("DS_TELEMETRY_OPT_IN").as_deref(),
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("DS Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("ds-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("ds-shell-turn"), "turn");
        assert_eq!(
            event_value("ds-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("ds-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("ds-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("DS Pro"), "ds_pro");
        assert_eq!(normalize_tier("DS Heavy"), "ds_heavy");
        assert_eq!(normalize_tier("ds_heavy"), "ds_heavy");
        ...
        assert_eq!(normalize_tier("DS Lite"), "ds_lite");
    ...
    /// (e.g. a future `"ds-shell-ext-"` origin), an earlier `ALL` entry could

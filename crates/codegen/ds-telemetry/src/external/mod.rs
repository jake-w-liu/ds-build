...
//!
//! Enterprise customers point the DS CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `DS_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `DS_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern DeepSeek-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &ds_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             DS_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }

...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "ds_code.session_start",
            Self::SessionEnd => "ds_code.session_end",
            Self::UserPrompt => "ds_code.user_prompt",
            Self::TurnCompleted => "ds_code.turn_completed",
            Self::ApiRequest => "ds_code.api_request",
            Self::ApiError => "ds_code.api_error",
            Self::ToolResult => "ds_code.tool_result",
            Self::ToolDecision => "ds_code.tool_decision",
            Self::McpServerConnection => "ds_code.mcp_server_connection",
            Self::PermissionModeChanged => "ds_code.permission_mode_changed",
            Self::SkillActivated => "ds_code.skill_activated",
            Self::PluginLoaded => "ds_code.plugin_loaded",
            Self::Compaction => "ds_code.compaction",
            Self::Subagent => "ds_code.subagent",
            Self::Auth => "ds_code.auth",
            Self::InternalError => "ds_code.internal_error",
            Self::ModelSwitched => "ds_code.model_switched",
            Self::ContextualTip => "ds_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `ds_code.session.count`.
    SessionCount,
    /// `ds_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "ds_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "ds_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "ds_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "ds_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "ds_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "ds_code.error.count";
...
/// Built-in tool names (closed enum surface in `ds-tools`) that pass
...
fn tool_outcome_label(o: &ds_file_utils::events::types::ToolOutcome) -> &'static str {
    use ds_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `ds_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `ds_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `ds_code.session_end`.
...
/// `PromptSubmitted` → `ds_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `ds_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `ds_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `ds_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `ds_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `ds_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `ds_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `ds_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `ds_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `ds_code.permission_mode_changed`.
...
/// `ContextualTip` → `ds_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `ds_code.permission_mode_changed`.
...
/// `SkillDispatched` → `ds_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `ds_code.plugin_loaded`.
...
/// `PluginUsed` → `ds_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `ds_code.compaction`.
...
/// `CompactionTriggered` → `ds_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `ds_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `ds_code.subagent` (`phase=completed`).
...
/// `InternalError` → `ds_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `ds_code.model_switched`.

...
//!
//! Extracted from `ds-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$DS_HOME` is resolved through `ds-config::ds_home`.
...
/// This function caches the result in `$DS_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = ds_config::ds_home().join("agent_id");
...
/// Returns true when workspace marker env vars (`DS_ROOT` and `DS_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("DS_ROOT").is_ok() && std::env::var("DS_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `DS_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("DS_TELEMETRY_SPECIAL_USER").as_deref(),

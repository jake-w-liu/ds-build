...
//!
//! Extracted from `ds-shell::agent::config` so the data-collector
...
pub struct TelemetryConfig {
    ...
    /// External OTEL master switch (`= DS_EXTERNAL_OTEL`, env wins).
...
/// No baked-in events URL, API key, Mixpanel token, or mixpanel enablement.
/// DS builds never ship product analytics credentials.
...
impl Default for TelemetryConfig {
    fn default() -> Self {
        // Ignore compile-time telemetry secrets — never bake DeepSeek/Mixpanel into DS.
        let _ = (
            option_env!("DS_TELEMETRY_BUILD_EVENTS_URL"),
            option_env!("DS_TELEMETRY_BUILD_EVENTS_API_KEY"),
            option_env!("DS_TELEMETRY_BUILD_MIXPANEL_TOKEN"),
            internal_defaults(),
            build_env_default,
        );
        Self {
...
impl TelemetryConfig {
    pub fn apply_env_overrides(&mut self) {
        self.normalize();
        if let Some(value) = Self::env_override("DS_TELEMETRY_EVENTS_URL") {
            self.events_url = value;
        }
        if let Some(value) = Self::env_override("DS_TELEMETRY_EVENTS_API_KEY") {
            self.events_api_key = value;
        }
        if let Some(value) = Self::env_override("DS_TELEMETRY_MIXPANEL_TOKEN") {
            self.mixpanel_token = value;
        }
        if let Some(value) = env_bool("DS_TELEMETRY_MIXPANEL_ENABLED") {
            self.mixpanel_enabled = value;
        }
        if let Some(value) = env_bool("DS_TELEMETRY_TRACE_UPLOAD") {
...
///
/// Local copy of `ds_shell::agent::config::env_bool` so this crate
...
mod tests {
    ...
    #[test]
    fn default_is_fully_disabled_in_ds_builds() {

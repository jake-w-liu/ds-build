...
//!
//! Extracted from `ds-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `ds-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `ds-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "ds-shell-",
            EmitterOrigin::Workspace => "ds-workspace-",
        }
    }
}
...
/// event-name prefix (`ds-shell-*` vs `ds-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "ds-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "ds-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("ds-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("ds-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "ds-workspace-turn");
    }

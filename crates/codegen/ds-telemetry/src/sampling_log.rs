//! Tracing layer for `target: "sampling_log"` → `~/.ds/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `DS_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "DS_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = ds_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);

...
use ds_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "deepseek-v4-pro-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    assert!(!ds_telemetry::is_enabled());

    ds_telemetry::log_event(ds_telemetry::events::SessionHarness {
        session_id: "sess-gates-on".into(),
        client_identifier: Some("ds-pager".into()),
        model_id: "deepseek-v4-pro".into(),
        agent_name: "ds-build-plan".into(),
        permission_mode: ds_telemetry::enums::PermissionMode::Ask,
    ...
    ds_telemetry::log_event(ds_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "deepseek-v4-pro".into(),
    ...
    ds_telemetry::log_event(ds_telemetry::events::ModelResponseReceived {
    ...
    ds_telemetry::log_event(ds_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "ds_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.ds.ds_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("ds-cli"),
        "service.name=ds-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("ds_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "ds_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "ds_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "ds_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "ds_code.session.count");

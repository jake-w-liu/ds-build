...
fn external_stream_end_to_end() {
    ...
    let mut cfg = ds_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = ds_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    ds_telemetry::external::init(Some(cfg));
    assert!(ds_telemetry::external::is_active());
    ...
    ds_telemetry::log_event(ds_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("ds-cli")),
        "service.name=ds-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "ds_code.session_start",
        "ds_code.user_prompt",
        "ds_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "ds_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "ds_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "ds_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "ds_code.token.usage"));
    assert_eq!(
    ...
    ds_telemetry::external::shutdown();
    assert!(

...
//! `ds-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use ds_telemetry::client;
use ds_telemetry::config::{TelemetryConfig, TelemetryMode};
use ds_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_ds_shell_manual_auth() {
    ...
    ds_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "ds-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no ds-shell-manual_auth POST received"
        );

//! Wire types for the `ds-workspace` API.
//!
...
//! `ds-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `ds-workspace-types` (instead of `ds-workspace` or
/// `ds-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `ds_workspace::permission`

...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] ds_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `ds-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `ds_tools::ToolError`) happens

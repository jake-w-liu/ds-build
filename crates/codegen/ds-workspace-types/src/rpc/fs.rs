...
//! the shell's `ds.cli/fs/*` ACP methods.
...
// workspace server (`ds-workspace`) and the ds.com backend
...
// `workspace.fs_*` ops above already serve the shell's `ds.cli/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `ds.cli/fs/list`
/// node `type` strings.
...
/// `ds.cli/fs/list` plus `offset` pagination).

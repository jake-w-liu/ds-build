...
//! `ds-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    ...
    /// ~/.ds/skills
    User,
    /// ~/.ds/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.ds/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the ds-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.ds/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.ds/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.ds/plugins/infra-plugin",
            "plugin_data": "/root/.ds/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],

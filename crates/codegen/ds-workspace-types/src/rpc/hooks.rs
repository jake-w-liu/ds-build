...
//!
//! `ds_hooks` pulls in `git2`/`reqwest`/`ds-tools`, too heavy for
...
/// Wire mirror of `ds_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `ds_hooks::config::HookSpec`.
///
...
/// `ds-hooks`, so a server-side test (`ds-workspace`'s
...
/// Wire mirror of `ds_hooks::event::HookEventName`.
///

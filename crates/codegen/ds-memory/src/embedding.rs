...
pub(super) fn build_middleware_client(
    ...
    ds_http::with_auth_retry(ds_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn ds_auth::AuthCredentialProvider> = std::sync::Arc::new(
        ds_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl ds_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = ds_http::shared_client()
...
/// can inject a deterministic embedder in their own test targets.

...
pub struct BeforeTurnPayload {
    ...
    pub turn_number: u64,
    /// Model being used for this turn (e.g. "deepseek-v4-flash").
    pub model_id: String,
    ...
    /// (`"primary"` | `"subagent"`). A `String`, not the `ds-file-utils`
...
pub struct AfterTurnPayload {
    ...
    /// the workspace decodes it into the `ds-file-utils`
...
pub enum TurnHookOutcome {
    ...
    Error,
    /// Forward-compatibility catch-all for variants added in newer
    /// protocol versions.  Prevents `SessionEvent::TurnEnded` from
    /// failing to deserialize when a future turn hook emits a new
    /// outcome value (e.g. `"timeout"`, `"interrupted"`).
    #[serde(other)]
    Unknown,
}
...
mod tests {
    ...
    #[test]
    fn unknown_outcome_maps_to_unknown_variant() {
        // Forward-compatibility: a future outcome value (e.g. "timeout")
        // must deserialize to TurnHookOutcome::Unknown instead of failing,
        // so that SessionEvent::TurnEnded carrying a new outcome doesn't
        // break the entire event deserialization.
        let result = serde_json::from_value::<TurnHookOutcome>(json!("timeout")).unwrap();
        assert_eq!(result, TurnHookOutcome::Unknown);
    }

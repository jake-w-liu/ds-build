//! JSON-RPC 2.0 envelope types with the DS `session_id` / `seq`
//! extensions.
//!
...
pub struct JsonRpcRequest<P = serde_json::Value> {
    ...
    pub id: JsonRpcId,
    /// DS extension: routing/sanity-check session id.
...
impl<'de, R: Deserialize<'de>> Deserialize<'de> for JsonRpcResponse<R> {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        // Two-pass deserialization: first extract the raw fields to
        // distinguish `"result": null` (valid JSON-RPC success with a
        // null result) from a missing `"result"` key.  `Option<R>` alone
        // cannot tell those apart because serde maps both to `None`.
        #[derive(Deserialize)]
        struct RawFlat {
        ...
        let raw = RawFlat::deserialize(deserializer)?;
        let outcome = match (raw.result, raw.error) {
            (Some(r), None) => {
                let typed: R = R::deserialize(r).map_err(de::Error::custom)?;
                ResponseOutcome::Result(typed)
            }

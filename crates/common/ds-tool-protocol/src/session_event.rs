...
mod tests {
    ...
    #[test]
    fn turn_ended_with_unknown_outcome_deserializes() {
        // Forward-compatibility: a future TurnHookOutcome value must not
        // break the entire SessionEvent deserialization.
        let v = json!({
        ...
        let event: SessionEvent = serde_json::from_value(v).unwrap();
        assert!(
            matches!(event, SessionEvent::TurnEnded { outcome, .. } if matches!(outcome, TurnHookOutcome::Unknown)),
            "future outcome must deserialize as TurnHookOutcome::Unknown, got {event:?}"
        );
    }

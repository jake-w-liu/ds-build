//! Observability seam for the full-replace (ds-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (ds-build:
...
//!
//! Emission points are part of the behavior contract: the ds-build observer

//! Compacted-history assembly (ds-build's rebuild structure, generic).
//!
//! Moved from `ds-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! ds-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // ds-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn ds_build_order_recent_before_summary() {

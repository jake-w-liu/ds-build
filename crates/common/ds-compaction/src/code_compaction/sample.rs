...
//!
//! The canonical `sample → classify → retry` loop, used by **both** ds-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and DS chat's intra `Shared` summarizer
...
//! the winning summary (ds-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the ds-build host uses to step down its input size).

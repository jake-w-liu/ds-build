...
//! harness implements [`CompactionItem`] for its `DsTurn`;
//! ds-build implements it for `ds_sampling_types::ConversationItem`.
//!
...
//! `<ds_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `DsRole` (DS chat) and the
/// `ConversationItem` variants (ds-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (DS chat) — maps to System on harnesses without a
...
/// extraction (`<ds_file id=".." name=".." />` lines in the
/// `<ds_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (DS chat's
    /// `DsTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (DS chat: a
    ...
    /// strips their `<ds_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<ds_file>` lines in the
    /// `<ds_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (DS chat: a `Developer` turn with category
    ...
    /// requests/results (DS chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up ds-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by DS chat's
/// `DsTurn`; adding these constructors to it as required methods would break
/// that impl. They are also ds-build-specific (DS chat's tail-keep path
...
///
/// The ds-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (DS chat stores turns as `Arc<DsTurn>`).

...
//!
//! - DS chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `DsTurn::get_num_tokens`.
//! - ds-build estimates with `bytes / 4`.
//!
...
//! (DS chat: `ItemTokenCounter<Arc<DsTurn>>`), so exactly one mechanism
//! is in play.

...
// The `Shared` summarizer reuses ds-build's full-replace summarization core
...
/// `FullReplace` strategy (default): ds-build's full-replace — summarize the
...
/// `<ds_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching ds-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through ds-build's shared core.
    ...
    //     Shared with DS chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    //     and the `<ds_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): ds-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<ds_user_queries>` preamble so
...
/// — ds-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and ds-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // ds-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // ds-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on DS chat's
...
/// the literal error messages produced by the DS chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the DS chat sampler when the
    ...
    /// — guards the forwarding that DS chat (`Arc<DsTurn>`) relies on.
    #[test]

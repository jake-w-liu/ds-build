...
///
/// - `FullReplace` (default): ds-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from ds-shell's
...
/// time (token threshold only, matching ds-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as ds-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<ds_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "deepseek-v4-pro.20";

...
pub enum CompactionTarget {
    ...
    /// with a single summary — ds-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by DS chat's
/// `StreamProcessor` (`Item = Arc<DsTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<DsTurn>`) already satisfies it.

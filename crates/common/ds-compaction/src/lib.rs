...
//! stay in each product host (for example `ds-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `ds-sampling-types`. It is decoupled from both DS chat and
//! ds-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — ds-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — DS chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — DS chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by DS chat and
//! ds-build; hosts still own snapshotting and host-only sections).
...
// ds-build's full-replace subsystem now lives under `code_compaction`;

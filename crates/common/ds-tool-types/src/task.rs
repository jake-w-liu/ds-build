...
/// A built-in subagent type shared by the CLI (`ds-agent`) and other
...
pub const ATTACKER_MATH_PROMPT: &str = concat!(
    ...
    "closed derivation this may mean every step; for a long research artifact,\n",
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "deepseek-v4-flash"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("deepseek-v4-flash"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("deepseek-v4-flash".into())).as_deref(),
    ...
    // match what the ds-shell MiniJinja templates render for the default
    // ds-build toolset (monitor + task + bash + read present, POSIX). The

...
//! `DsToolDyn` / `DsToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use ds_tool_protocol::{ToolCapabilities, ToolId};
use ds_tool_runtime::{
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&ds_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );

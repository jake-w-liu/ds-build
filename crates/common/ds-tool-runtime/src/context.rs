...
pub struct WorkspaceBindMetadata {
    ...
    /// Explicit toolset in the ds-tools gRPC wire shape. Empty = unset.
    #[serde(
        default,
        deserialize_with = "ok_or_default",
        skip_serializing_if = "Vec::is_empty"
    )]
    pub tools: Vec<ds_tools_api::ToolConfigEntry>,
    #[serde(
        default,
        deserialize_with = "ok_or_default",
        skip_serializing_if = "Option::is_none"
    ...
    /// DS consumers treat `None` as always-approve.
    #[serde(
        default,
        deserialize_with = "ok_or_default",
        skip_serializing_if = "Option::is_none"
...
mod bind_metadata_tests {
    ...
    fn round_trips_populated() {
        let md = WorkspaceBindMetadata {
            ...
            tools: vec![ds_tools_api::ToolConfigEntry {
                id: "DsBuild:grep".to_owned(),
                ..Default::default()
            }],
            viewer_ctx: Some(super::WorkspaceViewerContext {
                stream_tool_progress: true,
            }),
            yolo_mode: Some(true), // DS default: always-approve

...
use ds_tool_protocol::StreamingSpec;
...
pub fn stream_chunk(
    ...
    if cut == 0 && !delta_bytes.is_empty() {
        ...
        // If the extended cut still ends on an incomplete boundary, the data
        // itself is incomplete (not just the cap being too small).  Hold the
        // bytes back — the continuation will arrive in a subsequent chunk.
        if incomplete_utf8_suffix_len(&delta_bytes[..cut]) > 0 {
            return None;
        }
    }
    if cut == 0 {
...
mod tests {
    ...
    /// Regression: an entirely-incomplete solo delta (e.g. a lone UTF-8
    /// lead byte 0xC3 without 0xA9) must be held back intact, not emitted
    /// as U+FFFD.  Otherwise, when the continuation byte arrives next tick,
    /// both halves are permanently corrupted to replacement characters.
    #[test]
    fn incomplete_sole_delta_is_held_back_not_emitted_as_fffd() {
        let spec = spec_with(None);
        let mut last = 0;
        // The entire delta is an incomplete UTF-8 lead byte — must hold back.
        assert!(
            stream_chunk(&spec, b"\xC3", 1, &mut last, false).is_none(),
            "incomplete sole delta must be held back, not emitted"
        );
        assert_eq!(last, 0, "last_total must not advance");
    }
}

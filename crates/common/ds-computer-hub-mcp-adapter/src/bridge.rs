...
use ds_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use ds_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`ds_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`ds_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl ds_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            ds_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == ds_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
